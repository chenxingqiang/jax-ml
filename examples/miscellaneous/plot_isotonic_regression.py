...
y = rs.randint(-50, 50, size=(n,)) + 50.0 * jnp.log1p(jnp.arange(n))
...
lr = LinearRegression()
lr.fit(x[:, jnp.newaxis], y)  # x needs to be 2d for LinearRegression
...
lc.set_array(jnp.ones(len(y)))
lc.set_linewidths(jnp.full(n, 0.5))

fig, (ax0, ax1) = plt.subplots(ncols=2, figsize=(12, 6))

ax0.plot(x, y, "C0.", markersize=12)
ax0.plot(x, y_, "C1.-", markersize=12)
ax0.plot(x, lr.predict(x[:, jnp.newaxis]), "C2-")
ax0.add_collection(lc)
ax0.legend(("Training data", "Isotonic fit", "Linear fit"), loc="lower right")
ax0.set_title("Isotonic regression fit on noisy data (n=%d)" % n)

x_test = jnp.linspace(-10, 110, 1000)
