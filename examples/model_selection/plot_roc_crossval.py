...
for fold, (train, test) in enumerate(cv.split(X, y)):
    ...
    interp_tpr = jnp.interp(mean_fpr, viz.fpr, viz.tpr)
    interp_tpr[0] = 0.0
    tprs.append(interp_tpr)
    aucs.append(viz.roc_auc)

mean_tpr = jnp.mean(tprs, axis=0)
mean_tpr[-1] = 1.0
mean_auc = auc(mean_fpr, mean_tpr)
std_auc = jnp.std(aucs)
ax.plot(
    mean_fpr,
    mean_tpr,
    color="b",
...
std_tpr = jnp.std(tprs, axis=0)
tprs_upper = jnp.minimum(mean_tpr + std_tpr, 1)
tprs_lower = jnp.maximum(mean_tpr - std_tpr, 0)
ax.fill_between(
    mean_fpr,
    tprs_lower,
    tprs_upper,
