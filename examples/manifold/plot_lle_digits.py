...
for idx, ax in enumerate(axs.ravel()):
    ...
    ax.axis("off")
_ = fig.suptitle(
    "A selection from the 64-dimensional digits dataset", fontsize=16)
...
def plot_embedding(X, title):
    ...
    shown_images = jnp.array([[1.0, 1.0]])  # just something big
    for i in range(X.shape[0]):
        # plot every digit on the embedding
        # show an annotation box for a group of digits
        dist = jnp.sum((X[i] - shown_images) ** 2, 1)
        if jnp.min(dist) < 4e-3:
            # don't show points that are too close
            continue
        shown_images = jnp.concatenate([shown_images, [X[i]]], axis=0)
