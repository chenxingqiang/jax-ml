...
step = 4.0 * jnp.pi / n_samples

for i in range(X.shape[0]):
    x = i * step - 6.0
    X[i, 0] = x + np.random.normal(0, 0.1)
    X[i, 1] = 3.0 * (jnp.sin(x) + np.random.normal(0, 0.2))
...
plot_results(
    X, gmm.predict(
        X), gmm.means_, gmm.covariances_, 0, "Expectation-maximization"
)
...
dpgmm = mixture.BayesianGaussianMixture(
    ...
    covariance_prior=1e0 * jnp.eye(2),
    init_params="random",
    max_iter=100,
    random_state=2,
).fit(X)
