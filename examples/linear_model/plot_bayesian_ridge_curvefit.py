...
x_test = jnp.linspace(0.0, 1.0, 100)
...
n_order = 3
X_train = jnp.vander(x_train, n_order + 1, increasing=True)
X_test = jnp.vander(x_test, n_order + 1, increasing=True)
...
for i, ax in enumerate(axes):
    ...
    if i == 0:
        init = [1 / jnp.var(y_train), 1.0]  # Default values
    elif i == 1:
        init = [1.0, 1e-3]
