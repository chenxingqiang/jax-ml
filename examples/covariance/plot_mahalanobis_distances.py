...
gen_cov = jnp.eye(n_features)
gen_cov[0, 0] = 2.0
X = jnp.dot(np.random.randn(n_samples, n_features), gen_cov)
# add some outliers
outliers_cov = jnp.eye(n_features)
outliers_cov[jnp.arange(1, n_features), jnp.arange(1, n_features)] = 7.0
X[-n_outliers:] = jnp.dot(np.random.randn(n_outliers, n_features), outliers_cov)
...
xx, yy = jnp.meshgrid(
    jnp.linspace(plt.xlim()[0], plt.xlim()[1], 100),
    jnp.linspace(plt.ylim()[0], plt.ylim()[1], 100),
)
zz = jnp.c_[xx.ravel(), yy.ravel()]
# Calculate the MLE based Mahalanobis distances of the meshgrid
mahal_emp_cov = emp_cov.mahalanobis(zz)
mahal_emp_cov = mahal_emp_cov.reshape(xx.shape)
emp_cov_contour = plt.contour(
    xx, yy, jnp.sqrt(mahal_emp_cov), cmap=plt.cm.PuBu_r, linestyles="dashed"
)
# Calculate the MCD based Mahalanobis distances
mahal_robust_cov = robust_cov.mahalanobis(zz)
mahal_robust_cov = mahal_robust_cov.reshape(xx.shape)
robust_contour = ax.contour(
    xx, yy, jnp.sqrt(mahal_robust_cov), cmap=plt.cm.YlOrBr_r, linestyles="dotted"
)
...
emp_mahal = emp_cov.mahalanobis(X - jnp.mean(X, 0)) ** (0.33)
# Plot boxplots
ax1.boxplot([emp_mahal[:-n_outliers], emp_mahal[-n_outliers:]], widths=0.25)
# Plot individual samples
ax1.plot(
    jnp.full(n_samples - n_outliers, 1.26),
    emp_mahal[:-n_outliers],
    "+k",
    markeredgewidth=1,
)
ax1.plot(jnp.full(n_outliers, 2.26),
...
# Plot boxplots
ax2.boxplot([robust_mahal[:-n_outliers],
            robust_mahal[-n_outliers:]], widths=0.25)
# Plot individual samples
ax2.plot(
    jnp.full(n_samples - n_outliers, 1.26),
...
ax2.plot(jnp.full(n_outliers, 2.26),
