...
to use the online API of the jax-learn to process a very large
...
kmeans = MiniBatchKMeans(
    n_clusters=81, random_state=rng, verbose=True, n_init=3)
patch_size = (20, 20)
...
for _ in range(6):
    for img in faces.images:
        ...
        data = jnp.reshape(data, (len(data), -1))
        buffer.append(data)
        index += 1
        if index % 10 == 0:
            data = jnp.concatenate(buffer, axis=0)
