    ...
    for ns in n_samples:
        for nf in n_features:
            ...
            print("Iteration %s of %s" %
                  (it, max(len(n_samples), len(n_features))))
            print("==================")
            n_informative = nf // 10
            ...
            X /= jnp.sqrt(jnp.sum(X**2, axis=0))  # Normalize data
...
if __name__ == "__main__":
    ...
    n_features = 10
    list_n_samples = jnp.linspace(100, 1000000, 5).astype(int)
    lasso_results, lars_lasso_results = compute_bench(
        alpha, list_n_samples, [n_features], precompute=True
    )

    plt.figure("jax-learn LASSO benchmark results")
    plt.subplot(211)
    ...
    n_samples = 2000
    list_n_features = jnp.linspace(500, 3000, 5).astype(int)
    lasso_results, lars_lasso_results = compute_bench(
