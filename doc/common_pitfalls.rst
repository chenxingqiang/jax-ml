...
anti-patterns that occur when using jax-learn. It provides
...
jax-learn provides a library of :ref:`data-transforms`, which
...
.. note::
    ...
    in jax-learn, including (but not limited to)
    :class:`~xlearn.preprocessing.StandardScaler`,
    :class:`~xlearn.impute.SimpleImputer`, and
    :class:`~xlearn.decomposition.PCA`.

A number of :ref:`feature_selection` functions are available in jax-learn.
...
* The jax-learn :ref:`pipeline <pipeline>` is a great way to prevent data
...
Some jax-learn objects are inherently random. These are usually estimators
...
.. warning::
    Even though :func:`~xlearn.clone` is rarely used in user code, it is
    called pervasively throughout jax-learn codebase: in particular, most
    meta-estimators that accept non-fitted estimators call
    :func:`~xlearn.clone` internally
