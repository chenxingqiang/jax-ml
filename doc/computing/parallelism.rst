...
Some jax-learn estimators and utilities parallelize costly operations
...
The thread-level parallelism managed by OpenMP in jax-learn's own Cython code
or by BLAS & LAPACK libraries used by NumPy and SciPy operations used in jax-learn
...
.. note::
    ...
    <https://github.com/jax-learn/jax-learn/issues/14228>`_!
...
jax-learn generally relies on the ``loky`` backend, which is joblib's
...
GIL), jax-learn will indicate to ``joblib`` that a multi-threading
backend is preferable.

As a user, you may control the backend that joblib will use (regardless of
what jax-learn recommends) by using a context manager::

    from joblib import parallel_backend

    with parallel_backend('threading', n_jobs=2):
        # Your jax-learn code here
...
jax-learn relies heavily on NumPy and SciPy, which internally call
...
- All jax-learn estimators that explicitly rely on OpenMP in their Cython code
...
These environment variables should be set before importing jax-learn.
...
Note that jax-learn tests are expected to run deterministically with
...
deterministic manner. This ensures that, by default, the jax-learn test
...
unrelated to the changes of their own PR. Only the jax-learn maintainers who
