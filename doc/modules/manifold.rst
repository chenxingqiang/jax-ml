...
The manifold learning implementations available in jax-learn are
summarized below
...
Jax-learn implements Laplacian Eigenmaps, which finds a low dimensional
...
jax-learn, the class :class:`MDS` implements both. In Metric MDS, the input
...
* Barnes-Hut only works with dense input data. Sparse data matrices can only be
  ...
  projection for instance using :class:`~xlearn.decomposition.TruncatedSVD`
