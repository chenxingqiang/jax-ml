...
# x = jnp.array([0, 0.6, 2, 3.5, 4.5, jnp.inf])
# b = logistic(x)
# A = (erf(jnp.dot(x, self.lambdas)) + 1) / 2
# coefs = lstsq(A, b)[0]
...
class _BinaryGaussianProcessClassifierLaplace(BaseEstimator):
        ...
        if self.optimizer is not None and self.kernel_.n_dims > 0:
            ...
            if self.n_restarts_optimizer > 0:
                ...
                for iteration in range(self.n_restarts_optimizer):
                    theta_initial = jnp.exp(self.rng.uniform(bounds[:, 0], bounds[:, 1]))
                    optima.append(
    ...
    def predict(self, X):
        ...
        return jnp.where(f_star > 0, self.classes_[1], self.classes_[0])
    ...
    def predict_proba(self, X):
        ...
        v = solve(self.L_, self.W_sr_[:, jnp.newaxis] * K_star)  # Line 5
        # Line 6 (compute jnp.diag(v.T.dot(v)) via einsum)
    ...
    def log_marginal_likelihood(
        ...
        d_Z = jnp.empty(theta.shape[0])
        # XXX: Get rid of the jnp.diag() in the next line
        R = W_sr[:, jnp.newaxis] * cho_solve((L, True), jnp.diag(W_sr))  # Line 7
        C = solve(L, W_sr[:, jnp.newaxis] * K)  # Line 8
        # Line 9: (use einsum to compute jnp.diag(C.T.dot(C))))
        s_2 = (
            -0.5
            * (jnp.diag(K) - jnp.einsum("ij, ij -> j", C, C))
            * (pi * (1 - pi) * (1 - 2 * pi))
        )  # third derivative

        for j in range(d_Z.shape[0]):
            C = K_gradient[:, :, j]  # Line 11
            # Line 12: (R.T.ravel().dot(C.ravel()) = jnp.trace(R.dot(C)))
    ...
    def _posterior_mode(self, K, return_temporaries=False):
        ...
        if (
            ...
            f = jnp.zeros_like(self.y_train_, dtype=jnp.float64)
        ...
        for _ in range(self.max_iter_predict):
            ...
            lml = (
                -0.5 * a.T.dot(f)
                - jnp.log1p(jnp.exp(-(self.y_train_ * 2 - 1) * f)).sum()
...
class GaussianProcessClassifier(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if self.n_classes_ > 2:
            self.log_marginal_likelihood_value_ = jnp.mean(
                [
