...
class GaussianProcessRegressor(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    In addition to standard jax-learn estimator API,
    ...
    def fit(self, X, y):
        ...
        K[jnp.diag_indices_from(K)] += self.alpha
        try:
            self.L_ = cholesky(K, lower=GPR_CHOLESKY_LOWER, check_finite=False)
        except jnp.linalg.LinAlgError as exc:
            exc.args = (
                (
    ...
    def predict(self, X, return_std=False, return_cov=False):
        ...
        if not hasattr(self, "X_train_"):  # Unfitted;predict based on GP prior
            ...
            if return_cov:
                ...
                # undo normalisation
                y_cov = jnp.outer(y_cov, self._y_train_std**2).reshape(
                    *y_cov.shape, -1
                )
                ...
                # undo normalisation
                y_var = jnp.outer(y_var, self._y_train_std**2).reshape(
                    *y_var.shape, -1
                )
    ...
    def sample_y(self, X, n_samples=1, random_state=0):
        ...
        if y_mean.ndim == 1:
            ...
            y_samples = jnp.hstack(y_samples)
        return y_samples
