...
def test_kernel_theta(kernel):
    ...
    for i, hyperparameter in enumerate(kernel.hyperparameters):
        assert theta[i] == jnp.log(getattr(kernel, hyperparameter.name))
    ...
    for i, hyperparameter in enumerate(kernel.hyperparameters):
        ...
        assert_almost_equal(kernel.theta[i], jnp.log(43))
...
def test_kernel_anisotropic():
    ...
    kernel.theta = kernel.theta + jnp.log(2)
    assert_array_equal(kernel.theta, jnp.log([6.0, 1.0, 4.0]))
...
def test_kernel_stationary(kernel):
    ...
    K = kernel(X, X + 1)
    assert_almost_equal(K[0, 0], jnp.diag(K))
...
def test_kernel_clone_after_set_params(kernel):
    ...
    # See https://github.com/jax-learn/jax-learn/issues/6961
    # for more details.
    bounds = (1e-5, 1e5)
    kernel_cloned = clone(kernel)
    params = kernel.get_params()
...
def test_matern_kernel():
    ...
    assert_array_almost_equal(jnp.diag(K), jnp.ones(X.shape[0]))
    # matern kernel for coef0==0.5 is equal to absolute exponential kernel
    K_absexp = jnp.exp(-euclidean_distances(X, X, squared=False))
    K = Matern(nu=0.5, length_scale=1.0)(X)
    assert_array_almost_equal(K, K_absexp)
    # matern kernel with coef0==inf is equal to RBF kernel
    K_rbf = RBF(length_scale=1.0)(X)
    K = Matern(nu=jnp.inf, length_scale=1.0)(X)
...
def test_set_get_params(kernel):
    ...
    for hyperparameter in kernel.hyperparameters:
        ...
        if size > 1:  # anisotropic kernels
            assert_almost_equal(
                jnp.exp(kernel.theta[index: index + size]
                       ), params[hyperparameter.name]
            )
            index += size
        else:
            assert_almost_equal(
                jnp.exp(kernel.theta[index]), params[hyperparameter.name]
            )
            index += 1
    # Test set_params()
    index = 0
    ...
    for hyperparameter in kernel.hyperparameters:
        ...
        if size > 1:  # anisotropic kernels
            ...
            assert_almost_equal(
                jnp.exp(kernel.theta[index: index + size]), [value] * size
            )
            index += size
        else:
            kernel.set_params(**{hyperparameter.name: value})
            assert_almost_equal(jnp.exp(kernel.theta[index]), value)
            index += 1
