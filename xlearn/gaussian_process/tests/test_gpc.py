...
X = jnp.atleast_2d(jnp.linspace(0, 10, 30)).T
X2 = jnp.atleast_2d([2.0, 4.0, 5.5, 6.5, 7.5]).T
y = jnp.array(f(X).ravel() > 0, dtype=int)
fX = f(X).ravel()
y_mc = jnp.empty(y.shape, dtype=int)  # multi-class
y_mc[fX < -0.35] = 0
...
def test_lml_without_cloning_kernel(kernel):
    ...
    gpc = GaussianProcessClassifier(kernel=kernel).fit(X, y)
    input_theta = jnp.ones(gpc.kernel_.theta.shape, dtype=jnp.float64)
...
def test_multi_class(kernel):
    ...
    assert_array_equal(jnp.argmax(y_prob, 1), y_pred)
