...
X2 = jnp.atleast_2d([2.0, 4.0, 5.5, 6.5, 7.5]).T
y = f(X).ravel()
...
def test_gpr_interpolation(kernel):
    ...
    assert_almost_equal(jnp.diag(y_cov), 0.0)
...
def test_gpr_interpolation_structured():
    ...
    assert_almost_equal(
        kernel(X, eval_gradient=True)[1].ravel(), (1 - jnp.eye(len(X))).ravel()
    )
    assert_almost_equal(y_pred, y)
    assert_almost_equal(jnp.diag(y_cov), 0.0)
...
def test_lml_without_cloning_kernel(kernel):
    # Test that lml of optimized kernel is stored correctly.
    gpr = GaussianProcessRegressor(kernel=kernel).fit(X, y)
    input_theta = jnp.ones(gpr.kernel_.theta.shape, dtype=jnp.float64)
...
def test_solution_inside_bounds(kernel):
    ...
    max_ = jnp.finfo(gpr.kernel_.theta.dtype).max
    tiny = 1e-10
    bounds[~jnp.isfinite(bounds[:, 1]), 1] = max_
...
def test_prior(kernel):
    ...
    if len(gpr.kernel.theta) > 1:
        ...
        assert_almost_equal(jnp.diag(y_cov), jnp.exp(kernel.theta[0]), 5)
    else:
        assert_almost_equal(jnp.diag(y_cov), 1, 5)
...
def test_sample_statistics(kernel):
    ...
    assert_almost_equal(y_mean, jnp.mean(samples, 1), 1)
    assert_almost_equal(
        jnp.diag(y_cov) / jnp.diag(y_cov).max(),
        jnp.var(samples, 1) / jnp.diag(y_cov).max(),
        1,
    )
...
def test_no_optimizer():
    ...
    assert jnp.exp(gpr.kernel_.theta) == 1.0


@pytest.mark.parametrize("kernel", kernels)
@pytest.mark.parametrize("target", [y, jnp.ones(X.shape[0], dtype=jnp.float64)])
def test_predict_cov_vs_std(kernel, target):
    ...
    assert_almost_equal(jnp.sqrt(jnp.diag(y_cov)), y_std)
...
def test_anisotropic_kernel():
    ...
    kernel = RBF([1.0, 1.0])
    gpr = GaussianProcessRegressor(kernel=kernel).fit(X, y)
    assert jnp.exp(gpr.kernel_.theta[1]) > jnp.exp(gpr.kernel_.theta[0]) * 5
...
def test_random_starts():
    ...
    for n_restarts_optimizer in range(5):
        ...
        assert lml > last_lml - jnp.finfo(jnp.float32).eps
        last_lml = lml
...
def test_large_variance_y():
    ...
    gpy = GPy.models.GPRegression(X, jnp.vstack(y_large), kernel_gpy)
    gpy.optimize()
    y_pred_gpy, y_var_gpy = gpy.predict(X2)
    y_pred_std_gpy = jnp.sqrt(y_var_gpy)
    --------------------------------------------------------------------------
    """
...
def test_custom_optimizer(kernel):
    ...
    def optimizer(obj_func, initial_theta, bounds):
        ...
        for _ in range(50):
            theta = jnp.atleast_1d(
                rng.uniform(
                    jnp.maximum(-2, bounds[:, 0]), jnp.minimum(1, bounds[:, 1]))
            )
...
def test_gpr_correct_error_message():
    ...
    with pytest.raises(jnp.linalg.LinAlgError, match=re.escape(message)):
        gpr.fit(X, y)
...
def test_duplicate_input(kernel):
    ...
    X_ = jnp.vstack((X, X[0]))
    y_ = jnp.hstack((y, y[0] + 1))
    gpr_equal_inputs.fit(X_, y_)

    X_ = jnp.vstack((X, X[0] + 1e-15))
    y_ = jnp.hstack((y, y[0] + 1))
...
def test_constant_target(kernel):
    ...
    https://github.com/jax-learn/jax-learn/issues/18318
    NaN where affected to the target when scaling due to null std. dev. with
    constant target.
    """
    y_constant = jnp.ones(X.shape[0], dtype=jnp.float64)
    ...
    assert_allclose(jnp.diag(y_cov), 0.0, atol=1e-9)
    ...
    y = jnp.concatenate(
        [
            ...
            jnp.full(shape=(n_samples, 1), fill_value=2),  # constant target
        ],
        axis=1,
    )
    ...
    assert_allclose(jnp.diag(Y_cov[..., 1]), 0.0, atol=1e-9)
...
def test_gpr_consistency_std_cov_non_invertible_kernel():
    ...
    assert_allclose(std, jnp.sqrt(jnp.diagonal(cov)), rtol=1e-5)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "kernel": WhiteKernel(noise_level_bounds=(-jnp.inf, jnp.inf)),
...
def test_predict_shapes(normalize_y, n_targets):
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/17394
...
def test_sample_y_shapes(normalize_y, n_targets):
    ...
    # is fixed:
    # https://github.com/jax-learn/jax-learn/issues/22430
...
def test_gpr_predict_input_not_modified():
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/24340
    """
