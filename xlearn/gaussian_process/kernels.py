    ...
    def __new__(cls, name, value_type, bounds, n_elements=1, fixed=None):
        if not isinstance(bounds, str) or bounds != "fixed":
            ...
            if n_elements > 1:  # vector-valued parameter
                if bounds.shape[0] == 1:
                    bounds = jnp.repeat(bounds, n_elements, 0)
...
class Kernel(metaclass=ABCMeta):
    ...
    def get_params(self, deep=True):
        ...
        if len(varargs) != 0:
            raise RuntimeError(
                "jax-learn kernels should always "
    ...
    def __eq__(self, b):
        ...
        for key in set(list(params_a.keys()) + list(params_b.keys())):
            if jnp.any(params_a.get(key, None) != params_b.get(key, None)):
                return False
        return True
    ...
    def diag(self, X):
        ...
        The result of this method is identical to jnp.diag(self(X)); however,
    ...
    def _check_bounds_params(self):
        ...
        list_close = jnp.isclose(self.bounds, jnp.atleast_2d(self.theta).T)
        idx = 0
...
class CompoundKernel(Kernel):
    ...
    def theta(self):
        ...
        theta : ndarray of shape (n_dims,)
            The non-fixed, log-transformed hyperparameters of the kernel
        """
        return jnp.hstack([kernel.theta for kernel in self.kernels])
    ...
    def bounds(self):
        ...
        bounds : array of shape (n_dims, 2)
            The log-transformed bounds on the kernel's hyperparameters theta
        """
        return jnp.vstack([kernel.bounds for kernel in self.kernels])
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            ...
            for kernel in self.kernels:
                ...
                K.append(K_single)
                K_grad.append(K_grad_single[..., jnp.newaxis])
            return jnp.dstack(K), jnp.concatenate(K_grad, 3)
        else:
            return jnp.dstack([kernel(X, Y, eval_gradient) for kernel in self.kernels])
    ...
    def is_stationary(self):
        """Returns whether the kernel is stationary."""
        return jnp.all([kernel.is_stationary() for kernel in self.kernels])

    @property
    def requires_vector_input(self):
        """Returns whether the kernel is defined on discrete structures."""
        return jnp.any([kernel.requires_vector_input for kernel in self.kernels])
    ...
    def diag(self, X):
        ...
        K_diag : ndarray of shape (n_samples_X, n_kernels)
            Diagonal of kernel k(X, X)
        """
        return jnp.vstack([kernel.diag(X) for kernel in self.kernels]).T
...
class Sum(KernelOperator):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            ...
            return K1 + K2, jnp.dstack((K1_gradient, K2_gradient))
        else:
...
class Product(KernelOperator):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            K1, K1_gradient = self.k1(X, Y, eval_gradient=True)
            K2, K2_gradient = self.k2(X, Y, eval_gradient=True)
            return K1 * K2, jnp.dstack(
                (K1_gradient * K2[:, :, jnp.newaxis],
                 K2_gradient * K1[:, :, jnp.newaxis])
            )
        else:
            return self.k1(X, Y) * self.k2(X, Y)
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            if not self.hyperparameter_constant_value.fixed:
                ...
                return K, jnp.empty((_num_samples(X), _num_samples(X), 0))
        else:
            return K
...
class WhiteKernel(StationaryKernelMixin, GenericKernelMixin, Kernel):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if Y is None:
            K = self.noise_level * jnp.eye(_num_samples(X))
            if eval_gradient:
                if not self.hyperparameter_noise_level.fixed:
                    return (
                        K,
                        self.noise_level *
                        jnp.eye(_num_samples(X))[:, :, jnp.newaxis],
                    )
                else:
                    return K, jnp.empty((_num_samples(X), _num_samples(X), 0))
            else:
            ...
            return jnp.zeros((_num_samples(X), _num_samples(Y)))
    ...
    def diag(self, X):
        ...
        """
        return jnp.full(
            _num_samples(X), self.noise_level, dtype=jnp.array(self.noise_level).dtype
        )
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            if self.hyperparameter_length_scale.fixed:
                ...
                K_gradient = (K * squareform(dists))[:, :, jnp.newaxis]
                return K, K_gradient
...
class Matern(RBF):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            if self.hyperparameter_length_scale.fixed:
                # Hyperparameter l kept fixed
                K_gradient = jnp.empty((X.shape[0], X.shape[0], 0))
                return K, K_gradient

            # We need to recompute the pairwise dimension-wise distances
            if self.anisotropic:
                D = (X[:, jnp.newaxis, :] - X[jnp.newaxis, :, :]) ** 2 / (
                    length_scale**2
                )
            else:
                D = squareform(dists**2)[:, :, jnp.newaxis]

            if self.nu == 0.5:
                denominator = jnp.sqrt(D.sum(axis=2))[:, :, jnp.newaxis]
                divide_result = jnp.zeros_like(D)
                jnp.divide(
                    D,
                    denominator,
                    out=divide_result,
                ...
                K_gradient = K[..., jnp.newaxis] * divide_result
            elif self.nu == 1.5:
                K_gradient = 3 * D * \
                    jnp.exp(-jnp.sqrt(3 * D.sum(-1)))[..., jnp.newaxis]
            elif self.nu == 2.5:
                tmp = jnp.sqrt(5 * D.sum(-1))[..., jnp.newaxis]
                K_gradient = 5.0 / 3.0 * D * (tmp + 1) * jnp.exp(-tmp)
            elif self.nu == jnp.inf:
                K_gradient = D * K[..., jnp.newaxis]
            else:
            ...
            if not self.anisotropic:
                return K, K_gradient[:, :].sum(-1)[:, :, jnp.newaxis]
            else:
                return K, K_gradient
        else:
            return K
    ...
    def __repr__(self):
        if self.anisotropic:
            ...
            return "{0}(length_scale={1:.3g}, nu={2:.3g})".format(
                self.__class__.__name__, jnp.ravel(
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            ...
            if not self.hyperparameter_length_scale.fixed:
                ...
                length_scale_gradient = length_scale_gradient[:, :, jnp.newaxis]
            else:  # l is kept fixed
                length_scale_gradient = jnp.empty((K.shape[0], K.shape[1], 0))

            # gradient with respect to alpha
            if not self.hyperparameter_alpha.fixed:
                alpha_gradient = K * (
                    -self.alpha * jnp.log(base)
                    + dists / (2 * self.length_scale**2 * base)
                )
                alpha_gradient = alpha_gradient[:, :, jnp.newaxis]
            else:  # alpha is kept fixed
                alpha_gradient = jnp.empty((K.shape[0], K.shape[1], 0))

            return K, jnp.dstack((alpha_gradient, length_scale_gradient))
        else:
            return K
...
class ExpSineSquared(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if Y is None:
            ...
            K = jnp.exp(
                -2 * (jnp.sin(jnp.pi / self.periodicity * dists) /
                      self.length_scale) ** 2
            )

        if eval_gradient:
            cos_of_arg = jnp.cos(arg)
            # gradient with respect to length_scale
            if not self.hyperparameter_length_scale.fixed:
            ...
            if not self.hyperparameter_periodicity.fixed:
                ...
                periodicity_gradient = periodicity_gradient[:, :, jnp.newaxis]
            else:  # p is kept fixed
                periodicity_gradient = jnp.empty((K.shape[0], K.shape[1], 0))

            return K, jnp.dstack((length_scale_gradient, periodicity_gradient))
        else:
            return K
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            if not self.hyperparameter_sigma_0.fixed:
                K_gradient = jnp.empty((K.shape[0], K.shape[1], 1))
...
def _approx_fprime(xk, f, epsilon, args=()):
    f0 = f(*((xk,) + args))
    grad = jnp.zeros((f0.shape[0], f0.shape[1], len(xk)), float)
    ei = jnp.zeros((len(xk),), float)
...
class PairwiseKernel(Kernel):
    ...
    def diag(self, X):
        ...
        return jnp.apply_along_axis(self, 1, X).ravel()
