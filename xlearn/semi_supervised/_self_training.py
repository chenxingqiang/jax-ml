...
class SelfTrainingClassifier(MetaEstimatorMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if jnp.all(has_label):
            warnings.warn("y contains no unlabeled samples", UserWarning)

        if self.criterion == "k_best" and (
            self.k_best > X.shape[0] - jnp.sum(has_label)
        ):
            warnings.warn(
                (
        ...
        self.transduction_ = jnp.copy(y)
        self.labeled_iter_ = jnp.full_like(y, -1)
        self.labeled_iter_[has_label] = 0

        self.n_iter_ = 0

        while not jnp.all(has_label) and (
            ...
            pred = self.base_estimator_.classes_[jnp.argmax(prob, axis=1)]
            max_proba = jnp.max(prob, axis=1)
            ...
            if self.criterion == "threshold":
                ...
                if n_to_select == max_proba.shape[0]:
                    selected = jnp.ones_like(max_proba, dtype=bool)
                else:
                    # NB these are indices, not a mask
                    selected = jnp.argpartition(-max_proba,
                                               n_to_select)[:n_to_select]

            # Map selected indices into original array
            selected_full = jnp.nonzero(~has_label)[0][selected]
