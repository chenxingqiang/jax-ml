...
class BaseLabelPropagation(ClassifierMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def predict(self, X):
        ...
        # See https://github.com/jax-learn/jax-learn/pull/24898
        probas = self.predict_proba(X)
        return self.classes_[jnp.argmax(probas, axis=1)].ravel()
    ...
    def predict_proba(self, X):
        ...
        if self.kernel == "knn":
            probabilities = jnp.array(
                [
                    jnp.sum(self.label_distributions_[weight_matrix], axis=0)
        ...
        normalizer = jnp.atleast_2d(jnp.sum(probabilities, axis=1)).T
    ...
    def fit(self, X, y):
        ...
        self.label_distributions_ = jnp.zeros((n_samples, n_classes))
        for label in classes:
            self.label_distributions_[y == label, classes == label] = 1

        y_static = jnp.copy(self.label_distributions_)
        ...
        l_previous = jnp.zeros((self.X_.shape[0], n_classes))

        unlabeled = unlabeled[:, jnp.newaxis]
        if sparse.isspmatrix(graph_matrix):
            graph_matrix = graph_matrix.tocsr()

        for self.n_iter_ in range(self.max_iter):
            if jnp.abs(self.label_distributions_ - l_previous).sum() < self.tol:
                break
            ...
            if self._variant == "propagation":
                normalizer = jnp.sum(self.label_distributions_, axis=1)[
                    :, jnp.newaxis]
                normalizer[normalizer == 0] = 1
                self.label_distributions_ /= normalizer
                self.label_distributions_ = jnp.where(
        ...
        normalizer = jnp.sum(self.label_distributions_, axis=1)[:, jnp.newaxis]
        normalizer[normalizer == 0] = 1
        self.label_distributions_ /= normalizer

        # set the transduction item
        transduction = self.classes_[
            jnp.argmax(self.label_distributions_, axis=1)]
...
class LabelPropagation(BaseLabelPropagation):
    ...
    def _build_graph(self):
        ...
        if sparse.isspmatrix(affinity_matrix):
            affinity_matrix.data /= jnp.diag(jnp.array(normalizer))
        else:
            affinity_matrix /= normalizer[:, jnp.newaxis]
