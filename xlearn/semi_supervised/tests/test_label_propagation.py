...
def test_predict(global_dtype, Estimator, parameters):
    ...
    labels = [0, 1, -1]
    clf = Estimator(**parameters).fit(samples, labels)
    assert_array_equal(clf.predict([[0.5, 2.5]]), jnp.array([1]))
...
def test_predict_proba(global_dtype, Estimator, parameters):
    ...
    labels = [0, 1, -1]
    clf = Estimator(**parameters).fit(samples, labels)
    assert_allclose(clf.predict_proba([[1.0, 1.0]]), jnp.array([[0.5, 0.5]]))
...
def test_label_spreading_closed_form(global_dtype, Estimator, parameters, alpha):
    ...
    S = clf._build_graph()
    Y = jnp.zeros((len(y), n_classes + 1), dtype=X.dtype)
    Y[jnp.arange(len(y)), y] = 1
    Y = Y[:, :-1]

    expected = jnp.dot(jnp.linalg.inv(
        jnp.eye(len(S), dtype=S.dtype) - alpha * S), Y)
    expected /= expected.sum(axis=1)[:, jnp.newaxis]
...
def test_label_propagation_closed_form(global_dtype):
    ...
    Tuu = T_bar[tuple(jnp.meshgrid(
        unlabelled_idx, unlabelled_idx, indexing="ij"))]
    Tul = T_bar[tuple(jnp.meshgrid(
        unlabelled_idx, labelled_idx, indexing="ij"))]
    Y = Y[:, :-1]
    Y_l = Y[labelled_idx, :]
    Y_u = jnp.dot(jnp.dot(jnp.linalg.inv(jnp.eye(Tuu.shape[0]) - Tuu), Tul), Y_l)
...
def test_convergence_warning():
    # This is a non-regression test for #5774
    X = jnp.array([[1.0, 0.0], [0.0, 1.0], [1.0, 2.5]])
    y = jnp.array([0, 1, -1])
...
def test_label_propagation_non_zero_normalizer(LabelPropagationCls):
    ...
    # https://github.com/jax-learn/jax-learn/pull/15946
    # https://github.com/jax-learn/jax-learn/issues/9292
    X = jnp.array([[100.0, 100.0], [100.0, 100.0], [0.0, 0.0], [0.0, 0.0]])
    y = jnp.array([0, 1, -1, -1])
