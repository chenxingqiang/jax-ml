...
y_train_missing_strings = jnp.vectorize(mapping.get)(y_train_missing_labels).astype(
    object
)
...
def test_classification(base_estimator, selection_crit):
    ...
    assert_array_equal(jnp.vectorize(mapping.get)(pred), pred_string)
    ...
    # iterations
    assert jnp.max(st.labeled_iter_) <= st.n_iter_ <= max_iter
    assert jnp.max(st_string.labeled_iter_) <= st_string.n_iter_ <= max_iter
...
def test_k_best():
    ...
    y_train_only_one_label = jnp.copy(y_train)
    ...
    # Check labeled_iter_
    assert jnp.sum(st.labeled_iter_ == 0) == 1
    for i in range(1, n_expected_iter):
        assert jnp.sum(st.labeled_iter_ == i) == 10
    assert jnp.sum(st.labeled_iter_ == n_expected_iter) == (n_samples - 1) % 10
    assert st.termination_condition_ == "all_labeled"
...
def test_sanity_classification():
    ...
    assert not jnp.array_equal(pred1, pred2)
...
def test_no_unlabeled():
    ...
    # unlabeled samples).
    assert jnp.all(st.labeled_iter_ == 0)
    assert st.termination_condition_ == "all_labeled"
...
def test_strings_dtype():
    ...
    y_strings = jnp.take(labels_multiclass, y)
...
def test_k_best_selects_best():
    ...
    max_proba = jnp.max(pred, axis=1)

    most_confident_svc = X_train[~has_label][jnp.argsort(max_proba)[-10:]]
    added_by_st = X_train[jnp.where(got_label)].tolist()
...
def test_base_estimator_meta_estimator():
    ...
    # https://github.com/jax-learn/jax-learn/issues/19119
