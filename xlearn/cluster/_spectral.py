...
def cluster_qr(vectors):
    ...
    vectors = abs(jnp.dot(vectors, jnp.dot(ut, v.conj())))
...
def discretize(
    ...
    # search easier.
    norm_ones = jnp.sqrt(n_samples)
    for i in range(vectors.shape[1]):
        vectors[:, i] = (
            vectors[:, i] / jnp.linalg.norm(vectors[:, i])) * norm_ones
        if vectors[0, i] != 0:
            vectors[:, i] = -1 * vectors[:, i] * jnp.sign(vectors[0, i])

    # Normalize the rows of the eigenvectors.  Samples should lie on the unit
    # hypersphere centered at the origin.  This transforms the samples in the
    # embedding space to the space of partition matrices.
    vectors = vectors / jnp.sqrt((vectors**2).sum(axis=1))[:, jnp.newaxis]
    ...
    while (svd_restarts < max_svd_restarts) and not has_converged:
        ...
        # eigenvectors
        rotation = jnp.zeros((n_components, n_components))
        rotation[:, 0] = vectors[random_state.randint(n_samples), :].T

        # To initialize the rest of the rotation matrix, find the rows
        # of the eigenvectors that are as orthogonal to each other as
        # possible
        c = jnp.zeros(n_samples)
        for j in range(1, n_components):
            # Accumulate c to ensure row is as orthogonal as possible to
            # previous picks as well as current one
            c += jnp.abs(jnp.dot(vectors, rotation[:, j - 1]))
        ...
        while not has_converged:
            ...
            t_discrete = jnp.dot(vectors, rotation)

            labels = t_discrete.argmax(axis=1)
            vectors_discrete = csc_matrix(
                (jnp.ones(len(labels)), (jnp.arange(0, n_samples), labels)),
...
class SpectralClustering(ClusterMixin, BaseEstimator):
    ...
    affinity : str or callable, default='rbf'
        How to construct the affinity matrix.
         ...
         - one of the kernels supported by
           :func:`~xlearn.metrics.pairwise_kernels`.
    ...
    applying the Gaussian (aka RBF, heat) kernel::
        ...
        jnp.exp(- dist_matrix ** 2 / (2. * delta ** 2))
