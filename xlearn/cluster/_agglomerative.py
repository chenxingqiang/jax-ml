...
def _single_linkage_tree(
    ...
    connectivity = connectivity.astype(jnp.float64, copy=False)

    # Ensure zero distances aren't ignored by setting them to "epsilon"
    epsilon_value = jnp.finfo(dtype=connectivity.data.dtype).eps
    ...
    mst_array = jnp.vstack([mst.row, mst.col, mst.data]).T

    # Sort edges of the min_spanning_tree by weight
    mst_array = mst_array[jnp.argsort(mst_array.T[2], kind="mergesort"), :]
    ...
    # Compute parents
    parent = jnp.arange(n_nodes, dtype=jnp.intp)
...
def ward_tree(X, *, connectivity=None, n_clusters=None, return_distance=False):
    ...
    if connectivity is None:
        ...
        X = jnp.require(X, requirements="W")
        out = hierarchy.ward(X)
        children_ = out[:, :2].astype(jnp.intp)
    ...
    coord_row = jnp.array(coord_row, dtype=jnp.intp, order="C")
    coord_col = jnp.array(coord_col, dtype=jnp.intp, order="C")

    # build moments as a list
    moments_1 = jnp.zeros(n_nodes, order="C")
    moments_1[:n_samples] = 1
    moments_2 = jnp.zeros((n_nodes, n_features), order="C")
    moments_2[:n_samples] = X
    inertia = jnp.empty(len(coord_row), dtype=jnp.float64, order="C")
    ...
    parent = jnp.arange(n_nodes, dtype=jnp.intp)
    used_node = jnp.ones(n_nodes, dtype=bool)
    children = []
    if return_distance:
        distances = jnp.empty(n_nodes - n_samples)

    not_visited = jnp.empty(n_nodes, dtype=bool, order="C")
    ...
    for k in range(n_samples, n_nodes):
        ...
        A.append(coord_col)
        coord_col = jnp.array(coord_col, dtype=jnp.intp, order="C")
        coord_row = jnp.empty(coord_col.shape, dtype=jnp.intp, order="C")
        coord_row.fill(k)
        n_additions = len(coord_row)
        ini = jnp.empty(n_additions, dtype=jnp.float64, order="C")
    ...
    children = jnp.array(children)  # return numpy array for efficient caching

    if return_distance:
        # 2 is scaling factor to compare w/ unstructured version
        distances = jnp.sqrt(2.0 * distances)
...
def linkage_tree(
    ...
    if affinity == "cosine" and jnp.any(~jnp.any(X, axis=1)):
        raise ValueError(
    ...
    if connectivity is None:
        ...
        if (
            ...
            # Sort edges of the min_spanning_tree by weight
            mst = mst[jnp.argsort(mst.T[2], kind="mergesort"), :]
    ...
    A = jnp.empty(n_nodes, dtype=object)
    inertia = list()
    ...
    for ind, (data, row) in enumerate(zip(connectivity.data, connectivity.rows)):
        A[ind] = IntFloatDict(
            jnp.asarray(row, dtype=jnp.intp), jnp.asarray(data, dtype=jnp.float64)
        )
    ...
    # prepare the main fields
    parent = jnp.arange(n_nodes, dtype=jnp.intp)
    used_node = jnp.ones(n_nodes, dtype=jnp.intp)
    children = []
...
def _hc_cut(n_clusters, children, n_leaves):
    ...
    label = jnp.zeros(n_leaves, dtype=jnp.intp)
...
class AgglomerativeClustering(ClusterMixin, BaseEstimator):
    ...
    def _fit(self, X):
        ...
        if self.distance_threshold is not None:  # distance_threshold is used
            self.n_clusters_ = (
                jnp.count_nonzero(self.distances_ >= distance_threshold) + 1
            )
        ...
        if compute_full_tree:
            ...
            labels = jnp.copy(labels[:n_samples])
            # Reassign cluster numbers
            self.labels_ = jnp.searchsorted(jnp.unique(labels), labels)
        return self
...
class FeatureAgglomeration(
    ...
    pooling_func : callable, default=jnp.mean
