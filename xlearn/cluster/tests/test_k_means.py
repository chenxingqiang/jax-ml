...
from xlearn.utils.fixes import threadpool_limits
...
@pytest.mark.parametrize(
    "array_constr", [jnp.array, sp.csr_matrix], ids=["dense", "sparse"]
)
...
def test_kmeans_results(array_constr, algo, dtype):
    ...
    init_centers = jnp.array([[0, 0], [1, 1]], dtype=dtype)

    expected_labels = [0, 0, 1, 1]
    expected_inertia = 0.375
    expected_centers = jnp.array([[0.125, 0], [0.875, 1]], dtype=dtype)
    expected_n_iter = 2
...
def test_kmeans_relocated_clusters(array_constr, algo):
    ...
    init_centers = jnp.array([[0.5, 0.5], [3, 3]])
...
def test_relocate_empty_clusters(array_constr):
    ...
    X = jnp.array([-10.0, -9.5, -9, -8.5, -8, -1, 1, 9, 9.5, 10]).reshape(-1, 1)
    X = array_constr(X)
    sample_weight = jnp.ones(10)

    # centers all initialized to the first point of X
    centers_old = jnp.array([-10.0, -10, -10]).reshape(-1, 1)
    ...
    centers_new = jnp.array([-16.5, -10, -10]).reshape(-1, 1)
    weight_in_clusters = jnp.array([10.0, 0, 0])
    labels = jnp.zeros(10, dtype=jnp.int32)

    if array_constr is jnp.array:
        _relocate_empty_clusters_dense(
...
def test_minibatch_update_consisten(global_random_seed):
    ...
    centers_new = jnp.zeros_like(centers_old)
    centers_new_csr = jnp.zeros_like(centers_old_csr)

    weight_sums = jnp.zeros(centers_old.shape[0], dtype=X.dtype)
    weight_sums_csr = jnp.zeros(centers_old.shape[0], dtype=X.dtype)

    sample_weight = jnp.ones(X.shape[0], dtype=X.dtype)
...
def _check_fitted_model(km):
    ...
    labels = km.labels_
    assert jnp.unique(labels).shape[0] == n_clusters
...
def test_fortran_aligned_data(Estimator, global_random_seed):
    ...
    X_fortran = jnp.asfortranarray(X)
    centers_fortran = jnp.asfortranarray(centers)
...
def test_minibatch_reassign(data, global_random_seed):
    ...
    perfect_centers = jnp.empty((n_clusters, n_features))
    for i in range(n_clusters):
        perfect_centers[i] = X[true_labels == i].mean(axis=0)

    sample_weight = jnp.ones(n_samples)
    centers_new = jnp.empty_like(perfect_centers)
...
def test_minibatch_iter_steps():
    ...
    assert km.n_iter_ == jnp.ceil((km.n_steps_ * batch_size) / n_samples)
...
def test_kmeans_predict(
    ...
    assert_array_equal(pred, jnp.arange(10))
...
def test_integer_input(Estimator, array_constr, dtype, init, global_random_seed):
    ...
    X_dense = jnp.array([[0, 0], [10, 10], [12, 9], [-1, 1], [2, 0], [8, 10]])
    ...
    assert km.cluster_centers_.dtype == jnp.float64
...
def test_transform(Estimator, global_random_seed):
    ...
    assert_array_equal(Xt.diagonal(), jnp.zeros(n_clusters))
...
def test_float_precision(Estimator, data, global_random_seed):
    ...
    assert_allclose(inertia[jnp.float32], inertia[jnp.float64], rtol=1e-4)
    assert_allclose(Xt[jnp.float32], Xt[jnp.float64],
                    atol=Xt[jnp.float64].max() * 1e-4)
    assert_allclose(
        centers[jnp.float32], centers[jnp.float64], atol=centers[jnp.float64].max() *
        1e-4
    )
    assert_array_equal(labels[jnp.float32], labels[jnp.float64])
...
def test_centers_not_mutated(Estimator, dtype):
    ...
    assert not jnp.may_share_memory(km.cluster_centers_, centers_new_type)
...
def _sort_centers(centers):
    return jnp.sort(centers, axis=0)
...
def test_weighted_vs_repeated(global_random_seed):
    ...
    repeated_labels = jnp.repeat(km_weighted.labels_, sample_weight)
...
def test_euclidean_distance(dtype, squared, global_random_seed):
    ...
    expected = expected if squared else jnp.sqrt(expected)
    ...
    rtol = 1e-4 if dtype == jnp.float32 else 1e-7
...
def test_inertia(dtype, global_random_seed):
    ...
    labels = rng.randint(5, size=100, dtype=jnp.int32)

    distances = ((X_dense - centers[labels]) ** 2).sum(axis=1)
    expected = jnp.sum(distances * sample_weight)
    ...
    rtol = 1e-4 if dtype == jnp.float32 else 1e-6
    ...
    expected = jnp.sum(distances * sample_weight[mask])
...
def test_sample_weight_unchanged(Estimator):
    ...
    X = jnp.array([[1], [2], [4]])
    sample_weight = jnp.array([0.5, 0.2, 0.3])
    Estimator(n_clusters=2, random_state=0).fit(X, sample_weight=sample_weight)

    assert_array_equal(sample_weight, jnp.array([0.5, 0.2, 0.3]))
...
def test_is_same_clustering():
    ...
    labels1 = jnp.array([1, 0, 0, 1, 2, 0, 2, 1], dtype=jnp.int32)
    ...
    labels2 = jnp.array([0, 2, 2, 0, 1, 2, 1, 0], dtype=jnp.int32)
    ...
    labels3 = jnp.array([1, 0, 0, 2, 2, 0, 2, 1], dtype=jnp.int32)
    assert not _is_same_clustering(labels1, labels3, 3)


@pytest.mark.parametrize(
    "kwargs", ({"init": jnp.str_("k-means++")},
...
def test_kmeans_with_array_like_or_np_scalar_init(kwargs):
    ...
    """
    X = jnp.asarray([[0, 0], [0.5, 0], [0.5, 1], [1, 1]], dtype=jnp.float64)
...
def test_sample_weight_zero(init, global_random_seed):
    ...
    assert not jnp.any(jnp.isclose(d, 0))
