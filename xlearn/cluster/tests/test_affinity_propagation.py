...
n_clusters = 3
centers = jnp.array([[1, 1], [-1, -1], [1, -1]]) + 10
X, _ = make_blobs(
    n_samples=60,
    n_features=2,
    centers=centers,
...
# For more details, see: https://github.com/jax-learn/jax-learn/issues/11000
...
def test_affinity_propagation_precomputed():
    ...
    n_clusters_ = len(cluster_centers_indices)
    assert jnp.unique(labels).size == n_clusters_
    assert n_clusters == n_clusters_
...
def test_affinity_propagation_no_copy():
    ...
    S_original = S.copy()
    preference = jnp.median(S) * 10
    assert not jnp.allclose(S.diagonal(), preference)
    ...
    assert not jnp.allclose(S.diagonal(), preference)
    assert_allclose(S.diagonal(), jnp.zeros(S.shape[0]))
...
def test_affinity_propagation_fit_non_convergence(global_dtype):
    ...
    # as noise (-1)
    X = jnp.array([[0, 0], [1, 1], [-2, -2]], dtype=global_dtype)
    ...
    with pytest.warns(ConvergenceWarning):
        af.fit(X)
    assert_allclose(jnp.empty((0, 2)), af.cluster_centers_)
    assert_array_equal(jnp.array([-1, -1, -1]), af.labels_)


def test_affinity_propagation_equal_mutual_similarities(global_dtype):
    X = jnp.array([[-1, 1], [1, -1]], dtype=global_dtype)
    S = -euclidean_distances(X, squared=True)
...
def test_affinity_propagation_predict_non_convergence(global_dtype):
    ...
    # clusters
    to_predict = jnp.array([[2, 2], [3, 3], [4, 4]])
    with pytest.warns(ConvergenceWarning):
        y = af.predict(to_predict)
    assert_array_equal(jnp.array([-1, -1, -1]), y)
...
def test_affinity_propagation_non_convergence_regressiontest(global_dtype):
    ...
    assert_array_equal(jnp.array([0, 0, 0]), af.labels_)
...
def test_equal_similarities_and_preferences(global_dtype):
    ...
    assert not _equal_similarities_and_preferences(S, jnp.array(0))
    assert not _equal_similarities_and_preferences(S, jnp.array([0, 0]))
    assert not _equal_similarities_and_preferences(S, jnp.array([0, 1]))

    # Equal distances
    X = jnp.array([[0, 0], [1, 1]], dtype=global_dtype)
    S = -euclidean_distances(X, squared=True)
    ...
    # Same preferences
    assert _equal_similarities_and_preferences(S, jnp.array([0, 0]))
    assert _equal_similarities_and_preferences(S, jnp.array(0))
...
def test_affinity_propagation_random_state():
    ...
    assert jnp.mean((centers0 - centers76) ** 2) > 1


@pytest.mark.parametrize("centers", [csr_matrix(jnp.zeros((1, 10))), jnp.zeros((1, 10))])
def test_affinity_propagation_convergence_warning_dense_sparse(centers, global_dtype):
    ...
    with warnings.catch_warnings():
        ...
        assert_array_equal(ap.predict(X), jnp.zeros(X.shape[0], dtype=int))
