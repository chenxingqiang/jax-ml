...
def test_outlier_data(outlier_type):
    """
    Tests if jnp.inf and jnp.nan data are each treated as special outliers.
    """
    outlier = {
        "infinite": jnp.inf,
        "missing": jnp.nan,
    ...
    prob_check = {
        ...
        "missing": lambda x, y: jnp.isnan(x),
    }[outlier_type]
...
def test_hdbscan_algorithms(algo, metric):
    ...
    metric_params = {
        "mahalanobis": {"V": jnp.eye(X.shape[1])},
        "seuclidean": {"V": jnp.ones(X.shape[1])},
        "minkowski": {"p": 2},
        "wminkowski": {"p": 2, "w": jnp.ones(X.shape[1])},
    }.get(metric, None)
...
def test_dbscan_clustering_outlier_data(cut_distance):
    ...
    X_outlier = X.copy()
    X_outlier[0] = [jnp.inf, 1]
    X_outlier[2] = [1, jnp.nan]
    X_outlier[5] = [jnp.inf, jnp.nan]
    model = HDBSCAN().fit(X_outlier)
    labels = model.dbscan_clustering(cut_distance=cut_distance)

    missing_labels_idx = jnp.flatnonzero(labels == missing_label)
    assert_array_equal(missing_labels_idx, [2, 5])

    infinite_labels_idx = jnp.flatnonzero(labels == infinite_label)
...
def test_hdbscan_high_dimensional():
    ...
    labels = HDBSCAN(
        algorithm="auto",
        metric="seuclidean",
        metric_params={"V": jnp.ones(H.shape[1])},
    ).fit_predict(H)
    n_clusters = len(set(labels) - OUTLIER_SET)
    assert n_clusters == n_clusters_true
...
def test_hdbscan_best_balltree_metric():
    ...
    """
    labels = HDBSCAN(
        metric="seuclidean", metric_params={"V": jnp.ones(X.shape[1])}
    ).fit_predict(X)
    n_clusters = len(set(labels) - OUTLIER_SET)
    assert n_clusters == n_clusters_true
...
def test_hdbscan_min_cluster_size():
    ...
    for min_cluster_size in range(2, len(X), 1):
        ...
        if len(true_labels) != 0:
            assert jnp.min(jnp.bincount(true_labels)) >= min_cluster_size
...
@pytest.mark.parametrize(
    "kwargs, X",
    [
        ({"metric": "precomputed"}, jnp.array([[1, jnp.inf], [jnp.inf, 1]])),
...
def test_hdbscan_sparse_distances_too_few_nonzero():
    ...
    non-zero distances.
    """
    X = sparse.csr_matrix(jnp.zeros((10, 10)))
...
def test_hdbscan_precomputed_dense_nan():
    ...
    distances with `jnp.nan` values.
    """
    X_nan = X.copy()
    X_nan[0, 0] = jnp.nan
    msg = "jnp.nan values found in precomputed-dense"
...
def test_labelling_distinct(global_random_seed, allow_single_cluster, epsilon):
    ...
    first_with_label = {_y: jnp.where(y == _y)[0][0] for _y in list(set(y))}
    y_to_labels = {_y: labels[first_with_label[_y]] for _y in list(set(y))}
    aligned_target = jnp.vectorize(y_to_labels.get)(y)
