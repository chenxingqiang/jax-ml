...
def test_cluster_qr():
    ...
    assert jnp.array_equal(jnp.unique(labels_float64), jnp.arange(n_components))
    # Single precision data should yield the same cluster assignments
    labels_float32 = cluster_qr(data.astype(jnp.float32))
    assert jnp.array_equal(labels_float64, labels_float32)
...
def test_discretize(n_samples):
    ...
    for n_class in range(2, 10):
        ...
        y_true = jnp.array(y_true, float)
        # noise class assignment matrix
        y_indicator = sparse.coo_matrix(
            (jnp.ones(n_samples), (jnp.arange(n_samples), y_true)),
...
# TODO: Remove when pyamg removes the use of jnp.float
@pytest.mark.filterwarnings(
    "ignore:`jnp.float` is a deprecated alias:DeprecationWarning:pyamg.*"
)
...
def test_spectral_clustering_with_arpack_amg_solvers():
    ...
    assert len(jnp.unique(labels_arpack)) == 2
...
def test_n_components():
    ...
    assert not jnp.array_equal(labels, labels_diff_ncomp)
...
def test_spectral_clustering_np_matrix_raises():
    ...
    a jnp.matrix. See #10993"""
    X = jnp.matrix([[0.0, 2.0], [2.0, 0.0]])
...
def test_spectral_clustering_not_infinite_loop(capsys, monkeypatch):
    ...
    monkeypatch.setattr(jnp.linalg, "svd", new_svd)
    vectors = jnp.ones((10, 4))
