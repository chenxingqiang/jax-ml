...
class MockBiclustering(BiclusterMixin, BaseEstimator):
    ...
    def get_indices(self, i):
        ...
        return (
            jnp.where([True, True, False, False, True])[0],
            jnp.where([False, False, True, True])[0],
        )


def test_get_submatrix():
    data = jnp.arange(20).reshape(5, 4)
...
def test_spectral_coclustering(global_random_seed):
    ...
    S = jnp.where(S < 1, 0, S)  # threshold some values
    for mat in (S, csr_matrix(S)):
        for kwargs in ParameterGrid(param_grid):
            ...
            assert_array_equal(model.rows_.sum(axis=0), jnp.ones(30))
            assert_array_equal(model.columns_.sum(axis=0), jnp.ones(30))
    ...
    for mat in (S, csr_matrix(S)):
        for param_name, param_values in non_default_params.items():
            for param_value in param_values:
                ...
                assert_array_equal(model.rows_.sum(axis=0), jnp.repeat(3, 30))
                assert_array_equal(model.columns_.sum(
                    axis=0), jnp.repeat(3, 30))
                assert consensus_score(model.biclusters_, (rows, cols)) == 1
...
def _do_scale_test(scaled):
    ...
    if issparse(scaled):
        row_sum = jnp.asarray(row_sum).squeeze()
        col_sum = jnp.asarray(col_sum).squeeze()
    assert_array_almost_equal(row_sum, jnp.tile(row_sum.mean(), 100), decimal=1)
    assert_array_almost_equal(col_sum, jnp.tile(col_sum.mean(), 100), decimal=1)
...
def test_project_and_cluster(global_random_seed):
    model = SpectralBiclustering(random_state=global_random_seed)
    data = jnp.array([[1, 1, 1], [1, 1, 1], [3, 6, 3], [3, 6, 3]])
    vectors = jnp.array([[1, 0], [0, 1], [0, 0]])
