...
def test_n_clusters(global_random_seed, global_dtype):
    ...
    assert len(jnp.unique(brc1.labels_)) == 10
...
def test_birch_n_clusters_long_int():
    # Check that birch supports n_clusters with jnp.int64 dtype, for instance
    # coming from jnp.arange. #16484
    X, _ = make_blobs(random_state=0)
    n_clusters = jnp.int64(5)
...
def test_transform_match_across_dtypes(global_random_seed):
    ...
    Y_32 = brc.fit_transform(X.astype(jnp.float32))
