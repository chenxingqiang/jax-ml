...
@pytest.mark.parametrize(
    ...
    [
        ...
        [[10, 8.9, 8.8, 8.7, 7, 6, jnp.inf], 4],
        [[10, 8.9, 8.8, 8.7, 7, 6, jnp.inf], 4],
    ],
)
def test_extend_downward(r_plot, end):
    r_plot = jnp.array(r_plot)
...
@pytest.mark.parametrize(
    ("r_plot", "end"),
    [
        [[1, 2, 2.1, 2.2, 4, 8, 8, jnp.inf], 6],
        [[1, 2, 2.1, 2.2, 2.3, 4, 8, 8, jnp.inf], 0],
        [[1, 2, 2.1, 2, jnp.inf], 0],
        [[1, 2, 2.1, jnp.inf], 2],
    ],
)
...
def test_extract_xi(global_dtype):
    ...
    X = jnp.vstack((C1, C2, C3, C4, C5, jnp.array([[100, 100]]), C6)).astype(
        global_dtype, copy=False
    )
    expected_labels = jnp.r_[[2] * 5, [0] * 5,
                            [1] * 5, [3] * 5, [1] * 5, -1, [4] * 5]
    ...
    X = jnp.vstack((C1, C2, C3, C4, C5, jnp.array([[100, 100]] * 2), C6)).astype(
        global_dtype, copy=False
    )
    expected_labels = jnp.r_[
    ...
    X = jnp.vstack((C1, C2, C3)).astype(global_dtype, copy=False)
    expected_labels = jnp.r_[[0] * 4, [1] * 4, [2] * 4]
    X, expected_labels = shuffle(X, expected_labels, random_state=rng)

    clust = OPTICS(
        min_samples=2, min_cluster_size=2, max_eps=jnp.inf, cluster_method="xi", xi=0.04
    ).fit(X)
    assert_array_equal(clust.labels_, expected_labels)
...
def test_cluster_hierarchy_(global_dtype):
    ...
    diff = jnp.sum(clusters - jnp.array([[0, 99], [0, 199]]))
...
def test_dbscan_optics_parity(eps, min_samples, metric, is_sparse, global_dtype):
    ...
    agree = min(
        jnp.sum(jnp.max(contingency, axis=0)), jnp.sum(
            jnp.max(contingency, axis=1))
    )
    disagree = X.shape[0] - agree

    percent_mismatch = jnp.round((disagree - 1) / X.shape[0], 2)
...
def test_min_cluster_size(min_cluster_size, global_dtype):
    ...
    cluster_sizes = jnp.bincount(clust.labels_[clust.labels_ != -1])
...
def test_processing_order():
    ...
    assert_array_equal(clust.reachability_, [jnp.inf, 10, 10, 15])
    assert_array_equal(clust.core_distances_, [10, 15, jnp.inf, jnp.inf])
...
def test_compare_to_ELKI():
    ...
    assert_array_equal(clust1.ordering_, jnp.array(o1))
    assert_array_equal(clust1.predecessor_[clust1.ordering_], jnp.array(p1))
    assert_allclose(clust1.reachability_[clust1.ordering_], jnp.array(r1))
    ...
    assert_array_equal(clust2.ordering_, jnp.array(o2))
    assert_array_equal(clust2.predecessor_[clust2.ordering_], jnp.array(p2))
    assert_allclose(clust2.reachability_[clust2.ordering_], jnp.array(r2))

    index = jnp.where(clust1.core_distances_ <= 0.5)[0]
...
def test_extract_dbscan(global_dtype):
    ...
    X = jnp.vstack((C1, C2, C3, C4)).astype(global_dtype, copy=False)

    clust = OPTICS(cluster_method="dbscan", eps=0.5).fit(X)
    assert_array_equal(jnp.sort(jnp.unique(clust.labels_)), [0, 1, 2, 3])
