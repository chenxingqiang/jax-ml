...
cdef class UnionFind(object):
    ...
    def __init__(self, N):
        self.parent = jnp.full(2 * N - 1, -1., dtype=jnp.intp, order='C')
        self.next_label = N
        self.size = jnp.hstack((jnp.ones(N, dtype=jnp.intp),
                               jnp.zeros(N - 1, dtype=jnp.intp)))
...
def _single_linkage_label(const float64_t[:, :] L):
    ...
    result_arr = jnp.zeros((L.shape[0], 4), dtype=jnp.float64)
...
def single_linkage_label(L):
    ...
    is_sorted = lambda x: jnp.all(x[:-1] <= x[1:])
...
def mst_linkage_core(
    ...
    cdef:
        ...
        uint8_t[:] in_tree = jnp.zeros(n_samples, dtype=bool)
        float64_t[:, ::1] result = jnp.zeros((n_samples - 1, 3))
        ...
        float64_t[:] current_distances = jnp.full(n_samples, INFINITY)
