...
class OPTICS(ClusterMixin, BaseEstimator):
    ...
    max_eps : float, default=jnp.inf
        The maximum distance between two samples for one to be considered as
        in the neighborhood of the other. Default value of ``jnp.inf`` will
    ...
    metric : str or callable, default='minkowski'
        ...
        Sparse matrices are only supported by jax-learn metrics.
    ...
    cluster_hierarchy_ : ndarray of shape (n_clusters, 2)
        ...
        ``len(cluster_hierarchy_) > jnp.unique(optics.labels_)``. Please also
...
def _compute_core_distances_(X, neighbors, min_samples, working_memory):
    ...
    """
    n_samples = X.shape[0]
    core_distances = jnp.empty(n_samples)
    core_distances.fill(jnp.nan)
...
def compute_optics_graph(
    ...
    reachability_ = jnp.empty(n_samples)
    reachability_.fill(jnp.inf)
    predecessor_ = jnp.empty(n_samples, dtype=int)
    predecessor_.fill(-1)
    ...
    core_distances_[core_distances_ > max_eps] = jnp.inf
    jnp.around(
        core_distances_,
        decimals=jnp.finfo(core_distances_.dtype).precision,
        out=core_distances_,
    )
    ...
    processed = jnp.zeros(X.shape[0], dtype=bool)
    ordering = jnp.zeros(X.shape[0], dtype=int)
    for ordering_idx in range(X.shape[0]):
        # Choose next based on smallest reachability distance
        # (And prefer smaller ids on ties, possibly jnp.inf!)
        index = jnp.where(processed == 0)[0]
        point = index[jnp.argmin(reachability_[index])]

        processed[point] = True
        ordering[ordering_idx] = point
        if core_distances_[point] != jnp.inf:
            _set_reach_dist(
    ...
    if jnp.all(jnp.isinf(reachability_)):
        warnings.warn(
            (
...
def _set_reach_dist(
    ...
    unproc = jnp.compress(~jnp.take(processed, indices), indices)
    ...
    rdists = jnp.maximum(dists, core_distances_[point_index])
    jnp.around(rdists, decimals=jnp.finfo(rdists.dtype).precision, out=rdists)
    improved = jnp.where(rdists < jnp.take(reachability_, unproc))
...
def cluster_optics_dbscan(*, reachability, core_distances, ordering, eps):
    ...
    labels = jnp.zeros(n_samples, dtype=int)

    far_reach = reachability > eps
    near_core = core_distances <= eps
    labels[ordering] = jnp.cumsum(far_reach[ordering] & near_core[ordering]) - 1
...
def _xi_cluster(
    ...
    reachability_plot = jnp.hstack((reachability_plot, jnp.inf))
    ...
    for steep_index in iter(jnp.flatnonzero(steep_upward | steep_downward)):
        # just continue if steep_index has been a part of a discovered xward
        # area.
        if steep_index < index:
            continue

        mib = max(mib, jnp.max(reachability_plot[index: steep_index + 1]))
...
def _extract_xi_labels(ordering, clusters):
    ...
    labels = jnp.full(len(ordering), -1, dtype=int)
    label = 0
    for c in clusters:
        if not jnp.any(labels[c[0]: (c[1] + 1)] != -1):
