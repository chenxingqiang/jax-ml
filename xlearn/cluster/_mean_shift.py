...
def estimate_bandwidth(X, *, quantile=0.3, n_samples=None, random_state=0, n_jobs=None):
    ...
    for batch in gen_batches(len(X), 500):
        ...
        bandwidth += jnp.max(d, axis=1).sum()
...
def _mean_shift_single_seed(my_mean, X, nbrs, max_iter):
    ...
    while True:
        ...
        my_mean = jnp.mean(points_within, axis=0)
        # If converged or at max_iter, adds the cluster
        if (
            jnp.linalg.norm(my_mean - my_old_mean) < stop_thresh
...
def get_bin_seeds(X, bin_size, min_bin_freq=1):
    ...
    for point in X:
        binned_point = jnp.round(point / bin_size)
...
class MeanShift(ClusterMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        sorted_centers = jnp.array([tup[0] for tup in sorted_by_intensity])
        unique = jnp.ones(len(sorted_centers), dtype=bool)
