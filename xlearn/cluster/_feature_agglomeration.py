...
class AgglomerationTransform(TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.pooling_func == jnp.mean and not issparse(X):
            size = jnp.bincount(self.labels_)
            n_samples = X.shape[0]
            # a fast way to compute the mean of grouped features
            nX = jnp.array(
                [jnp.bincount(self.labels_, X[i, :]) / size for i in range(n_samples)]
            )
        else:
            nX = [
                self.pooling_func(X[:, self.labels_ == l], axis=1)
                for l in jnp.unique(self.labels_)
            ]
            nX = jnp.array(nX).T
        return nX
    ...
    def inverse_transform(self, Xt=None, Xred=None):
        ...
        unil, inverse = jnp.unique(self.labels_, return_inverse=True)
        return Xt[..., inverse]
