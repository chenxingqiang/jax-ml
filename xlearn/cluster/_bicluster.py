...
def _scale_normalize(X):
    ...
    row_diag = jnp.asarray(1.0 / jnp.sqrt(X.sum(axis=1))).squeeze()
    col_diag = jnp.asarray(1.0 / jnp.sqrt(X.sum(axis=0))).squeeze()
    row_diag = jnp.where(jnp.isnan(row_diag), 0, row_diag)
    col_diag = jnp.where(jnp.isnan(col_diag), 0, col_diag)
    if issparse(X):
        ...
        an = row_diag[:, jnp.newaxis] * X * col_diag
...
def _log_normalize(X):
    ...
    L = jnp.log(X)
    row_avg = L.mean(axis=1)[:, jnp.newaxis]
...
class BaseSpectral(BiclusterMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    _parameter_constraints: dict = {
        ...
        "mini_batch": ["boolean"],
        "init": [StrOptions({"k-means++", "random"}), jnp.ndarray],
    ...
    def fit(self, X, y=None):
        ...
        """
        X = self._validate_data(X, accept_sparse="csr", dtype=jnp.float64)
    ...
    def _svd(self, array, n_components, n_discard):
        ...
        if self.svd_method == "randomized":
            ...
            if jnp.any(jnp.isnan(vt)):
                # some eigenvalues of A * A.T are negative, causing
                # sqrt() to be jnp.nan. This causes some vectors in vt
                # to be jnp.nan.
...
class SpectralCoclustering(BaseSpectral):
    ...
    * :doi:`Dhillon, Inderjit S, 2001. Co-clustering documents and words using
    ...
    def _fit(self, X):
        ...
        n_sv = 1 + int(jnp.ceil(jnp.log2(self.n_clusters)))
        u, v = self._svd(normalized_data, n_sv, n_discard=1)
        z = jnp.vstack((row_diag[:, jnp.newaxis] * u,
                      col_diag[:, jnp.newaxis] * v))
...
class SpectralBiclustering(BaseSpectral):
    ...
    * :doi:`Kluger, Yuval, et. al., 2003. Spectral biclustering of microarray
    ...
    def _fit_best_piecewise(self, vectors, n_best, n_clusters):
        ...
        piecewise_vectors = jnp.apply_along_axis(
            make_piecewise, axis=1, arr=vectors)
        dists = jnp.apply_along_axis(
            norm, axis=1, arr=(vectors - piecewise_vectors))
        result = vectors[jnp.argsort(dists)[:n_best]]
        return result
