...
#          Jan Schlueter <jax-learn@jan-schlueter.de>
#          Nelle Varoquaux
...
def kmeans_plusplus(
    ...
    *,
    ...
    """
    # Check data
    check_array(X, accept_sparse="csr", dtype=[jnp.float64, jnp.float32])
...
def _kmeans_plusplus(
    ...
    centers = jnp.empty((n_clusters, n_features), dtype=X.dtype)
    ...
    closest_dist_sq = _euclidean_distances(
        centers[0, jnp.newaxis], X, Y_norm_squared=x_squared_norms, squared=True
    )
    ...
    for c in range(1, n_clusters):
        ...
        jnp.clip(candidate_ids, None, closest_dist_sq.size - 1, out=candidate_ids)
        ...
        jnp.minimum(closest_dist_sq, distance_to_candidates,
...
def _tolerance(X, tol):
    ...
    if sp.issparse(X):
        ...
        variances = jnp.var(X, axis=0)
    return jnp.mean(variances) * tol
...
def k_means(
    ...
    *,
    ...
    algorithm : {"lloyd", "elkan", "auto", "full"}, default="lloyd"
        ...
        Jax-Learn 1.3. They are both aliases for `"lloyd"`.
...
def _kmeans_single_elkan(
    ...
    labels = jnp.full(n_samples, -1, dtype=jnp.int32)
    labels_old = labels.copy()
    center_half_distances = euclidean_distances(centers) / 2
    distance_next_center = jnp.partition(
        jnp.asarray(center_half_distances), kth=1, axis=0
    )[1]
    upper_bounds = jnp.zeros(n_samples, dtype=X.dtype)
    lower_bounds = jnp.zeros((n_samples, n_clusters), dtype=X.dtype)
    ...
    for i in range(max_iter):
        ...
        if jnp.array_equal(labels, labels_old):
...
def _labels_inertia(X, sample_weight, centers, n_threads=1, return_inertia=True):
    ...
    labels = jnp.full(n_samples, -1, dtype=jnp.int32)
    center_shift = jnp.zeros(n_clusters, dtype=centers.dtype)
...
class _BaseKMeans(
    ...
    def _check_mkl_vcomp(self, X, n_samples):
        ...
        n_active_threads = int(jnp.ceil(n_samples / CHUNK_SIZE))
...
def _mini_batch_step(
    ...
    if random_reassign and reassignment_ratio > 0:
        ...
        if to_reassign.sum() > 0.5 * X.shape[0]:
            indices_dont_reassign = jnp.argsort(
        ...
        if n_reassigns:
            ...
            if sp.issparse(X):
                assign_rows_csr(
                    X,
                    new_centers.astype(jnp.intp, copy=False),
                    jnp.where(to_reassign)[0].astype(jnp.intp, copy=False),
                    centers_new,
                )
            else:
        ...
        weight_sums[to_reassign] = jnp.min(weight_sums[~to_reassign])
...
class MiniBatchKMeans(_BaseKMeans):
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        centers_new = jnp.empty_like(centers)

        # Initialize counts
        self._counts = jnp.zeros(self.n_clusters, dtype=X.dtype)
        ...
        with threadpool_limits(limits=1, user_api="blas"):
            ...
            for i in range(n_steps):
                ...
                if self._tol > 0.0:
                    centers_squared_diff = jnp.sum((centers_new - centers) ** 2)
                else:
        ...
        self.n_steps_ = i + 1
        self.n_iter_ = int(jnp.ceil(((i + 1) * self._batch_size) / n_samples))
