...
def _equal_similarities_and_preferences(S, preference):
    def all_equal_preferences():
        return jnp.all(preference == preference.flat[0])

    def all_equal_similarities():
        # Create mask to ignore diagonal of S
        mask = jnp.ones(S.shape, dtype=bool)
        jnp.fill_diagonal(mask, 0)

        return jnp.all(S[mask].flat == S[mask].flat[0])
    ...
    *,
    ...
    if n_samples == 1 or _equal_similarities_and_preferences(S, preference):
        ...
        if preference.flat[0] >= S.flat[n_samples - 1]:
            return (
                (jnp.arange(n_samples), jnp.arange(n_samples), 0)
                if return_n_iter
                else (jnp.arange(n_samples), jnp.arange(n_samples))
            )
        else:
            return (
                (jnp.array([0]), jnp.array([0] * n_samples), 0)
                if return_n_iter
                else (jnp.array([0]), jnp.array([0] * n_samples))
            )

    # Place preference on the diagonal of S
    S.flat[:: (n_samples + 1)] = preference

    A = jnp.zeros((n_samples, n_samples))
    R = jnp.zeros((n_samples, n_samples))  # Initialize messages
    # Intermediate results
    tmp = jnp.zeros((n_samples, n_samples))

    # Remove degeneracies
    S += (
        jnp.finfo(S.dtype).eps * S + jnp.finfo(S.dtype).tiny * 100
    ) * random_state.standard_normal(size=(n_samples, n_samples))

    # Execute parallel affinity propagation updates
    e = jnp.zeros((n_samples, convergence_iter))

    ind = jnp.arange(n_samples)

    for it in range(max_iter):
        # tmp = A + S; compute responsibilities
        jnp.add(A, S, tmp)
        I = jnp.argmax(tmp, axis=1)
        Y = tmp[ind, I]  # jnp.max(A + S, axis=1)
        tmp[ind, I] = -jnp.inf
        Y2 = jnp.max(tmp, axis=1)

        # tmp = Rnew
        jnp.subtract(S, Y[:, None], tmp)
        ...
        # tmp = -Anew
        tmp -= jnp.sum(tmp, axis=0)
        dA = jnp.diag(tmp).copy()
        tmp.clip(0, jnp.inf, tmp)
    ...
    if K > 0:
        ...
        c = jnp.argmax(S[:, I], axis=1)
        c[I] = jnp.arange(K)  # Identify clusters
        # Refine the final set of exemplars and clusters and return results
        for k in range(K):
            ii = jnp.where(c == k)[0]
            j = jnp.argmax(jnp.sum(S[ii[:, jnp.newaxis], ii], axis=0))
            I[k] = ii[j]
        ...
        cluster_centers_indices = jnp.unique(labels)
        labels = jnp.searchsorted(cluster_centers_indices, labels)
    else:
        warnings.warn(
            (
...
class AffinityPropagation(ClusterMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        if self.preference is None:
            preference = jnp.median(self.affinity_matrix_)
        else:
            preference = self.preference
        preference = jnp.array(preference, copy=False)
