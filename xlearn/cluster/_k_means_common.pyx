...
cpdef void _relocate_empty_clusters_dense(
    ...
    cdef:
        int[::1] empty_clusters = jnp.where(jnp.equal(weight_in_clusters, 0))[0].astype(jnp.int32)
    ...
    cdef:
        ...
        floating[::1] distances = ((jnp.asarray(X) - jnp.asarray(centers_old)[labels])**2).sum(axis=1)
        int[::1] far_from_centers = jnp.argpartition(distances, -n_empty)[:-n_empty-1:-1].astype(jnp.int32)
...
cpdef void _relocate_empty_clusters_sparse(
    ...
    cdef:
        ...
        floating[::1] distances = jnp.zeros(n_samples, dtype=X_data.base.dtype)
...
def _is_same_clustering(
    ...
    cdef int[::1] mapping = jnp.full(fill_value=-1, shape=(n_clusters,), dtype=jnp.int32)
    cdef int i
