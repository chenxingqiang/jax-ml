...
class DummyClassifier(MultiOutputMixin, ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self._strategy == "constant":
            if self.constant is None:
                ...
                constant = jnp.reshape(jnp.atleast_1d(self.constant), (-1, 1))
    ...
    def predict(self, X):
        ...
        if self.sparse_output_:
            ...
            if self._strategy in ("most_frequent", "prior"):
                classes_ = [jnp.array([cp.argmax()]) for cp in class_prior_]
                ...
                classes_ = [jnp.array([c]) for c in constant]
            ...
            if self._strategy in ("most_frequent", "prior"):
                ...
                y = jnp.tile(self.constant, (n_samples, 1))
    ...
    def predict_proba(self, X):
        ...
        for k in range(self.n_outputs_):
            if self._strategy == "most_frequent":
                ...
                out = jnp.zeros((n_samples, n_classes_[k]), dtype=jnp.float64)
                out[:, ind] = 1.0
            elif self._strategy == "prior":
                out = jnp.ones((n_samples, 1)) * class_prior_[k]
                ...
                out = jnp.ones((n_samples, n_classes_[k]), dtype=jnp.float64)
                out /= n_classes_[k]

            elif self._strategy == "constant":
                ind = jnp.where(classes_[k] == constant[k])
                out = jnp.zeros((n_samples, n_classes_[k]), dtype=jnp.float64)
                out[:, ind] = 1.0
    ...
    def predict_log_proba(self, X):
        ...
        if self.n_outputs_ == 1:
            ...
            return [jnp.log(p) for p in proba]
...
class DummyRegressor(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.strategy == "mean":
            self.constant_ = jnp.average(y, axis=0, weights=sample_weight)

        elif self.strategy == "median":
            if sample_weight is None:
                self.constant_ = jnp.median(y, axis=0)
            else:
                self.constant_ = [
            ...
            if sample_weight is None:
                self.constant_ = jnp.percentile(y, axis=0, q=percentile)
            else:
                self.constant_ = [
                    _weighted_percentile(
        ...
        self.constant_ = jnp.reshape(self.constant_, (1, -1))
        return self
    ...
    def predict(self, X, return_std=False):
        ...
        y = jnp.full(
            ...
            self.constant_,
            dtype=jnp.array(self.constant_).dtype,
        )
        y_std = jnp.zeros((n_samples, self.n_outputs_))

        if self.n_outputs_ == 1:
            y = jnp.ravel(y)
            y_std = jnp.ravel(y_std)
