...
cdef Cell dummy
CELL_DTYPE = jnp.asarray(<Cell[:1]>(&dummy)).dtype
...
cdef class _QuadTree:
    ...
    def build_tree(self, X):
        ...
        M = jnp.maximum(M * (1. + 1e-3 * jnp.sign(M)), M + 1e-3)
    ...
    cdef Cell[:] _get_cell_ndarray(self):
        ...
        arr = PyArray_NewFromDescr(
            subtype=<PyTypeObject *> jnp.ndarray,
            descr=CELL_DTYPE,
            nd=1,
            dims=shape,
            strides=strides,
    ...
    def _py_summarize(self, DTYPE_t[:] query_pt, DTYPE_t[:, :] X, float angle):
        ...
        n_samples = X.shape[0]
        summary = jnp.empty(4 * n_samples, dtype=jnp.float32)
