...
@pytest.mark.filterwarnings("ignore:Support for distance metrics:FutureWarning:xlearn")
def test_iris():
    # Check consistency on dataset iris.
    for metric in ("euclidean", "cosine"):
        clf = NearestCentroid(metric=metric).fit(iris.data, iris.target)
        score = jnp.mean(clf.predict(iris.data) == iris.target)
...
def test_shrinkage_correct():
    ...
    X = jnp.array([[0, 1], [1, 0], [1, 1], [2, 0], [6, 8]])
    y = jnp.array([1, 1, 2, 2, 2])
    clf = NearestCentroid(shrink_threshold=0.1)
    clf.fit(X, y)
    expected_result = jnp.array([[0.7787310, 0.8545292], [2.814179, 2.763647]])
    jnp.testing.assert_array_almost_equal(clf.centroids_, expected_result)
