...
def test_finite_differences(global_random_seed):
    ...
    nca = NeighborhoodComponentsAnalysis()
    nca.n_iter_ = 0
    mask = y[:, jnp.newaxis] == y[jnp.newaxis, :]
...
def test_transformation_dimensions():
    ...
    transformation = jnp.array([[1, 2], [3, 4]])
    ...
    transformation = jnp.array([[1, 2], [3, 4], [5, 6]])
    # len(transformation) > len(transformation[0])
    with pytest.raises(ValueError):
        NeighborhoodComponentsAnalysis(init=transformation).fit(X, y)

    # Pass otherwise
    transformation = jnp.arange(9).reshape(3, 3)
    NeighborhoodComponentsAnalysis(init=transformation).fit(X, y)
...
def test_warm_start_effectiveness():
    ...
    diff_warm = jnp.sum(
        jnp.abs(transformation_warm_plus_one - transformation_warm))
    diff_cold = jnp.sum(
        jnp.abs(transformation_cold_plus_one - transformation_cold))
...
def test_singleton_class():
    ...
    # one singleton class
    singleton_class = 1
    (ind_singleton,) = jnp.where(y == singleton_class)
    y[ind_singleton] = 2
...
@pytest.mark.parametrize(
    "param, value",
    [
        ("n_components", jnp.int32(3)),
        ("max_iter", jnp.int32(100)),
        ("tol", jnp.float32(0.0001)),
    ],
)
