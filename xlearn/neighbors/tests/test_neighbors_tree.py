...
def compute_kernel_slow(Y, X, kernel, h):
    d = jnp.sqrt(((Y[:, None, :] - X) ** 2).sum(-1))
    norm = kernel_norm(h, X.shape[1], kernel)

    if kernel == "gaussian":
        return norm * jnp.exp(-0.5 * (d * d) / (h * h)).sum(-1)
    elif kernel == "tophat":
        return norm * (d < h).sum(-1)
    elif kernel == "epanechnikov":
        return norm * ((1.0 - (d * d) / (h * h)) * (d < h)).sum(-1)
    elif kernel == "exponential":
        return norm * (jnp.exp(-d / h)).sum(-1)
    elif kernel == "linear":
        return norm * ((1 - d / h) * (d < h)).sum(-1)
    elif kernel == "cosine":
        return norm * (jnp.cos(0.5 * jnp.pi * d / h) * (d < h)).sum(-1)
    else:
...
def brute_force_neighbors(X, Y, k, metric, **kwargs):
    ...
    ind = jnp.argsort(D, axis=1)[:, :k]
    dist = D[jnp.arange(Y.shape[0])[:, None], ind]
    return dist, ind
...
def test_neighbor_tree_query_radius(Cls, n_samples=100, n_features=10):
    ...
    query_pt = jnp.zeros(n_features, dtype=float)

    eps = 1e-15  # roundoff error can cause test to fail
    tree = Cls(X, leaf_size=5)
    rad = jnp.sqrt(((X - query_pt) ** 2).sum(1))

    for r in jnp.linspace(rad[0], rad[-1], 100):
        ind = tree.query_radius([query_pt], r + eps)[0]
        i = jnp.where(rad <= r + eps)[0]
...
def test_neighbor_tree_query_radius_distance(Cls, n_samples=100, n_features=10):
    ...
    for r in jnp.linspace(rad[0], rad[-1], 100):
        ...
        d = jnp.sqrt(((query_pt - X[ind]) ** 2).sum(1))
...
def test_neighbors_heap(NeighborsHeap, n_pts=5, n_nbrs=10):
    ...
    for row in range(n_pts):
        d_in = rng.random_sample(2 * n_nbrs).astype(jnp.float64, copy=False)
        i_in = jnp.arange(2 * n_nbrs, dtype=jnp.intp)
        for d, i in zip(d_in, i_in):
            heap.push(row, d, i)

        ind = jnp.argsort(d_in)
        d_in = d_in[ind]
        i_in = i_in[ind]
...
def test_node_heap(nodeheap_sort, n_nodes=50):
    rng = check_random_state(0)
    vals = rng.random_sample(n_nodes).astype(jnp.float64, copy=False)
...
def test_simultaneous_sort(simultaneous_sort, n_rows=10, n_pts=201):
    rng = check_random_state(0)
    dist = rng.random_sample((n_rows, n_pts)).astype(jnp.float64, copy=False)
    ind = (jnp.arange(n_pts) + jnp.zeros((n_rows, 1))
           ).astype(jnp.intp, copy=False)
    ...
    i = jnp.argsort(dist2, axis=1)
    row_ind = jnp.arange(n_rows)[:, None]
...
def test_gaussian_kde(Cls, n_samples=1000):
    ...
    x_out = jnp.linspace(-5, 5, 30)

    for h in [0.01, 0.1, 1]:
        tree = Cls(x_in[:, None])
        gkde = gaussian_kde(x_in, bw_method=h / jnp.std(x_in))
