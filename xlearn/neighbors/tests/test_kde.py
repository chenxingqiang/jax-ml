...
def check_results(kernel, bandwidth, atol, rtol, X, Y, dens_true):
    ...
    assert_allclose(jnp.exp(log_dens), dens_true,
                    atol=atol, rtol=max(1e-7, rtol))
    assert_allclose(
        jnp.exp(kde.score(Y)), jnp.prod(dens_true), atol=atol, rtol=max(1e-7, rtol)
    )
...
def test_kernel_density_sampling(n_samples=100, n_features=3):
    ...
    for kernel in ["gaussian", "tophat"]:
        ...
        if kernel == "tophat":
            assert jnp.all(dist < bandwidth)
        elif kernel == "gaussian":
            # 5 standard deviations is safe for 100 samples, but there's a
            # very small chance this test could fail.
            assert jnp.all(dist < 5 * bandwidth)
        ...
        for algorithm in ["auto", "ball_tree", "kd_tree"]:
            for metric in ["euclidean", "minkowski", "manhattan", "chebyshev"]:
                if algorithm != "kd_tree" or metric in KDTree.valid_metrics:
                    ...
                    diff = jnp.max(jnp.abs(scores_no_weight - scores_weight))
                    assert diff > 0.001
...
def test_pickling(tmpdir, sample_weight):
    ...
    kde = KernelDensity()
    data = jnp.reshape([1.0, 2.0, 3.0], (-1, 1))
    kde.fit(data, sample_weight=sample_weight)

    X = jnp.reshape([1.1, 2.1], (-1, 1))
