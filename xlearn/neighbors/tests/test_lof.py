...
def test_lof(global_dtype):
    ...
    assert jnp.min(score[:-2]) > jnp.max(score[-2:])
...
def test_lof_performance(global_dtype):
    ...
    X_test = jnp.r_[X[100:], X_outliers]
    y_test = jnp.array([0] * 20 + [1] * 20)
...
def test_lof_values(global_dtype):
    # toy samples:
    X_train = jnp.asarray([[1, 1], [1, 2], [2, 1]], dtype=global_dtype)
...
def test_score_samples(global_dtype):
    X_train = jnp.asarray([[1, 1], [1, 2], [2, 1]], dtype=global_dtype)
    X_test = jnp.asarray([[2.0, 2.0]], dtype=global_dtype)
    clf1 = neighbors.LocalOutlierFactor(
        n_neighbors=2, contamination=0.1, novelty=True
    ).fit(X_train)
    clf2 = neighbors.LocalOutlierFactor(
