...
def test_quadtree_boundary_computation():
    ...
    # check a random case
    Xs.append(jnp.array([[-1, 1], [-4, -1]], dtype=jnp.float32))
    # check the case where only 0 are inserted
    Xs.append(jnp.array([[0, 0], [0, 0]], dtype=jnp.float32))
    # check the case where only negative are inserted
    Xs.append(jnp.array([[-1, -2], [-4, 0]], dtype=jnp.float32))
    # check the case where only small numbers are inserted
    Xs.append(jnp.array([[-1e-6, 1e-6], [-4e-6, -1e-6]], dtype=jnp.float32))
...
def test_quadtree_similar_point():
    ...
    Xs.append(jnp.array([[1, 2], [3, 4]], dtype=jnp.float32))
    # check the case where points are the same on X axis
    Xs.append(jnp.array([[1.0, 2.0], [1.0, 3.0]], dtype=jnp.float32))
    # check the case where points are arbitrarily close on X axis
    Xs.append(jnp.array([[1.00001, 2.0], [1.00002, 3.0]], dtype=jnp.float32))
    # check the case where points are the same on Y axis
    Xs.append(jnp.array([[1.0, 2.0], [3.0, 2.0]], dtype=jnp.float32))
    # check the case where points are arbitrarily close on Y axis
    Xs.append(jnp.array([[1.0, 2.00001], [3.0, 2.00002]], dtype=jnp.float32))
    # check the case where points are arbitrarily close on both axes
    Xs.append(
        jnp.array([[1.00001, 2.00001], [1.00002, 2.00002]], dtype=jnp.float32))

    # check the case where points are arbitrarily close on both axes
    # close to machine epsilon - x axis
    Xs.append(
        jnp.array([[1, 0.0003817754041], [2, 0.0003817753750]], dtype=jnp.float32))

    # check the case where points are arbitrarily close on both axes
    # close to machine epsilon - y axis
    Xs.append(
        jnp.array([[0.0003817754041, 1.0], [0.0003817753750, 2.0]],
                 dtype=jnp.float32)
    )
...
def test_summarize():
    ...
    angle = 0.9
    X = jnp.array(
        [[-10.0, -10.0], [9.0, 10.0], [10.0, 9.0], [10.0, 10.0]], dtype=jnp.float32
    )
    query_pt = X[0, :]
    ...
    assert jnp.isclose(node_dist, ds2c)
