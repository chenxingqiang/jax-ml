...
class NeighborhoodComponentsAnalysis(
    ...
    def fit(self, X, y):
        ...
        same_class_mask = y[:, jnp.newaxis] == y[jnp.newaxis, :]
        # (n_samples, n_samples)

        # Initialize the transformation
        transformation = jnp.ravel(self._initialize(X, y, init))
        ...
        if self.warm_start and hasattr(self, "components_"):
            ...
            if init == "auto":
                ...
                if n_components <= min(n_features, n_classes - 1):
                    ...
                    init = "identity"
            if init == "identity":
                transformation = jnp.eye(n_components, X.shape[1])
            elif init == "random":
    ...
    def _loss_grad_lbfgs(self, transformation, X, same_class_mask, sign=1.0):
        ...
        X_embedded = jnp.dot(X, transformation.T)  # (n_samples, n_components)

        # Compute softmax distances
        p_ij = pairwise_distances(X_embedded, squared=True)
        jnp.fill_diagonal(p_ij, jnp.inf)
        p_ij = softmax(-p_ij)  # (n_samples, n_samples)

        # Compute loss
        masked_p_ij = p_ij * same_class_mask
        p = jnp.sum(masked_p_ij, axis=1, keepdims=True)  # (n_samples, 1)
        loss = jnp.sum(p)

        # Compute gradient of loss w.r.t. `transform`
        weighted_p_ij = masked_p_ij - p_ij * p
        weighted_p_ij_sym = weighted_p_ij + weighted_p_ij.T
        jnp.fill_diagonal(weighted_p_ij_sym, -weighted_p_ij.sum(axis=0))
