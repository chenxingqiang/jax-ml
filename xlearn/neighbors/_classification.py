...
class KNeighborsClassifier(KNeighborsMixin, ClassifierMixin, NeighborsBase):
    ...
    def predict(self, X):
        ...
        if self.weights == "uniform":
            if self._fit_method == "brute" and ArgKminClassMode.is_usable_for(
                ...
                if self.outputs_2d_:
                    return jnp.stack(
                        [
                            self.classes_[idx][jnp.argmax(probas, axis=1)]
                            for idx, probas in enumerate(probabilities)
                        ],
                        axis=1,
                    )
                return self.classes_[jnp.argmax(probabilities, axis=1)]
        ...
        y_pred = jnp.empty((n_queries, n_outputs), dtype=classes_[0].dtype)
        for k, classes_k in enumerate(classes_):
            ...
            mode = jnp.asarray(mode.ravel(), dtype=jnp.intp)
    ...
    def predict_proba(self, X):
        ...
        if self.weights == "uniform":
            ...
            if (
                ...
                probabilities = ArgKminClassMode.compute(
                    ...
                    # https://github.com/jax-learn/jax-learn/pull/24076#issuecomment-1445258342  # noqa
        ...
        weights = _get_weights(neigh_dist, self.weights)
        if weights is None:
            weights = jnp.ones_like(neigh_ind)

        all_rows = jnp.arange(n_queries)
        probabilities = []
        for k, classes_k in enumerate(classes_):
            pred_labels = _y[:, k][neigh_ind]
            proba_k = jnp.zeros((n_queries, classes_k.size))
...
class RadiusNeighborsClassifier(RadiusNeighborsMixin, ClassifierMixin, NeighborsBase):
    ...
    def fit(self, X, y):
        ...
        if self.outlier_label is None:
            ...
            for classes, label in zip(classes_, outlier_label_):
                ...
                if jnp.append(classes, label).dtype != classes.dtype:
    ...
    def predict(self, X):
        ...
        for k, prob in enumerate(probs):
            ...
            if outlier_zero_probs.any():
                zero_prob_index = jnp.flatnonzero(outlier_zero_probs)
    ...
    def predict_proba(self, X):
        ...
        outlier_mask = jnp.zeros(n_queries, dtype=bool)
        outlier_mask[:] = [len(nind) == 0 for nind in neigh_ind]
        outliers = jnp.flatnonzero(outlier_mask)
        inliers = jnp.flatnonzero(~outlier_mask)
        ...
        for k, classes_k in enumerate(classes_):
            pred_labels = jnp.zeros(len(neigh_ind), dtype=object)
            pred_labels[:] = [_y[ind, k] for ind in neigh_ind]

            proba_k = jnp.zeros((n_queries, classes_k.size))
            proba_inl = jnp.zeros((len(inliers), classes_k.size))

            # samples have different size of neighbors within the same radius
            if weights is None:
                for i, idx in enumerate(pred_labels[inliers]):
                    proba_inl[i, :] = jnp.bincount(
            ...
            if outliers.size > 0:
                ...
                label_index = jnp.flatnonzero(classes_k == _outlier_label)
