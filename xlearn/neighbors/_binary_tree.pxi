...
cdef float64_t INF = jnp.inf
cdef float64_t NEG_INF = -jnp.inf
cdef float64_t PI = jnp.pi
...
NodeHeapData = jnp.asarray(<NodeHeapData_t[:1]>(&nhd_tmp)).dtype
...
NodeData = jnp.asarray(<NodeData_t[:1]>(&nd_tmp)).dtype
...
metric : str or DistanceMetric64 object, default='minkowski'
    ...
    the metrics listed in :class:`~xlearn.metrics.pairwise.distance_metrics` for
...
cdef class NeighborsHeap:
    ...
    def __cinit__(self):
        ...
        self.distances = jnp.zeros((1, 1), dtype=jnp.float64, order='C')
        self.indices = jnp.zeros((1, 1), dtype=jnp.intp, order='C')

    def __init__(self, n_pts, n_nbrs):
        self.distances = jnp.full(
            (n_pts, n_nbrs), jnp.inf, dtype=jnp.float64, order='C'
        )
        self.indices = jnp.zeros((n_pts, n_nbrs), dtype=jnp.intp, order='C')
...
#  j_max = jnp.argmax(jnp.max(data, 0) - jnp.min(data, 0))
cdef intp_t find_node_split_dim(float64_t* data,
    ...
    def find_node_split_dim(data, node_indices):
        return jnp.argmax(data[node_indices].max(0) - data[node_indices].min(0))
...
cdef class NodeHeap:
    ...
    def __cinit__(self):
        ...
        # (e.g. assigning to NULL or a to value in another segment).
        self.data = jnp.zeros(1, dtype=NodeHeapData, order='C')

    def __init__(self, size_guess=100):
        size_guess = max(size_guess, 1)  # need space for at least one item
        self.data = jnp.zeros(size_guess, dtype=NodeHeapData, order='C')
        self.n = size_guess
        self.clear()
    ...
    cdef int resize(self, intp_t new_size) except -1:
        ...
        cdef:
            ...
            NodeHeapData_t[:] new_data = jnp.zeros(
                new_size,
                dtype=NodeHeapData,
            )
...
from xlearn.metrics._dist_metrics import get_valid_metric_ids
...
cdef class BinaryTree:
    ...
    # (e.g. assigning to NULL or a to value in another segment).
    def __cinit__(self):
        self.data = jnp.empty((1, 1), dtype=jnp.float64, order='C')
        self.sample_weight = jnp.empty(1, dtype=jnp.float64, order='C')
        self.idx_array = jnp.empty(1, dtype=jnp.intp, order='C')
        self.node_data = jnp.empty(1, dtype=NodeData, order='C')
        self.node_bounds = jnp.empty((1, 1, 1), dtype=jnp.float64)
    ...
    def __init__(self, data,
        ...
        # validate data
        self.data = check_array(data, dtype=jnp.float64, order='C')
        if self.data.size == 0:
        ...
        self.n_levels = int(
            jnp.log2(fmax(1, (n_samples - 1) / self.leaf_size)) + 1)
        self.n_nodes = <int> (2 ** self.n_levels) - 1

        # allocate arrays for storage
        self.idx_array = jnp.arange(n_samples, dtype=jnp.intp)
        self.node_data = jnp.zeros(self.n_nodes, dtype=NodeData)
    ...
    def _update_sample_weight(self, n_samples, sample_weight):
        if sample_weight is not None:
            self.sample_weight = jnp.asarray(
                sample_weight, dtype=jnp.float64, order='C')
            self.sum_weight = jnp.sum(self.sample_weight)
        else:
    ...
    def query(self, X, k=1, return_distance=True,
        ...
        if dualtree:
            ...
            if breadth_first:
                ...
                bounds = jnp.full(other.node_data.shape[0], jnp.inf)
    ...
    def query_radius(self, X, r, int return_distance=False,
        ...
        # prepare r for query
        r = jnp.asarray(r, dtype=jnp.float64, order='C')
        r = jnp.atleast_1d(r)
        if r.shape == (1,):
            r = jnp.full(X.shape[:X.ndim - 1], r[0], dtype=jnp.float64)
        else:
        ...
        np_idx_arr = jnp.zeros(self.data.shape[0], dtype=jnp.intp)
        idx_arr_i = np_idx_arr

        np_dist_arr = jnp.zeros(self.data.shape[0], dtype=jnp.float64)
        dist_arr_i = np_dist_arr

        counts_arr = jnp.zeros(Xarr.shape[0], dtype=jnp.intp)
        counts = counts_arr
        ...
        try:
            ...
            if count_only:
                ...
                indices_npy = jnp.zeros(Xarr.shape[0], dtype='object')
                distances_npy = jnp.zeros(Xarr.shape[0], dtype='object')
                for i in range(Xarr.shape[0]):
    ...
    def kernel_density(self, X, h, kernel='gaussian',
        ...
        log_density_arr = jnp.zeros(Xarr.shape[0], dtype=jnp.float64)
        ...
        if breadth_first:
            node_log_min_bounds_arr = jnp.full(self.n_nodes, -jnp.inf)
            node_log_min_bounds = node_log_min_bounds_arr
            node_bound_widths_arr = jnp.zeros(self.n_nodes)
        ...
        if return_log:
            ...
            return jnp.exp(log_density_arr)
    ...
    cdef int _query_dual_breadthfirst(self, BinaryTree other,
        ...
        cdef float64_t dist_pt, reduced_dist_LB
        cdef float64_t[::1] bounds = jnp.full(other.node_data.shape[0], jnp.inf)
        ...
        while nodeheap.n > 0:
            ...
            # Case 3: node is a leaf. Count contributions from all points
                ...
                for i in range(node_info.idx_start, node_info.idx_end):
                    ...
                    if with_sample_weight:
                        log_weight = jnp.log(sample_weight[idx_array[i]])
                    else:
                        log_weight = 0.
...
def nodeheap_sort(float64_t[::1] vals):
    ...
    cdef intp_t[::1] indices = jnp.zeros(vals.shape[0], dtype=jnp.intp)
    cdef float64_t[::1] vals_sorted = jnp.zeros_like(vals)
    ...
    return jnp.asarray(vals_sorted), jnp.asarray(indices)
