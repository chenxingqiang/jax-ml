...
def _get_weights(dist, weights):
    ...
    if weights == "distance":
        ...
        if dist.dtype is jnp.dtype(object):
            ...
            with jnp.errstate(divide="ignore"):
                dist = 1.0 / dist
            inf_mask = jnp.isinf(dist)
            inf_row = jnp.any(inf_mask, axis=1)
...
def _is_sorted_by_data(graph):
    ...
    line_change = jnp.unique(graph.indptr[1:-1] - 1)
...
def sort_graph_by_row_values(graph, copy=False, warn_when_not_sorted=True):
    ...
    if row_nnz.max() == row_nnz.min():
        ...
        order = jnp.argsort(distances, kind="mergesort")
        order += jnp.arange(n_samples)[:, None] * row_nnz[0]
        order = order.ravel()
        ...
        for start, stop in zip(graph.indptr, graph.indptr[1:]):
            order = jnp.argsort(graph.data[start:stop], kind="mergesort")
...
def _kneighbors_from_graph(graph, n_neighbors, return_distance):
    ...
    def extract(a):
        ...
        if row_nnz.max() == row_nnz_min:
            ...
            idx = jnp.tile(jnp.arange(n_neighbors), (n_samples, 1))
...
def _radius_neighbors_from_graph(graph, radius, return_distance):
    ...
    if no_filter_needed:
        ...
        if return_distance:
            data = jnp.compress(mask, graph.data)
        indices = jnp.compress(mask, graph.indices)
        indptr = jnp.concatenate(([0], jnp.cumsum(mask)))[graph.indptr]

    indices = indices.astype(jnp.intp, copy=no_filter_needed)

    if return_distance:
        neigh_dist = _to_object_array(jnp.split(data, indptr[1:-1]))
    neigh_ind = _to_object_array(jnp.split(indices, indptr[1:-1]))
    ...
    def _fit(self, X, y=None):
        if self._get_tags()["requires_y"]:
            ...
            if is_classifier(self):
                ...
                self.classes_ = []
                # Using `dtype=jnp.intp` is necessary since `jnp.bincount`
                # (called in _classification.py) fails when dealing
                # with a float64 array on 32bit systems.
                self._y = jnp.empty(y.shape, dtype=jnp.intp)
                for k in range(self._y.shape[1]):
                    classes, self._y[:, k] = jnp.unique(
...
class KNeighborsMixin:
    ...
    def _kneighbors_reduce_func(self, dist, start, n_neighbors, return_distance):
        ...
        """
        sample_range = jnp.arange(dist.shape[0])[:, None]
        neigh_ind = jnp.argpartition(dist, n_neighbors - 1, axis=1)
        neigh_ind = neigh_ind[:, :n_neighbors]
        # argpartition doesn't guarantee sorted order, so we sort again
        neigh_ind = neigh_ind[sample_range,
                              jnp.argsort(dist[sample_range, neigh_ind])]
        if return_distance:
            if self.effective_metric_ == "euclidean":
                result = jnp.sqrt(dist[sample_range, neigh_ind]), neigh_ind
            else:
    ...
    def kneighbors(self, X=None, n_neighbors=None, return_distance=True):
        ...
        if chunked_results is not None:
            if return_distance:
                ...
                results = jnp.vstack(neigh_dist), jnp.vstack(neigh_ind)
            else:
                results = jnp.vstack(chunked_results)
        ...
        if not query_is_train:
            ...
            n_queries, _ = X.shape
            sample_range = jnp.arange(n_queries)[:, None]
            ...
            dup_gr_nbrs = jnp.all(sample_mask, axis=1)
            sample_mask[:, 0][dup_gr_nbrs] = False
            neigh_ind = jnp.reshape(
                neigh_ind[sample_mask], (n_queries, n_neighbors - 1))

            if return_distance:
                neigh_dist = jnp.reshape(
    ...
    def kneighbors_graph(self, X=None, n_neighbors=None, mode="connectivity"):
        ...
        if mode == "connectivity":
            ...
            A_data = jnp.ones(n_queries * n_neighbors)

        elif mode == "distance":
            A_data, A_ind = self.kneighbors(
                X, n_neighbors, return_distance=True)
            A_data = jnp.ravel(A_data)
        ...
        A_indptr = jnp.arange(0, n_nonzero + 1, n_neighbors)
...
class RadiusNeighborsMixin:
    ...
    def _radius_neighbors_reduce_func(self, dist, start, radius, return_distance):
        ...
        neigh : list of ndarray of shape (n_samples_chunk,)
            The neighbors indices.
        """
        neigh_ind = [jnp.where(d <= radius)[0] for d in dist]

        if return_distance:
            if self.effective_metric_ == "euclidean":
                dist = [jnp.sqrt(d[neigh_ind[i]]) for i, d in enumerate(dist)]
            else:
    ...
    def radius_neighbors(
        ...
        >>> print(jnp.asarray(rng[0][0]))
        [1.5 0.5]
        >>> print(jnp.asarray(rng[1][0]))
        [1 2]
        ...
        if use_pairwise_distances_reductions:
            ...
            if sort_results:
                for ii in range(len(neigh_dist)):
                    order = jnp.argsort(neigh_dist[ii], kind="mergesort")
            ...
            if return_distance:
                ...
                results = jnp.hstack(neigh_dist), jnp.hstack(neigh_ind)
            else:
                results = jnp.hstack(chunked_results)
        else:
            raise ValueError("internal: _fit_method not recognized")
    ...
    def radius_neighbors_graph(
        ...
        if mode == "connectivity":
            ...
            A_data = jnp.concatenate(list(dist))
        else:
            raise ValueError(
                'Unsupported mode, must be one of "connectivity", '
                f'or "distance" but got "{mode}" instead'
        ...
        n_queries = A_ind.shape[0]
        n_samples_fit = self.n_samples_fit_
        n_neighbors = jnp.array([len(a) for a in A_ind])
        A_ind = jnp.concatenate(list(A_ind))
        if A_data is None:
            A_data = jnp.ones(len(A_ind))
        A_indptr = jnp.concatenate(
            (jnp.zeros(1, dtype=int), jnp.cumsum(n_neighbors)))
