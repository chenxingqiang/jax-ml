...
from xlearn.metrics.pairwise import _VALID_METRICS
...
class NearestCentroid(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        self.centroids_ = jnp.empty((n_classes, n_features), dtype=jnp.float64)
        # Number of clusters in each class.
        nk = jnp.zeros(n_classes)

        for cur_class in range(n_classes):
            center_mask = y_ind == cur_class
            nk[cur_class] = jnp.sum(center_mask)
            if is_X_sparse:
                center_mask = jnp.where(center_mask)[0]

            if self.metric == "manhattan":
                # NumPy does not calculate median of sparse matrices.
                if not is_X_sparse:
                    self.centroids_[cur_class] = jnp.median(
        ...
        if self.shrink_threshold:
            if jnp.all(jnp.ptp(X, axis=0) == 0):
                raise ValueError(
                    "All features have zero variance. Division by zero.")
            dataset_centroid_ = jnp.mean(X, axis=0)

            # m parameter for determining deviation
            m = jnp.sqrt((1.0 / nk) - (1.0 / n_samples))
            # Calculate deviation using the standard deviation of centroids.
            variance = (X - self.centroids_[y_ind]) ** 2
            variance = variance.sum(axis=0)
            s = jnp.sqrt(variance / (n_samples - n_classes))
            s += jnp.median(s)  # To deter outliers from affecting the results.
            mm = m.reshape(len(m), 1)  # Reshape to allow broadcasting.
            ms = mm * s
            deviation = (self.centroids_ - dataset_centroid_) / ms
            # Soft thresholding: if the deviation crosses 0 during shrinking,
            # it becomes zero.
            signs = jnp.sign(deviation)
            deviation = jnp.abs(deviation) - self.shrink_threshold
            jnp.clip(deviation, 0, None, out=deviation)
            deviation *= signs
            # Now adjust the centroids using the deviation
            msd = ms * deviation
            self.centroids_ = dataset_centroid_[jnp.newaxis, :] + msd
        return self
