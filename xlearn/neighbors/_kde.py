...
class KernelDensity(BaseEstimator):
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        X = self._validate_data(X, order="C", dtype=jnp.float64)

        if sample_weight is not None:
            sample_weight = _check_sample_weight(
                sample_weight, X, dtype=jnp.float64, only_non_negative=True
            )
    ...
    def score_samples(self, X):
        ...
        X = self._validate_data(X, order="C", dtype=jnp.float64, reset=False)
    ...
    def score(self, X, y=None):
        ...
        logprob : float
            ...
            data.
        """
        return jnp.sum(self.score_samples(X))
    ...
    def sample(self, n_samples=1, random_state=None):
        ...
        data = jnp.asarray(self.tree_.data)

        rng = check_random_state(random_state)
        u = rng.uniform(0, 1, size=n_samples)
        if self.tree_.sample_weight is None:
            i = (u * data.shape[0]).astype(jnp.int64)
        else:
            cumsum_weight = jnp.cumsum(jnp.asarray(self.tree_.sample_weight))
            sum_weight = cumsum_weight[-1]
            i = jnp.searchsorted(cumsum_weight, u * sum_weight)
        if self.kernel == "gaussian":
            return jnp.atleast_2d(rng.normal(data[i], self.bandwidth_))
            ...
            return data[i] + X * correction[:, jnp.newaxis]
