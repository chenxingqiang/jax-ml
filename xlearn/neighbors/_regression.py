...
class KNeighborsRegressor(KNeighborsMixin, RegressorMixin, NeighborsBase):
    ...
    def predict(self, X):
        ...
        if weights is None:
            y_pred = jnp.mean(_y[neigh_ind], axis=1)
        else:
            y_pred = jnp.empty(
                (neigh_dist.shape[0], _y.shape[1]), dtype=jnp.float64)
            denom = jnp.sum(weights, axis=1)

            for j in range(_y.shape[1]):
                num = jnp.sum(_y[neigh_ind, j] * weights, axis=1)
                y_pred[:, j] = num / denom
...
class RadiusNeighborsRegressor(RadiusNeighborsMixin, RegressorMixin, NeighborsBase):
    ...
    def predict(self, X):
        ...
        empty_obs = jnp.full_like(_y[0], jnp.nan)

        if weights is None:
            y_pred = jnp.array(
                [
                    jnp.mean(_y[ind, :], axis=0) if len(ind) else empty_obs
            ...
            y_pred = jnp.array(
                [
                    (
                        jnp.average(_y[ind, :], axis=0, weights=weights[i])
                        if len(ind)
                        else empty_obs
                    )
                    for (i, ind) in enumerate(neigh_ind)
