...
class LocalOutlierFactor(KNeighborsMixin, OutlierMixin, NeighborsBase):
    ...
    p : int, default=2
        ...
        :func:`xlearn.metrics.pairwise.pairwise_distances`. When p = 1, this
    ...
    def fit(self, X, y=None):
        ...
        if self._fit_X.dtype == jnp.float32:
        ...
        lrd_ratios_array = (
            self._lrd[_neighbors_indices_fit_X_] / self._lrd[:, jnp.newaxis]
        )

        self.negative_outlier_factor_ = -jnp.mean(lrd_ratios_array, axis=1)

        if self.contamination == "auto":
            # inliers score around -1 (the higher, the less abnormal).
            self.offset_ = -1.5
        else:
            self.offset_ = jnp.percentile(
    ...
    def _predict(self, X=None):
        ...
        if X is not None:
            ...
            is_inlier = jnp.ones(X.shape[0], dtype=int)
            is_inlier[self.decision_function(X) < 0] = -1
        else:
            is_inlier = jnp.ones(self.n_samples_fit_, dtype=int)
    ...
    def score_samples(self, X):
        ...
        **Only available for novelty detection (when novelty is set to True).**
        ...
        # as bigger is better:
        return -jnp.mean(lrd_ratios_array, axis=1)
    ...
    def _local_reachability_density(self, distances_X, neighbors_indices):
        ...
        reach_dist_array = jnp.maximum(distances_X, dist_k)

        # 1e-10 to avoid `nan' when nb of duplicates > n_neighbors_:
        return 1.0 / (jnp.mean(reach_dist_array, axis=1) + 1e-10)
