...
def _pinv2_old(a):
    ...
    factor = {"f": 1e3, "d": 1e6}
    cond = jnp.max(s) * factor[t] * jnp.finfo(t).eps
    rank = jnp.sum(s > cond)

    u = u[:, :rank]
    u /= s[:rank]
    return jnp.transpose(jnp.conjugate(jnp.dot(u, vh[:rank])))
...
def _get_first_singular_vectors_power_method(
    ...
    try:
        y_score = next(col for col in Y.T if jnp.any(jnp.abs(col) > eps))
    ...
    for i in range(max_iter):
        if mode == "B":
            x_weights = jnp.dot(X_pinv, y_score)
        else:
            x_weights = jnp.dot(X.T, y_score) / jnp.dot(y_score, y_score)

        x_weights /= jnp.sqrt(jnp.dot(x_weights, x_weights)) + eps
        x_score = jnp.dot(X, x_weights)

        if mode == "B":
            y_weights = jnp.dot(Y_pinv, x_score)
        else:
            y_weights = jnp.dot(Y.T, x_score) / jnp.dot(x_score.T, x_score)

        if norm_y_weights:
            y_weights /= jnp.sqrt(jnp.dot(y_weights, y_weights)) + eps

        y_score = jnp.dot(Y, y_weights) / (jnp.dot(y_weights, y_weights) + eps)

        x_weights_diff = x_weights - x_weights_old
        if jnp.dot(x_weights_diff, x_weights_diff) < tol or Y.shape[1] == 1:
            break
...
def _center_scale_xy(X, Y, scale=True):
    ...
    if scale:
        ...
        x_std = jnp.ones(X.shape[1])
        y_std = jnp.ones(Y.shape[1])
...
def _svd_flip_1d(u, v):
    ...
    biggest_abs_val_idx = jnp.argmax(jnp.abs(u))
    sign = jnp.sign(u[biggest_abs_val_idx])
    u *= sign
    v *= sign
...
class _PLS(
    ...
    def fit(self, X, Y):
        ...
        X = self._validate_data(
            X, dtype=jnp.float64, copy=self.copy, ensure_min_samples=2
        )
        Y = check_array(
            Y, input_name="Y", dtype=jnp.float64, copy=self.copy, ensure_2d=False
        )
        if Y.ndim == 1:
            Y = Y.reshape(-1, 1)
        ...
        self.x_weights_ = jnp.zeros((p, n_components))  # U
        self.y_weights_ = jnp.zeros((q, n_components))  # V
        self._x_scores = jnp.zeros((n, n_components))  # Xi
        self._y_scores = jnp.zeros((n, n_components))  # Omega
        self.x_loadings_ = jnp.zeros((p, n_components))  # Gamma
        self.y_loadings_ = jnp.zeros((q, n_components))  # Delta
        self.n_iter_ = []

        # This whole thing corresponds to the algorithm in section 4.1 of the
        # review from Wegelin. See above for a notation mapping from code to
        # paper.
        Y_eps = jnp.finfo(Yk.dtype).eps
        for k in range(n_components):
            # Find first left and right singular vectors of the X.T.dot(Y)
            # cross-covariance matrix.
            if self.algorithm == "nipals":
                # Replace columns that are all close to zero with zeros
                Yk_mask = jnp.all(jnp.abs(Yk) < 10 * Y_eps, axis=0)
                Yk[:, Yk_mask] = 0.0
            ...
            x_scores = jnp.dot(Xk, x_weights)
            if norm_y_weights:
                y_ss = 1
            else:
                y_ss = jnp.dot(y_weights, y_weights)
            y_scores = jnp.dot(Yk, y_weights) / y_ss

            # Deflation: subtract rank-one approx to obtain Xk+1 and Yk+1
            x_loadings = jnp.dot(x_scores, Xk) / jnp.dot(x_scores, x_scores)
            Xk -= jnp.outer(x_scores, x_loadings)

            if self.deflation_mode == "canonical":
                # regress Yk on y_score
                y_loadings = jnp.dot(y_scores, Yk) / jnp.dot(y_scores, y_scores)
                Yk -= jnp.outer(y_scores, y_loadings)
            if self.deflation_mode == "regression":
                # regress Yk on x_score
                y_loadings = jnp.dot(x_scores, Yk) / jnp.dot(x_scores, x_scores)
                Yk -= jnp.outer(x_scores, y_loadings)
        ...
        self.x_rotations_ = jnp.dot(
            self.x_weights_,
            pinv2(jnp.dot(self.x_loadings_.T, self.x_weights_), check_finite=False),
        )
        self.y_rotations_ = jnp.dot(
            self.y_weights_,
            pinv2(jnp.dot(self.y_loadings_.T, self.y_weights_), check_finite=False),
        )
        self.coef_ = jnp.dot(self.x_rotations_, self.y_loadings_.T)
    ...
    def transform(self, X, Y=None, copy=True):
        ...
        X /= self._x_std
        # Apply rotation
        x_scores = jnp.dot(X, self.x_rotations_)
        if Y is not None:
            Y = check_array(
            ...
            Y -= self._y_mean
            Y /= self._y_std
            y_scores = jnp.dot(Y, self.y_rotations_)
    ...
    def inverse_transform(self, X, Y=None):
        ...
        X_reconstructed = jnp.matmul(X, self.x_loadings_.T)
        # Denormalize
        ...
        if Y is not None:
            ...
            # From pls space to original space
            Y_reconstructed = jnp.matmul(Y, self.y_loadings_.T)
            # Denormalize
...
class PLSSVD(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def transform(self, X, Y=None):
        ...
        """
        check_is_fitted(self)
        X = self._validate_data(X, dtype=jnp.float64, reset=False)
        Xr = (X - self._x_mean) / self._x_std
        x_scores = jnp.dot(Xr, self.x_weights_)
        if Y is not None:
            Y = check_array(Y, input_name="Y",
                            ensure_2d=False, dtype=jnp.float64)
            if Y.ndim == 1:
                Y = Y.reshape(-1, 1)
            Yr = (Y - self._y_mean) / self._y_std
            y_scores = jnp.dot(Yr, self.y_weights_)
            return x_scores, y_scores
        return x_scores
