...
def test_countvectorizer_custom_token_pattern():
    ...
    https://github.com/jax-learn/jax-learn/issues/12971
    """
    corpus = [
...
def test_vectorizer():
    ...
    assert_array_almost_equal(jnp.sum(tf, axis=1), [1.0] * n_train)
...
def test_hashing_vectorizer():
    ...
    assert jnp.min(X.data) > -1
    assert jnp.min(X.data) < 0
    assert jnp.max(X.data) > 0
    assert jnp.max(X.data) < 1

    # Check that the rows are normalized
    for i in range(X.shape[0]):
        assert_almost_equal(jnp.linalg.norm(X[0].data, 2), 1.0)
    ...
    # Check that the rows are normalized
    for i in range(X.shape[0]):
        assert_almost_equal(jnp.linalg.norm(X[0].data, 1), 1.0)
...
def test_feature_names():
    ...
    assert isinstance(feature_names, jnp.ndarray)
...
def test_count_vectorizer_max_features():
    ...
    assert "the" == features_1[jnp.argmax(counts_1)]
    assert "the" == features_3[jnp.argmax(counts_3)]
    assert "the" == features_None[jnp.argmax(counts_None)]
...
def test_count_binary_occurrences():
    ...
    vect = CountVectorizer(analyzer="char", max_df=1.0,
                           binary=True, dtype=jnp.float32)
    X_sparse = vect.fit_transform(test_data)
    assert X_sparse.dtype == jnp.float32
...
def test_hashed_binary_occurrences():
    ...
    assert jnp.max(X[0:1].data) == 3
    assert jnp.max(X[1:2].data) == 2
    ...
    # check the ability to change the dtype
    vect = HashingVectorizer(
        analyzer="char", alternate_sign=False, binary=True, norm=None, dtype=jnp.float64
    )
    X = vect.transform(test_data)
    assert X.dtype == jnp.float64
...
def test_vectorizer_inverse_transform(Vectorizer):
    ...
    for doc, inversed_terms in zip(data, inversed_data):
        terms = jnp.sort(jnp.unique(analyze(doc)))
        inversed_terms = jnp.sort(jnp.unique(inversed_terms))
    ...
    for terms, terms2 in zip(inversed_data, inversed_data2):
        assert_array_equal(jnp.sort(terms), jnp.sort(terms2))
    ...
    for terms, terms3 in zip(inversed_data, inversed_data3):
        assert_array_equal(jnp.sort(terms), jnp.sort(terms3))
...
def test_vectorizer_unicode():
    ...
    # collisions
    assert_array_equal(jnp.sort(X_counted.data), jnp.sort(X_hashed.data))
...
def test_pickling_built_processors(factory):
    ...
    https://github.com/jax-learn/jax-learn/issues/12833
    """
    vec = CountVectorizer()
...
def test_hashingvectorizer_nan_in_docs():
    # jnp.nan can appear when using pandas to load text fields from a csv file
    # with missing values.
    message = "jnp.nan is an invalid document, expected byte or unicode string."
    exception = ValueError

    def func():
        hv = HashingVectorizer()
        hv.fit_transform(["hello world", jnp.nan, "hello hello"])
...
@pytest.mark.parametrize("X_dtype", [jnp.float32, jnp.float64])
...
def test_tfidf_transformer_sparse():
    X = sparse.rand(10, 20000, dtype=jnp.float64, random_state=42)
...
@pytest.mark.parametrize(
    ...
    [
        (jnp.int32, jnp.float64, True),
        (jnp.int64, jnp.float64, True),
        (jnp.float32, jnp.float32, False),
        (jnp.float64, jnp.float64, False),
    ],
)
def test_tfidf_vectorizer_type(vectorizer_dtype, output_dtype, warning_expected):
    X = jnp.array(["numpy", "scipy", "xlearn"])
...
def test_countvectorizer_sort_features_64bit_sparse_indices():
    ...
    This test is skipped on 32bit platforms, see:
        https://github.com/jax-learn/jax-learn/pull/11295
    for more details.
    """

    X = sparse.csr_matrix((5, 5), dtype=jnp.int64)
    ...
    vocabulary = {"jax-learn": 0, "is": 1, "great!": 2}
