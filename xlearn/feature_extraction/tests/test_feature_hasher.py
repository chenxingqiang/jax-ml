...
def test_feature_hasher_pairs():
    ...
    x1_nz = sorted(jnp.abs(x1[x1 != 0]))
    x2_nz = sorted(jnp.abs(x2[x2 != 0]))
    assert [1, 2] == x1_nz
    assert [1, 3, 4] == x2_nz
...
def test_feature_hasher_pairs_with_string_values():
    ...
    x1, x2 = feature_hasher.transform(raw_X).toarray()
    x1_nz = jnp.abs(x1[x1 != 0])
    x2_nz = jnp.abs(x2[x2 != 0])
    assert [1] == x1_nz
    assert [1] == x2_nz
...
def test_hash_empty_input():
    ...
    assert_array_equal(X.A, jnp.zeros((len(raw_X), n_features)))
