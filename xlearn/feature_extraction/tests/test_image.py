...
def test_img_to_graph_sparse():
    ...
    desired = jnp.array([[1, 0, 0], [0, -1, 1], [0, 1, -2]])
    jnp.testing.assert_array_equal(grad_x, desired)
...
def test_grid_to_graph():
    ...
    mask = jnp.zeros((size, size), dtype=bool)
    mask[0:roi_size, 0:roi_size] = True
    mask[-roi_size:, -roi_size:] = True
    mask = mask.reshape(size**2)
    A = grid_to_graph(n_x=size, n_y=size, mask=mask, return_as=jnp.ndarray)
    ...
    desired = jnp.array([[1, 0, 0], [0, 1, 1], [0, 1, 1]])
    jnp.testing.assert_array_equal(graph, desired)

    # Checking that the function works whatever the type of mask is
    mask = jnp.ones((size, size), dtype=jnp.int16)
...
def orange_face(downsampled_face):
    ...
    face_color = jnp.zeros(face.shape + (3,))
...
def test_reconstruct_patches_perfect(downsampled_face):
    ...
    jnp.testing.assert_array_almost_equal(face, face_reconstructed)
...
def test_extract_patches_strided():
    ...
    for image_shape, patch_size, patch_step, expected_view, last_patch in zip(
        ...
        image = jnp.arange(jnp.prod(image_shape)).reshape(image_shape)
