"""
The :mod:`xlearn.feature_extraction.image` submodule gathers utilities to
...
def _make_edges_3d(n_x, n_y, n_z=1):
    ...
    """
    vertices = jnp.arange(n_x * n_y * n_z).reshape((n_x, n_y, n_z))
    edges_deep = jnp.vstack(
        (vertices[:, :, :-1].ravel(), vertices[:, :, 1:].ravel()))
    edges_right = jnp.vstack(
        (vertices[:, :-1].ravel(), vertices[:, 1:].ravel()))
    edges_down = jnp.vstack((vertices[:-1].ravel(), vertices[1:].ravel()))
    edges = jnp.hstack((edges_deep, edges_right, edges_down))
    return edges
...
def _mask_edges_weights(mask, edges, weights=None):
    ...
    inds = jnp.arange(mask.size)
    inds = inds[mask.ravel()]
    ind_mask = jnp.logical_and(jnp.in1d(edges[0], inds), jnp.in1d(edges[1], inds))
    ...
    if len(edges.ravel()):
        ...
        maxval = 0
    order = jnp.searchsorted(jnp.flatnonzero(mask), jnp.arange(maxval + 1))
    edges = order[edges]
    if weights is None:
        return edges
    else:
...
def _to_graph(
    ...
    if img is not None:
        ...
        if mask is not None:
            ...
            n_voxels = jnp.sum(mask)
        else:
            n_voxels = n_x * n_y * n_z
        weights = jnp.ones(edges.shape[1], dtype=dtype)
        diag = jnp.ones(n_voxels, dtype=dtype)

    diag_idx = jnp.arange(n_voxels)
    i_idx = jnp.hstack((edges[0], edges[1]))
    j_idx = jnp.hstack((edges[1], edges[0]))
    graph = sparse.coo_matrix(
        (
            jnp.hstack((weights, weights, diag)),
            (jnp.hstack((i_idx, diag_idx)), jnp.hstack((j_idx, diag_idx))),
        ),
        (n_voxels, n_voxels),
        dtype=dtype,
    )
    if return_as is jnp.ndarray:
...
def img_to_graph(img, *, mask=None, return_as=sparse.coo_matrix, dtype=None):
    ...
    mask : ndarray of shape (height, width) or \
        ...
        pixels.
    return_as : jnp.ndarray or a sparse matrix class, \
    ...
    Notes
    -----
    For jax-learn versions 0.14.1 and prior, return_as=jnp.ndarray was
    handled by returning a dense jnp.matrix instance.  Going forward, jnp.ndarray
    returns an jnp.ndarray, as expected.

    For compatibility, user code relying on this method should wrap its
    calls in ``jnp.asarray`` to avoid type issues.
    """
    img = jnp.atleast_3d(img)
...
def grid_to_graph(
    ...
    Returns
    -------
    graph : jnp.ndarray or a sparse matrix class
        The computed adjacency matrix.
...
def _extract_patches(arr, patch_shape=8, extraction_step=1):
    ...
    patch_indices_shape = (
        (jnp.array(arr.shape) - jnp.array(patch_shape)) // jnp.array(extraction_step)
    ) + 1
...
@validate_params(
    {"patches": [jnp.ndarray], "image_size": [tuple, Hidden(list)]},
    prefer_skip_nested_validation=True,
)
...
class PatchExtractor(TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        ...
        n_imgs, img_height, img_width = X.shape[:3]
        X = jnp.reshape(X, (n_imgs, img_height, img_width, -1))
        ...
        # extract the patches
        patches = jnp.empty(patches_shape)
