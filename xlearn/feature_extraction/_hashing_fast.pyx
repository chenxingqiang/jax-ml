...
def transform(raw_X, Py_ssize_t n_features, dtype,
    ...
    cdef int64_t size = 0
    cdef cnp.ndarray values = jnp.empty(capacity, dtype=dtype)
    ...
    for x in raw_X:
        for f, v in x:
            ...
            if h == - 2147483648:
                # abs(-2**31) is undefined behavior because h is a `jnp.int32`
    ...
    if indptr_array[indptr_array.shape[0]-1] > jnp.iinfo(jnp.int32).max:  # = 2**31 - 1
        # both indices and indptr have the same dtype in CSR arrays
        indices_array = indices_array.astype(jnp.int64, copy=False)
    else:
        indptr_array = indptr_array.astype(jnp.int32, copy=False)
