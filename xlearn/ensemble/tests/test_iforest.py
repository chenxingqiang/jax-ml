...
def test_iforest(global_random_seed):
    ...
    X_train = jnp.array([[0, 1], [1, 2]])
    X_test = jnp.array([[2, 1], [1, 1]])
...
def test_iforest_error():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("error", UserWarning)
        IsolationForest(max_samples=jnp.int64(2)).fit(X)
...
def test_recalculate_max_depth():
    ...
    for est in clf.estimators_:
        assert est.max_depth == int(jnp.ceil(jnp.log2(X.shape[0])))
...
def test_iforest_performance(global_random_seed):
    ...
    X = rng.permutation(jnp.vstack((X + 2, X - 2)))
    X_train = X[:1000]

    # Generate some abnormal novel observations
    X_outliers = rng.uniform(low=-1, high=1, size=(200, 2))
    X_test = jnp.vstack((X[1000:], X_outliers))
    y_test = jnp.array([0] * 200 + [1] * 200)
...
def test_iforest_works(contamination, global_random_seed):
    ...
    assert jnp.min(decision_func[-2:]) > jnp.max(decision_func[:-2])
...
def test_iforest_average_path_length():
    ...
    result_one = 2.0 * (jnp.log(4.0) + jnp.euler_gamma) - 2.0 * 4.0 / 5.0
    result_two = 2.0 * (jnp.log(998.0) + jnp.euler_gamma) - 2.0 * 998.0 / 999.0
    ...
    assert_allclose(
        _average_path_length(jnp.array([1, 2, 5, 999])),
        [0.0, 1.0, result_one, result_two],
    )
    # _average_path_length is increasing
    avg_path_length = _average_path_length(jnp.arange(5))
    assert_array_equal(avg_path_length, jnp.sort(avg_path_length))
...
def test_iforest_with_uniform_data():
    ...
    X = jnp.repeat(rng.randn(1, 10), 100, 0)
    iforest = IsolationForest()
    iforest.fit(X)

    assert all(iforest.predict(X) == 1)
    assert all(iforest.predict(rng.randn(100, 10)) == 1)
    assert all(iforest.predict(jnp.ones((100, 10))) == 1)
