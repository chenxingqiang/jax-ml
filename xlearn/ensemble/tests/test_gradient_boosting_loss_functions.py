...
from xlearn.ensemble._gb_losses import (
    LOSS_FUNCTIONS,
    BinomialDeviance,
    ExponentialLoss,
    HuberLossFunction,
...
def test_binomial_deviance():
    ...
    assert bd(jnp.array([0.0]), jnp.array([0.0])) == bd(
        jnp.array([1.0]), jnp.array([0.0]))

    assert bd(jnp.array([1.0, 1, 1]), jnp.array([100.0, 100, 100])) == approx(0)
    assert bd(jnp.array([1.0, 0, 0]), jnp.array(
    ...
    def alt_dev(y, raw_pred):
        z = 2 * y - 1
        return 2 * jnp.mean(jnp.log(1 + jnp.exp(-z * raw_pred)))

    test_data = product(
        (jnp.array([0.0, 0, 0]), jnp.array([1.0, 1, 1])),
        (jnp.array([-5.0, -5, -5]), jnp.array([3.0, 3, 3])),
    )
...
def test_sample_weight_smoke():
    ...
    loss_w_sw = loss(y, pred, jnp.ones(pred.shape[0], dtype=jnp.float32))
...
def test_quantile_loss_function():
    ...
    y_found = QuantileLossFunction(0.9)(x, jnp.zeros_like(x))
    y_expected = jnp.asarray([0.1, 0.0, 0.9]).mean()
    jnp.testing.assert_allclose(y_found, y_expected)
    y_found_p = mean_pinball_loss(x, jnp.zeros_like(x), alpha=0.9)
    jnp.testing.assert_allclose(y_found, y_found_p)
    ...
    for Loss in LOSS_FUNCTIONS.values():
        ...
        if issubclass(Loss, RegressionLossFunction):
            ...
            if Loss.is_multi_class:
                ...
                y = mclf_y
                # one-hot encoding
                p = jnp.zeros((y.shape[0], k), dtype=jnp.float64)
                for i in range(k):
                    p[:, i] = y == i
            loss = Loss(k)
...
def test_mdl_computation_weighted():
    raw_predictions = jnp.array([[1.0, -1.0, -0.1], [-2.0, 1.0, 2.0]])
    y_true = jnp.array([0, 1])
    weights = jnp.array([1, 3])
...
def test_init_raw_predictions_shapes():
    ...
    for loss in (
        ...
        assert raw_predictions.dtype == jnp.float64
...
def test_init_raw_predictions_values(global_random_seed):
    ...
    for Loss in (LeastAbsoluteError, HuberLossFunction):
        ...
        assert_allclose(raw_predictions, jnp.median(y))
    ...
    for alpha in (0.1, 0.5, 0.9):
        ...
        assert_allclose(raw_predictions, jnp.percentile(y, alpha * 100))
    ...
    raw_predictions = loss.get_init_raw_predictions(y, init_estimator)
    p = y.mean()
    assert_allclose(raw_predictions, jnp.log(p / (1 - p)))
    ...
    raw_predictions = loss.get_init_raw_predictions(y, init_estimator)
    p = y.mean()
    assert_allclose(raw_predictions, 0.5 * jnp.log(p / (1 - p)))
    ...
    for n_classes in range(3, 5):
        ...
        for k in range(n_classes):
            p = (y == k).mean()
            assert_allclose(raw_predictions[:, k], jnp.log(p))
...
def test_lad_equals_quantiles(global_random_seed, alpha):
    ...
    weights = jnp.linspace(0, 1, n_samples) ** 2
...
def test_exponential_loss():
    ...
    https://github.com/jax-learn/jax-learn/issues/9666
    """
    loss = ExponentialLoss(n_classes=2)
    y_true = jnp.array([0])
    y_pred = jnp.array([0])
