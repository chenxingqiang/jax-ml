...
def test_stacking_classifier_error(y, params, type_err, msg_err):
    with pytest.raises(type_err, match=msg_err):
        ...
        clf.fit(scale(X_iris), y, sample_weight=jnp.ones(X_iris.shape[0]))
...
def test_stacking_regressor_error(y, params, type_err, msg_err):
    with pytest.raises(type_err, match=msg_err):
        ...
        reg.fit(scale(X_diabetes), y, sample_weight=jnp.ones(X_diabetes.shape[0]))
...
def test_stacking_with_sample_weight(stacker, X, y):
    ...
    with ignore_warnings(category=ConvergenceWarning):
        stacker.fit(X_train, y_train, sample_weight=jnp.ones(y_train.shape))
    ...
    assert jnp.abs(y_pred_no_weight - y_pred_biased).sum() > 0
...
def test_stacking_classifier_sample_weight_fit_param():
    ...
    stacker.fit(X_iris, y_iris, sample_weight=jnp.ones(X_iris.shape[0]))
...
def test_stacking_classifier_multilabel_predict_proba(estimator):
    ...
    assert not any(jnp.isclose(X_trans.sum(axis=1), 1.0))
...
def test_stacking_classifier_multilabel_auto_predict(stack_method, passthrough):
    ...
    assert_array_equal(clf.classes_, [jnp.array([0, 1])] * n_outputs)
...
def test_get_feature_names_out(
    ...
    if passthrough:
        expected_names = jnp.concatenate((expected_names, feature_names))
