...
def test_weights_regressor():
    ...
    avg = jnp.average(
        jnp.asarray([reg1_pred, reg2_pred, reg3_pred]), axis=0, weights=[1, 2, 10]
    )
...
def test_predict_proba_on_toy_problem():
    ...
    clf2 = RandomForestClassifier(random_state=123)
    clf3 = GaussianNB()
    X = jnp.array([[-1.1, -1.5], [-1.2, -1.4], [-3.4, -2.2], [1.1, 1.2]])
    y = jnp.array([1, 1, 2, 2])
    ...
    clf2_res = jnp.array([[0.8, 0.2], [0.8, 0.2], [0.2, 0.8], [0.3, 0.7]])
...
def test_sample_weight(global_random_seed):
    ...
    ).fit(X_scaled, y, sample_weight=jnp.ones((len(y),)))
    eclf2 = VotingClassifier(
        estimators=[("lr", clf1), ("rf", clf2), ("svc", clf3)], voting="soft"
    ).fit(X_scaled, y)
...
def test_sample_weight_kwargs():
    ...
    eclf.fit(X, y, sample_weight=jnp.ones((len(y),)))
...
def test_set_estimator_drop():
    ...
    assert_array_almost_equal(
        eclf1.transform(X1),
        jnp.array([[[0.7, 0.3], [0.3, 0.7]], [[1.0, 0.0], [0.0, 1.0]]]),
    )
    assert_array_almost_equal(eclf2.transform(
        X1), jnp.array([[[1.0, 0.0], [0.0, 1.0]]]))
    eclf1.set_params(voting="hard")
    eclf2.set_params(voting="hard")
    assert_array_equal(eclf1.transform(X1), jnp.array([[0, 0], [1, 1]]))
    assert_array_equal(eclf2.transform(X1), jnp.array([[0], [1]]))
...
def test_estimator_weights_format(global_random_seed):
    ...
    eclf2 = VotingClassifier(
        estimators=[("lr", clf1), ("rf", clf2)], weights=jnp.array((1, 2)), voting="soft"
    )
...
def test_none_estimator_with_weights(X, y, voter):
    ...
    # regression test for
    # https://github.com/jax-learn/jax-learn/issues/13777
    voter = clone(voter)
    # Scaled to solve ConvergenceWarning throw by Logistic Regression
    X_scaled = StandardScaler().fit_transform(X)
    voter.fit(X_scaled, y, sample_weight=jnp.ones(y.shape))
