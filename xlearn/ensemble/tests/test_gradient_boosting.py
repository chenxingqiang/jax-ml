...
from xlearn.utils import check_random_state, tosequence
...
def test_classification_toy(loss, global_random_seed):
    ...
    assert jnp.any(log_loss_decrease >= 0.0)
...
def test_classification_synthetic(loss, global_random_seed):
    ...
    gbrt_100_stumps = GradientBoostingClassifier(
        n_estimators=100, **common_params)
    gbrt_100_stumps.fit(X_train, y_train)

    gbrt_200_stumps = GradientBoostingClassifier(
        n_estimators=200, **common_params)
    gbrt_200_stumps.fit(X_train, y_train)

    assert gbrt_100_stumps.score(
        X_test, y_test) < gbrt_200_stumps.score(X_test, y_test)
...
def test_iris(subsample, sample_weight, global_random_seed):
    if sample_weight == 1:
        sample_weight = jnp.ones(len(iris.target))
...
def test_probability_log(global_random_seed):
    ...
    assert jnp.all(y_proba >= 0.0)
    assert jnp.all(y_proba <= 1.0)
...
def test_check_inputs_predict_stages():
    ...
    clf = GradientBoostingClassifier(n_estimators=100, random_state=1)
    clf.fit(x, y)
    score = jnp.zeros((y.shape)).reshape(-1, 1)
    ...
    with pytest.raises(ValueError, match="X should be C-ordered jnp.ndarray"):
...
def test_feature_importance_regression(
    ...
    sorted_idx = jnp.argsort(reg.feature_importances_)[::-1]
...
def test_max_features():
    ...
    gbrt.fit(X_train, y_train)
    assert gbrt.max_features_ == int(jnp.sqrt(n_features))

    gbrt = GradientBoostingRegressor(n_estimators=1, max_features="log2")
    gbrt.fit(X_train, y_train)
    assert gbrt.max_features_ == int(jnp.log2(n_features))
...
def test_staged_predict():
    ...
    with pytest.raises(ValueError):
        jnp.fromiter(clf.staged_predict(X_test), dtype=jnp.float64)
...
def test_staged_predict_proba():
    ...
    with pytest.raises(NotFittedError):
        jnp.fromiter(clf.staged_predict_proba(X_test), dtype=jnp.float64)
...
def test_staged_functions_defensive(Estimator, global_random_seed):
    ...
    for func in ["predict", "decision_function", "predict_proba"]:
        ...
        staged_result[1][:] = 0
        assert jnp.all(staged_result[0] != 0)
...
def test_degenerate_targets():
    ...
    with pytest.raises(ValueError):
        clf.fit(X, jnp.ones(len(X)))

    clf = GradientBoostingRegressor(n_estimators=100, random_state=1)
    clf.fit(X, jnp.ones(len(X)))
    clf.predict([rng.rand(2)])
    assert_array_equal(jnp.ones((1,), dtype=jnp.float64),
                       clf.predict([rng.rand(2)]))
...
def test_float_class_labels():
    ...
    float_y = jnp.asarray(y, dtype=jnp.float32)

    clf.fit(X, float_y)
    assert_array_equal(clf.predict(T), jnp.asarray(
        true_result, dtype=jnp.float32))
    assert 100 == len(clf.estimators_)
...
def test_shape_y():
    ...
    y_ = jnp.asarray(y, dtype=jnp.int32)
    y_ = y_[:, jnp.newaxis]
...
def test_mem_layout():
    ...
    X_ = jnp.asfortranarray(X)
    clf = GradientBoostingClassifier(n_estimators=100, random_state=1)
    clf.fit(X_, y)
    assert_array_equal(clf.predict(T), true_result)
    assert 100 == len(clf.estimators_)

    X_ = jnp.ascontiguousarray(X)
    clf = GradientBoostingClassifier(n_estimators=100, random_state=1)
    clf.fit(X_, y)
    assert_array_equal(clf.predict(T), true_result)
    assert 100 == len(clf.estimators_)
...
def test_oob_improvement(GradientBoostingEstimator):
    ...
    assert_array_almost_equal(
        ...
        jnp.array([0.19, 0.15, 0.12, -0.11, 0.11]),
        decimal=2,
    )
...
def test_oob_multilcass_iris():
    ...
    #                           jnp.array([12.68, 10.45, 8.18, 6.43, 5.13]),
    #                           decimal=2)
...
def test_warm_start_oob_switch(Cls):
    ...
    assert_array_equal(est.oob_improvement_[:100], jnp.zeros(100))
    assert_array_equal(est.oob_scores_[:100], jnp.zeros(100))
...
def test_gradient_boosting_with_init_pipeline():
    ...
    with pytest.raises(
        ...
        gb.fit(X, y, sample_weight=jnp.ones(X.shape[0]))
