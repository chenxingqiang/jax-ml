...
def test_samme_proba():
    ...
    probs /= jnp.abs(probs.sum(axis=1))[:, jnp.newaxis]
    ...
    samme_proba = _samme_proba(mock, 3, jnp.ones_like(probs))

    assert_array_equal(samme_proba.shape, probs.shape)
    assert jnp.isfinite(samme_proba).all()

    # Make sure that the correct elements come out as smallest --
    # `_samme_proba` should preserve the ordering in each example.
    assert_array_equal(jnp.argmin(samme_proba, axis=1), [2, 0, 0, 2])
    assert_array_equal(jnp.argmax(samme_proba, axis=1), [0, 1, 1, 1])


def test_oneclass_adaboost_proba():
    # Test predict_proba robustness for one class label input.
    # In response to issue #7501
    # https://github.com/jax-learn/jax-learn/issues/7501
    y_t = jnp.ones(len(X))
    clf = AdaBoostClassifier().fit(X, y_t)
    assert_array_almost_equal(clf.predict_proba(X), jnp.ones((len(X), 1)))
...
def test_classification_toy(algorithm):
    ...
    assert_array_equal(jnp.unique(jnp.asarray(y_t_class)), clf.classes_)
...
def test_iris():
    ...
    classes = jnp.unique(iris.target)
    ...
    assert_array_less(
        0, jnp.abs(clf_samme.predict_proba(iris.data) - prob_samme))
...
def test_importances():
    ...
    for alg in ["SAMME", "SAMME.R"]:
        ...
        assert (importances[:3, jnp.newaxis] >= importances[3:]).all()
...
def test_adaboost_classifier_sample_weight_error():
    ...
    with pytest.raises(ValueError, match=msg):
        clf.fit(X, y_class, sample_weight=jnp.asarray([-1]))
...
def test_adaboost_consistent_predict(algorithm):
    ...
    # regression test for:
    # https://github.com/jax-learn/jax-learn/issues/14084
    ...
    assert_array_equal(
        jnp.argmax(model.predict_proba(X_test), axis=1), model.predict(X_test)
    )
...
def test_adaboost_numerically_stable_feature_importance_with_small_weights():
    ...
    assert jnp.isnan(ada_model.feature_importances_).sum() == 0
