...
hastie_X = hastie_X.astype(jnp.float32)
...
def test_poisson_vs_mse():
    ...
    for X, y, data_name in [(X_train, y_train, "train"), (X_test, y_test, "test")]:
        ...
        metric_mse = mean_poisson_deviance(
            y, jnp.clip(forest_mse.predict(X), 1e-6, None)
        )
...
def check_probability(name):
    ...
    with jnp.errstate(divide="ignore"):
        ...
        clf.fit(iris.data, iris.target)
        assert_array_almost_equal(
            jnp.sum(clf.predict_proba(iris.data),
                   axis=1), jnp.ones(iris.data.shape[0])
        )
        assert_array_almost_equal(
...
def check_importances(name, criterion, dtype, tolerance):
    ...
    assert jnp.all(importances[:3] > 0.1)
    ...
    importances = est.feature_importances_
    assert jnp.all(importances >= 0.0)
    ...
    for scale in [0.5, 100]:
        ...
        assert jnp.abs(importances - importances_bis).mean() < tolerance
...
def test_importances_asymptotic():
    ...
    def entropy(samples):
        ...
        for count in jnp.bincount(samples):
            p = 1.0 * count / n_samples
            if p > 0:
                entropy -= p * jnp.log2(p)
    ...
    def mdi_importance(X_m, X, y):
        ...
        features.pop(X_m)
        values = [jnp.unique(X[:, i]) for i in range(n_features)]
        ...
        for k in range(n_features):
            ...
            for B in combinations(features, k):
                ...
                for b in product(*[values[B[j]] for j in range(k)]):
                    mask_b = jnp.ones(n_samples, dtype=bool)
    ...
    X, y = jnp.array(data[:, :7], dtype=bool), data[:, 7]
    n_features = X.shape[1]

    # Compute true importances
    true_importances = jnp.zeros(n_features)
    ...
    assert jnp.abs(true_importances - importances).mean() < 0.01
...
def test_random_trees_dense_type():
    ...
    assert type(X_transformed) == jnp.ndarray
...
def check_min_samples_split(name):
    ...
    assert jnp.min(node_samples) > len(X) * 0.5 - \
...
def check_min_samples_leaf(name):
    ...
    # drop inner nodes
    leaf_count = node_counts[node_counts != 0]
    assert jnp.min(leaf_count) > len(X) * 0.25 - \
        1, "Failed with {0}".format(name)
...
def check_min_weight_fraction_leaf(name):
    ...
    for frac in jnp.linspace(0, 0.5, 6):
        est = ForestEstimator(
...
def check_decision_path(name):
    ...
    assert_array_equal(
        jnp.diff(n_nodes_ptr), [e.tree_.node_count for e in est.estimators_]
    )
...
def test_max_samples_boundary_classifiers(name):
    ...
    jnp.testing.assert_allclose(ms_1_proba, ms_None_proba)
...
def test_mse_criterion_object_segfault_smoke_test(Forest):
    ...
    # https://github.com/jax-learn/jax-learn/issues/12623
...
def test_read_only_buffer(monkeypatch):
    ...
    Non-regression test for: https://github.com/jax-learn/jax-learn/issues/25333
    """
    monkeypatch.setattr(
