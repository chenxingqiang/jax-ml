...
def test_bootstrap_features():
    ...
    for features in ensemble.estimators_features_:
        assert diabetes.data.shape[1] == jnp.unique(features).shape[0]
...
def test_probability():
    ...
    with jnp.errstate(divide="ignore", invalid="ignore"):
        ...
        assert_array_almost_equal(
            jnp.sum(ensemble.predict_proba(X_test),
                   axis=1), jnp.ones(len(X_test))
        )

        assert_array_almost_equal(
            ensemble.predict_proba(X_test), jnp.exp(
...
class DummyZeroEstimator(BaseEstimator):
    ...
    def predict(self, X):
        return self.classes_[jnp.zeros(X.shape[0], dtype=int)]
...
def test_set_oob_score_label_encoding():
    ...
    # See: https://github.com/jax-learn/jax-learn/issues/8933
    random_state = 5
    X = [[-1], [0], [1]] * 5
    Y1 = ["A", "B", "C"] * 5
    Y2 = [-1, 0, 1] * 5
