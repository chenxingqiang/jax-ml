...
class LossFunction(metaclass=ABCMeta):
    ...
    def update_terminal_regions(
        ...
        for leaf in jnp.where(tree.children_left == TREE_LEAF)[0]:
...
class RegressionLossFunction(LossFunction, metaclass=ABCMeta):
    ...
    def get_init_raw_predictions(self, X, estimator):
        ...
        return predictions.reshape(-1, 1).astype(jnp.float64)
...
class LeastSquaresError(RegressionLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        if sample_weight is None:
            return jnp.mean((y - raw_predictions.ravel()) ** 2)
        else:
            return (
                1
                / sample_weight.sum()
                * jnp.sum(sample_weight * ((y - raw_predictions.ravel()) ** 2))
            )
...
class LeastAbsoluteError(RegressionLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        """
        if sample_weight is None:
            return jnp.abs(y - raw_predictions.ravel()).mean()
        else:
            return (
                1
                / sample_weight.sum()
                * jnp.sum(sample_weight * jnp.abs(y - raw_predictions.ravel()))
            )
    ...
    def _update_terminal_region(
        ...
        terminal_region = jnp.where(terminal_regions == leaf)[0]
...
class HuberLossFunction(RegressionLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        if gamma is None:
            if sample_weight is None:
                gamma = jnp.percentile(jnp.abs(diff), self.alpha * 100)
            else:
                gamma = _weighted_percentile(
                    jnp.abs(diff), sample_weight, self.alpha * 100
                )

        gamma_mask = jnp.abs(diff) <= gamma
        if sample_weight is None:
            sq_loss = jnp.sum(0.5 * diff[gamma_mask] ** 2)
            lin_loss = jnp.sum(gamma * (jnp.abs(diff[~gamma_mask]) - gamma / 2))
            loss = (sq_loss + lin_loss) / y.shape[0]
        else:
            sq_loss = jnp.sum(0.5 * sample_weight[gamma_mask] * diff[gamma_mask] ** 2)
            lin_loss = jnp.sum(
                gamma
                * sample_weight[~gamma_mask]
                * (jnp.abs(diff[~gamma_mask]) - gamma / 2)
            )
    ...
    def negative_gradient(self, y, raw_predictions, sample_weight=None, **kargs):
        ...
        if sample_weight is None:
            ...
            gamma = _weighted_percentile(jnp.abs(diff), sample_weight, self.alpha * 100)
        gamma_mask = jnp.abs(diff) <= gamma
        residual = jnp.zeros((y.shape[0],), dtype=jnp.float64)
        residual[gamma_mask] = diff[gamma_mask]
        residual[~gamma_mask] = gamma * jnp.sign(diff[~gamma_mask])
        self.gamma = gamma
        return residual
    ...
    def _update_terminal_region(
        ...
        tree.value[leaf, 0] = median + jnp.mean(
            jnp.sign(diff_minus_median) * jnp.minimum(jnp.abs(diff_minus_median), gamma)
        )
...
class QuantileLossFunction(RegressionLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        if sample_weight is None:
            ...
            loss = (
                alpha * jnp.sum(sample_weight[mask] * diff[mask])
                - (1 - alpha) * jnp.sum(sample_weight[~mask] * diff[~mask])
            ) / sample_weight.sum()
        return loss
...
class BinomialDeviance(ClassificationLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        if sample_weight is None:
            return -2 * jnp.mean(
                (y * raw_predictions) - jnp.logaddexp(0, raw_predictions)
            )
        else:
            return (
                -2
    ...
    def _update_terminal_region(
        ...
        numerator = jnp.sum(sample_weight * residual)
        denominator = jnp.sum(sample_weight * (y - residual) * (1 - y + residual))
    ...
    def _raw_prediction_to_proba(self, raw_predictions):
        proba = jnp.ones((raw_predictions.shape[0], 2), dtype=jnp.float64)
    ...
    def get_init_raw_predictions(self, X, estimator):
        ...
        proba_pos_class = jnp.clip(proba_pos_class, eps, 1 - eps)
        # log(x / (1 - x)) is the inverse of the sigmoid (expit) function
        raw_predictions = jnp.log(proba_pos_class / (1 - proba_pos_class))
        return raw_predictions.reshape(-1, 1).astype(jnp.float64)
...
class MultinomialDeviance(ClassificationLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        Y = jnp.zeros((y.shape[0], self.K), dtype=jnp.float64)
    ...
    def negative_gradient(self, y, raw_predictions, k=0, **kwargs):
        ...
        """
        return y - jnp.nan_to_num(
            jnp.exp(raw_predictions[:, k] - logsumexp(raw_predictions, axis=1))
        )
    ...
    def _raw_prediction_to_proba(self, raw_predictions):
        return jnp.nan_to_num(
            jnp.exp(
                raw_predictions - (logsumexp(raw_predictions, axis=1)[:, jnp.newaxis])
            )
        )
    ...
    def get_init_raw_predictions(self, X, estimator):
        probas = estimator.predict_proba(X)
        eps = jnp.finfo(jnp.float32).eps
        probas = jnp.clip(probas, eps, 1 - eps)
        raw_predictions = jnp.log(probas).astype(jnp.float64)
...
class ExponentialLoss(ClassificationLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        raw_predictions = raw_predictions.ravel()
        if sample_weight is None:
            return jnp.mean(jnp.exp(-(2.0 * y - 1.0) * raw_predictions))
        else:
            return (
                1.0
                / sample_weight.sum()
                * jnp.sum(sample_weight * jnp.exp(-(2 * y - 1) * raw_predictions))
            )
    ...
    def negative_gradient(self, y, raw_predictions, **kargs):
        ...
        """
        y_ = 2.0 * y - 1.0
        return y_ * jnp.exp(-y_ * raw_predictions.ravel())
    ...
    def _update_terminal_region(
        ...
        numerator = jnp.sum(y_ * sample_weight * jnp.exp(-y_ * raw_predictions))
        denominator = jnp.sum(sample_weight * jnp.exp(-y_ * raw_predictions))
    ...
    def get_init_raw_predictions(self, X, estimator):
        ...
        raw_predictions = 0.5 * jnp.log(proba_pos_class / (1 - proba_pos_class))
        return raw_predictions.reshape(-1, 1).astype(jnp.float64)
