...
class BaseWeightBoosting(BaseEnsemble, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        sample_weight = _check_sample_weight(
            sample_weight, X, jnp.float64, copy=True, only_non_negative=True
        )
        ...
        self.estimators_ = []
        self.estimator_weights_ = jnp.zeros(self.n_estimators, dtype=jnp.float64)
        self.estimator_errors_ = jnp.ones(self.n_estimators, dtype=jnp.float64)
        ...
        for iboost in range(self.n_estimators):
            ...
            if not jnp.isfinite(sample_weight_sum):
                warnings.warn(
                    (
...
def _samme_proba(estimator, n_classes, X):
    ...
    jnp.clip(proba, jnp.finfo(proba.dtype).eps, None, out=proba)
    log_proba = jnp.log(proba)

    return (n_classes - 1) * (
        log_proba - (1.0 / n_classes) * log_proba.sum(axis=1)[:, jnp.newaxis]
    )
...
class AdaBoostClassifier(ClassifierMixin, BaseWeightBoosting):
    ...
    def _boost_real(self, iboost, X, y, sample_weight, random_state):
        ...
        y_predict = self.classes_.take(
            jnp.argmax(y_predict_proba, axis=1), axis=0)

        # Instances incorrectly classified
        incorrect = y_predict != y

        # Error fraction
        estimator_error = jnp.mean(jnp.average(
        ...
        y_codes = jnp.array([-1.0 / (n_classes - 1), 1.0])
        y_coding = y_codes.take(classes == y[:, jnp.newaxis])
    ...
    def predict(self, X):
        ...
        return self.classes_.take(jnp.argmax(pred, axis=1), axis=0)
    ...
    def staged_predict(self, X):
        ...
        if n_classes == 2:
            for pred in self.staged_decision_function(X):
                yield jnp.array(classes.take(pred > 0, axis=0))

        else:
            for pred in self.staged_decision_function(X):
                yield jnp.array(classes.take(jnp.argmax(pred, axis=1), axis=0))
    ...
    def decision_function(self, X):
        ...
        n_classes = self.n_classes_
        classes = self.classes_[:, jnp.newaxis]
    ...
    def _compute_proba_from_decision(decision, n_classes):
        ...
        """
        if n_classes == 2:
            decision = jnp.vstack([-decision, decision]).T / 2
        else:
    ...
    def predict_proba(self, X):
        ...
        if n_classes == 1:
            return jnp.ones((_num_samples(X), 1))
...
class AdaBoostRegressor(RegressorMixin, BaseWeightBoosting):
    ...
    def _boost(self, iboost, X, y, sample_weight, random_state):
        ...
        error_vect = jnp.abs(y_predict - y)
        ...
        if self.loss == "square":
            ...
            masked_error_vector = 1.0 - jnp.exp(-masked_error_vector)
        ...
        estimator_weight = self.learning_rate * jnp.log(1.0 / beta)

        if not iboost == self.n_estimators - 1:
            sample_weight[sample_mask] *= jnp.power(
    ...
    def _get_median_predict(self, X, limit):
        ...
        predictions = jnp.array([est.predict(X)
                               for est in self.estimators_[:limit]]).T

        # Sort the predictions
        sorted_idx = jnp.argsort(predictions, axis=1)

        # Find index of median prediction for each sample
        weight_cdf = stable_cumsum(self.estimator_weights_[sorted_idx], axis=1)
        median_or_above = weight_cdf >= 0.5 * weight_cdf[:, -1][:, jnp.newaxis]
        median_idx = median_or_above.argmax(axis=1)

        median_estimators = sorted_idx[jnp.arange(_num_samples(X)), median_idx]

        # Return median predictions
        return predictions[jnp.arange(_num_samples(X)), median_estimators]
