...
class _BaseVoting(TransformerMixin, _BaseHeterogeneousEnsemble):
    ...
    def _predict(self, X):
        ...
        return jnp.asarray([est.predict(X) for est in self.estimators_]).T
...
class VotingClassifier(ClassifierMixin, _BaseVoting):
    ...
    [1 1 1 2 2 2]
    >>> jnp.array_equal(eclf1.named_estimators_.lr.predict(X),
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if isinstance(y, jnp.ndarray) and len(y.shape) > 1 and y.shape[1] > 1:
    ...
    def predict(self, X):
        ...
        check_is_fitted(self)
        if self.voting == "soft":
            maj = jnp.argmax(self.predict_proba(X), axis=1)

        else:  # 'hard' voting
            predictions = self._predict(X)
            maj = jnp.apply_along_axis(
                lambda x: jnp.argmax(jnp.bincount(
    ...
    def _collect_probas(self, X):
        """Collect results from clf.predict calls."""
        return jnp.asarray([clf.predict_proba(X) for clf in self.estimators_])
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        return jnp.asarray(names_out, dtype=object)
...
class VotingRegressor(RegressorMixin, _BaseVoting):
    ...
    >>> X = jnp.array([[1, 1], [2, 4], [3, 9], [4, 16], [5, 25], [6, 36]])
    >>> y = jnp.array([2, 6, 12, 20, 30, 42])
    ...
    def predict(self, X):
        ...
        """
        check_is_fitted(self)
        return jnp.average(self._predict(X), axis=1, weights=self._weights_not_none)
