...
class BaseGradientBoosting(BaseEnsemble, metaclass=ABCMeta):
    ...
    def _fit_stage(
        ...
        for k in range(loss.K):
            if loss.is_multi_class:
                y = jnp.array(original_y == k, dtype=jnp.float64)
            ...
            if self.subsample < 1.0:
                ...
                sample_weight = sample_weight * sample_mask.astype(jnp.float64)
    ...
    def _check_params(self):
        ...
        if isinstance(self.max_features, str):
            if self.max_features == "auto":
                if is_classifier(self):
                    max_features = max(1, int(jnp.sqrt(self.n_features_in_)))
                else:
                    max_features = self.n_features_in_
            elif self.max_features == "sqrt":
                max_features = max(1, int(jnp.sqrt(self.n_features_in_)))
            else:  # self.max_features == "log2"
                max_features = max(1, int(jnp.log2(self.n_features_in_)))
    ...
    def _init_state(self):
        ...
        self.estimators_ = jnp.empty((self.n_estimators, self._loss.K), dtype=object)
        self.train_score_ = jnp.zeros((self.n_estimators,), dtype=jnp.float64)
        # do oob?
        if self.subsample < 1.0:
            self.oob_improvement_ = jnp.zeros((self.n_estimators), dtype=jnp.float64)
            self.oob_scores_ = jnp.zeros((self.n_estimators), dtype=jnp.float64)
            self.oob_score_ = jnp.nan

    def _clear_state(self):
        """Clear the state of the gradient boosting model."""
        if hasattr(self, "estimators_"):
            self.estimators_ = jnp.empty((0, 0), dtype=object)
    ...
    def _resize_state(self):
        ...
        self.estimators_ = jnp.resize(
            self.estimators_, (total_n_estimators, self._loss.K)
        )
        self.train_score_ = jnp.resize(self.train_score_, total_n_estimators)
        if self.subsample < 1 or hasattr(self, "oob_improvement_"):
            # if do oob resize arrays or create new if not available
            if hasattr(self, "oob_improvement_"):
                self.oob_improvement_ = jnp.resize(
                    self.oob_improvement_, total_n_estimators
                )
                self.oob_scores_ = jnp.resize(self.oob_scores_, total_n_estimators)
                self.oob_score_ = jnp.nan
            else:
                self.oob_improvement_ = jnp.zeros(
                    (total_n_estimators,), dtype=jnp.float64
                )
                self.oob_scores_ = jnp.zeros((total_n_estimators,), dtype=jnp.float64)
                self.oob_score_ = jnp.nan
    ...
    def fit(self, X, y, sample_weight=None, monitor=None):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            ...
            ``dtype=jnp.float32`` and if a sparse matrix is provided
        ...
        if self.n_iter_no_change is not None:
            ...
            if is_classifier(self):
                if self._n_classes != jnp.unique(y).shape[0]:
        ...
        if not self._is_initialized():
            ...
            if self.init_ == "zero":
                raw_predictions = jnp.zeros(
                    shape=(X.shape[0], self._loss.K), dtype=jnp.float64
                )
            else:
    ...
    def _fit_stages(
        ...
        sample_mask = jnp.ones((n_samples,), dtype=bool)
        ...
        if self.n_iter_no_change is not None:
            loss_history = jnp.full(self.n_iter_no_change, jnp.inf)
        ...
        for i in range(begin_at_stage, self.n_estimators):
            ...
            if self.n_iter_no_change is not None:
                ...
                if jnp.any(validation_loss + self.tol < loss_history):
    ...
    def feature_importances_(self):
        ...
        if not relevant_trees:
            ...
            return jnp.zeros(shape=self.n_features_in_, dtype=jnp.float64)

        relevant_feature_importances = [
            tree.tree_.compute_feature_importances(normalize=False)
            for tree in relevant_trees
        ]
        avg_feature_importances = jnp.mean(
            relevant_feature_importances, axis=0, dtype=jnp.float64
        )
        return avg_feature_importances / jnp.sum(avg_feature_importances)
    ...
    def _compute_partial_dependence_recursion(self, grid, target_features):
        ...
        averaged_predictions = jnp.zeros(
            (n_trees_per_stage, grid.shape[0]), dtype=jnp.float64, order="C"
        )
    ...
    def apply(self, X):
        ...
        X : {array-like, sparse matrix} of shape (n_samples, n_features)
            ...
            ``dtype=jnp.float32``. If a sparse matrix is provided, it will
        ...
        leaves = jnp.zeros((X.shape[0], n_estimators, n_classes))
...
class GradientBoostingClassifier(ClassifierMixin, BaseGradientBoosting):
    ...
    def _validate_y(self, y, sample_weight):
        ...
        n_trim_classes = jnp.count_nonzero(jnp.bincount(y, sample_weight))
        if n_trim_classes < 2:
            raise ValueError(
