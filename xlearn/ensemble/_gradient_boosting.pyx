...
cdef void _predict_regression_tree_inplace_fast_dense(
    ...
    k : int
        ...
        0 <= ``k`` < ``K``.
    out : memory view on array of type jnp.float64_t
...
def predict_stages(
    ...
    if issparse(X):
        ...
        if not isinstance(X, jnp.ndarray) or jnp.isfortran(X):
            raise ValueError(f"X should be C-ordered jnp.ndarray, got {type(X)}")
...
def _random_sample_mask(
    ...
    Returns
    -------
    sample_mask : jnp.ndarray, shape=[n_total_samples]
