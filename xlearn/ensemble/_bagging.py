    ...
    for i in range(n_estimators):
        ...
        if support_sample_weight:
            if sample_weight is None:
                curr_sample_weight = jnp.ones((n_samples,))
            else:
...
def _parallel_predict_proba(estimators, estimators_features, X, n_classes):
    ...
    n_samples = X.shape[0]
    proba = jnp.zeros((n_samples, n_classes))
...
def _parallel_predict_log_proba(estimators, estimators_features, X, n_classes):
    ...
    n_samples = X.shape[0]
    log_proba = jnp.empty((n_samples, n_classes))
    log_proba.fill(-jnp.inf)
    all_classes = jnp.arange(n_classes, dtype=int)

    for estimator, features in zip(estimators, estimators_features):
        log_proba_estimator = estimator.predict_log_proba(X[:, features])

        if n_classes == len(estimator.classes_):
            log_proba = jnp.logaddexp(log_proba, log_proba_estimator)

        else:
            log_proba[:, estimator.classes_] = jnp.logaddexp(
                log_proba[:, estimator.classes_],
                log_proba_estimator[:, range(len(estimator.classes_))],
            )

            missing = jnp.setdiff1d(all_classes, estimator.classes_)
            log_proba[:, missing] = jnp.logaddexp(
                log_proba[:, missing], -jnp.inf)
...
class BaggingClassifier(ClassifierMixin, BaseBagging):
    ...
    def _set_oob_score(self, X, y):
        ...
        predictions = jnp.zeros((n_samples, n_classes_))
        ...
        oob_decision_function = predictions / \
            predictions.sum(axis=1)[:, jnp.newaxis]
        oob_score = accuracy_score(y, jnp.argmax(predictions, axis=1))
    ...
    def predict(self, X):
        ...
        return self.classes_.take((jnp.argmax(predicted_probabilitiy, axis=1)), axis=0)
    ...
    def predict_log_proba(self, X):
        ...
        if hasattr(self.estimator_, "predict_log_proba"):
            ...
            for j in range(1, len(all_log_proba)):
                log_proba = jnp.logaddexp(log_proba, all_log_proba[j])

            log_proba -= jnp.log(self.n_estimators)

        else:
            log_proba = jnp.log(self.predict_proba(X))
...
class BaggingRegressor(RegressorMixin, BaseBagging):
    ...
    def _set_oob_score(self, X, y):
        ...
        predictions = jnp.zeros((n_samples,))
        n_predictions = jnp.zeros((n_samples,))
