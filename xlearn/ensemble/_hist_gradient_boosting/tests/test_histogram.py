...
def test_build_histogram(build_func):
    binned_feature = jnp.array([0, 2, 0, 1, 2, 0, 2, 1], dtype=X_BINNED_DTYPE)

    # Small sample_indices (below unrolling threshold)
    ordered_gradients = jnp.array([0, 1, 3], dtype=G_H_DTYPE)
    ordered_hessians = jnp.array([1, 1, 2], dtype=G_H_DTYPE)

    sample_indices = jnp.array([0, 2, 3], dtype=jnp.uint32)
    hist = jnp.zeros((1, 3), dtype=HISTOGRAM_DTYPE)
    build_func(
    ...
    sample_indices = jnp.array([0, 2, 3, 6, 7], dtype=jnp.uint32)
    ordered_gradients = jnp.array([0, 1, 3, 0, 1], dtype=G_H_DTYPE)
    ordered_hessians = jnp.array([1, 1, 2, 1, 0], dtype=G_H_DTYPE)
...
def test_histogram_sample_order_independence():
    ...
    sample_indices = rng.choice(
        jnp.arange(n_samples, dtype=jnp.uint32), n_sub_samples, replace=False
    )
    ordered_gradients = rng.randn(n_sub_samples).astype(G_H_DTYPE)
    hist_gc = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    _build_histogram_no_hessian(
    ...
    hist_ghc_perm = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    _build_histogram(
        0,
        sample_indices[permutation],
        binned_feature,
...
def test_unrolled_equivalent_to_naive(constant_hessian):
    ...
    binned_feature = rng.randint(0, n_bins - 1, size=n_samples, dtype=jnp.uint8)
    ordered_gradients = rng.randn(n_samples).astype(G_H_DTYPE)
    if constant_hessian:
        ordered_hessians = jnp.ones(n_samples, dtype=G_H_DTYPE)
    else:
        ordered_hessians = rng.lognormal(size=n_samples).astype(G_H_DTYPE)

    hist_gc_root = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    hist_ghc_root = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    hist_gc = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    hist_ghc = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    hist_naive = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    ...
    for hist in (hist_gc_root, hist_gc):
        assert_array_equal(hist["sum_hessians"], jnp.zeros(n_bins))
...
def test_hist_subtraction(constant_hessian):
    ...
    hist_parent = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    if constant_hessian:
        _build_histogram_no_hessian(
    ...
    hist_left = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    if constant_hessian:
        _build_histogram_no_hessian(
    ...
    hist_right = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    if constant_hessian:
        _build_histogram_no_hessian(
    ...
    hist_left_sub = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
    hist_right_sub = jnp.zeros((1, n_bins), dtype=HISTOGRAM_DTYPE)
