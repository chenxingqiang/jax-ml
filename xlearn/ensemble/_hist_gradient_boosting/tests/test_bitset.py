...
@pytest.mark.parametrize(
    ...
    [
        ([0, 4, 33], jnp.array([2**0 + 2**4, 2**1, 0], dtype=jnp.uint32)),
        (
            [31, 32, 33, 79],
            jnp.array([2**31, 2**0 + 2**1, 2**15], dtype=jnp.uint32),
        ),
    ],
)
def test_set_get_bitset(values_to_insert, expected_bitset):
    n_32bits_ints = 3
    bitset = jnp.zeros(n_32bits_ints, dtype=jnp.uint32)
...
def test_raw_bitset_from_binned_bitset(
    ...
    binned_bitset = jnp.zeros(2, dtype=jnp.uint32)
    raw_bitset = jnp.zeros(2, dtype=jnp.uint32)
    raw_categories = jnp.asarray(raw_categories, dtype=X_DTYPE)
