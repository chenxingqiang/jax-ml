...
def test_predictions(global_random_seed, use_feature_names):
    ...
    y = 5 * f_0 + jnp.sin(10 * jnp.pi * f_0) - 5 * f_1 - jnp.cos(10 * jnp.pi * f_1) + noise
    ...
    linspace = jnp.linspace(0, 1, 100)
    sin = jnp.sin(linspace)
    constant = jnp.full_like(linspace, fill_value=0.5)
...
def test_bounded_value_min_gain_to_split():
    ...
    min_samples_leaf = 1
    n_bins = n_samples = 5
    X_binned = jnp.arange(n_samples).reshape(-1, 1).astype(X_BINNED_DTYPE)
    sample_indices = jnp.arange(n_samples, dtype=jnp.uint32)
    all_hessians = jnp.ones(n_samples, dtype=G_H_DTYPE)
    all_gradients = jnp.array([1, 1, 100, 1, 1], dtype=G_H_DTYPE)
    ...
    n_bins_non_missing = jnp.array([n_bins - 1] * X_binned.shape[1], dtype=jnp.uint32)
    ...
    children_lower_bound, children_upper_bound = -jnp.inf, jnp.inf
    ...
    current_lower_bound, current_upper_bound = -jnp.inf, jnp.inf
    ...
    current_lower_bound, current_upper_bound = -10, jnp.inf
    value = compute_node_value(
        sum_gradients,
        sum_hessians,
        current_lower_bound,
