...
def test_histogram_split(n_bins):
    ...
    sample_indices = jnp.arange(binned_feature.shape[0], dtype=jnp.uint32)
    ordered_hessians = jnp.ones_like(binned_feature, dtype=G_H_DTYPE)
    ...
    for true_bin in range(1, n_bins - 2):
        for sign in [-1, 1]:
            ordered_gradients = jnp.full_like(
            ...
            n_bins_non_missing = jnp.array(
                [n_bins - 1] * X_binned.shape[1], dtype=jnp.uint32
            )
            has_missing_values = jnp.array(
                [False] * X_binned.shape[1], dtype=jnp.uint8)
            monotonic_cst = jnp.array(
                [MonotonicConstraint.NO_CST] * X_binned.shape[1], dtype=jnp.int8
            )
            is_categorical = jnp.zeros_like(monotonic_cst, dtype=jnp.uint8)
            ...
            value = compute_node_value(
                sum_gradients, sum_hessians, -jnp.inf, jnp.inf, l2_regularization
            )
...
def test_gradient_and_hessian_sanity(constant_hessian):
    ...
    sample_indices = jnp.arange(n_samples, dtype=jnp.uint32)
    all_gradients = rng.randn(n_samples).astype(G_H_DTYPE)
    sum_gradients = all_gradients.sum()
    if constant_hessian:
        all_hessians = jnp.ones(1, dtype=G_H_DTYPE)
    ...
    n_bins_non_missing = jnp.array(
        [n_bins - 1] * X_binned.shape[1], dtype=jnp.uint32)
    has_missing_values = jnp.array([False] * X_binned.shape[1], dtype=jnp.uint8)
    monotonic_cst = jnp.array(
        [MonotonicConstraint.NO_CST] * X_binned.shape[1], dtype=jnp.int8
    )
    is_categorical = jnp.zeros_like(monotonic_cst, dtype=jnp.uint8)
    ...
    for si, indices in (
        ...
        assert jnp.isclose(hessian, expected_hessian)
    ...
    hists_parent = jnp.asarray(hists_parent, dtype=HISTOGRAM_DTYPE)
    hists_left = jnp.asarray(hists_left, dtype=HISTOGRAM_DTYPE)
    hists_right = jnp.asarray(hists_right, dtype=HISTOGRAM_DTYPE)
    for hists, indices in (
        ...
        assert jnp.allclose(gradients, expected_gradient)
        assert jnp.allclose(hessians, expected_hessian)
...
def test_split_indices():
    ...
    X_binned = jnp.asfortranarray(X_binned, dtype=X_BINNED_DTYPE)
    sample_indices = jnp.arange(n_samples, dtype=jnp.uint32)
    all_gradients = rng.randn(n_samples).astype(G_H_DTYPE)
    all_hessians = jnp.ones(1, dtype=G_H_DTYPE)
    sum_gradients = all_gradients.sum()
    ...
    n_bins_non_missing = jnp.array(
        [n_bins] * X_binned.shape[1], dtype=jnp.uint32)
    has_missing_values = jnp.array([False] * X_binned.shape[1], dtype=jnp.uint8)
    monotonic_cst = jnp.array(
        [MonotonicConstraint.NO_CST] * X_binned.shape[1], dtype=jnp.int8
    )
    ...
    assert jnp.all(sample_indices == splitter.partition)
...
def test_min_gain_to_split():
    ...
    sample_indices = jnp.arange(n_samples, dtype=jnp.uint32)
    all_hessians = jnp.ones_like(binned_feature, dtype=G_H_DTYPE)
    all_gradients = jnp.ones_like(binned_feature, dtype=G_H_DTYPE)
    sum_gradients = all_gradients.sum()
    sum_hessians = all_hessians.sum()
    hessians_are_constant = False
...
def test_splitting_missing_values(
    ...
    sample_indices = jnp.arange(n_samples, dtype=jnp.uint32)
    X_binned = jnp.array(X_binned, dtype=X_BINNED_DTYPE).reshape(-1, 1)
    X_binned = jnp.asfortranarray(X_binned)
    all_gradients = jnp.array(all_gradients, dtype=G_H_DTYPE)
    has_missing_values = jnp.array([has_missing_values], dtype=jnp.uint8)
    all_hessians = jnp.ones(1, dtype=G_H_DTYPE)
    sum_gradients = all_gradients.sum()
    sum_hessians = 1 * n_samples
    hessians_are_constant = True
    ...
    n_bins_non_missing = jnp.array([n_bins_non_missing], dtype=jnp.uint32)
    monotonic_cst = jnp.array(
        [MonotonicConstraint.NO_CST] * X_binned.shape[1], dtype=jnp.int8
    )
    is_categorical = jnp.zeros_like(monotonic_cst, dtype=jnp.uint8)
    ...
    if not expected_split_on_nan:
        ...
        # to the right child.
        missing_samples_indices = jnp.flatnonzero(
            jnp.array(X_binned) == missing_values_bin_idx
        )
        non_missing_samples_indices = jnp.flatnonzero(
            jnp.array(X_binned) != missing_values_bin_idx
        )
...
def test_splitting_categorical_cat_smooth(
    ...
    n_bins = max(X_binned) + 1
    n_samples = len(X_binned)
    X_binned = jnp.array([X_binned], dtype=X_BINNED_DTYPE).T
    X_binned = jnp.asfortranarray(X_binned)
    ...
    sample_indices = jnp.arange(n_samples, dtype=jnp.uint32)
    all_gradients = jnp.ones(n_samples, dtype=G_H_DTYPE)
    has_missing_values = jnp.array([has_missing_values], dtype=jnp.uint8)
    all_hessians = jnp.ones(1, dtype=G_H_DTYPE)
    sum_gradients = all_gradients.sum()
    ...
    is_categorical = jnp.ones_like(monotonic_cst, dtype=jnp.uint8)
    missing_values_bin_idx = n_bins - 1
...
def _assert_categories_equals_bitset(categories, bitset):
    ...
    expected_bitset = jnp.zeros(8, dtype=jnp.uint32)
...
def test_splitting_categorical_sanity(
    ...
    left_mask = jnp.isin(X_binned.ravel(), expected_categories_left)
...
def test_split_interaction_constraints():
    ...
    allowed_features = jnp.array([0, 3], dtype=jnp.uint32)
    n_bins = 5
    n_samples = 10
    l2_regularization = 0.0
    min_hessian_to_split = 1e-3
