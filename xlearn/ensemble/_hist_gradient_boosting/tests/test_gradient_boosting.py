...
from xlearn.metrics import mean_gamma_deviance, mean_poisson_deviance
...
def test_gamma_y_positive(y):
    ...
    with pytest.raises(ValueError, match=err_msg):
        gbdt.fit(jnp.zeros(shape=(len(y), 1)), y)
...
def test_gamma():
    ...
    # invalid (jnp.inf) for the Gamma deviance. A Poisson HGBT (having a log link)
    ...
    dispersion = 0.5
    y = rng.gamma(shape=1 / dispersion, scale=dispersion * jnp.exp(X @ coef))
    ...
    for X, y in [(X_train, y_train), (X_test, y_test)]:
        ...
        loss_gbdt_mse = mean_gamma_deviance(
            y, jnp.maximum(jnp.min(y_train), gbdt_mse.predict(X))
        )
...
def test_quantile_asymmetric_error(quantile):
    ...
    y = rng.exponential(
        scale=-(X @ coef + intercept) / jnp.log(1 - quantile), size=n_samples
    )
    ...
    assert_allclose(jnp.mean(model.predict(X) > y), quantile, rtol=1e-2)
...
def test_poisson():
    ...
    for X, y in [(X_train, y_train), (X_test, y_test)]:
        ...
        metric_ls = mean_poisson_deviance(
            y, jnp.clip(gbdt_ls.predict(X), 1e-15, None))
...
def test_small_trainset():
    ...
    unique, counts = jnp.unique(y_small, return_counts=True)
...
def test_missing_values_minmax_imputation():
    ...
    class MinMaxImputer(TransformerMixin, BaseEstimator):
        ...
        def transform(self, X):
            ...
            for feature_idx in range(X.shape[1]):
                nan_mask = jnp.isnan(X[:, feature_idx])
                X_min[nan_mask, feature_idx] = self.data_min_[feature_idx] - 1
                X_max[nan_mask, feature_idx] = self.data_max_[feature_idx] + 1

            return jnp.concatenate([X_min, X_max], axis=1)
    ...
    def make_missing_value_data(n_samples=int(1e4), seed=0):
        ...
        # strategies and also make it easier to insert jnp.nan in a structured
        # way:
        X = KBinsDiscretizer(n_bins=42, encode="ordinal").fit_transform(X)

        # First feature has missing values completely at random:
        rnd_mask = rng.rand(X.shape[0]) > 0.9
        X[rnd_mask, 0] = jnp.nan

        # Second and third features have missing values for extreme values
        # (censoring missingness):
        low_mask = X[:, 1] == 0
        X[low_mask, 1] = jnp.nan

        high_mask = X[:, 2] == X[:, 2].max()
        X[high_mask, 2] = jnp.nan

        # Make the last feature nan pattern very informative:
        y_max = jnp.percentile(y, 70)
        y_max_mask = y >= y_max
        y[y_max_mask] = y_max
        X[y_max_mask, 3] = jnp.nan

        # Check that there is at least one missing value in each feature:
        for feature_idx in range(X.shape[1]):
            assert any(jnp.isnan(X[:, feature_idx]))
...
def test_infinite_values():
    ...
    X = jnp.array([-jnp.inf, 0, 1, jnp.inf]).reshape(-1, 1)
    y = jnp.array([0, 0, 1, 1])

    gbdt = HistGradientBoostingRegressor(min_samples_leaf=1)
    gbdt.fit(X, y)
    jnp.testing.assert_allclose(gbdt.predict(X), y, atol=1e-4)


def test_consistent_lengths():
    X = jnp.array([-jnp.inf, 0, 1, jnp.inf]).reshape(-1, 1)
    y = jnp.array([0, 0, 1, 1])
    sample_weight = jnp.array([0.1, 0.3, 0.1])
...
def test_infinite_values_missing_values():
    ...
    X = jnp.asarray([-jnp.inf, 0, 1, jnp.inf, jnp.nan]).reshape(-1, 1)
    y_isnan = jnp.isnan(X.ravel())
    y_isinf = X.ravel() == jnp.inf
...
def test_string_target_early_stopping(scoring):
    ...
    rng = np.random.RandomState(42)
    X = rng.randn(100, 10)
    y = jnp.array(["x"] * 50 + ["y"] * 50, dtype=object)
...
def test_sample_weight_effect(problem, duplication):
    ...
    if duplication == "half":
        ...
        lim = n_samples
    X_dup = jnp.r_[X, X[:lim]]
    y_dup = jnp.r_[y, y[:lim]]
    sample_weight = jnp.ones(shape=(n_samples))
    sample_weight[:lim] = 2
    ...
    assert jnp.allclose(est_sw._raw_predict(X_dup), est_dup._raw_predict(X_dup))
...
def test_sum_hessians_are_sample_weight(Loss):
    ...
    sum_sw = jnp.zeros(shape=(n_features, bin_mapper.n_bins))
...
def test_max_depth_max_leaf_nodes():
    ...
    # https://github.com/jax-learn/jax-learn/issues/16179
...
def test_unknown_categories_nan(
    ...
    X_test = jnp.zeros((10, X.shape[1]), dtype=float)
    X_test[:5, 1] = 30
    X_test[5:, 1] = missing_value
    assert len(jnp.unique(est.predict(X_test))) == 1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            jnp.array([b"hello", 1.3], dtype=object),
            None,
            re.escape(
                "categorical_features must be an array-like of bool, int or str, "
                "got: bytes, float."
...
def test_categorical_spec_no_categories(Est, categorical_features, as_array):
    ...
    y = jnp.arange(5)
    if as_array:
        categorical_features = jnp.asarray(categorical_features)
...
def test_uint8_predict(Est):
    # Non regression test for
    # https://github.com/jax-learn/jax-learn/issues/18408
    # Make sure X can be of dtype uint8 (i.e. X_BINNED_DTYPE) in predict. It
    # will be converted to X_DTYPE.

    rng = np.random.RandomState(0)

    X = rng.randint(0, 100, size=(10, 2)).astype(jnp.uint8)
    y = rng.randint(0, 2, size=10).astype(jnp.uint8)
    est = Est()
    est.fit(X, y)
    est.predict(X)
...
def test_interaction_cst_numerically():
    ...
    # y = x0 + x1 + 5 * x0 * x1
    y = jnp.hstack((X, 5 * X[:, [0]] * X[:, [1]])).sum(axis=1)
...
def test_class_weights():
    ...
    X_imb = jnp.concatenate((X[~y_is_1], X[y_is_1][:10]))
    y_imb = jnp.concatenate((y[~y_is_1], y[y_is_1][:10]))

    # class_weight="balanced" is the same as sample_weights to be
    # inversely proportional to n_samples / (n_classes * jnp.bincount(y))
    clf_balanced = clone(clf).set_params(class_weight="balanced")
    clf_balanced.fit(X_imb, y_imb)

    class_weight = y_imb.shape[0] / (2 * jnp.bincount(y_imb))
...
def test_unknown_category_that_are_negative():
    ...
    rng = np.random.RandomState(42)
    n_samples = 1000
    X = jnp.c_[rng.rand(n_samples), rng.randint(4, size=n_samples)]
    y = jnp.zeros(shape=n_samples)
    y[X[:, 1] % 2 == 0] = 1
    ...
    # missing category
    X_test_neg = jnp.asarray([[1, -2], [3, -4]])
    X_test_nan = jnp.asarray([[1, jnp.nan], [3, jnp.nan]])
