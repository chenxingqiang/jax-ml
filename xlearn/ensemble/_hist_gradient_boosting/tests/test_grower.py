...
def _make_training_data(n_bins=256, constant_hessian=True):
    ...
    target = jnp.array([true_decision_function(x)
    ...
    all_hessians = jnp.ones(shape=shape_hessians, dtype=G_H_DTYPE)
...
def test_predictor_from_grower():
    ...
    predictor = grower.make_predictor(
        binning_thresholds=jnp.zeros((X_binned.shape[1], n_bins))
    )
    ...
    assert jnp.allclose(predictions, expected_targets)

    # Check that training set can be recovered exactly:
    predictions = predictor.predict_binned(
        X_binned, missing_values_bin_idx, n_threads)
    assert jnp.allclose(predictions, -all_gradients)
...
def test_min_samples_leaf(n_samples, min_samples_leaf, n_bins, constant_hessian, noise):
    ...
    all_hessians = jnp.ones(shape=shape_hessian, dtype=G_H_DTYPE)
    grower = TreeGrower(
        X,
        all_gradients,
        all_hessians,
...
def test_min_samples_leaf_root(n_samples, min_samples_leaf):
    ...
    all_gradients = y.astype(G_H_DTYPE)
    all_hessians = jnp.ones(shape=1, dtype=G_H_DTYPE)
    grower = TreeGrower(
        X,
        all_gradients,
        all_hessians,
...
def test_input_validation():
    ...
    X_binned_float = X_binned.astype(jnp.float32)
    ...
    X_binned_C_array = jnp.ascontiguousarray(X_binned)
    with pytest.raises(
...
def test_missing_value_predict_only():
    ...
    hessians = jnp.ones(shape=1, dtype=G_H_DTYPE)
    ...
    # We pass undefined binning_thresholds because we won't use predict anyway
    predictor = grower.make_predictor(
        binning_thresholds=jnp.zeros((X_binned.shape[1], X_binned.max() + 1))
    )
    ...
    all_nans = jnp.full(shape=(n_samples, 1), fill_value=jnp.nan)
    known_cat_bitsets = jnp.zeros((0, 8), dtype=X_BITSET_INNER_DTYPE)
    f_idx_map = jnp.zeros(0, dtype=jnp.uint32)

    y_pred = predictor.predict(
        all_nans, known_cat_bitsets, f_idx_map, n_threads)
    assert jnp.all(y_pred == prediction_main_path)
...
def test_split_on_nan_with_infinite_values():
    ...
    X = jnp.array([0, 1, jnp.inf, jnp.nan, jnp.nan]).reshape(-1, 1)
    # the gradient values will force a split on nan situation
    gradients = jnp.array([0, 0, 0, 100, 100], dtype=G_H_DTYPE)
    hessians = jnp.ones(shape=1, dtype=G_H_DTYPE)
    ...
    assert predictor.nodes[0]["num_threshold"] == jnp.inf
    ...
    jnp.testing.assert_allclose(predictions, -gradients)
    jnp.testing.assert_allclose(predictions_binned, -gradients)


def test_grow_tree_categories():
    # Check that the grower produces the right predictor tree when a split is
    # categorical
    X_binned = jnp.array([[0, 1] * 11 + [1]], dtype=X_BINNED_DTYPE).T
    X_binned = jnp.asfortranarray(X_binned)

    all_gradients = jnp.array([10, 1] * 11 + [1], dtype=G_H_DTYPE)
    all_hessians = jnp.ones(1, dtype=G_H_DTYPE)
    is_categorical = jnp.ones(1, dtype=jnp.uint8)
    ...
    categories = [jnp.array([4, 9], dtype=X_DTYPE)]
    ...
    prediction_binned = predictor.predict_binned(
        jnp.asarray([[6]]).astype(X_BINNED_DTYPE),
    ...
    # prediction
    known_cat_bitsets = jnp.zeros((1, 8), dtype=jnp.uint32)  # ignored anyway
    f_idx_map = jnp.array([0], dtype=jnp.uint32)
    prediction = predictor.predict(
        jnp.array([[jnp.nan]]), known_cat_bitsets, f_idx_map, n_threads
    )
...
def test_ohe_equivalence(min_samples_leaf, n_unique_categories, target):
    ...
    X_ohe = jnp.asfortranarray(X_ohe).astype(jnp.uint8)
    ...
    predictor = grower.make_predictor(
        binning_thresholds=jnp.zeros((1, n_unique_categories))
    )
    ...
    predictor_ohe = grower_ohe.make_predictor(
        binning_thresholds=jnp.zeros((X_ohe.shape[1], n_unique_categories))
    )
    ...
    jnp.testing.assert_allclose(preds, preds_ohe)
