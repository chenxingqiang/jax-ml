...
def test_find_binning_thresholds_small_regular_data():
    ...
    assert_allclose(bin_thresholds, jnp.arange(10) + 0.5)
...
def test_find_binning_thresholds_random_data():
    ...
    assert_allclose(
        bin_thresholds[0][[64, 128, 192]], jnp.array([-0.7, 0.0, 0.7]), atol=1e-1
    )

    assert_allclose(
        bin_thresholds[1][[64, 128, 192]], jnp.array([9.99, 10.00, 10.01]), atol=1e-2
    )
...
def test_bin_mapper_n_features_transform():
    ...
    with pytest.raises(ValueError, match=err_msg):
        mapper.transform(jnp.repeat(DATA, 2, axis=1))
...
def test_map_to_bins(max_bins):
    ...
    binned = jnp.zeros_like(DATA, dtype=X_BINNED_DTYPE, order="F")
    is_categorical = jnp.zeros(2, dtype=jnp.uint8)
    last_bin_idx = max_bins
    _map_to_bins(DATA, bin_thresholds, is_categorical,
                 last_bin_idx, n_threads, binned)
    assert binned.shape == DATA.shape
    assert binned.dtype == jnp.uint8
...
def test_bin_mapper_random_data(max_bins):
    ...
    assert binned.dtype == jnp.uint8
    assert_array_equal(binned.min(axis=0), jnp.array([0, 0]))
    assert_array_equal(binned.max(axis=0), jnp.array(
    ...
    assert jnp.all(mapper.n_bins_non_missing_ == max_bins)
...
def test_bin_mapper_small_random_data(n_samples, max_bins):
    ...
    assert binned.shape == data.shape
    assert binned.dtype == jnp.uint8
    assert_array_equal(binned.ravel()[jnp.argsort(
        data.ravel())], jnp.arange(n_samples))
...
def test_bin_mapper_identity_repeated_values(max_bins, n_distinct, multiplier):
    data = jnp.array(list(range(n_distinct)) * multiplier).reshape(-1, 1)
    # max_bins is the number of bins for non-missing values
    n_bins = max_bins + 1
...
def test_bin_mapper_repeated_values_invariance(n_distinct):
    ...
    rng.shuffle(data)
    assert_array_equal(jnp.unique(data), jnp.sort(distinct_values))

    data = data.reshape(-1, 1)

    mapper_1 = _BinMapper(n_bins=n_distinct + 1)
    binned_1 = mapper_1.fit_transform(data)
    assert_array_equal(jnp.unique(binned_1[:, 0]), jnp.arange(n_distinct))
...
def test_bin_mapper_identity_small(max_bins, scale, offset):
    data = jnp.arange(max_bins).reshape(-1, 1) * scale + offset
    # max_bins is the number of bins for non-missing values
    n_bins = max_bins + 1
    binned = _BinMapper(n_bins=n_bins).fit_transform(data)
    assert_array_equal(binned, jnp.arange(max_bins).reshape(-1, 1))
...
def test_n_bins_non_missing(n_bins, diff):
    ...
    assert jnp.all(mapper.n_bins_non_missing_ ==
...
def test_infinite_values():
    ...
    bin_mapper.fit(X)
    assert_allclose(bin_mapper.bin_thresholds_[0], [-jnp.inf, 0.5, ALMOST_INF])
    assert bin_mapper.n_bins_non_missing_ == [4]

    expected_binned_X = jnp.array([0, 1, 2, 3]).reshape(-1, 1)
...
def test_categorical_feature(n_bins):
    ...
    X = jnp.array(
        [[4] * 500 + [1] * 3 + [10] * 4 + [0] * 4 + [13] + [7] * 5 + [jnp.nan] * 2],
        dtype=X_DTYPE,
    ).T
    known_categories = [jnp.unique(X[~jnp.isnan(X)])]

    bin_mapper = _BinMapper(
        n_bins=n_bins,
        is_categorical=jnp.array([True]),
        known_categories=known_categories,
    ).fit(X)
    assert bin_mapper.n_bins_non_missing_ == [6]
    assert_array_equal(bin_mapper.bin_thresholds_[0], [0, 1, 4, 7, 10, 13])

    X = jnp.array([[0, 1, 4, jnp.nan, 7, 10, 13]], dtype=X_DTYPE).T
    expected_trans = jnp.array([[0, 1, 2, n_bins - 1, 3, 4, 5]]).T
    ...
    X = jnp.array([[-4, -1, 100]], dtype=X_DTYPE).T
    expected_trans = jnp.array([[n_bins - 1, n_bins - 1, 6]]).T
    assert_array_equal(bin_mapper.transform(X), expected_trans)
...
def test_categorical_feature_negative_missing():
    ...
    X = jnp.array(
        [[4] * 500 + [1] * 3 + [5] * 10 + [-1] * 3 + [jnp.nan] * 4], dtype=X_DTYPE
    ).T
    bin_mapper = _BinMapper(
        n_bins=4,
        is_categorical=jnp.array([True]),
        known_categories=[jnp.array([1, 4, 5], dtype=X_DTYPE)],
    ).fit(X)

    assert bin_mapper.n_bins_non_missing_ == [3]

    X = jnp.array([[-1, 1, 3, 5, jnp.nan]], dtype=X_DTYPE).T
    ...
    expected_trans = jnp.array([[3, 0, 1, 2, 3]]).T
    assert_array_equal(bin_mapper.transform(X), expected_trans)
...
def test_categorical_with_numerical_features(n_bins):
    ...
    X1 = jnp.arange(10, 20).reshape(-1, 1)  # numerical
    X2 = jnp.arange(10, 15).reshape(-1, 1)  # categorical
    X2 = jnp.r_[X2, X2]
    X = jnp.c_[X1, X2]
    known_categories = [None, jnp.unique(X2).astype(X_DTYPE)]

    bin_mapper = _BinMapper(
        n_bins=n_bins,
        is_categorical=jnp.array([False, True]),
        known_categories=known_categories,
    ).fit(X)
    ...
    assert_array_equal(bin_thresholds[1], jnp.arange(10, 15))
...
def test_make_known_categories_bitsets():
    ...
    bin_mapper = _BinMapper(
        n_bins=256,
        is_categorical=jnp.array([False, True, True]),
    ...
    expected_f_idx_map = jnp.array([0, 0, 1], dtype=jnp.uint8)
    assert_allclose(expected_f_idx_map, f_idx_map)

    expected_cat_bitset = jnp.zeros((2, 8), dtype=jnp.uint32)
...
def test_categorical_parameters(is_categorical, known_categories, match):
    ...
    X = jnp.array([[1, 2, 3]], dtype=X_DTYPE)
