...
def test_regression_dataset(n_bins):
    ...
    hessians = jnp.ones(1, dtype=G_H_DTYPE)
...
def test_infinite_values_and_thresholds(num_threshold, expected_predictions):
    ...
    X = jnp.array([-jnp.inf, 10, 20, jnp.inf]).reshape(-1, 1)
    nodes = jnp.zeros(3, dtype=PREDICTOR_RECORD_DTYPE)
    ...
    binned_cat_bitsets = jnp.zeros((0, 8), dtype=X_BITSET_INNER_DTYPE)
    raw_categorical_bitsets = jnp.zeros((0, 8), dtype=X_BITSET_INNER_DTYPE)
    known_cat_bitset = jnp.zeros((0, 8), dtype=X_BITSET_INNER_DTYPE)
    f_idx_map = jnp.zeros(0, dtype=jnp.uint32)
    ...
    assert jnp.all(predictions == expected_predictions)
...
def test_categorical_predictor(bins_go_left, expected_predictions):
    ...
    X_binned = jnp.array([[0, 1, 2, 3, 4, 5]], dtype=X_BINNED_DTYPE).T
    categories = jnp.array([2, 5, 6, 8, 10, 15], dtype=X_DTYPE)

    bins_go_left = jnp.array(bins_go_left, dtype=X_BINNED_DTYPE)
    ...
    binned_cat_bitsets = jnp.zeros((1, 8), dtype=X_BITSET_INNER_DTYPE)
    raw_categorical_bitsets = jnp.zeros((1, 8), dtype=X_BITSET_INNER_DTYPE)
    ...
    known_cat_bitsets = jnp.zeros((1, 8), dtype=jnp.uint32)
    known_cat_bitsets[0, 0] = jnp.sum(2**categories, dtype=jnp.uint32)
    ...
    X_binned_missing = jnp.array([[6]], dtype=X_BINNED_DTYPE).T
    ...
    predictions = predictor.predict(
        jnp.array([[jnp.nan, 17]], dtype=X_DTYPE).T,
        known_cat_bitsets,
        f_idx_map,
        n_threads,
    )
