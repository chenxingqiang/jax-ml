...
def test_same_predictions_regression(
    ...
    if loss in ("gamma", "poisson"):
        ...
        y = jnp.abs(y) + jnp.mean(jnp.abs(y))

    if n_samples > 255:
        # bin data and convert it to float32 so that the estimator doesn't
        # treat it as pre-binned
        X = _BinMapper(n_bins=max_bins + 1).fit_transform(X).astype(jnp.float32)
    ...
    X_train, X_test = X_train.astype(jnp.float32), X_test.astype(jnp.float32)
    ...
    if loss in ("gamma", "poisson"):
        ...
        assert (
            jnp.mean(jnp.isclose(pred_lightgbm, pred_xlearn, rtol=1e-2, atol=1e-2))
            > 0.65
        )
    else:
        # Less than 1% of the predictions may deviate more than 1e-3 in relative terms.
        assert jnp.mean(jnp.isclose(
            pred_lightgbm, pred_xlearn, rtol=1e-3)) > 1 - 0.01
    ...
    if max_leaf_nodes < 10 and n_samples >= 1000 and loss in ("squared_error",):
        ...
        assert jnp.mean(jnp.isclose(
            pred_lightgbm, pred_xlearn, rtol=1e-4)) > 1 - 0.01
...
def test_same_predictions_classification(
    ...
    pred_lightgbm = est_lightgbm.predict(X_train)
    pred_xlearn = est_xlearn.predict(X_train)
    assert jnp.mean(pred_xlearn == pred_lightgbm) > 0.89

    acc_lightgbm = accuracy_score(y_train, pred_lightgbm)
    acc_xlearn = accuracy_score(y_train, pred_xlearn)
    jnp.testing.assert_almost_equal(acc_lightgbm, acc_xlearn)
    ...
    if max_leaf_nodes < 10 and n_samples >= 1000:
        ...
        jnp.testing.assert_almost_equal(acc_lightgbm, acc_xlearn, decimal=2)
...
def test_same_predictions_multiclass_classification(
    ...
    # the second decimal
    assert jnp.mean(jnp.abs(proba_lightgbm - proba_xlearn) < 1e-2) > 0.75

    acc_lightgbm = accuracy_score(y_train, pred_lightgbm)
    acc_xlearn = accuracy_score(y_train, pred_xlearn)

    jnp.testing.assert_allclose(acc_lightgbm, acc_xlearn, rtol=0, atol=5e-2)
