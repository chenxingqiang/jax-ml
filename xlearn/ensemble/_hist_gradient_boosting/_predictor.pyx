...
from xlearn.utils._typedefs cimport intp_t
...
def _compute_partial_dependence(
    ...
    cdef:
        ...
        unsigned int [:] node_idx_stack = jnp.zeros(shape=nodes.shape[0],
                                                   dtype=jnp.uint32)
        Y_DTYPE_C [::1] weight_stack = jnp.zeros(shape=nodes.shape[0],
                                                dtype=Y_DTYPE)
