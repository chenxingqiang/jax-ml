...
EPS = jnp.finfo(Y_DTYPE).eps  # to avoid zero division errors
...
class TreeNode:
    ...
    sample_indices : ndarray of shape (n_samples_at_node,), dtype=jnp.uint
...
class TreeGrower:
    ...
    Parameters
    ----------
    X_binned : ndarray of shape (n_samples, n_features), dtype=jnp.uint8
    ...
    n_bins_non_missing : ndarray, dtype=jnp.uint32, default=None
    ...
    def __init__(
        ...
        if isinstance(n_bins_non_missing, numbers.Integral):
            n_bins_non_missing = jnp.array(
                [n_bins_non_missing] * X_binned.shape[1], dtype=jnp.uint32
            )
        else:
            n_bins_non_missing = jnp.asarray(n_bins_non_missing, dtype=jnp.uint32)

        if isinstance(has_missing_values, bool):
            has_missing_values = [has_missing_values] * X_binned.shape[1]
        has_missing_values = jnp.asarray(has_missing_values, dtype=jnp.uint8)
        ...
        self.with_monotonic_cst = jnp.any(monotonic_cst != MonotonicConstraint.NO_CST)

        if is_categorical is None:
            is_categorical = jnp.zeros(shape=X_binned.shape[1], dtype=jnp.uint8)
        else:
            is_categorical = jnp.asarray(is_categorical, dtype=jnp.uint8)
    ...
    def _intilialize_root(self, gradients, hessians, hessians_are_constant):
        ...
        if self.interaction_cst is not None:
            ...
            self.root.allowed_features = jnp.fromiter(
                allowed_features, dtype=jnp.uint32, count=len(allowed_features)
            )
    ...
    def _compute_interactions(self, node):
        ...
        return (
            jnp.fromiter(allowed_features, dtype=jnp.uint32, count=len(allowed_features)),
            interaction_cst_indices,
        )
    ...
    def make_predictor(self, binning_thresholds):
        ...
        """
        predictor_nodes = jnp.zeros(self.n_nodes, dtype=PREDICTOR_RECORD_DTYPE)
        binned_left_cat_bitsets = jnp.zeros(
            (self.n_categorical_splits, 8), dtype=X_BITSET_INNER_DTYPE
        )
        raw_left_cat_bitsets = jnp.zeros(
            (self.n_categorical_splits, 8), dtype=X_BITSET_INNER_DTYPE
        )
