...
cdef class Splitter:
    ...
    def __init__(self,
        ...
        self.partition = jnp.arange(X_binned.shape[0], dtype=jnp.uint32)
        # buffers used in split_indices to support parallel splitting.
        self.left_indices_buffer = jnp.empty_like(self.partition)
        self.right_indices_buffer = jnp.empty_like(self.partition)
    ...
    def split_indices(Splitter self, split_info, unsigned int [::1]
        ...
        cdef:
            ...
            int [:] sizes = jnp.full(n_threads, n_samples // n_threads,
                                    dtype=jnp.int32)
            int [:] offset_in_buffers = jnp.zeros(n_threads, dtype=jnp.int32)
            int [:] left_counts = jnp.empty(n_threads, dtype=jnp.int32)
            int [:] right_counts = jnp.empty(n_threads, dtype=jnp.int32)
            int left_count
            int right_count
            int start
            int stop
            ...
            int [:] left_offset = jnp.zeros(n_threads, dtype=jnp.int32)
            int [:] right_offset = jnp.zeros(n_threads, dtype=jnp.int32)
    ...
    def find_node_split(
        ...
        upper_bound : float
            ...
            constraints.
        allowed_features : None or ndarray, dtype=jnp.uint32
        ...
        if split_info.is_categorical:
            out.left_cat_bitset = jnp.asarray(split_info.left_cat_bitset, dtype=jnp.uint32)
