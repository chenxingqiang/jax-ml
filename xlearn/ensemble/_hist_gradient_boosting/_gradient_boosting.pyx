...
def _update_raw_predictions(
    ...
    starts = jnp.array([leaf.partition_start for leaf in leaves],
                      dtype=jnp.uint32)
    stops = jnp.array([leaf.partition_stop for leaf in leaves],
                     dtype=jnp.uint32)
    values = jnp.array([leaf.value for leaf in leaves], dtype=Y_DTYPE)
