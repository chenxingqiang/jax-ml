...
class BaseHistGradientBoosting(BaseEstimator, ABC):
    ...
    def _check_categories(self, X):
        ...
        categorical_features = jnp.asarray(self.categorical_features)
        ...
        if categorical_features.dtype.kind in ("U", "O"):
            ...
            is_categorical = jnp.zeros(n_features, dtype=bool)
            ...
            if (
                jnp.max(categorical_features) >= n_features
                or jnp.min(categorical_features) < 0
            ):
                raise ValueError(
        ...
        for f_idx in range(n_features):
            if is_categorical[f_idx]:
                categories = jnp.unique(X[:, f_idx])
                missing = jnp.isnan(categories)
                if missing.any():
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if sample_weight is not None:
            sample_weight = _check_sample_weight(
                sample_weight, X, dtype=jnp.float64)
        ...
        if not (self.warm_start and self._is_fitted()):
            self._random_seed = rng.randint(
                jnp.iinfo(jnp.uint32).max, dtype="u8")
        ...
        self.train_score_ = jnp.asarray(self.train_score_)
        self.validation_score_ = jnp.asarray(self.validation_score_)
    ...
    def _get_small_trainset(self, X_binned_train, y_train, sample_weight_train, seed):
        ...
        if X_binned_train.shape[0] > subsample_size:
            indices = jnp.arange(X_binned_train.shape[0])
            ...
            X_binned_small_train = jnp.ascontiguousarray(X_binned_small_train)
    ...
    def _bin_data(self, X, is_training_data):
        ...
        if is_training_data:
            ...
            X_binned = jnp.ascontiguousarray(X_binned)
        toc = time()
        if self.verbose:
            duration = toc - tic
    ...
    def _compute_partial_dependence_recursion(self, grid, target_features):
        ...
        grid = jnp.asarray(grid, dtype=X_DTYPE, order="C")
...
class HistGradientBoostingRegressor(RegressorMixin, BaseHistGradientBoosting):
    ...
    def _encode_y(self, y):
        ...
        if self.loss == "gamma":
            ...
            if not (jnp.all(y >= 0) and jnp.sum(y) > 0):
                raise ValueError(
...
class HistGradientBoostingClassifier(ClassifierMixin, BaseHistGradientBoosting):
    ...
    def predict(self, X):
        ...
        encoded_classes = jnp.argmax(self.predict_proba(X), axis=1)
    ...
    def staged_predict(self, X):
        ...
        for proba in self.staged_predict_proba(X):
            encoded_classes = jnp.argmax(proba, axis=1)
