...
def _find_binning_thresholds(col_data, max_bins):
    ...
    missing_mask = jnp.isnan(col_data)
    if missing_mask.any():
        col_data = col_data[~missing_mask]
    col_data = jnp.ascontiguousarray(col_data, dtype=X_DTYPE)
    distinct_values = jnp.unique(col_data)
    if len(distinct_values) <= max_bins:
        ...
        # jnp.unique(col_data, return_counts) instead but this is more
        # work and the performance benefit will be limited because we
        # work on a fixed-size subsample of the full data.
        percentiles = jnp.linspace(0, 100, num=max_bins + 1)
    ...
    jnp.clip(midpoints, a_min=None, a_max=ALMOST_INF, out=midpoints)
    return midpoints
...
class _BinMapper(TransformerMixin, BaseEstimator):
    ...
    n_bins_non_missing_ : ndarray, dtype=jnp.uint32
        For each feature, gives the number of bins actually used for
        non-missing values. For features with a lot of unique values, this is
        equal to ``n_bins - 1``.
    is_categorical_ : ndarray of shape (n_features,), dtype=jnp.uint8
        Indicator for categorical features.
    missing_values_bin_idx_ : jnp.uint8
    ...
    def fit(self, X, y=None):
        ...
        if self.is_categorical is None:
            self.is_categorical_ = jnp.zeros(X.shape[1], dtype=jnp.uint8)
        else:
            self.is_categorical_ = jnp.asarray(self.is_categorical, dtype=jnp.uint8)
        ...
        self.n_bins_non_missing_ = jnp.array(n_bins_non_missing, dtype=jnp.uint32)
        return self
    ...
    def transform(self, X):
        ...
        binned = jnp.zeros_like(X, dtype=X_BINNED_DTYPE, order="F")
        _map_to_bins(
            X,
            self.bin_thresholds_,
    ...
    def make_known_categories_bitsets(self):
        ...
        categorical_features_indices = jnp.flatnonzero(self.is_categorical_)

        n_features = self.is_categorical_.size
        n_categorical_features = categorical_features_indices.size

        f_idx_map = jnp.zeros(n_features, dtype=jnp.uint32)
        f_idx_map[categorical_features_indices] = jnp.arange(
            n_categorical_features, dtype=jnp.uint32
        )

        known_categories = self.bin_thresholds_

        known_cat_bitsets = jnp.zeros(
