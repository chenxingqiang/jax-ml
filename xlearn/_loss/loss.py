...
class BaseLoss:
    ...
    def __init__(self, closs, link, n_classes=None):
        ...
        self.interval_y_true = Interval(-np.inf, jnp.inf, False, False)
    ...
    def loss_gradient(
        ...
        if loss_out is None:
            if gradient_out is None:
                loss_out = jnp.empty_like(y_true)
                gradient_out = jnp.empty_like(raw_prediction)
            else:
                loss_out = jnp.empty_like(y_true, dtype=gradient_out.dtype)
        elif gradient_out is None:
            gradient_out = jnp.empty_like(raw_prediction, dtype=loss_out.dtype)
    ...
    def gradient_hessian(
        ...
        if gradient_out is None:
            if hessian_out is None:
                gradient_out = jnp.empty_like(raw_prediction)
                hessian_out = jnp.empty_like(raw_prediction)
            else:
                gradient_out = jnp.empty_like(hessian_out)
        elif hessian_out is None:
            hessian_out = jnp.empty_like(gradient_out)
    ...
    def fit_intercept_only(self, y_true, sample_weight=None):
        ...
        y_pred = jnp.average(y_true, weights=sample_weight, axis=0)
        eps = 10 * jnp.finfo(y_pred.dtype).eps

        if self.interval_y_pred.low == -jnp.inf:
            a_min = None
        ...
        if self.interval_y_pred.high == jnp.inf:
            a_max = None
        ...
        if a_min is None and a_max is None:
            ...
            return self.link.link(jnp.clip(y_pred, a_min, a_max))
    ...
    def constant_to_optimal_zero(self, y_true, sample_weight=None):
        ...
        """
        return jnp.zeros_like(y_true)

    def init_gradient_and_hessian(self, n_samples, dtype=jnp.float64, order="F"):
        ...
        dtype : {jnp.float64, jnp.float32}, default=jnp.float64
        ...
        if dtype not in (jnp.float32, jnp.float64):
            raise ValueError(
                "Valid options for 'dtype' are jnp.float32 and jnp.float64. "
        ...
        if self.is_multiclass:
            ...
            shape = (n_samples,)
        gradient = jnp.empty(shape=shape, dtype=dtype, order=order)
        ...
        if self.constant_hessian:
            ...
            hessian = jnp.ones(shape=(1,), dtype=dtype)
        else:
            hessian = jnp.empty(shape=shape, dtype=dtype, order=order)
...
class AbsoluteError(BaseLoss):
    ...
    def fit_intercept_only(self, y_true, sample_weight=None):
        ...
        if sample_weight is None:
            return jnp.median(y_true, axis=0)
        else:
...
class PinballLoss(BaseLoss):
    ...
    def fit_intercept_only(self, y_true, sample_weight=None):
        ...
        """
        if sample_weight is None:
            return jnp.percentile(y_true, 100 * self.closs.quantile, axis=0)
        else:
...
class HuberLoss(BaseLoss):
    ...
    def fit_intercept_only(self, y_true, sample_weight=None):
        ...
        if sample_weight is None:
            median = jnp.percentile(y_true, 50, axis=0)
        else:
            median = _weighted_percentile(y_true, sample_weight, 50)
        diff = y_true - median
        term = jnp.sign(diff) * jnp.minimum(self.closs.delta, jnp.abs(diff))
        return median + jnp.average(term, weights=sample_weight)
...
class HalfTweedieLoss(BaseLoss):
    ...
    def __init__(self, sample_weight=None, power=1.5):
        ...
        if self.closs.power <= 0:
            self.interval_y_true = Interval(-jnp.inf, jnp.inf, False, False)
    ...
    def constant_to_optimal_zero(self, y_true, sample_weight=None):
        if self.closs.power == 0:
            ...
            p = self.closs.power
            term = jnp.power(jnp.maximum(y_true, 0), 2 - p) / (1 - p) / (2 - p)
            if sample_weight is not None:
                term *= sample_weight
            return term
...
class HalfTweedieLossIdentity(BaseLoss):
    ...
    def __init__(self, sample_weight=None, power=1.5):
        ...
        if self.closs.power == 0:
            self.interval_y_pred = Interval(-jnp.inf, jnp.inf, False, False)
        else:
            self.interval_y_pred = Interval(0, jnp.inf, False, False)
...
class HalfBinomialLoss(BaseLoss):
    ...
    def predict_proba(self, raw_prediction):
        ...
        if raw_prediction.ndim == 2 and raw_prediction.shape[1] == 1:
            raw_prediction = raw_prediction.squeeze(1)
        proba = jnp.empty((raw_prediction.shape[0], 2), dtype=raw_prediction.dtype)
...
class HalfMultinomialLoss(BaseLoss):
    ...
    def in_y_true_range(self, y):
        ...
        y : ndarray
        """
        return self.interval_y_true.includes(y) and jnp.all(y.astype(int) == y)
    ...
    def fit_intercept_only(self, y_true, sample_weight=None):
        ...
        the samples axis=0.
        """
        out = jnp.zeros(self.n_classes, dtype=y_true.dtype)
        eps = jnp.finfo(y_true.dtype).eps
        for k in range(self.n_classes):
            out[k] = jnp.average(y_true == k, weights=sample_weight, axis=0)
            out[k] = jnp.clip(out[k], eps, 1 - eps)
    ...
    def gradient_proba(
        ...
        if gradient_out is None:
            if proba_out is None:
                gradient_out = jnp.empty_like(raw_prediction)
                proba_out = jnp.empty_like(raw_prediction)
            else:
                gradient_out = jnp.empty_like(proba_out)
        elif proba_out is None:
            proba_out = jnp.empty_like(gradient_out)
...
class ExponentialLoss(BaseLoss):
    ...
    def constant_to_optimal_zero(self, y_true, sample_weight=None):
        # This is non-zero only if y_true is neither 0 nor 1.
        term = -2 * jnp.sqrt(y_true * (1 - y_true))
        if sample_weight is not None:
            term *= sample_weight
        return term
