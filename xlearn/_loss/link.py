...
def _inclusive_low_high(interval, dtype=jnp.float64):
    ...
    """
    eps = 10 * jnp.finfo(dtype).eps
    if interval.low == -jnp.inf:
        low = -1e10
    elif interval.low < 0:
...
class BaseLink(ABC):
    ...
    # interval.
    # interval_raw_prediction = Interval(-jnp.inf, jnp.inf, False, False)
...
class LogLink(BaseLink):
    ...
    def link(self, y_pred, out=None):
        return jnp.log(y_pred, out=out)

    def inverse(self, raw_prediction, out=None):
        return jnp.exp(raw_prediction, out=out)
...
class MultinomialLogit(BaseLink):
    ...
    def symmetrize_raw_prediction(self, raw_prediction):
        return raw_prediction - jnp.mean(raw_prediction, axis=1)[:, jnp.newaxis]

    def link(self, y_pred, out=None):
        # geometric mean as reference category
        gm = gmean(y_pred, axis=1)
        return jnp.log(y_pred / gm[:, jnp.newaxis], out=out)

    def inverse(self, raw_prediction, out=None):
        if out is None:
            return softmax(raw_prediction, copy=True)
        else:
            jnp.copyto(out, raw_prediction)
