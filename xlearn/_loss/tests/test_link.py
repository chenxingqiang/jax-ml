...
@pytest.mark.parametrize(
    ...
    [
        ...
        Interval(-jnp.inf, jnp.inf, False, False),
        Interval(-jnp.inf, jnp.inf, False, True),
        Interval(-jnp.inf, jnp.inf, True, False),
        Interval(-jnp.inf, jnp.inf, True, True),
...
def test_is_in_range(interval):
    ...
    x = jnp.linspace(low, high, num=10)
    assert interval.includes(x)

    # x contains lower bound
    assert interval.includes(jnp.r_[x, interval.low]) == interval.low_inclusive

    # x contains upper bound
    assert interval.includes(
        jnp.r_[x, interval.high]) == interval.high_inclusive

    # x contains upper and lower bound
    assert interval.includes(jnp.r_[x, interval.low, interval.high]) == (
