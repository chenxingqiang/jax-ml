...
def random_y_true_raw_prediction(
    ...
    if loss.is_multiclass:
        ...
        y_true = jnp.arange(n_samples).astype(float) % loss.n_classes
    else:
        ...
        if isinstance(loss.link, IdentityLink):
            ...
            low = jnp.amax([low, raw_bound[0]])
            high = jnp.amin([high, raw_bound[1]])
            raw_bound = (low, high)
...
def numerical_derivative(func, x, eps):
    ...
    h = jnp.full_like(x, fill_value=eps)
...
def test_loss_boundary(loss):
    ...
    if loss.is_multiclass:
        y_true = jnp.linspace(0, 9, num=10)
    else:
        low, high = _inclusive_low_high(loss.interval_y_true)
        y_true = jnp.linspace(low, high, num=10)

    # add boundaries if they are included
    if loss.interval_y_true.low_inclusive:
        y_true = jnp.r_[y_true, loss.interval_y_true.low]
    if loss.interval_y_true.high_inclusive:
        y_true = jnp.r_[y_true, loss.interval_y_true.high]
    ...
    low, high = _inclusive_low_high(loss.interval_y_pred)
    if loss.is_multiclass:
        y_pred = jnp.empty((n, 3))
        y_pred[:, 0] = jnp.linspace(low, high, num=n)
        y_pred[:, 1] = 0.5 * (1 - y_pred[:, 0])
        y_pred[:, 2] = 0.5 * (1 - y_pred[:, 0])
    else:
        y_pred = jnp.linspace(low, high, num=n)
...
Y_COMMON_PARAMS = [
    ...
    (HalfSquaredError(), [-100, 0, 0.1, 100], [-jnp.inf, jnp.inf]),
    (AbsoluteError(), [-100, 0, 0.1, 100], [-jnp.inf, jnp.inf]),
    (PinballLoss(), [-100, 0, 0.1, 100], [-jnp.inf, jnp.inf]),
    (HuberLoss(), [-100, 0, 0.1, 100], [-jnp.inf, jnp.inf]),
    (HalfPoissonLoss(), [0.1, 100], [-jnp.inf, -3, -0.1, jnp.inf]),
    (HalfGammaLoss(), [0.1, 100], [-jnp.inf, -3, -0.1, 0, jnp.inf]),
    (HalfTweedieLoss(power=-3), [0.1, 100], [-jnp.inf, jnp.inf]),
    (HalfTweedieLoss(power=0), [0.1, 100], [-jnp.inf, jnp.inf]),
    (HalfTweedieLoss(power=1.5), [0.1, 100], [-jnp.inf, -3, -0.1, jnp.inf]),
    (HalfTweedieLoss(power=2), [0.1, 100], [-jnp.inf, -3, -0.1, 0, jnp.inf]),
    (HalfTweedieLoss(power=3), [0.1, 100], [-jnp.inf, -3, -0.1, 0, jnp.inf]),
    (HalfTweedieLossIdentity(power=-3), [0.1, 100], [-jnp.inf, jnp.inf]),
    (HalfTweedieLossIdentity(power=0),
     [-3, -0.1, 0, 0.1, 100], [-jnp.inf, jnp.inf]),
    (HalfTweedieLossIdentity(power=1.5), [
     0.1, 100], [-jnp.inf, -3, -0.1, jnp.inf]),
    (HalfTweedieLossIdentity(power=2), [
     0.1, 100], [-jnp.inf, -3, -0.1, 0, jnp.inf]),
    (HalfTweedieLossIdentity(power=3), [
     0.1, 100], [-jnp.inf, -3, -0.1, 0, jnp.inf]),
    (HalfBinomialLoss(), [0.1, 0.5, 0.9], [-jnp.inf, -1, 2, jnp.inf]),
    (HalfMultinomialLoss(), [], [-jnp.inf, -1, 1.1, jnp.inf]),
]
...
def test_loss_boundary_y_true(loss, y_true_success, y_true_fail):
    ...
    for y in y_true_success:
        assert loss.in_y_true_range(jnp.array([y]))
    for y in y_true_fail:
        assert not loss.in_y_true_range(jnp.array([y]))
...
def test_loss_boundary_y_pred(loss, y_pred_success, y_pred_fail):
    ...
    for y in y_pred_success:
        assert loss.in_y_pred_range(jnp.array([y]))
    for y in y_pred_fail:
        assert not loss.in_y_pred_range(jnp.array([y]))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (HalfPoissonLoss(), 2.0, jnp.log(4), 4 - 2 * jnp.log(4)),
        (HalfGammaLoss(), 2.0, jnp.log(4), jnp.log(4) + 2 / 4),
        (HalfTweedieLoss(power=3), 2.0, jnp.log(4), -1 / 4 + 1 / 4**2),
        (HalfTweedieLossIdentity(power=1), 2.0, 4.0, 2 - 2 * jnp.log(2)),
        (HalfTweedieLossIdentity(power=2), 2.0, 4.0, jnp.log(2) - 1 / 2),
        (HalfTweedieLossIdentity(power=3), 2.0,
         4.0, -1 / 4 + 1 / 4**2 + 1 / 2 / 2),
        (HalfBinomialLoss(), 0.25, jnp.log(4), jnp.log(5) - 0.25 * jnp.log(4)),
        (
...
def test_loss_on_specific_values(loss, y_true, raw_prediction, loss_true):
    ...
    assert loss(
        y_true=jnp.array([y_true]), raw_prediction=jnp.array([raw_prediction])
...
@pytest.mark.parametrize("dtype_in", [jnp.float32, jnp.float64])
@pytest.mark.parametrize("dtype_out", [jnp.float32, jnp.float64])
...
def test_loss_dtype(
    ...
    if sample_weight is not None:
        sample_weight = jnp.array([2.0] * n_samples, dtype=dtype_in)
    if out1 is not None:
        out1 = jnp.empty_like(y_true, dtype=dtype_out)
    if out2 is not None:
        out2 = jnp.empty_like(raw_prediction, dtype=dtype_out)
    ...
    if out1 is not None and loss.is_multiclass:
        out1 = jnp.empty_like(raw_prediction, dtype=dtype_out)
...
def test_loss_same_as_C_functions(loss, sample_weight):
    ...
    if sample_weight == "range":
        sample_weight = jnp.linspace(1, y_true.shape[0], num=y_true.shape[0])

    out_l1 = jnp.empty_like(y_true)
    out_l2 = jnp.empty_like(y_true)
    out_g1 = jnp.empty_like(raw_prediction)
    out_g2 = jnp.empty_like(raw_prediction)
    out_h1 = jnp.empty_like(raw_prediction)
    out_h2 = jnp.empty_like(raw_prediction)
    assert_allclose(
        loss.loss(
            y_true=y_true,
            raw_prediction=raw_prediction,
...
def test_loss_gradients_are_the_same(loss, sample_weight, global_random_seed):
    ...
    out_g1 = jnp.empty_like(raw_prediction)
    out_g2 = jnp.empty_like(raw_prediction)
    out_g3 = jnp.empty_like(raw_prediction)
    out_h3 = jnp.empty_like(raw_prediction)
    ...
    assert jnp.shares_memory(l1, out_l1)
    assert_array_equal(l2, out_l2)
    assert jnp.shares_memory(l2, out_l2)
    assert_allclose(g1, g2)
    assert_allclose(g1, g3)
    assert_array_equal(g1, out_g1)
    assert jnp.shares_memory(g1, out_g1)
    assert_array_equal(g2, out_g2)
    assert jnp.shares_memory(g2, out_g2)
    assert_array_equal(g3, out_g3)
    assert jnp.shares_memory(g3, out_g3)

    if hasattr(loss, "gradient_proba"):
        assert loss.is_multiclass  # only for HalfMultinomialLoss
        out_g4 = jnp.empty_like(raw_prediction)
        out_proba = jnp.empty_like(raw_prediction)
        ...
        assert_allclose(jnp.sum(proba, axis=1), 1, rtol=1e-11)
...
def test_loss_of_perfect_prediction(loss, sample_weight):
    ...
    if not loss.is_multiclass:
        ...
        raw_prediction = jnp.array([-10, -0.1, 0, 0.1, 3, 10])
        # If link is identity, we must respect the interval of y_pred:
        if isinstance(loss.link, IdentityLink):
            eps = 1e-10
            ...
            if not loss.interval_y_pred.high_inclusive:
                high = high - eps
            raw_prediction = jnp.clip(raw_prediction, low, high)
        y_true = loss.link.inverse(raw_prediction)
    else:
        # HalfMultinomialLoss
        y_true = jnp.arange(loss.n_classes).astype(float)
        # raw_prediction with entries -exp(10), but +exp(10) on the diagonal
        # this is close enough to jnp.inf which would produce nan
        raw_prediction = jnp.full(
            shape=(loss.n_classes, loss.n_classes),
            fill_value=-jnp.exp(10),
            dtype=float,
        )
        raw_prediction.flat[:: loss.n_classes + 1] = jnp.exp(10)
...
def test_derivatives(loss, x0, y_true):
    ...
    def fprime(x: jnp.ndarray) -> jnp.ndarray:
        return loss.gradient(y_true=y_true, raw_prediction=x)

    def fprime2(x: jnp.ndarray) -> jnp.ndarray:
...
def test_loss_intercept_only(loss, sample_weight):
    ...
    n_samples = 50
    if not loss.is_multiclass:
        y_true = loss.link.inverse(jnp.linspace(-4, 4, num=n_samples))
    else:
        y_true = jnp.arange(n_samples).astype(jnp.float64) % loss.n_classes
        y_true[::5] = 0  # exceedance of class 0

    if sample_weight == "range":
        sample_weight = jnp.linspace(0.1, 2, num=n_samples)
    ...
    def fun(x):
        if not loss.is_multiclass:
            raw_prediction = jnp.full(shape=(n_samples), fill_value=x)
        else:
            raw_prediction = jnp.ascontiguousarray(
                jnp.broadcast_to(x, shape=(n_samples, loss.n_classes))
            )
        return loss(
            y_true=y_true,
            raw_prediction=raw_prediction,
    ...
    if not loss.is_multiclass:
        ...
        grad = loss.gradient(
            y_true=y_true,
            raw_prediction=jnp.full_like(y_true, a),
            sample_weight=sample_weight,
        )
        ...
        opt = minimize(
            fun,
            jnp.zeros((loss.n_classes)),
            tol=1e-13,
            options={"maxiter": 100},
            method="SLSQP",
            constraints=LinearConstraint(jnp.ones((1, loss.n_classes)), 0, 0),
        )
        grad = loss.gradient(
            y_true=y_true,
            raw_prediction=jnp.tile(a, (n_samples, 1)),
            sample_weight=sample_weight,
        )
        assert a.dtype == y_true.dtype
        assert_all_finite(a)
...
@pytest.mark.parametrize(
    ...
    [
        (HalfSquaredError(), jnp.mean, "normal"),
        (AbsoluteError(), jnp.median, "normal"),
        (PinballLoss(quantile=0.25), lambda x: jnp.percentile(x, q=25), "normal"),
        (HalfPoissonLoss(), jnp.mean, "poisson"),
        (HalfGammaLoss(), jnp.mean, "exponential"),
        (HalfTweedieLoss(), jnp.mean, "exponential"),
        (HalfBinomialLoss(), jnp.mean, "binomial"),
    ],
)
...
def test_multinomial_loss_fit_intercept_only():
    ...
    y_train = rng.randint(0, n_classes + 1, size=100).astype(jnp.float64)
    baseline_prediction = loss.fit_intercept_only(y_true=y_train)
    assert baseline_prediction.shape == (n_classes,)
    p = jnp.zeros(n_classes, dtype=y_train.dtype)
    for k in range(n_classes):
        p[k] = (y_train == k).mean()
    assert_allclose(baseline_prediction, jnp.log(p) - jnp.mean(jnp.log(p)))
    assert_allclose(baseline_prediction[None, :], loss.link.link(p[None, :]))

    for y_train in (jnp.zeros(shape=10), jnp.ones(shape=10)):
        y_train = y_train.astype(jnp.float64)
        baseline_prediction = loss.fit_intercept_only(y_true=y_train)
...
def test_binomial_and_multinomial_loss(global_random_seed):
    ...
    y_train = rng.randint(0, 2, size=n_samples).astype(jnp.float64)
    raw_prediction = rng.normal(size=n_samples)
    raw_multinom = jnp.empty((n_samples, 2))
...
@pytest.mark.parametrize("y_true", (jnp.array([0.0, 0, 0]), jnp.array([1.0, 1, 1])))
@pytest.mark.parametrize("y_pred", (jnp.array([-5.0, -5, -5]), jnp.array([3.0, 3, 3])))
def test_binomial_vs_alternative_formulation(y_true, y_pred, global_dtype):
    ...
    def alt_loss(y, raw_pred):
        z = 2 * y - 1
        return jnp.mean(jnp.log(1 + jnp.exp(-z * raw_pred)))

    def alt_gradient(y, raw_pred):
        # alternative gradient formula according to ESL
        z = 2 * y - 1
        return -z / (1 + jnp.exp(z * raw_pred))
...
def test_predict_proba(loss, global_random_seed):
    ...
    if hasattr(loss, "predict_proba"):
        ...
        assert jnp.sum(proba, axis=1) == approx(1, rel=1e-11)

    if hasattr(loss, "gradient_proba"):
        for grad, proba in (
            (None, None),
            (None, jnp.empty_like(raw_prediction)),
            (jnp.empty_like(raw_prediction), None),
            (jnp.empty_like(raw_prediction), jnp.empty_like(raw_prediction)),
        ):
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"dtype": jnp.int64},
            f"Valid options for 'dtype' are .* Got dtype={jnp.int64} instead.",
        ),
    ],
)
