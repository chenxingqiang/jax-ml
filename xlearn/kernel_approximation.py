"""
The :mod:`xlearn.kernel_approximation` module implements several
...
class PolynomialCountSketch(
    ...
    def transform(self, X):
        ...
        X_gamma = jnp.sqrt(self.gamma) * X

        if sp.issparse(X_gamma) and self.coef0 != 0:
            X_gamma = sp.hstack(
                [X_gamma, jnp.sqrt(self.coef0) *
                 jnp.ones((X_gamma.shape[0], 1))],
                format="csc",
            )

        elif not sp.issparse(X_gamma) and self.coef0 != 0:
            X_gamma = jnp.hstack(
                [X_gamma, jnp.sqrt(self.coef0) * jnp.ones((X_gamma.shape[0], 1))]
            )
        ...
        count_sketches_fft_prod = jnp.prod(count_sketches_fft, axis=1)
        data_sketch = jnp.real(ifft(count_sketches_fft_prod, overwrite_x=True))
...
class RBFSampler(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    Attributes
    ----------
    random_offset_ : ndarray of shape (n_components,), dtype={jnp.float64, jnp.float32}
    ...
    def fit(self, X, y=None):
        ...
        self.random_offset_ = random_state.uniform(
            0, 2 * jnp.pi, size=self.n_components)
...
class SkewedChi2Sampler(
    ...
    def fit(self, X, y=None):
        ...
        self.random_weights_ = 1.0 / jnp.pi * \
            jnp.log(jnp.tan(jnp.pi / 2.0 * uniform))
        self.random_offset_ = random_state.uniform(
            0, 2 * jnp.pi, size=self.n_components)
    ...
    def transform(self, X):
        ...
        X = self._validate_data(
            X, copy=True, dtype=[jnp.float64, jnp.float32], reset=False
        )
        ...
        projection += self.random_offset_
        jnp.cos(projection, projection)
        projection *= jnp.sqrt(2.0) / jnp.sqrt(self.n_components)
        return projection
...
class AdditiveChi2Sampler(TransformerMixin, BaseEstimator):
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        return jnp.asarray(names_list, dtype=object)
    ...
    def _transform_dense(X, sample_steps, sample_interval):
        ...
        X_step = jnp.zeros_like(X)
        X_step[non_zero] = jnp.sqrt(X_nz * sample_interval)

        X_new = [X_step]

        log_step_nz = sample_interval * jnp.log(X_nz)
        step_nz = 2 * X_nz * sample_interval

        for j in range(1, sample_steps):
            factor_nz = jnp.sqrt(step_nz / jnp.cosh(jnp.pi * j * sample_interval))

            X_step = jnp.zeros_like(X)
            X_step[non_zero] = factor_nz * jnp.cos(j * log_step_nz)
            X_new.append(X_step)

            X_step = jnp.zeros_like(X)
            X_step[non_zero] = factor_nz * jnp.sin(j * log_step_nz)
            X_new.append(X_step)

        return jnp.hstack(X_new)
    ...
    def _transform_sparse(X, sample_steps, sample_interval):
        ...
        data_step = jnp.sqrt(X.data * sample_interval)
        X_step = sp.csr_matrix(
            (data_step, indices, indptr), shape=X.shape, dtype=X.dtype, copy=False
        )
        X_new = [X_step]

        log_step_nz = sample_interval * jnp.log(X.data)
        step_nz = 2 * X.data * sample_interval

        for j in range(1, sample_steps):
            factor_nz = jnp.sqrt(step_nz / jnp.cosh(jnp.pi * j * sample_interval))

            data_step = factor_nz * jnp.cos(j * log_step_nz)
            X_step = sp.csr_matrix(
                (data_step, indices, indptr), shape=X.shape, dtype=X.dtype, copy=False
            )
            X_new.append(X_step)

            data_step = factor_nz * jnp.sin(j * log_step_nz)
            X_step = sp.csr_matrix(
                (data_step, indices, indptr), shape=X.shape, dtype=X.dtype, copy=False
            )
            X_new.append(X_step)
...
class Nystroem(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    * T. Yang, Y. Li, M. Mahdavi, R. Jin and Z. Zhou
    ...
    def fit(self, X, y=None):
        ...
        self.normalization_ = jnp.dot(U / jnp.sqrt(S), V)
    ...
    def transform(self, X):
        ...
        return jnp.dot(embedded, self.normalization_.T)
