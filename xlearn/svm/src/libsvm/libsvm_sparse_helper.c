 ...
 */
struct svm_csr_node **csr_to_libsvm(double *values, int *indices, int *indptr,
                                    npy_int n_samples) {
...
set_parameter(int svm_type, int kernel_type, int degree, double gamma,
              double coef0, double nu, double cache_size, double C, double eps,
              double p, int shrinking, int probability, int nr_weight,
...
/*
 * Create and return a svm_csr_problem struct from a scipy.sparse.csr matrix. It
 * is up to the user to free resulting structure.
 *
 * TODO: precomputed kernel.
 */
struct svm_csr_problem *csr_set_problem(char *values, npy_intp *n_indices,
                                        char *indices, npy_intp *n_indptr,
                                        char *indptr, char *Y,
...
struct svm_csr_model *csr_set_model(struct svm_parameter *param, int nr_class,
                                    ...
                                    char *SV_intptr, char *sv_coef, char *rho,
 ...
 */
int csr_copy_SV(char *data, npy_intp *n_indices, char *indices,
                npy_intp *n_indptr, char *indptr, struct svm_csr_model *model,
                int n_features) {
  int i, j, k = 0, index;
...
/*
 * Predict using a model, where data is expected to be encoded into a csr
 * matrix.
 */
int csr_copy_predict(npy_intp *data_size, char *data, npy_intp *index_size,
                     char *index, npy_intp *intptr_size, char *intptr,
                     struct svm_csr_model *model, char *dec_values,
                     BlasFunctions *blas_functions) {
  double *t = (double *)dec_values;
  struct svm_csr_node **predict_nodes;
  npy_intp i;

  predict_nodes = csr_to_libsvm((double *)data, (int *)index, (int *)intptr,
                                intptr_size[0] - 1);
...
int csr_copy_predict_values(npy_intp *data_size, char *data,
                            npy_intp *index_size, char *index,
                            npy_intp *intptr_size, char *intptr,
                            struct svm_csr_model *model, char *dec_values,
                            int nr_class, BlasFunctions *blas_functions) {
  struct svm_csr_node **predict_nodes;
  npy_intp i;
...
int csr_copy_predict_proba(npy_intp *data_size, char *data,
                           npy_intp *index_size, char *index,
                           npy_intp *intptr_size, char *intptr,
                           struct svm_csr_model *model, char *dec_values,
                           BlasFunctions *blas_functions) {
  ...
  if (predict_nodes == NULL)
    return -1;
  for (i = 0; i < intptr_size[0] - 1; ++i) {
    svm_csr_predict_probability(model, predict_nodes[i],
                                ((double *)dec_values) + i * m, blas_functions);
    free(predict_nodes[i]);
  }
  free(predict_nodes);
  return 0;
 ...
 */
npy_intp get_l(struct svm_csr_model *model) { return (npy_intp)model->l; }
