...
static int solve_l2r_l1l2_svc(const problem *prob, double *w, double eps,
...
static int solve_l2r_l1l2_svr(const problem *prob, double *w,
...
int solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp,
                      double Cn, int max_iter) {
  int l = prob->l;
  int w_size = prob->n;
  int i, s, iter = 0;
...
static int solve_l1r_l2_svc(problem *prob_col, double *w, double eps, double Cp,
                            double Cn, int max_iter) {
  int l = prob_col->l;
...
static int solve_l1r_lr(const problem *prob_col, double *w, double eps,
...
static int train_one(const problem *prob, const parameter *param, double *w,
                     double Cp, double Cn, BlasFunctions *blas_functions) {
  double eps = param->eps;
...
model *train(const problem *prob, const parameter *param,
  ...
  if (check_regression_model(model_)) {
    ...
    model_->label = NULL;
    model_->n_iter[0] =
        train_one(prob, param, &model_->w[0], 0, 0, blas_functions);
  } else {
    int nr_class;
    int *label = NULL;
    int *start = NULL;
    ...
    if (param->solver_type == MCSVM_CS) {
      ...
      if (nr_class == 2) {
        ...
        model_->n_iter[0] =
            train_one(&sub_prob, param, &model_->w[0], weighted_C[1],
                      weighted_C[0], blas_functions);
      } else {
        ...
        for (i = 0; i < nr_class; i++) {
          ...
          model_->n_iter[i] = train_one(&sub_prob, param, w, weighted_C[i],
                                        param->C, blas_functions);
