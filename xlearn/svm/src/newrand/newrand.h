...
// Jax-Learn-specific random number generator replacing `rand()` originally
...
// - (2) public `set_seed()` function that should be used instead of `srand()`
// to set a new seed.
void set_seed(unsigned custom_seed) { mt_rand.seed(custom_seed); }

// - (3) New internal `bounded_rand_int` function, used instead of rand()
// everywhere.
