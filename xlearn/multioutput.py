...
class _MultiOutputEstimator(MetaEstimatorMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None, **partial_fit_params):
        ...
        classes : list of ndarray of shape (n_outputs,), default=None
            ...
            Can be obtained via
            ``[jnp.unique(y[:, i]) for i in range(y.shape[1])]``, where `y`
...
class MultiOutputClassifier(ClassifierMixin, _MultiOutputEstimator):
    ...
    def score(self, X, y):
        ...
        return jnp.mean(jnp.all(y == y_pred, axis=1))
...
class _BaseChain(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, Y, **fit_params):
        ...
        **fit_params : dict of string -> object
        ...
        if isinstance(self.order_, tuple):
            self.order_ = jnp.array(self.order_)

        if self.order_ is None:
            self.order_ = jnp.array(range(Y.shape[1]))
        ...
        if self.cv is None:
            ...
            if sp.issparse(X):
                ...
                X_aug = jnp.hstack((X, Y_pred_chain))
            ...
            Y_pred_chain = jnp.zeros((X.shape[0], Y.shape[1]))
            X_aug = jnp.hstack((X, Y_pred_chain))
        ...
        for chain_idx, estimator in enumerate(self.estimators_):
            ...
            if self.cv is not None and chain_idx < len(self.estimators_) - 1:
                ...
                if sp.issparse(X_aug):
                    X_aug[:, col_idx] = jnp.expand_dims(cv_result, 1)
                else:
    ...
    def predict(self, X):
        ...
        Y_pred_chain = jnp.zeros((X.shape[0], len(self.estimators_)))
        for chain_idx, estimator in enumerate(self.estimators_):
            ...
            if sp.issparse(X):
                ...
                X_aug = jnp.hstack((X, previous_predictions))
            Y_pred_chain[:, chain_idx] = estimator.predict(X_aug)

        inv_order = jnp.empty_like(self.order_)
        inv_order[self.order_] = jnp.arange(len(self.order_))
...
class ClassifierChain(MetaEstimatorMixin, ClassifierMixin, _BaseChain):
    ...
    def predict_proba(self, X):
        ...
        """
        X = self._validate_data(X, accept_sparse=True, reset=False)
        Y_prob_chain = jnp.zeros((X.shape[0], len(self.estimators_)))
        Y_pred_chain = jnp.zeros((X.shape[0], len(self.estimators_)))
        for chain_idx, estimator in enumerate(self.estimators_):
            previous_predictions = Y_pred_chain[:, :chain_idx]
            if sp.issparse(X):
    ...
    def decision_function(self, X):
        ...
        """
        X = self._validate_data(X, accept_sparse=True, reset=False)
        Y_decision_chain = jnp.zeros((X.shape[0], len(self.estimators_)))
        Y_pred_chain = jnp.zeros((X.shape[0], len(self.estimators_)))
        for chain_idx, estimator in enumerate(self.estimators_):
            previous_predictions = Y_pred_chain[:, :chain_idx]
            if sp.issparse(X):
