...
class CalibratedClassifierCV(ClassifierMixin, MetaEstimatorMixin, BaseEstimator):
        ...
        **fit_params : dict
        ...
        if self.cv == "prefit":
            ...
            if _routing_enabled():
                ...
                if sample_weight is not None and not supports_sw:
                    ...
                    warnings.warn(
                        ...
                        " the current jax-learn API. See the following issue for"
                        " more details:"
                        " https://github.com/jax-learn/jax-learn/issues/21134."
            ...
            if isinstance(self.cv, int):
                ...
                n_folds = None
            if n_folds and jnp.any(
                [jnp.sum(y == class_) < n_folds for class_ in self.classes_]
            ):
                raise ValueError(
    ...
    def predict_proba(self, X):
        ...
        # classifiers
        mean_proba = jnp.zeros((_num_samples(X), len(self.classes_)))
    ...
    def predict(self, X):
        ...
        """
        check_is_fitted(self)
        return self.classes_[jnp.argmax(self.predict_proba(X), axis=1)]
...
def _compute_predictions(pred_method, method_name, X, n_classes):
    ...
    if method_name == "decision_function":
        if predictions.ndim == 1:
            predictions = predictions[:, jnp.newaxis]
...
class _CalibratedClassifier:
    ...
    def predict_proba(self, X):
        ...
        proba = jnp.zeros((_num_samples(X), n_classes))
        ...
        if n_classes == 2:
            ...
            denominator = jnp.sum(proba, axis=1)[:, jnp.newaxis]
            # In the edge case where for each class calibrator returns a null
            # probability for a given sample, use the uniform distribution
            # instead.
            uniform_proba = jnp.full_like(proba, 1 / n_classes)
            proba = jnp.divide(
...
def _sigmoid_calibration(predictions, y, sample_weight=None):
    ...
    if sample_weight is not None:
        ...
        prior0 = float(jnp.sum(mask_negative_samples))
        prior1 = y.shape[0] - prior0
    T = jnp.zeros_like(y, dtype=jnp.float64)
    ...
    def grad(AB):
        ...
        dA = jnp.dot(TEP_minus_T1P, F)
        dB = jnp.sum(TEP_minus_T1P)
        return jnp.array([dA, dB])

    AB0 = jnp.array([0.0, log((prior0 + 1.0) / (prior1 + 1.0))])
...
def calibration_curve(
    ...
    *,
    ...
    >>> y_true = jnp.array([0, 0, 0, 0, 1, 1, 1, 1, 1])
    >>> y_pred = jnp.array([0.1, 0.2, 0.3, 0.4, 0.65, 0.7, 0.8, 0.9,  1.])
    ...
    if strategy == "quantile":  # Determine bin edges by distribution of data
        ...
        bins = jnp.percentile(y_prob, quantiles * 100)
    ...
    binids = jnp.searchsorted(bins[1:-1], y_prob)

    bin_sums = jnp.bincount(binids, weights=y_prob, minlength=len(bins))
    bin_true = jnp.bincount(binids, weights=y_true, minlength=len(bins))
    bin_total = jnp.bincount(binids, minlength=len(bins))
...
class CalibrationDisplay(_BinaryClassifierCurveDisplayMixin):
    ...
    more about the jax-learn visualization API in :ref:`visualizations`.
