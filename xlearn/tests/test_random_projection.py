...
from xlearn.exceptions import DataDimensionalityWarning
...
def check_zero_mean_and_unit_norm(random_matrix):
    ...
    assert_array_almost_equal(0, jnp.mean(A), 3)
    assert_array_almost_equal(1.0, jnp.linalg.norm(A), 1)
...
def test_gaussian_random_matrix():
    ...
    assert_array_almost_equal(0.0, jnp.mean(A), 2)
    assert_array_almost_equal(jnp.var(A, ddof=1), 1 / n_components, 1)
...
def test_sparse_random_matrix():
    ...
    for density in [0.3, 1.0]:
        ...
        values = jnp.unique(A)
        assert jnp.sqrt(s) / jnp.sqrt(n_components) in values
        assert -jnp.sqrt(s) / jnp.sqrt(n_components) in values

        if density == 1.0:
            assert jnp.size(values) == 2
        else:
            assert 0.0 in values
            assert jnp.size(values) == 3
        ...
        # - +sqrt(s) / sqrt(n_components)   with probability 1 / 2s
        #
        assert_almost_equal(jnp.mean(A == 0.0), 1 - 1 / s, decimal=2)
        assert_almost_equal(
            jnp.mean(A == jnp.sqrt(s) / jnp.sqrt(n_components)), 1 / (2 * s), decimal=2
        )
        assert_almost_equal(
            jnp.mean(A == -jnp.sqrt(s) / jnp.sqrt(n_components)), 1 / (2 * s), decimal=2
        )

        assert_almost_equal(jnp.var(A == 0.0, ddof=1),
                            (1 - 1 / s) * 1 / s, decimal=2)
        assert_almost_equal(
            jnp.var(A == jnp.sqrt(s) / jnp.sqrt(n_components), ddof=1),
            (1 - 1 / (2 * s)) * 1 / (2 * s),
            decimal=2,
        )
        assert_almost_equal(
...
def test_SparseRandomProj_output_representation():
    for SparseRandomProj in all_SparseRandomProjection:
        ...
        rp.fit(data)
        assert isinstance(rp.transform(data), jnp.ndarray)

        sparse_data = sp.csr_matrix(data)
        assert isinstance(rp.transform(sparse_data), jnp.ndarray)
...
def test_random_projection_numerical_consisten(random_projection_cls):
    ...
    projection_32 = rp_32.fit_transform(X.astype(jnp.float32))
    projection_64 = rp_64.fit_transform(X.astype(jnp.float64))
