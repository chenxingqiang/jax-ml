...
def test_multi_target_regression():
    ...
    references = jnp.zeros_like(y_test)
    for n in range(3):
...
y = jnp.column_stack((y1, y2, y3))
n_samples, n_features = X.shape
n_outputs = y.shape[1]
n_classes = len(jnp.unique(y1))
classes = list(map(jnp.unique, (y1, y2, y3)))
...
def test_multi_output_classification():
    ...
    assert_array_equal(
        jnp.argmax(jnp.dstack(predict_proba), axis=1), predictions)
...
def test_multiclass_multioutput_estimator_predict_proba():
    ...
    # random labels
    y1 = jnp.array(["b", "a", "a", "b", "a"]).reshape(5, 1)  # 2 classes
    y2 = jnp.array(["d", "e", "f", "e", "d"]).reshape(5, 1)  # 3 classes
...
def test_base_chain_crossval_fit_and_predict():
    ...
    for chain in [ClassifierChain(LogisticRegression()), RegressorChain(Ridge())]:
        ...
        assert not jnp.all(Y_pred == Y_pred_cv)
...
@pytest.mark.parametrize("order_type", [list, jnp.array, tuple])
