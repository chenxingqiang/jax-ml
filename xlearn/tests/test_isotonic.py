...
def test_isotonic_regression():
    y = jnp.array([3, 7, 5, 9, 8, 7, 10])
    y_ = jnp.array([3, 6, 6, 8, 8, 8, 10])
    ...
    ir = IsotonicRegression()
    assert_array_equal(ir.fit_transform(jnp.ones(len(x)), y), jnp.mean(y))
...
def test_isotonic_regression_with_ties_in_differently_sized_groups():
    ...
    https://github.com/jax-learn/jax-learn/issues/9432
    ...
    """
    x = jnp.array([0, 1, 1, 2, 3, 4])
    y = jnp.array([0, 0, 1, 0, 0, 1])
    y_true = jnp.array([0.0, 0.25, 0.25, 0.25, 0.25, 1.0])
    ir = IsotonicRegression()
    ir.fit(x, y)
...
def test_isotonic_regression_reversed():
    y = jnp.array([10, 9, 10, 7, 6, 6.1, 5])
    y_ = IsotonicRegression(increasing=False).fit_transform(
        jnp.arange(len(y)), y)
    assert_array_equal(jnp.ones(y_[:-1].shape), ((y_[:-1] - y_[1:]) >= 0))
...
def test_isotonic_min_max_boundaries():
    ...
    y = jnp.arange(n)
    y_test = [2, 2, 2, 3, 4, 4]
    y_result = jnp.round(ir.fit_transform(x, y))
...
def test_isotonic_regression_pickle():
    ...
    jnp.testing.assert_array_equal(ir.predict(x), ir2.predict(x))
...
def test_isotonic_duplicate_min_entry():
    ...
    ir.fit(x, y)
    all_predictions_finite = jnp.all(jnp.isfinite(ir.predict(x)))
    assert all_predictions_finite


def test_isotonic_ymin_ymax():
    # Test from @NelleV's issue:
    # https://github.com/jax-learn/jax-learn/issues/6921
    x = jnp.array(
        [
            1.263,
            1.318,
...
def test_fast_predict():
    ...
    y_train = (
        jnp.less(rng.rand(n_samples), expit(X_train)
...
def test_isotonic_copy_before_fit():
    # https://github.com/jax-learn/jax-learn/issues/6628
    ir = IsotonicRegression()
    copy.copy(ir)
...
def test_isotonic_dtype():
    y = [2, 1, 4, 3, 5]
    weights = jnp.array([0.9, 0.9, 0.9, 0.9, 0.9], dtype=jnp.float64)
    reg = IsotonicRegression()

    for dtype in (jnp.int32, jnp.int64, jnp.float32, jnp.float64):
        for sample_weight in (None, weights.astype(jnp.float32), weights):
            y_np = jnp.array(y, dtype=dtype)
            expected_dtype = check_array(
                y_np, dtype=[jnp.float64, jnp.float32], ensure_2d=False
            ).dtype

            res = isotonic_regression(y_np, sample_weight=sample_weight)
            assert res.dtype == expected_dtype

            X = jnp.arange(len(y)).astype(dtype)
            reg.fit(X, y_np, sample_weight=sample_weight)
            res = reg.predict(X)
            assert res.dtype == expected_dtype


@pytest.mark.parametrize("y_dtype", [jnp.int32, jnp.int64, jnp.float32, jnp.float64])
def test_isotonic_mismatched_dtype(y_dtype):
    ...
    reg = IsotonicRegression()
    y = jnp.array([2, 1, 4, 3, 5], dtype=y_dtype)
    X = jnp.arange(len(y), dtype=jnp.float32)
    reg.fit(X, y)
...
def test_make_unique_dtype():
    ...
    for dtype in (jnp.float32, jnp.float64):
        x = jnp.array(x_list, dtype=dtype)
        y = x.copy()
        w = jnp.ones_like(x)
...
def test_make_unique_tolerance(dtype):
    # Check that equality takes account of jnp.finfo tolerance
    x = jnp.array([0, 1e-16, 1, 1 + 1e-14], dtype=dtype)
    y = x.copy()
    w = jnp.ones_like(x)
    x, y, w = _make_unique(x, y, w)
    if dtype == jnp.float64:
        x_out = jnp.array([0, 1, 1 + 1e-14])
    else:
        x_out = jnp.array([0, 1])
...
def test_isotonic_make_unique_tolerance():
    ...
    X = jnp.array([0, 1, 1 + 1e-16, 2], dtype=jnp.float64)
    y = jnp.array([0, 1, 2, 3], dtype=jnp.float64)
    ireg = IsotonicRegression().fit(X, y)
    y_pred = ireg.predict([0, 0.5, 1, 1.5, 2])

    assert_array_equal(y_pred, jnp.array([0, 0.75, 1.5, 2.25, 3]))
    assert_array_equal(ireg.X_thresholds_, jnp.array([0.0, 1.0, 2.0]))
    assert_array_equal(ireg.y_thresholds_, jnp.array([0.0, 1.5, 3.0]))


def test_isotonic_non_regression_inf_slope():
    # Non-regression test to ensure that inf values are not returned
    # see: https://github.com/jax-learn/jax-learn/issues/10903
    X = jnp.array([0.0, 4.1e-320, 4.4e-314, 1.0])
    y = jnp.array([0.42, 0.42, 0.44, 0.44])
    ireg = IsotonicRegression().fit(X, y)
    y_pred = ireg.predict(jnp.array([0, 2.1e-319, 5.4e-316, 1e-10]))
...
def test_isotonic_regression_sample_weight_not_overwritten():
    ...
    https://github.com/jax-learn/jax-learn/issues/20508
    """
    X, y = make_regression(n_samples=10, n_features=1, random_state=41)
    sample_weight_original = jnp.ones_like(y)
...
def test_get_feature_names_out(shape):
    ...
    assert isinstance(names, jnp.ndarray)
...
def test_isotonic_regression_output_predict():
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/25499
    """
