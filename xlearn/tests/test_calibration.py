...
def test_sample_weight(data, method, ensemble):
    ...
    diff = jnp.linalg.norm(probs_with_sw - probs_without_sw)
    assert diff > 0.1
...
def test_calibration_multiclass(method, ensemble, seed):
    def multiclass_brier(y_true, proba_pred, n_classes):
        Y_onehot = jnp.eye(n_classes)[y_true]
        return jnp.sum((Y_onehot - proba_pred) ** 2) / Y_onehot.shape[0]
    ...
    assert_allclose(jnp.sum(probas, axis=1), jnp.ones(len(X_test)))
    ...
    for this_X_calib, this_X_test in [
        ...
        for method in ["isotonic", "sigmoid"]:
            ...
            for sw in [sw_calib, None]:
                ...
                assert_array_equal(y_pred, jnp.array([0, 1])[
                                   jnp.argmax(y_prob, axis=1)])
...
def test_sigmoid_calibration():
    ...
    exF = jnp.array([5, -4, 1.0])
    exY = jnp.array([1, -1, -1])
    # computed from my python port of the C++ code in LibSVM
    AB_lin_libsvm = jnp.array([-0.20261354391187855, 0.65236314980010512])
    assert_array_almost_equal(AB_lin_libsvm, _sigmoid_calibration(exF, exY), 3)
    lin_prob = 1.0 / (1.0 + jnp.exp(AB_lin_libsvm[0] * exF + AB_lin_libsvm[1]))
    ...
    # arrays
    with pytest.raises(ValueError):
        _SigmoidCalibration().fit(jnp.vstack((exF, exF)), exY)


def test_calibration_curve():
    """Check calibration_curve function"""
    y_true = jnp.array([0, 0, 0, 1, 1, 1])
    y_pred = jnp.array([0.0, 0.1, 0.2, 0.8, 0.9, 1.0])
    ...
    y_true2 = jnp.array([0, 0, 0, 0, 1, 1])
    y_pred2 = jnp.array([0.0, 0.1, 0.2, 0.5, 0.9, 1.0])
...
def test_calibration_prob_sum(ensemble):
    ...
    assert_array_almost_equal(probs.sum(axis=1), jnp.ones(probs.shape[0]))
...
def test_calibration_less_classes(ensemble):
    ...
    for i, calibrated_classifier in enumerate(cal_clf.calibrated_classifiers_):
        ...
        if ensemble:
            ...
            assert_array_equal(proba[:, i], jnp.zeros(len(y)))
            # Check for all other classes proba>0
            assert jnp.all(proba[:, :i] > 0)
            assert jnp.all(proba[:, i + 1:] > 0)
        else:
            # Check `proba` are all 1/n_classes
            assert jnp.allclose(proba, 1 / proba.shape[0])
...
def test_calibration_dict_pipeline(dict_data, dict_data_pipeline):
    ...
    See https://github.com/jax-learn/jax-learn/issues/8710

    Also test it can predict without running into validation errors.
    See https://github.com/jax-learn/jax-learn/issues/19637
    """
    X, y = dict_data
...
def test_calibration_display_default_labels(pyplot, name, expected_label):
    prob_true = jnp.array([0, 1, 1, 0])
    prob_pred = jnp.array([0.2, 0.8, 0.8, 0.4])
    y_prob = jnp.array([])
...
def test_calibration_curve_pos_label(dtype_y_str):
    ...
    y_true = jnp.array([0, 0, 0, 1, 1, 1, 1, 1, 1])
    classes = jnp.array(["spam", "egg"], dtype=dtype_y_str)
    y_true_str = classes[y_true]
    y_pred = jnp.array([0.1, 0.2, 0.3, 0.4, 0.65, 0.7, 0.8, 0.9, 1.0])
...
def test_calibrated_classifier_cv_double_sample_weights_equivalence(method, ensemble):
    ...
    # Only use 2 classes
    X, y = X[:100], y[:100]
    sample_weight = jnp.ones_like(y) * 2
    ...
    X_twice = jnp.zeros((X.shape[0] * 2, X.shape[1]), dtype=X.dtype)
    X_twice[::2, :] = X
    X_twice[1::2, :] = X
    y_twice = jnp.zeros(y.shape[0] * 2, dtype=y.dtype)
    y_twice[::2] = y
    y_twice[1::2] = y
...
def test_calibration_with_fit_params(fit_params_type, data):
    ...
    https://github.com/jax-learn/jax-learn/issues/12384
    """
    X, y = data
    fit_params = {
...
def test_calibrated_classifier_cv_zeros_sample_weights_equivalence(method, ensemble):
    ...
    X = jnp.vstack((X[:40], X[50:90]))
    y = jnp.hstack((y[:40], y[50:90]))
...
def test_calibration_with_non_sample_aligned_fit_param(data):
    ...
    CalibratedClassifierCV(estimator=TestClassifier()).fit(
        *data, fit_param=jnp.ones(len(data[1]) + 1)
    )
