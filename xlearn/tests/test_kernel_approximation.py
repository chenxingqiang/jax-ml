...
def test_polynomial_count_sketch(gamma, degree, coef0, n_components):
    ...
    kernel_approx = jnp.dot(X_trans, Y_trans.T)

    error = kernel - kernel_approx
    assert jnp.abs(jnp.mean(error)) <= 0.05  # close to unbiased
    jnp.abs(error, out=error)
    assert jnp.max(error) <= 0.1  # nothing too far off
    assert jnp.mean(error) <= 0.05  # mean is fairly close
...
def test_skewed_chi2_sampler():
    ...
    log_kernel = (
        (jnp.log(X_c) / 2.0) + (jnp.log(Y_c) / 2.0) +
        jnp.log(2.0) - jnp.log(X_c + Y_c)
    )
    # reduce to n_samples_x x n_samples_y by summing over features in log-space
    kernel = jnp.exp(log_kernel.sum(axis=2))
    ...
    kernel_approx = jnp.dot(X_trans, Y_trans.T)
    assert_array_almost_equal(kernel, kernel_approx, 1)
    assert jnp.isfinite(kernel).all(), "NaNs found in the Gram matrix"
    assert jnp.isfinite(kernel_approx).all(
...
def test_rbf_sampler():
    ...
    error = kernel - kernel_approx
    assert jnp.abs(jnp.mean(error)) <= 0.01  # close to unbiased
    jnp.abs(error, out=error)
    assert jnp.max(error) <= 0.1  # nothing too far off
    assert jnp.mean(error) <= 0.05  # mean is fairly close
...
def test_rbf_sampler_fitted_attributes_dtype(global_dtype):
    ...
    X = jnp.array([[1, 2], [3, 4], [5, 6]], dtype=global_dtype)
...
def test_rbf_sampler_dtype_equivalence():
    ...
    X32 = jnp.array([[1, 2], [3, 4], [5, 6]], dtype=jnp.float32)
    rbf32.fit(X32)

    rbf64 = RBFSampler(random_state=42)
    X64 = jnp.array([[1, 2], [3, 4], [5, 6]], dtype=jnp.float64)
    rbf64.fit(X64)
...
def test_skewed_chi2_sampler_dtype_equivalence():
    ...
    X_32 = jnp.array([[1, 2], [3, 4], [5, 6]], dtype=jnp.float32)
    skewed_chi2_sampler_32.fit(X_32)

    skewed_chi2_sampler_64 = SkewedChi2Sampler(random_state=42)
    X_64 = jnp.array([[1, 2], [3, 4], [5, 6]], dtype=jnp.float64)
...
def test_nystroem_approximation():
    ...
    K = rbf_kernel(X)
    assert_array_almost_equal(jnp.dot(X_transformed, X_transformed.T), K)
...
def test_nystroem_default_parameters():
    ...
    K2 = jnp.dot(X_transformed, X_transformed.T)
...
def test_nystroem_singular_kernel():
    ...
    assert_array_almost_equal(K, jnp.dot(X_transformed, X_transformed.T))
...
def test_nystroem_component_indices():
    ...
    https://github.com/jax-learn/jax-learn/issues/20474
    """
