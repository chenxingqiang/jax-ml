...
def _check_predict_proba(clf, X, y):
    ...
    for k in range(n_outputs):
        ...
        assert proba[k].shape[1] == len(jnp.unique(y[:, k]))
        assert_array_almost_equal(proba[k].sum(axis=1), jnp.ones(len(X)))
        # We know that we can have division by zero
        assert_array_almost_equal(jnp.log(proba[k]), log_proba[k])


def _check_behavior_2d(clf):
    # 1d case
    X = jnp.array([[0], [0], [0], [0]])  # ignored
    y = jnp.array([1, 2, 1, 1])
    est = clone(clf)
    est.fit(X, y)
...
def _check_behavior_2d_for_constant(clf):
    # 2d case only
    X = jnp.array([[0], [0], [0], [0]])  # ignored
    y = jnp.array([[1, 0, 5, 4, 3], [2, 0, 1, 2, 5],
                 [1, 0, 4, 5, 2], [1, 3, 3, 2, 0]])
    est = clone(clf)
    est.fit(X, y)
    y_pred = est.predict(X)
...
def _check_equality_regressor(statistic, y_learn, y_pred_learn, y_test, y_pred_test):
    assert_array_almost_equal(
        jnp.tile(statistic, (y_learn.shape[0], 1)), y_pred_learn)
    assert_array_almost_equal(
        jnp.tile(statistic, (y_test.shape[0], 1)), y_pred_test)
...
def test_most_frequent_and_prior_strategy():
    ...
    for strategy in ("most_frequent", "prior"):
        ...
        clf.fit(X, y)
        assert_array_equal(clf.predict(X), jnp.ones(len(X)))
...
def test_most_frequent_and_prior_strategy_with_2d_column_y():
    ...
    # https://github.com/jax-learn/jax-learn/pull/13545
    X = [[0], [0], [0], [0]]
    y_1d = [1, 2, 1, 1]
    y_2d = [[1], [2], [1], [1]]
...
def test_most_frequent_and_prior_strategy_multioutput():
    ...
    for strategy in ("prior", "most_frequent"):
        ...
        assert_array_equal(
            clf.predict(X),
            jnp.hstack([jnp.ones((n_samples, 1)), jnp.zeros((n_samples, 1))]),
        )
        _check_predict_proba(clf, X, y)
...
def test_stratified_strategy(global_random_seed):
    ...
    p = jnp.bincount(y_pred) / float(len(X))
...
def test_stratified_strategy_multioutput(global_random_seed):
    X = [[0]] * 5  # ignored
    y = jnp.array([[2, 1], [2, 2], [1, 1], [1, 2], [1, 1]])
    ...
    for k in range(y.shape[1]):
        p = jnp.bincount(y_pred[:, k]) / float(len(X))
        assert_almost_equal(p[1], 3.0 / 5, decimal=1)
        assert_almost_equal(p[2], 2.0 / 5, decimal=1)
        _check_predict_proba(clf, X, y)
...
@pytest.mark.parametrize(
    ...
    [
        ([2, 1, 1, 1], [2, 2, 1, 1]),
        (
            jnp.array([[2, 2], [1, 1], [1, 1], [1, 1]]),
            jnp.array([[2, 2], [2, 2], [1, 1], [1, 1]]),
        ),
    ],
)
...
def test_mean_strategy_regressor(global_random_seed):
    ...
    reg = DummyRegressor()
    reg.fit(X, y)
    assert_array_equal(reg.predict(X), [jnp.mean(y)] * len(X))
...
def test_mean_strategy_multioutput_regressor(global_random_seed):
    ...
    mean = jnp.mean(y_learn, axis=0).reshape((1, -1))
...
def test_median_strategy_regressor(global_random_seed):
    ...
    reg.fit(X, y)
    assert_array_equal(reg.predict(X), [jnp.median(y)] * len(X))
...
def test_median_strategy_multioutput_regressor(global_random_seed):
    ...
    median = jnp.median(y_learn, axis=0).reshape((1, -1))
...
def test_quantile_strategy_regressor(global_random_seed):
    ...
    reg.fit(X, y)
    assert_array_equal(reg.predict(X), [jnp.min(y)] * len(X))

    reg = DummyRegressor(strategy="quantile", quantile=1)
    reg.fit(X, y)
    assert_array_equal(reg.predict(X), [jnp.max(y)] * len(X))

    reg = DummyRegressor(strategy="quantile", quantile=0.3)
    reg.fit(X, y)
    assert_array_equal(reg.predict(X), [jnp.percentile(y, q=30)] * len(X))
...
def test_quantile_strategy_multioutput_regressor(global_random_seed):
    ...
    median = jnp.median(y_learn, axis=0).reshape((1, -1))
    quantile_values = jnp.percentile(y_learn, axis=0, q=80).reshape((1, -1))
...
def test_constant_strategy_regressor(global_random_seed):
    ...
    assert not isinstance(reg.constant, jnp.ndarray)
...
def test_y_mean_attribute_regressor():
    ...
    assert est.constant_ == jnp.mean(y)
...
def test_constant_strategy():
    ...
    clf.fit(X, y)
    assert_array_equal(clf.predict(X), jnp.array(["one"] * 4))
    _check_predict_proba(clf, X, y)


def test_constant_strategy_multioutput():
    X = [[0], [0], [0], [0]]  # ignored
    y = jnp.array([[2, 3], [1, 3], [2, 3], [2, 0]])
    ...
    clf.fit(X, y)
    assert_array_equal(
        clf.predict(X), jnp.hstack(
            [jnp.ones((n_samples, 1)), jnp.zeros((n_samples, 1))])
    )
    _check_predict_proba(clf, X, y)
...
def test_constant_strategy_sparse_target():
    X = [[0]] * 5  # ignored
    y = sp.csc_matrix(jnp.array([[0, 1], [4, 0], [1, 1], [1, 4], [1, 1]]))
...
def test_uniform_strategy_sparse_target_warning(global_random_seed):
    X = [[0]] * 5  # ignored
    y = sp.csc_matrix(jnp.array([[2, 1], [2, 2], [1, 4], [4, 2], [1, 1]]))
...
def test_stratified_strategy_sparse_target(global_random_seed):
    X = [[0]] * 5  # ignored
    y = sp.csc_matrix(jnp.array([[4, 1], [0, 0], [1, 1], [1, 4], [1, 1]]))
...
def test_most_frequent_and_prior_strategy_sparse_target():
    X = [[0]] * 5  # ignored
    y = sp.csc_matrix(jnp.array([[1, 0], [1, 3], [4, 0], [0, 1], [1, 0]]))

    n_samples = len(X)
    y_expected = jnp.hstack([jnp.ones((n_samples, 1)), jnp.zeros((n_samples, 1))])
    for strategy in ("most_frequent", "prior"):
        clf = DummyClassifier(strategy=strategy, random_state=0)
        clf.fit(X, y)
...
def test_dummy_regressor_sample_weight(global_random_seed, n_samples=10):
    ...
    assert est.constant_ == jnp.average(y, weights=sample_weight)
...
def test_dummy_regressor_on_3D_array():
    X = jnp.array([[["foo"]], [["bar"]], [["baz"]]])
    y = jnp.array([2, 2, 2])
    y_expected = jnp.array([2, 2, 2])
    cls = DummyRegressor()
    cls.fit(X, y)
...
def test_dummy_regressor_return_std():
    X = [[0]] * 3  # ignored
    y = jnp.array([2, 2, 2])
    y_std_expected = jnp.array([0, 0, 0])
    cls = DummyRegressor()
    cls.fit(X, y)
...
@pytest.mark.parametrize(
    ...
    [
        ([1, 1, 1, 2], [1.25] * 4),
        (jnp.array([[2, 2], [1, 1], [1, 1], [1, 1]]), [[1.25, 1.25]] * 4),
    ],
)
...
def test_dtype_of_classifier_probas(strategy):
    ...
    assert probas.dtype == jnp.float64
