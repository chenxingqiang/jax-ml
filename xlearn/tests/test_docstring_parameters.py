...
from xlearn.utils import IS_PYPY, all_estimators
...
with warnings.catch_warnings():
    ...
    xlearn_path = xlearn.__path__  # type: ignore  # mypy issue #1422
    PUBLIC_MODULES = set(
        [
            pckg[1]
...
def test_docstring_parameters():
    ...
    for name in PUBLIC_MODULES:
        if name.endswith(".conftest"):
            # pytest tooling, not part of the jax-learn API
            continue
        ...
        # Exclude non-jax-learn classes
        classes = [
            cls for cls in classes if cls[1].__module__.startswith("xlearn")]
...
def test_tabs():
    ...
    for importer, modname, ispkg in walk_packages(xlearn.__path__, prefix="xlearn."):
        if IS_PYPY and (
