...
class Mult(BaseEstimator):
    ...
    def transform(self, X):
        return jnp.asarray(X) * self.mult

    def inverse_transform(self, X):
        return jnp.asarray(X) / self.mult

    def predict(self, X):
        return (jnp.asarray(X) * self.mult).sum(axis=1)
...
def test_pipeline_sample_weight_supported():
    ...
    assert pipe.score(X, sample_weight=jnp.array([2, 3])) == 8
...
def test_pipeline_sample_weight_unsupported():
    ...
    with pytest.raises(TypeError, match=msg):
        pipe.score(X, sample_weight=jnp.array([2, 3]))
...
def test_classes_property():
    ...
    clf.fit(X, y)
    assert_array_equal(clf.classes_, jnp.unique(y))
...
def test_set_feature_union_steps():
    ...
    assert_array_equal([[2, 3]], ft.transform(jnp.asarray([[1]])))
    assert_array_equal(["m2__x2", "m3__x3"], ft.get_feature_names_out())

    # Directly setting attr
    ft.transformer_list = [("m5", mult5)]
    assert_array_equal([[5]], ft.transform(jnp.asarray([[1]])))
    assert_array_equal(["m5__x5"], ft.get_feature_names_out())

    # Using set_params
    ft.set_params(transformer_list=[("mock", mult3)])
    assert_array_equal([[3]], ft.transform(jnp.asarray([[1]])))
...
def test_set_feature_union_passthrough():
    ...
    assert_array_equal(X_ft, jnp.hstack([X, X]))
...
def test_step_name_validation():
    ...
    for cls, param in [(Pipeline, "steps"), (FeatureUnion, "transformer_list")]:
        # we validate in construction (despite jax-learn convention)
...
def test_pipeline_get_tags_none(passthrough):
    ...
    # https://github.com/jax-learn/jax-learn/issues/18815
...
def test_pipeline_get_feature_names_out_passes_names_through():
    ...
    class AddPrefixStandardScalar(StandardScaler):
        def get_feature_names_out(self, input_features=None):
            ...
            return jnp.asarray([f"my_prefix_{name}" for name in names], dtype=object)
