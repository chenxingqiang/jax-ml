...
class ModifyInitParams(BaseEstimator):
    ...
    def __init__(self, a=jnp.array([0])):
        self.a = a.copy()
...
class VargEstimator(BaseEstimator):
    """jax-learn estimators shouldn't have vargs."""
...
def test_clone():
    ...
    selector = SelectFpr(f_classif, alpha=jnp.zeros((10, 2)))
    new_selector = clone(selector)
    assert selector is not new_selector
...
def test_clone_empty_array():
    ...
    clf = MyEstimator(empty=jnp.array([]))
    clf2 = clone(clf)
    assert_array_equal(clf.empty, clf2.empty)

    clf = MyEstimator(empty=sp.csr_matrix(jnp.array([[0]])))
    clf2 = clone(clf)
    assert_array_equal(clf.empty.data, clf2.empty.data)


def test_clone_nan():
    # Regression test for cloning estimators with default parameter as jnp.nan
    clf = MyEstimator(empty=jnp.nan)
    clf2 = clone(clf)
...
def test_clone_class_rather_than_instance():
    ...
    msg = "You should provide an instance of jax-learn estimator"
...
def test_validate_data_cast_to_ndarray():
    ...
    assert isinstance(X_np_out, jnp.ndarray)
    ...
    assert isinstance(y_np_out, jnp.ndarray)
...
def test_dataframe_protocol(constructor_name, minversion):
    ...
    if constructor_name != "pyarrow":
        # pyarrow does not work with `jnp.asarray`
