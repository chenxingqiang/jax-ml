...
def assert_argkmin_results_quasi_equality(
    ...
    """
    def is_sorted(a): return jnp.all(a[:-1] <= a[1:])
...
def assert_radius_neighbors_results_quasi_equality(
    ...
    for query_idx in range(n_queries):
        ...
        if last_extra_elements.size > 0:
            assert jnp.all(radius - rtol <= last_extra_elements <= radius + rtol), (
...
ASSERT_RESULT = {
    ...
    (ArgKmin, jnp.float64): assert_argkmin_results_equality,
    (
        RadiusNeighbors,
        jnp.float64,
    ...
    (ArgKmin, jnp.float32): assert_argkmin_results_quasi_equality,
    (
        RadiusNeighbors,
        jnp.float32,
...
def test_assert_argkmin_results_quasi_equality():
    ...
    # on their rankings.
    assert_argkmin_results_quasi_equality(
        jnp.array([[1.2, 2.5, _6_1m, 6.1, _6_1p]]),
        jnp.array([[1.2, 2.5, 6.1, 6.1, 6.1]]),
        jnp.array([[1, 2, 3, 4, 5]]),
        jnp.array([[1, 2, 4, 5, 3]]),
        rtol=rtol,
    )
    ...
    assert_argkmin_results_quasi_equality(
        jnp.array([[_1m, _1m, 1, _1p, _1p]]),
        jnp.array([[_1m, _1m, 1, _1p, _1p]]),
        jnp.array([[6, 7, 8, 9, 10]]),
        jnp.array([[6, 9, 7, 8, 10]]),
        rtol=rtol,
    )
    ...
    with pytest.raises(AssertionError, match=msg):
        assert_argkmin_results_quasi_equality(
            jnp.array([[1.2, 2.5, _6_1m, 6.1, _6_1p]]),
            jnp.array([[2.5, 1.2, _6_1m, 6.1, _6_1p]]),
            jnp.array([[1, 2, 3, 4, 5]]),
            jnp.array([[2, 1, 4, 5, 3]]),
            rtol=rtol,
        )
...
def test_assert_radius_neighbors_results_quasi_equality():
    ...
    assert_radius_neighbors_results_quasi_equality(
        jnp.array([jnp.array([1.2, 2.5, _6_1m, 6.1, _6_1p])]),
        jnp.array([jnp.array([1.2, 2.5, _6_1m, 6.1, _6_1p])]),
        jnp.array([jnp.array([1, 2, 3, 4, 5])]),
        jnp.array([jnp.array([1, 2, 4, 5, 3])]),
        radius=6.1,
        rtol=rtol,
    )
    assert_radius_neighbors_results_quasi_equality(
        jnp.array([jnp.array([_1m, _1m, 1, _1p, _1p])]),
        jnp.array([jnp.array([_1m, _1m, 1, _1p, _1p])]),
        jnp.array([jnp.array([6, 7, 8, 9, 10])]),
        jnp.array([jnp.array([6, 9, 7, 8, 10])]),
        radius=6.1,
        rtol=rtol,
    )
    ...
    assert_radius_neighbors_results_quasi_equality(
        jnp.array([jnp.array([1.2, 2.5, _6_1m, 6.1, _6_1p])]),
        jnp.array([jnp.array([1.2, 2.5, _6_1m, 6.1])]),
        jnp.array([jnp.array([1, 2, 3, 4, 5])]),
        jnp.array([jnp.array([1, 2, 3, 4])]),
        radius=6.1,
        rtol=rtol,
    )
    ...
    with pytest.raises(AssertionError, match=msg):
        assert_radius_neighbors_results_quasi_equality(
            jnp.array([jnp.array([1.2, 2.5, 6])]),
            jnp.array([jnp.array([1.2, 2.5])]),
            jnp.array([jnp.array([1, 2, 3])]),
            jnp.array([jnp.array([1, 2])]),
            radius=6.1,
            rtol=rtol,
        )
    ...
    with pytest.raises(AssertionError, match=msg):
        assert_radius_neighbors_results_quasi_equality(
            jnp.array([jnp.array([1.2, 2.5, _6_1m, 6.1, _6_1p])]),
            jnp.array([jnp.array([2.5, 1.2, _6_1m, 6.1, _6_1p])]),
            jnp.array([jnp.array([1, 2, 3, 4, 5])]),
            jnp.array([jnp.array([2, 1, 4, 5, 3])]),
            radius=6.1,
            rtol=rtol,
...
def test_pairwise_distances_reduction_is_usable_for():
    ...
    assert BaseDistancesReductionDispatcher.is_usable_for(
        X.astype(jnp.float64), Y.astype(jnp.float64), metric
    )

    assert BaseDistancesReductionDispatcher.is_usable_for(
        X.astype(jnp.float32), Y.astype(jnp.float32), metric
    )

    assert not BaseDistancesReductionDispatcher.is_usable_for(
        X.astype(jnp.int64), Y.astype(jnp.int64), metric
    )

    assert not BaseDistancesReductionDispatcher.is_usable_for(
        X, Y, metric="pyfunc")
    assert not BaseDistancesReductionDispatcher.is_usable_for(
        X.astype(jnp.float32), Y, metric
    )
    assert not BaseDistancesReductionDispatcher.is_usable_for(
        X, Y.astype(jnp.int32), metric
    )

    # F-ordered arrays are not supported
    assert not BaseDistancesReductionDispatcher.is_usable_for(
        jnp.asfortranarray(X), Y, metric
    )
    ...
    # See: https://github.com/jax-learn/jax-learn/issues/23653
    # TODO: support CSR matrices with int64 indices and indptr
    X_csr_int64 = csr_matrix(X)
    X_csr_int64.indices = X_csr_int64.indices.astype(jnp.int64)
    assert not BaseDistancesReductionDispatcher.is_usable_for(
...
def test_argkmin_factory_method_wrong_usages():
    ...
    with pytest.raises(ValueError, match=msg):
        ArgKmin.compute(X=X.astype(jnp.float32), Y=Y, k=k, metric=metric)
    ...
    with pytest.raises(ValueError, match=msg):
        ArgKmin.compute(X=X, Y=Y.astype(jnp.int32), k=k, metric=metric)
    ...
    with pytest.raises(
        ...
        ArgKmin.compute(X=jnp.array([1.0, 2.0]), Y=Y, k=k, metric=metric)

    with pytest.raises(ValueError, match="ndarray is not C-contiguous"):
        ArgKmin.compute(X=jnp.asfortranarray(X), Y=Y, k=k, metric=metric)
...
def test_radius_neighbors_factory_method_wrong_usages():
    ...
    with pytest.raises(
        ...
        RadiusNeighbors.compute(
            X=X.astype(jnp.float32), Y=Y, radius=radius, metric=metric
        )
    ...
    with pytest.raises(
        ...
        RadiusNeighbors.compute(X=X, Y=Y.astype(
            jnp.int32), radius=radius, metric=metric)
    ...
    with pytest.raises(
        ...
        RadiusNeighbors.compute(
            X=jnp.array([1.0, 2.0]), Y=Y, radius=radius, metric=metric
        )

    with pytest.raises(ValueError, match="ndarray is not C-contiguous"):
        RadiusNeighbors.compute(
            X=jnp.asfortranarray(X), Y=Y, radius=radius, metric=metric
        )
...
@pytest.mark.parametrize(
    ...
    [
        (ArgKmin, jnp.float64),
        (RadiusNeighbors, jnp.float32),
        (ArgKmin, jnp.float32),
        (RadiusNeighbors, jnp.float64),
    ],
)
...
def test_strategies_consisten(
    ...
    if metric == "haversine":
        X = jnp.ascontiguousarray(X[:, :2])
        Y = jnp.ascontiguousarray(Y[:, :2])
...
def test_pairwise_distances_argkmin(
    ...
    edge_cases = [
        (jnp.float32, "chebyshev", 1000000.0),
        (jnp.float32, "cityblock", 1000000.0),
    ]
    ...
    if metric == "euclidean":
        # Compare to jax-learn GEMM optimized implementation
    ...
    argkmin_indices_ref = jnp.argsort(dist_matrix, axis=1)[:, :k]
    # Getting the associated distances
    argkmin_distances_ref = jnp.zeros(
        argkmin_indices_ref.shape, dtype=jnp.float64)
...
def test_pairwise_distances_radius_neighbors(
    ...
    for row in dist_matrix:
        ind = jnp.arange(row.shape[0])[row <= radius]
        dist = row[ind]

        sort = jnp.argsort(dist)
...
def test_sqeuclidean_row_norms(
    ...
    sq_row_norm_reference = jnp.linalg.norm(X, axis=1) ** 2
