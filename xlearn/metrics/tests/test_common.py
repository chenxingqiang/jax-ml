...
CLASSIFICATION_METRICS = {
    ...
    "normalized_confusion_matrix": lambda *args, **kwargs: (
        ...
        / confusion_matrix(*args, **kwargs).sum(axis=1)[:, jnp.newaxis]
    ),
...
def precision_recall_curve_padded_thresholds(*args, **kwargs):
    ...
    dimensional jnp.array of dtype('float64') (it will be of dtype('object)
...
def test_format_invariance_with_1d_vectors(name):
    ...
    y1_1d, y2_1d = jnp.array(y1), jnp.array(y2)
    assert_array_equal(y1_1d.ndim, 1)
    assert_array_equal(y2_1d.ndim, 1)
    y1_column = jnp.reshape(y1_1d, (-1, 1))
    y2_column = jnp.reshape(y2_1d, (-1, 1))
    y1_row = jnp.reshape(y1_1d, (1, -1))
    y2_row = jnp.reshape(y2_1d, (1, -1))
...
def test_classification_invariance_string_vs_numbers_labels(name):
    ...
    y1_str = jnp.array(["eggs", "spam"])[y1]
    y2_str = jnp.array(["eggs", "spam"])[y2]
...
@pytest.mark.parametrize(
    ...
    # https://github.com/jax-learn/jax-learn/issues/6809
    [
        ([jnp.nan, 1, 2], [1, 2, 3]),
        ([jnp.inf, 1, 2], [1, 2, 3]),
    ],  # type: ignore
...
def test_classification_inf_nan_input(metric, y_true, y_score):
    ...
    if not jnp.isfinite(y_true).all():
        input_name = "y_true"
        if jnp.isnan(y_true).any():
            unexpected_value = "NaN"
        else:
            unexpected_value = "infinity or a value too large"
    else:
        input_name = "y_pred"
        if jnp.isnan(y_score).any():
            unexpected_value = "NaN"
        else:
            unexpected_value = "infinity or a value too large"
...
def check_single_sample(name):
    ...
    # Score functions tested are those that formerly called jnp.squeeze,
...
def check_single_sample_multioutput(name):
    ...
    for i, j, k, l in product([0, 1], repeat=4):
        metric(jnp.array([[i, j]]), jnp.array([[k, l]]))
...
def test_multilabel_representation_invariance():
    ...
    y1 = jnp.vstack([y1, [[0] * n_classes]])
    y2 = jnp.vstack([y2, [[0] * n_classes]])
...
def _check_averaging(
    ...
    assert_allclose(macro_measure, jnp.mean(label_measure))

    # Weighted measure
    weights = jnp.sum(y_true_binarize, axis=0, dtype=int)

    if jnp.sum(weights) != 0:
        weighted_measure = metric(y_true, y_pred, average="weighted")
        assert_allclose(weighted_measure, jnp.average(
...
def test_averaging_multilabel_all_zeroes(name):
    ...
    y_pred = jnp.zeros((20, 3))
    y_score = jnp.zeros((20, 3))
    y_true_binarize = y_true
    y_pred_binarize = y_pred
...
def test_averaging_multilabel_all_ones(name):
    y_true = jnp.ones((20, 3))
    y_pred = jnp.ones((20, 3))
    y_score = jnp.ones((20, 3))
    y_true_binarize = y_true
    y_pred_binarize = y_pred
...
def check_sample_weight_invariance(name, metric, y1, y2):
    ...
    assert_allclose(
        unweighted_score,
        metric(y1, y2, sample_weight=jnp.ones(shape=len(y1))),
    ...
    repeat_weighted_score = metric(
        jnp.repeat(y1, sample_weight, axis=0),
        jnp.repeat(y2, sample_weight, axis=0),
        sample_weight=None,
    )
    assert_allclose(
        weighted_score,
    ...
    sample_weight_zeroed = jnp.copy(sample_weight)
    ...
    with pytest.raises(ValueError, match=error_message):
        metric(y1, y2, sample_weight=jnp.hstack([sample_weight, sample_weight]))
...
def test_multilabel_sample_weight_invariance(name):
    ...
    y_true = jnp.vstack([ya, yb])
    y_pred = jnp.vstack([ya, ya])
...
def test_no_averaging_labels():
    ...
    y_true_multilabel = jnp.array([[1, 1, 0, 0], [1, 1, 0, 0]])
    y_pred_multilabel = jnp.array([[0, 0, 1, 1], [0, 1, 1, 0]])
    y_true_multiclass = jnp.array([0, 1, 2])
    y_pred_multiclass = jnp.array([0, 2, 3])
    labels = jnp.array([3, 0, 1, 2])
    _, inverse_labels = jnp.unique(labels, return_inverse=True)
...
def test_thresholded_multilabel_multioutput_permutations_invariance(name):
    ...
    for perm in permutations(range(n_classes), n_classes):
        ...
        if metric == mean_absolute_percentage_error:
            assert jnp.isfinite(current_score)
...
def test_thresholded_metric_permutation_invariance(name):
    ...
    score = metric(y_true, y_score)
    for perm in permutations(range(n_classes), n_classes):
        inverse_perm = jnp.zeros(n_classes, dtype=int)
        inverse_perm[list(perm)] = jnp.arange(n_classes)
        y_score_perm = y_score[:, inverse_perm]
        y_true_perm = jnp.take(perm, y_true)
...
def test_metrics_pos_label_error_str(metric, y_pred_threshold, dtype_y_str):
    ...
    if not y_pred_threshold:
        y2 = jnp.array(["spam", "eggs"], dtype=dtype_y_str)[y2]
