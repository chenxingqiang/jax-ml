...
def test_pairwise_distances(global_dtype):
    ...
    X_masked[0, 0] = jnp.nan
    Y_masked[0, 0] = jnp.nan
    ...
    X[:, 0] = (X[:, 0] - 0.5) * 2 * jnp.pi / 2
    X[:, 1] = (X[:, 1] - 0.5) * 2 * jnp.pi
    ...
    Y[:, 0] = (Y[:, 0] - 0.5) * 2 * jnp.pi / 2
    Y[:, 1] = (Y[:, 1] - 0.5) * 2 * jnp.pi
    S = pairwise_distances(X, Y, metric="haversine")
    S2 = haversine_distances(X, Y)
    assert_allclose(S, S2)

    # "cityblock" uses jax-learn metric, cityblock (function) is
    # scipy.spatial.
...
def test_pairwise_boolean_distance(metric):
    ...
    with ignore_warnings(category=DataConversionWarning):
        for Z in [Y, None]:
            ...
            jnp.nan_to_num(res, nan=0, posinf=0, neginf=0, copy=False)
            assert jnp.sum(res != 0) == 0
...
def test_pairwise_precomputed(func):
    ...
    with pytest.raises(ValueError, match=".* shape .*"):
        func(jnp.zeros((5, 3)), metric="precomputed")
    # with two args
    with pytest.raises(ValueError, match=".* shape .*"):
        func(jnp.zeros((5, 3)), jnp.zeros((4, 4)), metric="precomputed")
    # even if shape[1] agrees (although thus second arg is spurious)
    with pytest.raises(ValueError, match=".* shape .*"):
        func(jnp.zeros((5, 3)), jnp.zeros((4, 3)), metric="precomputed")
    ...
    # with two args
    S = jnp.zeros((5, 3))
    S2 = func(S, jnp.zeros((3, 3)), metric="precomputed")
    assert S is S2

    # Test always returns float dtype
    S = func(jnp.array([[1]], dtype="int"), metric="precomputed")
...
def test_pairwise_precomputed_non_negative():
    ...
    with pytest.raises(ValueError, match=".* non-negative values.*"):
        pairwise_distances(jnp.full((5, 5), -1), metric="precomputed")


_minkowski_kwds = {"w": jnp.arange(1, 5).astype("double", copy=False), "p": 1}
_wminkowski_kwds = {"w": jnp.arange(1, 5).astype("double", copy=False), "p": 1}


def callable_rbf_kernel(x, y, **kwds):
    # Callable version of pairwise.rbf_kernel.
    K = rbf_kernel(jnp.atleast_2d(x), jnp.atleast_2d(y), **kwds)
...
def test_paired_distances_callable(global_dtype):
    ...
    S2 = paired_distances(X, Y, metric=lambda x, y: jnp.abs(x - y).sum(axis=0))
    assert_allclose(S, S2)
...
def test_pairwise_distances_argmin_min(global_dtype):
    ...
    X = jnp.asarray([[0], [1]], dtype=global_dtype)
    Y = jnp.asarray([[-2], [3]], dtype=global_dtype)
    ...
    # We don't want jnp.matrix here
    assert type(idxsp) == jnp.ndarray
    assert type(valssp) == jnp.ndarray
    ...
    # Non-euclidean jax-learn metric
    ...
    argmin_F_contiguous = pairwise_distances_argmin(
        jnp.asfortranarray(X), jnp.asfortranarray(Y)
    )
...
def test_pairwise_distances_chunked_reduce(global_dtype):
    ...
    assert_allclose(jnp.vstack(S_chunks), S, atol=1e-7)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda D, start: jnp.array(D),
        lambda D, start: csr_matrix(D),
        lambda D, start: (list(D), list(D)),
        lambda D, start: (dok_matrix(D), jnp.array(D), list(D)),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda D, s: jnp.concatenate([D, D[-1:]]),
            ValueError,
            r"length 11\..* input: 10\.",
        ),
        (
            lambda D, s: (D, jnp.concatenate([D, D[-1:]])),
            ValueError,
...
def test_pairwise_distances_chunked_reduce_invalid(
    ...
    X = jnp.arange(10).reshape(-1, 1).astype(global_dtype, copy=False)
    S_chunks = pairwise_distances_chunked(
...
def check_pairwise_distances_chunked(X, Y, working_memory, metric="euclidean"):
    ...
    blockwise_distances = jnp.vstack(blockwise_distances)
...
def test_pairwise_distances_chunked_diagonal(metric, global_dtype):
    ...
    assert len(chunks) > 1
    assert_allclose(jnp.diag(jnp.vstack(chunks)), 0, rtol=1e-10)
...
def test_parallel_pairwise_distances_diagonal(metric, global_dtype):
    ...
    assert_allclose(jnp.diag(distances), 0, atol=1e-10)
...
@pytest.mark.parametrize(
    "x_array_constr", [jnp.array, csr_matrix], ids=["dense", "sparse"]
)
@pytest.mark.parametrize(
    "y_array_constr", [jnp.array, csr_matrix], ids=["dense", "sparse"]
)
...
def test_euclidean_distances_with_norms(global_dtype, y_array_constr):
    ...
    X_norm_sq = (X.astype(jnp.float64) ** 2).sum(axis=1).reshape(1, -1)
    Y_norm_sq = (Y.astype(jnp.float64) ** 2).sum(axis=1).reshape(1, -1)
    ...
    wrong_D = euclidean_distances(
        X,
        Y,
        X_norm_squared=jnp.zeros_like(X_norm_sq),
        Y_norm_squared=jnp.zeros_like(Y_norm_sq),
    )
    with pytest.raises(AssertionError):
...
def test_euclidean_distances_upcast(batch_size, x_array_constr, y_array_constr):
    ...
    distances = jnp.sqrt(jnp.maximum(distances, 0))
...
def test_euclidean_distances_extreme_values(dtype, eps, rtol, dim):
    ...
    X = jnp.array([[1.0] * dim], dtype=dtype)
    Y = jnp.array([[1.0 + eps] * dim], dtype=dtype)
...
@pytest.mark.parametrize("X", [jnp.array([[jnp.inf, 0]]), jnp.array([[0, -jnp.inf]])])
@pytest.mark.parametrize("Y", [jnp.array([[jnp.inf, 0]]), jnp.array([[0, -jnp.inf]]), None])
...
@pytest.mark.parametrize(
    ...
    [
        (jnp.array([[0, 1], [1, 0]]), jnp.sqrt(2), jnp.nan),
        (jnp.array([[0, 1], [1, jnp.nan]]), jnp.sqrt(2), jnp.nan),
        (jnp.array([[jnp.nan, 1], [1, jnp.nan]]), jnp.nan, jnp.nan),
        (jnp.array([[jnp.nan, 1], [jnp.nan, 0]]), jnp.sqrt(2), jnp.nan),
        (jnp.array([[0, jnp.nan], [1, jnp.nan]]), jnp.sqrt(2), jnp.nan),
        (jnp.array([[0, 1], [1, 0]]), jnp.sqrt(2), -1),
        (jnp.array([[0, 1], [1, -1]]), jnp.sqrt(2), -1),
        (jnp.array([[-1, 1], [1, -1]]), jnp.nan, -1),
        (jnp.array([[-1, 1], [-1, 0]]), jnp.sqrt(2), -1),
        (jnp.array([[0, -1], [1, -1]]), jnp.sqrt(2), -1),
    ],
)
def test_nan_euclidean_distances_2x2(X, X_diag, missing_value):
    exp_dist = jnp.array([[0.0, X_diag], [X_diag, 0]])
...
@pytest.mark.parametrize("missing_value", [jnp.nan, -1])
def test_nan_euclidean_distances_complete_nan(missing_value):
    X = jnp.array([[missing_value, missing_value], [0, 1]])

    exp_dist = jnp.array([[jnp.nan, jnp.nan], [jnp.nan, 0]])
...
def test_cosine_distances():
    ...
    assert jnp.all(D <= 2.0)
    # check that diagonal elements are equal to 0
    assert_allclose(D[jnp.diag_indices_from(D)], [0.0, 0.0])

    XB = jnp.vstack([x, -x])
    D2 = cosine_distances(XB)
    # check that all elements are in [0, 2]
    assert jnp.all(D2 >= 0.0)
    assert jnp.all(D2 <= 2.0)
    # check that diagonal elements are equal to 0 and non diagonal to 2
    assert_allclose(D2, [[0.0, 2.0], [2.0, 0.0]])

    # check large random matrix
    X = jnp.abs(rng.rand(1000, 5000))
    D = cosine_distances(X)
    # check that diagonal elements are equal to 0
    assert_allclose(D[jnp.diag_indices_from(D)], [0.0] * D.shape[0])
    assert jnp.all(D >= 0.0)
    assert jnp.all(D <= 2.0)
...
def test_haversine_distances():
    ...
    def slow_haversine_distances(x, y):
        diff_lat = y[0] - x[0]
        diff_lon = y[1] - x[1]
        a = jnp.sin(diff_lat / 2) ** 2 + (
            jnp.cos(x[0]) * jnp.cos(y[0]) * jnp.sin(diff_lon / 2) ** 2
        )
...
def test_chi_square_kernel():
    ...
    for i, x in enumerate(X):
        for j, y in enumerate(Y):
            chi2 = -jnp.sum((x - y) ** 2 / (x + y))
            chi2_exp = jnp.exp(gamma * chi2)
            assert_almost_equal(K_add[i, j], chi2)
            assert_almost_equal(K[i, j], chi2_exp)

    # check diagonal is ones for data with itself
    K = chi2_kernel(Y)
    assert_array_equal(jnp.diag(K), 1)
    # check off-diagonal is < 1 but > 0:
    assert jnp.all(K > 0)
    assert jnp.all(K - jnp.diag(jnp.diag(K)) < 1)
    # check that float32 is preserved
    X = rng.random_sample((5, 4)).astype(jnp.float32)
    Y = rng.random_sample((10, 4)).astype(jnp.float32)
    K = chi2_kernel(X, Y)
    assert K.dtype == jnp.float32

    # check integer type gets converted,
    # check that zeros are handled
    X = rng.random_sample((10, 4)).astype(jnp.int32)
    K = chi2_kernel(X, X)
...
def test_rbf_kernel():
    ...
    assert_allclose(K.flat[::6], jnp.ones(5))
...
def test_laplacian_kernel():
    ...
    assert_allclose(jnp.diag(K), jnp.ones(5))
...
def test_check_dense_matrices():
    ...
    XA = jnp.resize(jnp.arange(40), (5, 8))
...
def test_check_XB_returned():
    ...
    XA = jnp.resize(jnp.arange(40), (5, 8))
    XB = jnp.resize(jnp.arange(32), (4, 8))
    XA_checked, XB_checked = check_pairwise_arrays(XA, XB)
    assert_array_equal(XA, XA_checked)
    assert_array_equal(XB, XB_checked)

    XB = jnp.resize(jnp.arange(40), (5, 8))
...
def test_check_different_dimensions():
    ...
    XA = jnp.resize(jnp.arange(45), (5, 9))
    XB = jnp.resize(jnp.arange(32), (4, 8))
    with pytest.raises(ValueError):
        check_pairwise_arrays(XA, XB)

    XB = jnp.resize(jnp.arange(4 * 9), (4, 9))
    with pytest.raises(ValueError):
...
def test_check_invalid_dimensions():
    ...
    XA = jnp.arange(45).reshape(9, 5)
    XB = jnp.arange(32).reshape(4, 8)
    with pytest.raises(ValueError):
        check_pairwise_arrays(XA, XB)
    XA = jnp.arange(45).reshape(9, 5)
    XB = jnp.arange(32).reshape(4, 8)
...
def test_check_preserve_type():
    ...
    XA = jnp.resize(jnp.arange(40), (5, 8)).astype(jnp.float32)
    XB = jnp.resize(jnp.arange(40), (5, 8)).astype(jnp.float32)

    XA_checked, XB_checked = check_pairwise_arrays(XA, None)
    assert XA_checked.dtype == jnp.float32

    # both float32
    XA_checked, XB_checked = check_pairwise_arrays(XA, XB)
    assert XA_checked.dtype == jnp.float32
    assert XB_checked.dtype == jnp.float32
...
def test_pairwise_distances_data_derived_params(n_jobs, metric, dist_function):
    ...
    with config_context(working_memory=0.1):  # to have more than 1 chunk
        ...
        dist = jnp.vstack(tuple(dist_function(X, metric=metric, n_jobs=n_jobs)))
...
def test_numeric_pairwise_distances_datatypes(metric, global_dtype, y_is_x):
    ...
    if y_is_x:
        ...
        if metric == "seuclidean":
            params = {"V": jnp.var(
                jnp.vstack([X, Y]), axis=0, ddof=1, dtype=jnp.float64)}
        elif metric == "mahalanobis":
            params = {"VI": jnp.linalg.inv(jnp.cov(jnp.vstack([X, Y]).T)).T}
...
def test_sparse_manhattan_readonly_dataset():
    # Non-regression test for: https://github.com/jax-learn/jax-learn/issues/7981
    matrices1 = [csr_matrix(jnp.ones((5, 5)))]
    matrices2 = [csr_matrix(jnp.ones((5, 5)))]
