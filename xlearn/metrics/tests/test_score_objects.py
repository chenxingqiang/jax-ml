...
def setup_module():
    ...
    TEMP_FOLDER = tempfile.mkdtemp(prefix="xlearn_test_score_objects_")
...
def test_thresholded_scorers():
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=0)
    clf = DecisionTreeClassifier()
    clf.fit(X_train, jnp.zeros_like(y_train))
...
def test_thresholded_scorers_multilabel_indicator_data():
    ...
    score1 = get_scorer("roc_auc")(clf, X_test, y_test)
    score2 = roc_auc_score(y_test, jnp.vstack([p[:, -1] for p in y_proba]).T)
    assert_almost_equal(score1, score2)
    ...
    score1 = get_scorer("roc_auc")(clf, X_test, y_test)
    score2 = roc_auc_score(y_test, jnp.vstack([p for p in y_proba]).T)
    assert_almost_equal(score1, score2)
...
def test_classification_scorer_sample_weight():
    ...
    sample_weight = jnp.ones_like(y_test)
    sample_weight[:10] = 0
...
def test_multimetric_scorer_calls_method_once(
    ...
    X, y = jnp.array([[1], [1], [0], [0], [0]]), jnp.array([0, 1, 1, 1, 0])
    ...
    mock_est.classes_ = jnp.array([0, 1])
...
def string_labeled_classification_problem():
    ...
    y_pred_proba : ndarray of shape (n_samples, 2), dtype=jnp.float64
        Probabilities of `classifier` when predicting for `X_test`.
    y_pred_decision : ndarray of shape (n_samples,), dtype=jnp.float64
    ...
    idx_positive = jnp.flatnonzero(y == 1)
    idx_negative = jnp.flatnonzero(y == 0)
    idx_selected = jnp.hstack([idx_negative, idx_positive[:25]])
...
def test_scorer_select_proba_error(scorer):
    ...
    assert scorer._kwargs["pos_label"] not in jnp.unique(y).tolist()
...
def test_scorer_no_op_multiclass_select_proba():
    ...
    assert_array_equal(jnp.unique(y_test), lr.classes_[:-1])
