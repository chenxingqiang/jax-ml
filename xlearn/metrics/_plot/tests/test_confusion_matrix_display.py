...
def test_confusion_matrix_display_plotting(
    ...
    if include_values:
        ...
        fmt = ".2g"
        expected_text = jnp.array([format(v, fmt) for v in cm.ravel(order="C")])
        text_text = jnp.array([t.get_text()
...
def test_confusion_matrix_display(pyplot, constructor_name):
    ...
    expected_text = jnp.array([format(v, "e") for v in cm.ravel(order="C")])
    text_text = jnp.array([t.get_text() for t in disp.text_.ravel(order="C")])
    assert_array_equal(expected_text, text_text)
...
def test_confusion_matrix_with_unknown_labels(pyplot, constructor_name):
    ...
    https://github.com/jax-learn/jax-learn/pull/18405
    """
    n_classes = 5
    X, y = make_classification(
        n_samples=100, n_informative=5, n_classes=n_classes, random_state=0
...
def test_colormap_max(pyplot):
    ...
    confusion_matrix = jnp.array([[1.0, 0.0], [0.0, 1.0]])
...
def test_im_kw_adjust_vmin_vmax(pyplot):
    ...
    confusion_matrix = jnp.array([[0.48, 0.04], [0.08, 0.4]])
    disp = ConfusionMatrixDisplay(confusion_matrix)
