...
def test_precision_recall_display_plotting(
    ...
    jnp.testing.assert_allclose(display.precision, precision)
    jnp.testing.assert_allclose(display.recall, recall)
...
def test_default_labels(pyplot, average_precision, estimator_name, expected_label):
    ...
    precision = jnp.array([1, 0.5, 0])
    recall = jnp.array([0, 0.5, 1])
...
def test_plot_precision_recall_pos_label(pyplot, constructor_name, response_method):
    ...
    assert -jnp.trapz(display.precision, display.recall) < avg_prec_limit
