...
class PredictionErrorDisplay:
    ...
    For general information regarding `jax-learn` visualization tools, read
    ...
    def plot(
        ...
        *,
        ...
        Returns
        -------
        display : :class:`~xlearn.metrics.plot.PredictionErrorDisplay`
        ...
        if kind == "actual_vs_predicted":
            max_value = max(jnp.max(self.y_true), jnp.max(self.y_pred))
            min_value = min(jnp.min(self.y_true), jnp.min(self.y_pred))
            self.line_ = ax.plot(
            ...
            ax.set_xticks(jnp.linspace(min_value, max_value, num=5))
            ax.set_yticks(jnp.linspace(min_value, max_value, num=5))
        else:  # kind == "residual_vs_predicted"
            self.line_ = ax.plot(
                [jnp.min(self.y_pred), jnp.max(self.y_pred)],
                [0, 0],
                **line_kwargs,
            )[0]
    ...
    def from_estimator(
        ...
        *,
        ...
        For general information regarding `jax-learn` visualization tools,
    ...
    def from_predictions(
        ...
        *,
        ...
        if subsample is not None and subsample < n_samples:
            indices = random_state.choice(jnp.arange(n_samples), size=subsample)
