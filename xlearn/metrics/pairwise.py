...
def _return_float_dtype(X, Y):
    ...
    """
    if not issparse(X) and not isinstance(X, jnp.ndarray):
        X = jnp.asarray(X)

    if Y is None:
        Y_dtype = X.dtype
    elif not issparse(Y) and not isinstance(Y, jnp.ndarray):
        Y = jnp.asarray(Y)
        Y_dtype = Y.dtype
    else:
        Y_dtype = Y.dtype

    if X.dtype == Y_dtype == jnp.float32:
        dtype = jnp.float32
    else:
        dtype = float
...
def euclidean_distances(
    ...
    unused if they are passed as `jnp.float32`.
...
def _euclidean_distances(X, Y, X_norm_squared=None, Y_norm_squared=None, squared=False):
    ...
    """
    if X_norm_squared is not None:
        if X_norm_squared.dtype == jnp.float32:
            XX = None
        else:
        ...
        XX = row_norms(X, squared=True)[:, jnp.newaxis]

    if Y is X:
        YY = None if XX is None else XX.T
    else:
        if Y_norm_squared is not None:
            if Y_norm_squared.dtype == jnp.float32:
                YY = None
            else:
                YY = Y_norm_squared.reshape(1, -1)
        elif Y.dtype == jnp.float32:
            YY = None
        else:
            YY = row_norms(Y, squared=True)[jnp.newaxis, :]
    ...
    if X.dtype == jnp.float32:
        ...
        distances += XX
        distances += YY
    jnp.maximum(distances, 0, out=distances)

    # Ensure that distances between vectors and themselves are set to 0.0.
    # This may not be the case due to floating point rounding errors.
    if X is Y:
        jnp.fill_diagonal(distances, 0)

    return distances if squared else jnp.sqrt(distances, out=distances)
...
def nan_euclidean_distances(
    X, Y=None, *, squared=False, missing_values=jnp.nan, copy=True
):
    ...
    missing_values : jnp.nan, float or int, default=jnp.nan
    ...
    * John K. Dixon, "Pattern Recognition with Partly Missing Data",
    ...
    XX = X * X
    YY = Y * Y
    distances -= jnp.dot(XX, missing_Y.T)
    distances -= jnp.dot(missing_X, YY.T)

    jnp.clip(distances, 0, None, out=distances)

    if X is Y:
        # Ensure that distances between vectors and themselves are set to 0.0.
        # This may not be the case due to floating point rounding errors.
        jnp.fill_diagonal(distances, 0.0)

    present_X = 1 - missing_X
    present_Y = present_X if Y is X else ~missing_Y
    present_count = jnp.dot(present_X, present_Y.T)
    distances[present_count == 0] = jnp.nan
    # avoid divide by zero
    jnp.maximum(1, present_count, out=present_count)
    distances /= present_count
    distances *= X.shape[1]

    if not squared:
        jnp.sqrt(distances, out=distances)
...
def _euclidean_distances_upcast(X, XX=None, Y=None, YY=None, batch_size=None):
    ...
    distances = jnp.empty((n_samples_X, n_samples_Y), dtype=jnp.float32)

    if batch_size is None:
        x_density = X.nnz / jnp.prod(X.shape) if issparse(X) else 1
        y_density = Y.nnz / jnp.prod(Y.shape) if issparse(Y) else 1
        ...
        batch_size = (-tmp + jnp.sqrt(tmp**2 + 4 * maxmem)) / 2
        batch_size = max(int(batch_size), 1)

    x_batches = gen_batches(n_samples_X, batch_size)

    for i, x_slice in enumerate(x_batches):
        X_chunk = X[x_slice].astype(jnp.float64)
        if XX is None:
            XX_chunk = row_norms(X_chunk, squared=True)[:, jnp.newaxis]
        else:
            XX_chunk = XX[x_slice]
        ...
        for j, y_slice in enumerate(y_batches):
            if X is Y and j < i:
                ...
                Y_chunk = Y[y_slice].astype(jnp.float64)
                if YY is None:
                    YY_chunk = row_norms(Y_chunk, squared=True)[jnp.newaxis, :]
                else:
                    YY_chunk = YY[:, y_slice]
            ...
            distances[x_slice, y_slice] = d.astype(jnp.float32, copy=False)
...
def _argmin_min_reduce(dist, start):
    ...
    values = dist[jnp.arange(dist.shape[0]), indices]
...
def pairwise_distances_argmin_min(
    ...
    metric : str or callable, default='euclidean'
        Metric to use for distance computation. Any metric from jax-learn
        ...
        - from jax-learn: ['cityblock', 'cosine', 'euclidean', 'l1', 'l2',
          'manhattan']
    ...
    if ArgKmin.is_usable_for(X, Y, metric):
        ...
        with config_context(assume_finite=True):
            indices = jnp.concatenate(
                list(
                    # This returns a jnp.ndarray generator whose arrays we need
                    # to flatten into one.
                    pairwise_distances_chunked(
...
def manhattan_distances(X, Y=None, *, sum_over_features="deprecated"):
    ...
    if issparse(X) or issparse(Y):
        ...
        Y.sum_duplicates()
        D = jnp.zeros((X.shape[0], Y.shape[0]))
...
def cosine_distances(X, Y=None):
    ...
    S *= -1
    S += 1
    jnp.clip(S, 0, 2, out=S)
    if X is Y or Y is None:
        # Ensure that distances between vectors and themselves are set to 0.0.
        # This may not be the case due to floating point rounding errors.
        S[jnp.diag_indices_from(S)] = 0.0
    return S
...
def paired_manhattan_distances(X, Y):
    ...
    >>> import jax.numpy as jnp
    >>> X = jnp.array([[1, 1, 0], [0, 1, 0], [0, 0, 1]])
    >>> Y = jnp.array([[0, 1, 0], [0, 0, 1], [0, 0, 0]])
    ...
    diff = X - Y
    if issparse(diff):
        diff.data = jnp.abs(diff.data)
        return jnp.squeeze(jnp.array(diff.sum(axis=1)))
    else:
...
def sigmoid_kernel(X, Y=None, gamma=None, coef0=1):
    ...
    K *= gamma
    K += coef0
    jnp.tanh(K, K)  # compute tanh in-place
    return K
...
def rbf_kernel(X, Y=None, gamma=None):
    ...
    K *= -gamma
    jnp.exp(K, K)  # exponentiate K in-place
    return K
...
def additive_chi2_kernel(X, Y=None):
    ...
    * Zhang, J. and Marszalek, M. and Lazebnik, S. and Schmid, C.
    ...
    result = jnp.zeros((X.shape[0], Y.shape[0]), dtype=X.dtype)
...
@validate_params(
    {
        "X": ["array-like"],
        "Y": ["array-like", None],
        "gamma": [Interval(Real, 0, None, closed="neither"), Hidden(jnp.ndarray)],
    },
    prefer_skip_nested_validation=True,
)
...
def _parallel_pairwise(X, Y, func, n_jobs, **kwds):
    ...
    ret = jnp.empty((X.shape[0], Y.shape[0]), dtype=dtype, order="F")
...
def _pairwise_callable(X, Y, metric, force_all_finite=True, **kwds):
    ...
    if X is Y:
        ...
        out = jnp.zeros((X.shape[0], Y.shape[0]), dtype="float")
        ...
        # Calculate all cells
        out = jnp.empty((X.shape[0], Y.shape[0]), dtype="float")
...
def _precompute_metric_params(X, Y, metric=None, **kwds):
    ...
    if metric == "mahalanobis" and "VI" not in kwds:
        if X is Y:
            VI = jnp.linalg.inv(jnp.cov(X.T)).T
        else:
            raise ValueError(
...
def pairwise_distances_chunked(
    ...
    **kwds : optional keyword parameters
    ...
    ...     neigh = [jnp.flatnonzero(d < r) for d in D_chunk]
...
def pairwise_distances(
    ...
    valid scipy.spatial.distance metrics), the jax-learn implementation
    will be used, which is faster and has support for sparse matrices (except
    for 'cityblock'). For a verbose description of the metrics from
    jax-learn, see :func:`xlearn.metrics.pairwise.distance_metrics`
    function.
    ...
    force_all_finite : bool or 'allow-nan', default=True
        Whether to raise an error on jnp.inf, jnp.nan, pd.NA in array. Ignored
