...
def mean_absolute_error(
    ...
    output_errors = jnp.average(
        jnp.abs(y_pred - y_true), weights=sample_weight, axis=0)
    if isinstance(multioutput, str):
        if multioutput == "raw_values":
            return output_errors
        elif multioutput == "uniform_average":
            # pass None as weights to jnp.average: uniform mean
            multioutput = None
...
def mean_pinball_loss(
    ...
    output_errors = jnp.average(loss, weights=sample_weight, axis=0)
...
def mean_absolute_percentage_error(
    ...
    mape = jnp.abs(y_pred - y_true) / jnp.maximum(jnp.abs(y_true), epsilon)
    output_errors = jnp.average(mape, weights=sample_weight, axis=0)
    if isinstance(multioutput, str):
        if multioutput == "raw_values":
            return output_errors
        elif multioutput == "uniform_average":
...
def median_absolute_error(
    ...
    if sample_weight is None:
        output_errors = jnp.median(jnp.abs(y_pred - y_true), axis=0)
    else:
        sample_weight = _check_sample_weight(sample_weight, y_pred)
        output_errors = _weighted_percentile(
            jnp.abs(y_pred - y_true), sample_weight=sample_weight
        )
    if isinstance(multioutput, str):
        if multioutput == "raw_values":
            return output_errors
...
def _assemble_r2_explained_variance(
    ...
    if not force_finite:
        ...
        output_scores = jnp.ones([n_outputs])
    ...
    if isinstance(multioutput, str):
        if multioutput == "raw_values":
            ...
            # Passing None as weights to jnp.average results is uniform mean
            avg_weights = None
        elif multioutput == "variance_weighted":
            avg_weights = denominator
            if not jnp.any(nonzero_denominator):
                # All weights are zero, jnp.average would raise a ZeroDiv error.
...
def explained_variance_score(
    ...
    *,
    ...
    denominator = jnp.average((y_true - y_true_avg) **
...
def r2_score(
    ...
    *,
    ...
    if sample_weight is not None:
        ...
        weight = sample_weight[:, jnp.newaxis]
    else:
        weight = 1.0

    numerator = (weight * (y_true - y_pred) ** 2).sum(axis=0, dtype=jnp.float64)
    denominator = (
        weight * (y_true - jnp.average(y_true, axis=0,
                  weights=sample_weight)) ** 2
    ).sum(axis=0, dtype=jnp.float64)
...
def mean_tweedie_deviance(y_true, y_pred, *, sample_weight=None, power=0):
    ...
    y_type, y_true, y_pred, _ = _check_reg_targets(
        y_true, y_pred, None, dtype=[jnp.float64, jnp.float32]
    )
    if y_type == "continuous-multioutput":
...
def d2_pinball_score(
    ...
    if sample_weight is None:
        y_quantile = jnp.tile(
            jnp.percentile(y_true, q=alpha * 100, axis=0), (len(y_true), 1)
        )
    else:
    ...
    output_scores = jnp.ones(y_true.shape[1])
    ...
    if isinstance(multioutput, str):
        if multioutput == "raw_values":
            ...
            # passing None as weights to jnp.average results in uniform mean
            avg_weights = None
    else:
        avg_weights = multioutput
