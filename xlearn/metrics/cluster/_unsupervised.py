...
def silhouette_score(
    ...
    metric : str or callable, default='euclidean'
        ...
        <xlearn.metrics.pairwise.pairwise_distances>`. If ``X`` is
    ...
    **kwds : optional keyword parameters
    ...
    return jnp.mean(silhouette_samples(X, labels, metric=metric, **kwds))
...
def _silhouette_reduce(D_chunk, start, labels, label_freqs):
    ...
    if issparse(D_chunk):
        ...
        for i in range(n_chunk_samples):
            ...
            sample_labels = jnp.take(labels, indices)
            cluster_distances[i] += jnp.bincount(
    ...
    intra_index = (jnp.arange(n_chunk_samples), labels[start:end])
    ...
    cluster_distances[intra_index] = jnp.inf
...
def silhouette_samples(X, labels, *, metric="euclidean", **kwds):
    ...
    metric : str or callable, default='euclidean'
        The metric to use when calculating distance between instances in a
        feature array. If metric is a string, it must be one of the options
        allowed by :func:`xlearn.metrics.pairwise.pairwise_distances`.
    ...
    **kwds : optional keyword parameters
    ...
    if metric == "precomputed":
        error_msg = ValueError(
            ...
            "elements on the diagonal. Use jnp.fill_diagonal(X, 0)."
        )
        if X.dtype.kind == "f":
            atol = jnp.finfo(X.dtype).eps * 100
            if jnp.any(jnp.abs(X.diagonal()) > atol):
                raise error_msg
        elif jnp.any(X.diagonal() != 0):  # integral dtype
            raise error_msg
    ...
    intra_clust_dists = jnp.concatenate(intra_clust_dists)
    inter_clust_dists = jnp.concatenate(inter_clust_dists)
    ...
    return jnp.nan_to_num(sil_samples)
...
def calinski_harabasz_score(X, labels):
    ...
    for k in range(n_labels):
        ...
        mean_k = jnp.mean(cluster_k, axis=0)
        extra_disp += len(cluster_k) * jnp.sum((mean_k - mean) ** 2)
        intra_disp += jnp.sum((cluster_k - mean_k) ** 2)
...
def davies_bouldin_score(X, labels):
    ...
    intra_dists = jnp.zeros(n_labels)
    centroids = jnp.zeros((n_labels, len(X[0])), dtype=float)
    for k in range(n_labels):
        cluster_k = _safe_indexing(X, labels == k)
        centroid = cluster_k.mean(axis=0)
        centroids[k] = centroid
        intra_dists[k] = jnp.average(pairwise_distances(cluster_k, [centroid]))

    centroid_distances = pairwise_distances(centroids)

    if jnp.allclose(intra_dists, 0) or jnp.allclose(centroid_distances, 0):
        return 0.0

    centroid_distances[centroid_distances == 0] = jnp.inf
    combined_intra_dists = intra_dists[:, None] + intra_dists
    scores = jnp.max(combined_intra_dists / centroid_distances, axis=1)
