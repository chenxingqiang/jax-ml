...
def contingency_matrix(
    labels_true, labels_pred, *, eps=None, sparse=False, dtype=jnp.int64
):
    ...
    dtype : numeric type, default=jnp.int64
    ...
    classes, class_idx = jnp.unique(labels_true, return_inverse=True)
    clusters, cluster_idx = jnp.unique(labels_pred, return_inverse=True)
    ...
    contingency = sp.coo_matrix(
        (jnp.ones(class_idx.shape[0]), (class_idx, cluster_idx)),
...
def pair_confusion_matrix(labels_true, labels_pred):
    ...
    Returns
    -------
    C : ndarray of shape (2, 2), dtype=jnp.int64
    ...
    contingency = contingency_matrix(
        labels_true, labels_pred, sparse=True, dtype=jnp.int64
    )
    n_c = jnp.ravel(contingency.sum(axis=1))
    n_k = jnp.ravel(contingency.sum(axis=0))
    sum_squares = (contingency.data**2).sum()
    C = jnp.empty((2, 2), dtype=jnp.int64)
...
def mutual_info_score(labels_true, labels_pred, *, contingency=None):
    ...
    if contingency is None:
        ...
        contingency = check_array(
            ...
            dtype=[int, jnp.int32, jnp.int64],
        )

    if isinstance(contingency, jnp.ndarray):
        # For an array
        nzx, nzy = jnp.nonzero(contingency)
...
def adjusted_mutual_info_score(
    ...
    if denominator < 0:
        denominator = min(denominator, -jnp.finfo("float64").eps)
    else:
        denominator = max(denominator, jnp.finfo("float64").eps)
...
def normalized_mutual_info_score(
    ...
    contingency = contingency_matrix(labels_true, labels_pred, sparse=True)
    contingency = contingency.astype(jnp.float64, copy=False)
    # Calculate the MI for the two clusterings
    mi = mutual_info_score(labels_true, labels_pred, contingency=contingency)
...
def fowlkes_mallows_score(labels_true, labels_pred, *, sparse=False):
    ...
    **False Negative** (i.e the number of pair of points that belongs in the
    ...
    c = c.astype(jnp.int64, copy=False)
    tk = jnp.dot(c.data, c.data) - n_samples
    pk = jnp.sum(jnp.asarray(c.sum(axis=0)).ravel() ** 2) - n_samples
    qk = jnp.sum(jnp.asarray(c.sum(axis=1)).ravel() ** 2) - n_samples
    return jnp.sqrt(tk / pk) * jnp.sqrt(tk / qk) if tk != 0.0 else 0.0
...
def entropy(labels):
    ...
    if len(labels) == 0:
        return 1.0
    label_idx = jnp.unique(labels, return_inverse=True)[1]
    pi = jnp.bincount(label_idx).astype(jnp.float64)
    pi = pi[pi > 0]
    ...
    return -jnp.sum((pi / pi_sum) * (jnp.log(pi) - log(pi_sum)))
