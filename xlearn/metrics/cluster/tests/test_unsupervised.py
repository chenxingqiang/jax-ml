...
def test_cluster_size_1():
    ...
    assert not jnp.isnan(silhouette)
...
def test_silhouette_paper_example():
    ...
    for labels, expected, score in [
        ...
        pytest.approx(
            expected,
            silhouette_samples(D, jnp.array(labels), metric="precomputed"),
            abs=1e-2,
        )
        pytest.approx(
            score, silhouette_score(D, jnp.array(labels), metric="precomputed"), abs=1e-2
        )
...
def test_correct_labelsize():
    ...
    err_msg = (
        ...
        r"to n_samples - 1 \(inclusive\)" % len(jnp.unique(y))
    )
...
def test_silhouette_nonzero_diag(dtype):
    ...
    dists = pairwise_distances(
        jnp.array([[0.2, 0.1, 0.12, 1.34, 1.11, 1.6]], dtype=dtype).T
    )
    labels = [0, 0, 0, 1, 1, 1]

    # small values on the diagonal are OK
    dists[2][2] = jnp.finfo(dists.dtype).eps * 10
    silhouette_samples(dists, labels, metric="precomputed")

    # values bigger than eps * 100 are not
    dists[2][2] = jnp.finfo(dists.dtype).eps * 1000
...
def test_silhouette_samples_precomputed_sparse(to_sparse):
    ...
    X = jnp.array([[0.2, 0.1, 0.1, 0.2, 0.1, 1.6, 0.2, 0.1]],
                 dtype=jnp.float32).T
    y = [0, 0, 0, 0, 1, 1, 1, 1]
...
def test_calinski_harabasz_score():
    ...
    assert 1.0 == calinski_harabasz_score(jnp.ones((10, 2)), [0] * 5 + [1] * 5)
...
def test_davies_bouldin_score():
    ...
    assert davies_bouldin_score(jnp.ones((10, 2)), [0] * 5 + [1] * 5) == pytest.approx(
        0.0
    )
    ...
    labels = [0] * 10 + [1] * 10 + [2] * 10 + [3] * 10
    pytest.approx(davies_bouldin_score(X, labels), 2 * jnp.sqrt(0.5) / 3)
