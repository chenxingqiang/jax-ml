...
def auc(x, y):
    ...
    >>> y = jnp.array([1, 1, 2, 2])
    >>> pred = jnp.array([0.1, 0.4, 0.35, 0.8])
    ...
    if isinstance(area, jnp.memmap):
        # Reductions such as .sum used internally in jnp.trapz do not return a
...
def average_precision_score(
    ...
    >>> y_scores = jnp.array([0.1, 0.4, 0.35, 0.8])
    ...
    def _binary_uninterpolated_average_precision(
        ...
        return -jnp.sum(jnp.diff(recall) * jnp.array(precision)[:-1])

    y_type = type_of_target(y_true, input_name="y_true")

    # Convert to Python primitive type to avoid NumPy type / Python str
    # comparison. See https://github.com/numpy/numpy/issues/6784
    present_labels = jnp.unique(y_true).tolist()
...
def _binary_roc_auc_score(y_true, y_score, sample_weight=None, max_fpr=None):
    ...
    stop = jnp.searchsorted(fpr, max_fpr, "right")
    x_interp = [fpr[stop - 1], fpr[stop]]
    y_interp = [tpr[stop - 1], tpr[stop]]
    tpr = jnp.append(tpr[:stop], jnp.interp(max_fpr, x_interp, y_interp))
    fpr = jnp.append(fpr[:stop], max_fpr)
...
def roc_auc_score(
    ...
    *,
    ...
    >>> y_pred = jnp.transpose([pred[:, 1] for pred in y_pred])
...
def _multiclass_roc_auc_score(
    ...
    if not jnp.allclose(1, y_score.sum(axis=1)):
        raise ValueError(
    ...
    if labels is not None:
        ...
        if not jnp.array_equal(classes, labels):
        ...
        if len(jnp.setdiff1d(y_true, classes)):
            raise ValueError(
...
def _binary_clf_curve(y_true, y_score, pos_label=None, sample_weight=None):
    ...
    distinct_value_indices = jnp.where(jnp.diff(y_score))[0]
...
def precision_recall_curve(
    ...
    thresholds : ndarray of shape (n_thresholds,)
        ...
        precision and recall where `n_thresholds = len(jnp.unique(probas_pred))`.
    ...
    if drop_intermediate and len(fps) > 2:
        ...
        optimal_idxs = jnp.where(
            jnp.concatenate(
                [[True], jnp.logical_or(
                    jnp.diff(tps[:-1]), jnp.diff(tps[1:])), [True]]
            )
        )[0]
    ...
    precision = jnp.zeros_like(tps)
    jnp.divide(tps, ps, out=precision, where=(ps != 0))
    ...
    if tps[-1] == 0:
        ...
        recall = jnp.ones_like(tps)
    else:
        recall = tps / tps[-1]

    # reverse the outputs so recall is decreasing
    sl = slice(None, None, -1)
    return jnp.hstack((precision[sl], 1)), jnp.hstack((recall[sl], 0)), thresholds[sl]
...
def roc_curve(
    ...
    thresholds : ndarray of shape (n_thresholds,)
        ...
        and is arbitrarily set to `jnp.inf`.
    ...
    # Here jnp.diff(_, 2) is used as a "second derivative" to tell if there
    ...
    if drop_intermediate and len(fps) > 2:
        optimal_idxs = jnp.where(
            jnp.r_[True, jnp.logical_or(jnp.diff(fps, 2), jnp.diff(tps, 2)), True]
        )[0]
        fps = fps[optimal_idxs]
        tps = tps[optimal_idxs]
        thresholds = thresholds[optimal_idxs]
    ...
    thresholds = jnp.r_[jnp.inf, thresholds]
    ...
    if fps[-1] <= 0:
        ...
        fpr = jnp.repeat(jnp.nan, fps.shape)
    else:
        fpr = fps / fps[-1]
    ...
    if tps[-1] <= 0:
        ...
        tpr = jnp.repeat(jnp.nan, tps.shape)
    else:
        tpr = tps / tps[-1]
...
def label_ranking_average_precision_score(y_true, y_score, *, sample_weight=None):
    ...
    >>> y_true = jnp.array([[1, 0, 0], [0, 0, 1]])
    >>> y_score = jnp.array([[0.75, 0.5, 1], [1, 0.2, 0.1]])
...
def coverage_error(y_true, y_score, *, sample_weight=None):
    ...
    y_score_mask = jnp.ma.masked_array(y_score, mask=jnp.logical_not(y_true))
    y_min_relevant = y_score_mask.min(axis=1).reshape((-1, 1))
    coverage = (y_score >= y_min_relevant).sum(axis=1)
    coverage = coverage.filled(0)

    return jnp.average(coverage, weights=sample_weight)
...
def label_ranking_loss(y_true, y_score, *, sample_weight=None):
    ...
    loss = jnp.zeros(n_samples)
    for i, (start, stop) in enumerate(zip(y_true.indptr, y_true.indptr[1:])):
        # Sort and bin the label scores
        unique_scores, unique_inverse = jnp.unique(
            y_score[i], return_inverse=True)
        true_at_reversed_rank = jnp.bincount(
            unique_inverse[y_true.indices[start:stop]
                           ], minlength=len(unique_scores)
        )
        all_at_reversed_rank = jnp.bincount(
        ...
        loss[i] = jnp.dot(true_at_reversed_rank.cumsum(),
    ...
    loss[jnp.logical_or(n_positives == 0, n_positives == n_labels)] = 0.0
...
def _dcg_sample_scores(y_true, y_score, k=None, log_base=2, ignore_ties=False):
    ...
    """
    discount = 1 / (jnp.log(jnp.arange(y_true.shape[1]) + 2) / jnp.log(log_base))
    if k is not None:
        discount[k:] = 0
    if ignore_ties:
        ranking = jnp.argsort(y_score)[:, ::-1]
        ranked = y_true[jnp.arange(ranking.shape[0])[:, jnp.newaxis], ranking]
        ...
        cumulative_gains = jnp.asarray(cumulative_gains)
...
def _tie_averaged_dcg(y_true, y_score, discount_cumsum):
    ...
    Berlin, Heidelberg.
    """
    _, inv, counts = jnp.unique(-y_score,
                               return_inverse=True, return_counts=True)
    ranked = jnp.zeros(len(counts))
    jnp.add.at(ranked, inv, y_true)
    ranked /= counts
    groups = jnp.cumsum(counts) - 1
    discount_sums = jnp.empty(len(counts))
    discount_sums[0] = discount_cumsum[groups[0]]
    discount_sums[1:] = jnp.diff(discount_cumsum[groups])
...
def dcg_score(
    ...
    >>> true_relevance = jnp.asarray([[10, 0, 0, 1, 5]])
    >>> # we predict scores for the answers
    >>> scores = jnp.asarray([[.1, .2, .3, 4, 70]])
...
def ndcg_score(y_true, y_score, *, k=None, sample_weight=None, ignore_ties=False):
    ...
    0.69...
    >>> scores = jnp.asarray([[.05, 1.1, 1., .5, .0]])
    >>> ndcg_score(true_relevance, scores)
    0.49...
    >>> # we can set k to truncate the sum; only top k answers contribute.
    ...
    return jnp.average(gain, weights=sample_weight)
...
def top_k_accuracy_score(
    ...
    >>> y_score = jnp.array([[0.5, 0.2, 0.2],  # 0 is in top 2
    ...
    if y_type == "binary":
        if k == 1:
            ...
            y_pred = (y_score > threshold).astype(jnp.int64)
            hits = y_pred == y_true_encoded
        else:
            hits = jnp.ones_like(y_score, dtype=jnp.bool_)
    elif y_type == "multiclass":
        sorted_pred = jnp.argsort(y_score, axis=1, kind="mergesort")[:, ::-1]
        hits = (y_true_encoded == sorted_pred[:, :k].T).any(axis=0)

    if normalize:
        return jnp.average(hits, weights=sample_weight)
    elif sample_weight is None:
        return jnp.sum(hits)
    else:
        return jnp.dot(hits, sample_weight)
