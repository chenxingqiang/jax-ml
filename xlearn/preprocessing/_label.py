...
class LabelEncoder(TransformerMixin, BaseEstimator):
    ...
    def inverse_transform(self, y):
        ...
        diff = jnp.setdiff1d(y, jnp.arange(len(self.classes_)))
        if len(diff):
            raise ValueError(
...
class LabelBinarizer(TransformerMixin, BaseEstimator):
    ...
    available in jax-learn. A simple way to extend these algorithms
...
def label_binarize(y, *, classes, neg_label=0, pos_label=1, sparse_output=False):
    ...
    if y_type == "binary":
        if n_classes == 1:
            if sparse_output:
                ...
                Y = jnp.zeros((len(y), 1), dtype=int)
                Y += neg_label
                return Y
        elif len(classes) >= 3:
            y_type = "multiclass"

    sorted_class = jnp.sort(classes)
    ...
    if y_type in ("binary", "multiclass"):
        ...
        y_in_classes = jnp.in1d(y, classes)
        y_seen = y[y_in_classes]
        indices = jnp.searchsorted(sorted_class, y_seen)
        indptr = jnp.hstack((0, jnp.cumsum(y_in_classes)))

        data = jnp.empty_like(indices)
        data.fill(pos_label)
    ...
    if jnp.any(classes != sorted_class):
        indices = jnp.searchsorted(sorted_class, classes)
        Y = Y[:, indices]
...
def _inverse_binarize_multiclass(y, classes):
    ...
    if sp.issparse(y):
        ...
        outputs = jnp.arange(n_outputs)
        row_max = min_max_axis(y, 1)[1]
        row_nnz = jnp.diff(y.indptr)

        y_data_repeated_max = jnp.repeat(row_max, row_nnz)
        # picks out all indices obtaining the maximum per row
        y_i_all_argmax = jnp.flatnonzero(y_data_repeated_max == y.data)

        # For corner case where last row has a max of 0
        if row_max[-1] == 0:
            y_i_all_argmax = jnp.append(y_i_all_argmax, [len(y.data)])

        # Gets the index of the first argmax in each row from y_i_all_argmax
        index_first_argmax = jnp.searchsorted(y_i_all_argmax, y.indptr[:-1])
        # first argmax of each row
        y_ind_ext = jnp.append(y.indices, [0])
        y_i_argmax = y_ind_ext[y_i_all_argmax[index_first_argmax]]
        # Handle rows of all 0
        y_i_argmax[jnp.where(row_nnz == 0)[0]] = 0

        # Handles rows with max of 0 that contain negative numbers
        samples = jnp.arange(n_samples)[(row_nnz > 0) & (row_max.ravel() == 0)]
        for i in samples:
            ind = y.indices[y.indptr[i]: y.indptr[i + 1]]
            y_i_argmax[i] = classes[jnp.setdiff1d(outputs, ind)][0]
...
def _inverse_binarize_thresholding(y, output_type, classes, threshold):
    ...
    if sp.issparse(y):
        if threshold > 0:
            if y.format not in ("csr", "csc"):
                y = y.tocsr()
            y.data = jnp.array(y.data > threshold, dtype=int)
            y.eliminate_zeros()
        else:
            y = jnp.array(y.toarray() > threshold, dtype=int)
    else:
    ...
    if output_type == "binary":
        ...
        if y.ndim == 2 and y.shape[1] == 2:
            ...
            if len(classes) == 1:
                return jnp.repeat(classes[0], len(y))
            else:
...
class MultiLabelBinarizer(TransformerMixin, BaseEstimator):
    ...
    def fit(self, y):
        ...
        self.classes_ = jnp.empty(len(classes), dtype=dtype)
    ...
    def fit_transform(self, y):
        ...
        class_mapping = jnp.empty(len(tmp), dtype=dtype)
        class_mapping[:] = tmp
        self.classes_, inverse = jnp.unique(class_mapping, return_inverse=True)
        # ensure yt.indices keeps its current dtype
        yt.indices = jnp.array(
    ...
    def _transform(self, y, class_mapping):
        ...
        data = jnp.ones(len(indices), dtype=int)
    ...
    def inverse_transform(self, yt):
        ...
        if sp.issparse(yt):
            yt = yt.tocsr()
            if len(yt.data) != 0 and len(jnp.setdiff1d(yt.data, [0, 1])) > 0:
            ...
            unexpected = jnp.setdiff1d(yt, [0, 1])
            if len(unexpected) > 0:
                raise ValueError(
