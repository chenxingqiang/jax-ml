...
def test_np_log():
    ...
    assert_array_equal(
        FunctionTransformer(jnp.log1p).transform(X),
        jnp.log1p(X),
    )


def test_kw_arg():
    X = jnp.linspace(0, 1, num=10).reshape((5, 2))

    F = FunctionTransformer(jnp.around, kw_args=dict(decimals=3))

    # Test that rounding is correct
    assert_array_equal(F.transform(X), jnp.around(X, decimals=3))
...
def test_kw_arg_update():
    ...
    # Test that rounding is correct
    assert_array_equal(F.transform(X), jnp.around(X, decimals=1))
...
def test_inverse_transform():
    X = jnp.array([1, 4, 9, 16]).reshape((2, 2))

    # Test that inverse_transform works correctly
    F = FunctionTransformer(
        func=jnp.sqrt,
        inverse_func=jnp.around,
        inv_kw_args=dict(decimals=3),
    )
    assert_array_equal(
        F.inverse_transform(F.transform(X)),
        jnp.around(jnp.sqrt(X), decimals=3),
    )


def test_check_inverse():
    X_dense = jnp.array([1, 4, 9, 16], dtype=jnp.float64).reshape((2, 2))
    ...
    # provided.
    trans = FunctionTransformer(
        func=jnp.expm1, inverse_func=None, check_inverse=True, validate=True
    )
    with warnings.catch_warnings():
        warnings.simplefilter("error", UserWarning)
        trans.fit(X_dense)
    trans = FunctionTransformer(
        func=None, inverse_func=jnp.expm1, check_inverse=True, validate=True
    )
    with warnings.catch_warnings():
        warnings.simplefilter("error", UserWarning)
        trans.fit(X_dense)
...
def test_function_transformer_validate_inverse():
    ...
    def add_constant_feature(X):
        X_one = jnp.ones((X.shape[0], 1))
        return jnp.concatenate((X, X_one), axis=1)

    def inverse_add_constant(X):
        return X[:, :-1]

    X = jnp.array([[1, 2], [3, 4], [3, 4]])
    trans = FunctionTransformer(
...
def test_set_output_func():
    ...
    ft = FunctionTransformer(jnp.log, feature_names_out="one-to-one")
