...
class _BaseEncoder(TransformerMixin, BaseEstimator):
    ...
    def _check_X(self, X, force_all_finite=True):
        ...
        if not (hasattr(X, "iloc") and getattr(X, "ndim", 0) == 2):
            ...
            if not hasattr(X, "dtype") and jnp.issubdtype(X_temp.dtype, jnp.str_):
    ...
    def _fit(
        ...
        for i in range(n_features):
            ...
            if self.categories == "auto":
                ...
                if jnp.issubdtype(Xi.dtype, jnp.str_):
                ...
                cats = jnp.array(self.categories[i], dtype=Xi_dtype)
                if (
                    cats.dtype == object
                ...
                if Xi.dtype.kind not in "OUS":
                    sorted_cats = jnp.sort(cats)
                    error_msg = (
                        "Unsorted categories are not supported for numerical categories"
                    )
                    # if there are nans, nan should be the last element
                    stop_idx = -1 if jnp.isnan(sorted_cats[-1]) else None
                    if jnp.any(sorted_cats[:stop_idx] != cats[:stop_idx]) or (
                        jnp.isnan(sorted_cats[-1]
                                 ) and not jnp.isnan(sorted_cats[-1])
                    ):
    ...
    def _transform(
        ...
        X_int = jnp.zeros((n_samples, n_features), dtype=int)
        X_mask = jnp.ones((n_samples, n_features), dtype=bool)
    ...
    def _identify_infrequent(self, category_count, n_samples, col_idx):
        ...
        if self.max_categories is not None and self.max_categories < n_current_features:
            ...
            if frequent_category_count == 0:
                ...
                smallest_levels = jnp.argsort(category_count, kind="mergesort")[
                    :-frequent_category_count
                ]
                infrequent_mask[smallest_levels] = True

        output = jnp.flatnonzero(infrequent_mask)
    ...
    def _fit_infrequent_category_mapping(
            ...
            for feature_idx, count in enumerate(category_counts):
                if feature_idx in missing_indices:
                    category_counts_.append(
                        jnp.delete(count, missing_indices[feature_idx])
                    )
                else:
        ...
        for feature_idx, infreq_idx in enumerate(self._infrequent_indices):
            ...
            mapping = jnp.empty(n_cats, dtype=jnp.int64)
            ...
            frequent_indices = jnp.setdiff1d(jnp.arange(n_cats), infreq_idx)
            mapping[frequent_indices] = jnp.arange(n_frequent_cats)
    ...
    def _map_infrequent_categories(self, X_int, X_mask, ignore_category_indices):
        ...
        for i, mapping in enumerate(self._default_to_infrequent_mappings):
            ...
            X_int[rows_to_update, i] = jnp.take(
...
class OneHotEncoder(_BaseEncoder):
    ...
    This encoding is needed for feeding categorical data to many jax-learn
    ...
    def _set_drop_idx(self):
        ...
        if self.drop is None:
            ...
            if self.drop == "first":
                drop_idx_after_grouping = jnp.zeros(
                ...
                drop_idx_after_grouping = jnp.array(
                    [
            ...
            drop_array = jnp.asarray(self.drop, dtype=object)
            ...
            for feature_idx, (drop_val, cat_list) in enumerate(
                ...
                if not is_scalar_nan(drop_val):
                    drop_idx = jnp.where(cat_list == drop_val)[0]
            ...
            if any(missing_drops):
                ...
                raise ValueError(msg)
            drop_idx_after_grouping = jnp.array(drop_indices, dtype=object)
        ...
        if not self._infrequent_enabled or drop_idx_after_grouping is None:
            ...
            for feature_idx, drop_idx in enumerate(drop_idx_after_grouping):
                ...
                if drop_idx is None or default_to_infrequent is None:
                    ...
                    orig_drop_idx = jnp.flatnonzero(
                        default_to_infrequent == drop_idx)[0]

                drop_idx_.append(orig_drop_idx)

            self.drop_idx_ = jnp.asarray(drop_idx_, dtype=object)
    ...
    def _remove_dropped_categories(self, categories, i):
        ...
        if (
            ...
            return jnp.delete(categories, self._drop_idx_after_grouping[i])
        return categories
    ...
    def transform(self, X):
        ...
        mask = X_mask.ravel()
        feature_indices = jnp.cumsum([0] + self._n_features_outs)
        indices = (X_int + feature_indices[:-1]).ravel()[mask]

        indptr = jnp.empty(n_samples + 1, dtype=int)
        indptr[0] = 0
        jnp.sum(X_mask, axis=1, out=indptr[1:], dtype=indptr.dtype)
        jnp.cumsum(indptr[1:], out=indptr[1:])
        data = jnp.ones(indptr[-1])
    ...
    def inverse_transform(self, X):
        ...
        n_features_out = jnp.sum(self._n_features_outs)
        ...
        dt = jnp.result_type(*[cat.dtype for cat in transformed_features])
        X_tr = jnp.empty((n_samples, n_features), dtype=dt)
        ...
        for i in range(n_features):
            ...
            labels = jnp.asarray(sub.argmax(axis=1)).flatten()
            ...
            if self.handle_unknown == "ignore" or (
                ...
                unknown = jnp.asarray(sub.sum(axis=1) == 0).flatten()
                ...
                dropped = jnp.asarray(sub.sum(axis=1) == 0).flatten()
                if dropped.any():
                    if self._drop_idx_after_grouping is None:
                        all_zero_samples = jnp.flatnonzero(dropped)
                        raise ValueError(
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        return jnp.array(feature_names, dtype=object)
...
class OrdinalEncoder(OneToOneFeatureMixin, _BaseEncoder):
    ...
    dtype : number type, default=jnp.float64
        Desired dtype of output.
    ...
    unknown_value : int or jnp.nan, default=None
        When the parameter handle_unknown is set to 'use_encoded_value', this
        parameter is required and will set the encoded value of unknown
        categories. It has to be distinct from the values used to encode any of
        the categories in `fit`. If set to jnp.nan, the `dtype` parameter must
        be a float dtype.

        .. versionadded:: 0.24

    encoded_missing_value : int or jnp.nan, default=jnp.nan
        Encoded value of missing categories. If set to `jnp.nan`, then the `dtype`
    ...
    >>> import jax.numpy as jnp
    >>> X = [['Male', 1], ['Female', 3], ['Female', jnp.nan]]
    >>> enc.fit_transform(X)
    array([[ 1.,  0.],
           [ 0.,  1.],
           [ 0., nan]])
    ...
    _parameter_constraints: dict = {
        "categories": [StrOptions({"auto"}), list],
        "dtype": "no_validation",  # validation delegated to numpy
        "encoded_missing_value": [Integral, type(jnp.nan)],
    ...
    def fit(self, X, y=None):
        ...
        if self.handle_unknown == "use_encoded_value":
            if is_scalar_nan(self.unknown_value):
                if jnp.dtype(self.dtype).kind != "f":
                    raise ValueError(
                        "When unknown_value is jnp.nan, the dtype "
                        "parameter should be "
        ...
        if self._missing_indices:
            if jnp.dtype(self.dtype).kind != "f" and is_scalar_nan(
    ...
    def inverse_transform(self, X):
        ...
        # create resulting array of appropriate dtype
        dt = jnp.result_type(*[cat.dtype for cat in self.categories_])
        X_tr = jnp.empty((n_samples, n_features), dtype=dt)
        ...
        for i in range(n_features):
            ...
            if infrequent_indices is not None and infrequent_indices[i] is not None:
                ...
                frequent_categories_mask = jnp.ones_like(categories, dtype=bool)
            ...
            if self.handle_unknown == "use_encoded_value":
                ...
                if isinstance(rows_to_update, jnp.ndarray):
