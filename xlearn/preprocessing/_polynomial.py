...
def _create_expansion(X, interaction_only, deg, n_features, cumulative_size=0):
    ...
    index_dtype = jnp.int64 if needs_int64 else jnp.int32
    ...
    expanded_data = jnp.empty(shape=total_nnz, dtype=X.data.dtype)
    expanded_indices = jnp.empty(shape=total_nnz, dtype=index_dtype)
    expanded_indptr = jnp.empty(shape=X.indptr.shape[0], dtype=index_dtype)
    _csr_polynomial_expansion(
        X.data,
        X.indices,
        X.indptr,
...
class PolynomialFeatures(TransformerMixin, BaseEstimator):
    ...
    def powers_(self):
        ...
        return jnp.vstack(
            [jnp.bincount(c, minlength=self.n_features_in_)
    ...
    def fit(self, X, y=None):
        ...
        if self.n_output_features_ > jnp.iinfo(jnp.intp).max:
            msg = (
                ...
                f" indexed by {jnp.intp().dtype.name}. Please change some or all of the"
            ...
            if (
                jnp.intp == jnp.int32
                and self.n_output_features_ <= jnp.iinfo(jnp.int64).max
            ):  # pragma: nocover
                msg += (
    ...
    def transform(self, X):
        ...
        if sparse.isspmatrix_csr(X):
            ...
            if self.include_bias:
                to_stack.append(
                    sparse.csr_matrix(
                        jnp.ones(shape=(n_samples, 1), dtype=X.dtype))
                )
            ...
            for combi in combinations:
                if combi:
                    ...
                    bias = sparse.csc_matrix(jnp.ones((X.shape[0], 1)))
                    columns.append(bias)
...
class SplineTransformer(TransformerMixin, BaseEstimator):
    ...
    def _get_base_knot_positions(X, n_knots=10, knots="uniform", sample_weight=None):
        ...
        Returns
        -------
        knots : ndarray of shape (n_knots, n_features), dtype=jnp.float64
            Knot positions (points) of base interval.
        """
        if knots == "quantile":
            percentiles = 100 * jnp.linspace(
                start=0, stop=1, num=n_knots, dtype=jnp.float64
            )

            if sample_weight is None:
                knots = jnp.percentile(X, percentiles, axis=0)
            else:
                knots = jnp.array(
                    [
            ...
            x_min = jnp.amin(X[mask], axis=0)
            x_max = jnp.amax(X[mask], axis=0)
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if isinstance(self.knots, str):
            ...
            base_knots = check_array(self.knots, dtype=jnp.float64)
            if base_knots.shape[0] < 2:
                raise ValueError(
                    "Number of knots, knots.shape[0], must be >= 2.")
            elif base_knots.shape[1] != n_features:
                raise ValueError("knots.shape[1] == n_features is violated.")
            elif not jnp.all(jnp.diff(base_knots, axis=0) > 0):
        ...
        if self.extrapolation == "periodic":
            ...
            # knots = jnp.r_[
            #     jnp.tile(base_knots.min(axis=0), reps=[degree, 1]),
            #     base_knots,
            #     jnp.tile(base_knots.max(axis=0), reps=[degree, 1])
            # ]
        ...
        coef = jnp.eye(n_splines, dtype=jnp.float64)
        if self.extrapolation == "periodic":
            coef = jnp.concatenate((coef, coef[:degree, :]))
    ...
    def transform(self, X):
        ...
        if use_sparse:
            ...
            XBS = jnp.zeros((n_samples, n_out), dtype=dtype, order=self.order)
        ...
        for i in range(n_features):
            ...
            if self.extrapolation == "error":
                ...
                # outputs jnp.nan.
                if (use_sparse and jnp.any(jnp.isnan(XBS_sparse.data))) or (
                    not use_sparse
                    and jnp.any(
                        jnp.isnan(
        ...
        if use_sparse:
            ...
            for mat in output_list:
                all_int32 &= mat.indices.dtype == jnp.int32
            if (
                sp_version < parse_version("1.9.2")
