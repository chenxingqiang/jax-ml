...
class KBinsDiscretizer(TransformerMixin, BaseEstimator):
    ...
    dtype : {jnp.float32, jnp.float64}, default=None
        The desired data-type for the output. If None, output dtype is
        consistent with input dtype. Only jnp.float32 and jnp.float64 are
        supported.
    ...
    subsample : int or None, default='warn'
        ...
        efficien. Defaults to 200_000 when `strategy='quantile'` and to `None`
    ...
    n_bins_ : ndarray of shape (n_features,), dtype=jnp.int_
    ...
    ``inverse_transform``. During transform, bin edges are extended to::
      ...
      jnp.concatenate([-jnp.inf, bin_edges_[i][1:-1], jnp.inf])
    ...
    _parameter_constraints: dict = {
        ...
        "dtype": [Options(type, {jnp.float64, jnp.float32}), None],
        "subsample": [
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        bin_edges = jnp.zeros(n_features, dtype=object)
        for jj in range(n_features):
            ...
            if self.strategy == "uniform":
                bin_edges[jj] = jnp.linspace(col_min, col_max, n_bins[jj] + 1)

            elif self.strategy == "quantile":
                quantiles = jnp.linspace(0, 100, n_bins[jj] + 1)
                if sample_weight is None:
                    bin_edges[jj] = jnp.asarray(
                        jnp.percentile(column, quantiles))
                else:
                    bin_edges[jj] = jnp.asarray(
                        [
                ...
                uniform_edges = jnp.linspace(col_min, col_max, n_bins[jj] + 1)
                ...
                bin_edges[jj] = jnp.r_[col_min, bin_edges[jj], col_max]
        ...
        if "onehot" in self.encode:
            self._encoder = OneHotEncoder(
                categories=[jnp.arange(i) for i in self.n_bins_],
                sparse_output=self.encode == "onehot",
                dtype=output_dtype,
            )
            # Fit the OneHotEncoder with toy datasets
            # so that it's ready for use after the KBinsDiscretizer is fitted
            self._encoder.fit(jnp.zeros((1, len(self.n_bins_))))
    ...
    def _validate_n_bins(self, n_features):
        ...
        if isinstance(orig_bins, Integral):
            return jnp.full(n_features, orig_bins, dtype=int)
        ...
        violating_indices = jnp.where(bad_nbins_value)[0]
    ...
    def transform(self, X):
        ...
        Returns
        -------
        Xt : {ndarray, sparse matrix}, dtype={jnp.float32, jnp.float64}
    ...
    def inverse_transform(self, Xt):
        ...
        Returns
        -------
        Xinv : ndarray, dtype={jnp.float32, jnp.float64}
        ...
        Xinv = check_array(Xt, copy=True, dtype=(jnp.float64, jnp.float32))
        ...
        for jj in range(n_features):
            ...
            Xinv[:, jj] = bin_centers[jnp.int_(Xinv[:, jj])]
