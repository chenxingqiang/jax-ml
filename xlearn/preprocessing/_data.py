...
def _is_constant_feature(var, mean, n_samples):
    ...
    """
    # In jax-learn, variance is always computed using float64 accumulators.
...
def _handle_zeros_in_scale(scale, copy=True, constant_mask=None):
    ...
    if jnp.isscalar(scale):
        if scale == 0.0:
            scale = 1.0
        return scale
    elif isinstance(scale, jnp.ndarray):
...
def scale(X, *, axis=0, with_mean=True, with_std=True, copy=True):
    ...
    if sparse.issparse(X):
        ...
        if with_std:
            ...
            inplace_column_scale(X, 1 / jnp.sqrt(var))
    else:
        X = jnp.asarray(X)
        if with_mean:
            mean_ = jnp.nanmean(X, axis)
        if with_std:
            scale_ = jnp.nanstd(X, axis)
        # Xr is a view on the original array that enables easy use of
        # broadcasting on the axis in which we are interested in
        Xr = jnp.rollaxis(X, axis)
        if with_mean:
            Xr -= mean_
            mean_1 = jnp.nanmean(Xr, axis=0)
            # Verify that mean_1 is 'close to zero'. If X contains very
            # large values, mean_1 can also be very large, due to a lack of
            # precision of mean_. In this case, a pre-scaling of the
            # concerned feature is efficient, for instance by its mean or
            # maximum.
            if not jnp.allclose(mean_1, 0):
                warnings.warn(
        ...
        if with_std:
            ...
            Xr /= scale_
            if with_mean:
                mean_2 = jnp.nanmean(Xr, axis=0)
                # If mean_2 is not 'close to zero', it comes from the fact that
                # scale_ is very small so that mean_2 = mean_1/scale_ > 0, even
                # if mean_1 was close to zero. The problem is thus essentially
                # due to the lack of precision of mean_. A solution is then to
                # subtract the mean again:
                if not jnp.allclose(mean_2, 0):
                    warnings.warn(
                        "Numerical issues were encountered "
...
class MinMaxScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        ...
        X += self.min_
        if self.clip:
            jnp.clip(X, self.feature_range[0], self.feature_range[1], out=X)
        return X
...
class StandardScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    scale_ : ndarray of shape (n_features,) or None
        ...
        variance. Generally this is calculated using `jnp.sqrt(var_)`. If a
    ...
    def partial_fit(self, X, y=None, sample_weight=None):
        ...
        dtype = jnp.int64 if sample_weight is None else X.dtype
        if not hasattr(self, "n_samples_seen_"):
            self.n_samples_seen_ = jnp.zeros(n_features, dtype=dtype)
        elif jnp.size(self.n_samples_seen_) == 1:
            self.n_samples_seen_ = jnp.repeat(self.n_samples_seen_, X.shape[1])
        ...
        if sparse.issparse(X):
            ...
            if self.with_std:
                ...
                # See https://github.com/jax-learn/jax-learn/pull/12338
                self.mean_ = self.mean_.astype(jnp.float64, copy=False)
                self.var_ = self.var_.astype(jnp.float64, copy=False)
            else:
                self.mean_ = None  # as with_mean must be False for sparse
                self.var_ = None
                weights = _check_sample_weight(sample_weight, X)
                sum_weights_nan = weights @ sparse_constructor(
                    (jnp.isnan(X.data), X.indices, X.indptr), shape=X.shape
                )
                self.n_samples_seen_ += (jnp.sum(weights) - sum_weights_nan).astype(
                    dtype
                )
        else:
            # First pass
            ...
            if not self.with_mean and not self.with_std:
                self.mean_ = None
                self.var_ = None
                self.n_samples_seen_ += X.shape[0] - jnp.isnan(X).sum(axis=0)
        ...
        # missing values)
        if jnp.ptp(self.n_samples_seen_) == 0:
        ...
        if self.with_std:
            ...
            self.scale_ = _handle_zeros_in_scale(
                jnp.sqrt(self.var_), copy=False, constant_mask=constant_mask
            )
        else:
            self.scale_ = None
    ...
    def _more_tags(self):
        return {"allow_nan": True, "preserves_dtype": [jnp.float64, jnp.float32]}
...
class MaxAbsScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    def partial_fit(self, X, y=None):
        ...
        if sparse.issparse(X):
            ...
            max_abs = jnp.maximum(jnp.abs(mins), jnp.abs(maxs))
        else:
        ...
        if self.with_scaling:
            ...
            for feature_idx in range(X.shape[1]):
                if sparse.issparse(X):
                    ...
                    column_data = jnp.zeros(shape=X.shape[0], dtype=X.dtype)
                    column_data[: len(column_nnz_data)] = column_nnz_data
                else:
                    column_data = X[:, feature_idx]

                quantiles.append(jnp.nanpercentile(
                    column_data, self.quantile_range))

            quantiles = jnp.transpose(quantiles)
...
def normalize(X, norm="l2", *, axis=1, copy=True, return_norm=False):
    ...
    if sparse.issparse(X):
        ...
        if norm == "l1":
            ...
            norms = jnp.maximum(abs(mins), maxes)
            norms_elementwise = norms.repeat(jnp.diff(X.indptr))
...
class Normalizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        (ii) be consistent with the jax-learn transformer API.
...
class KernelCenterer(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def fit(self, K, y=None):
        ...
        n_samples = K.shape[0]
        self.K_fit_rows_ = jnp.sum(K, axis=0) / n_samples
    ...
    def transform(self, K, copy=True):
        ...
        K_pred_cols = (jnp.sum(K, axis=1) /
                       self.K_fit_rows_.shape[0])[:, jnp.newaxis]
...
def add_dummy_feature(X, value=1.0):
    ...
    if sparse.issparse(X):
        if sparse.isspmatrix_coo(X):
            ...
            col = jnp.concatenate((jnp.zeros(n_samples), col))
            # Row indices of dummy feature are 0, ..., n_samples-1.
            row = jnp.concatenate((jnp.arange(n_samples), X.row))
            # Prepend the dummy feature n_samples times.
            data = jnp.concatenate((jnp.full(n_samples, value), X.data))
            ...
            # Row indices of dummy feature are 0, ..., n_samples-1.
            indices = jnp.concatenate((jnp.arange(n_samples), X.indices))
            # Prepend the dummy feature n_samples times.
            data = jnp.concatenate((jnp.full(n_samples, value), X.data))
        ...
        return jnp.hstack((jnp.full((n_samples, 1), value), X))
...
class QuantileTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    def _dense_fit(self, X, random_state):
        ...
        for col in X.T:
            ...
            self.quantiles_.append(jnp.nanpercentile(col, references))
        self.quantiles_ = jnp.transpose(self.quantiles_)
        # Due to floating-point precision error in `jnp.nanpercentile`,
        # make sure that quantiles are monotonically increasing.
        # Upstream issue in numpy:
        # https://github.com/numpy/numpy/issues/14685
        self.quantiles_ = jnp.maximum.accumulate(self.quantiles_)
    ...
    def _sparse_fit(self, X, random_state):
        ...
        for feature_idx in range(n_features):
            ...
            if len(column_nnz_data) > self.subsample:
                ...
                if self.ignore_implicit_zeros:
                    column_data = jnp.zeros(
                        shape=column_subsample, dtype=X.dtype)
                else:
                    column_data = jnp.zeros(shape=self.subsample, dtype=X.dtype)
                ...
                if self.ignore_implicit_zeros:
                    ...
                    column_data = jnp.zeros(shape=n_samples, dtype=X.dtype)
                column_data[: len(column_nnz_data)] = column_nnz_data
            ...
            if not column_data.size:
                ...
                self.quantiles_.append(
                    jnp.nanpercentile(column_data, references))
        self.quantiles_ = jnp.transpose(self.quantiles_)
        # due to floating-point precision error in `jnp.nanpercentile`,
    ...
    def fit(self, X, y=None):
        ...
        self.references_ = jnp.linspace(0, 1, self.n_quantiles_, endpoint=True)
        if sparse.issparse(X):
    ...
    def _transform_col(self, X_col, quantiles, inverse):
        ...
        if not inverse:
            ...
            with jnp.errstate(invalid="ignore"):  # hide NaN comparison warnings
        ...
        if not inverse:
            ...
            X_col[isfinite_mask] = 0.5 * (
                jnp.interp(X_col_finite, quantiles, self.references_)
                - jnp.interp(-X_col_finite, -
                            quantiles[::-1], -self.references_[::-1])
            )
        else:
            X_col[isfinite_mask] = jnp.interp(
        ...
        if not inverse:
            with jnp.errstate(invalid="ignore"):  # hide NaN comparison warnings
                if output_distribution == "normal":
                    ...
                    # consistent
                    clip_min = stats.norm.ppf(BOUNDS_THRESHOLD - jnp.spacing(1))
                    clip_max = stats.norm.ppf(
                        1 - (BOUNDS_THRESHOLD - jnp.spacing(1)))
    ...
    def _check_inputs(self, X, in_fit, accept_sparse_negative=False, copy=False):
        ...
        with jnp.errstate(invalid="ignore"):  # hide NaN comparison warnings
            if (
                ...
                and (sparse.issparse(X) and jnp.any(X.data < 0))
            ):
                raise ValueError(
...
class PowerTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    def _fit(self, X, y=None, force_transform=False):
        ...
        n_samples = X.shape[0]
        mean = jnp.mean(X, axis=0, dtype=jnp.float64)
        var = jnp.var(X, axis=0, dtype=jnp.float64)
        ...
        with jnp.errstate(invalid="ignore"):  # hide NaN warnings
            self.lambdas_ = jnp.empty(X.shape[1], dtype=X.dtype)
    ...
    def _yeo_johnson_inverse_transform(self, x, lmbda):
        ...
        transform with parameter lambda.
        """
        x_inv = jnp.zeros_like(x)
        pos = x >= 0

        # when x >= 0
        if abs(lmbda) < jnp.spacing(1.0):
            x_inv[pos] = jnp.exp(x[pos]) - 1
        else:  # lmbda != 0
            x_inv[pos] = jnp.power(x[pos] * lmbda + 1, 1 / lmbda) - 1

        # when x < 0
        if abs(lmbda - 2) > jnp.spacing(1.0):
            x_inv[~pos] = 1 - \
                jnp.power(-(2 - lmbda) * x[~pos] + 1, 1 / (2 - lmbda))
        else:  # lmbda == 2
            x_inv[~pos] = 1 - jnp.exp(-x[~pos])
    ...
    def _yeo_johnson_transform(self, x, lmbda):
        ...
        # when x >= 0
        if abs(lmbda) < jnp.spacing(1.0):
            out[pos] = jnp.log1p(x[pos])
        else:  # lmbda != 0
            out[pos] = (jnp.power(x[pos] + 1, lmbda) - 1) / lmbda

        # when x < 0
        if abs(lmbda - 2) > jnp.spacing(1.0):
            out[~pos] = -(jnp.power(-x[~pos] + 1, 2 - lmbda) - 1) / (2 - lmbda)
        else:  # lmbda == 2
            out[~pos] = -jnp.log1p(-x[~pos])
    ...
    def _yeo_johnson_optimize(self, x):
        ...
        """
        x_tiny = jnp.finfo(jnp.float64).tiny
        ...
        def _neg_log_likelihood(lmbda):
            ...
            # Reject transformed data that would raise a RuntimeWarning in jnp.log
            if x_trans_var < x_tiny:
                return jnp.inf

            log_var = jnp.log(x_trans_var)
            loglike = -n_samples / 2 * log_var
            loglike += (lmbda - 1) * (jnp.sign(x) * jnp.log1p(jnp.abs(x))).sum()
    ...
    def _check_input(self, X, in_fit, check_positive=False, check_shape=False):
        ...
        with warnings.catch_warnings():
            ...
            if check_positive and self.method == "box-cox" and jnp.nanmin(X) <= 0:
                raise ValueError(
