...
def _fit_encoding_fast(
    ...
    cdef:
        ...
        cnp.int64_t max_n_cats = jnp.max(n_categories)
        double[::1] sums = jnp.empty(max_n_cats, dtype=jnp.float64)
        double[::1] counts = jnp.empty(max_n_cats, dtype=jnp.float64)
        list encodings = []
    ...
    for feat_idx in range(n_features):
        current_encoding = jnp.empty(shape=n_categories[feat_idx], dtype=jnp.float64)
        encoding_vec[feat_idx] = &current_encoding[0]
        encodings.append(jnp.asarray(current_encoding))
...
def _fit_encoding_fast_auto_smooth(
    ...
    cdef:
        ...
        int n_features = X_int.shape[1]
        cnp.int64_t max_n_cats = jnp.max(n_categories)
        double[::1] means = jnp.empty(max_n_cats, dtype=jnp.float64)
        cnp.int64_t[::1] counts = jnp.empty(max_n_cats, dtype=jnp.int64)
        double[::1] sum_of_squared_diffs = jnp.empty(max_n_cats, dtype=jnp.float64)
        double lambda_
        list encodings = []
        double[::1] current_encoding
        # Gives access to encodings without gil
