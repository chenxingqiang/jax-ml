...
class TargetEncoder(OneToOneFeatureMixin, _BaseEncoder):
    ...
    >>> X = jnp.array([["dog"] * 20 + ["cat"] * 30 + ["snake"] * 38], dtype=object).T
    ...
    def fit_transform(self, X, y):
        ...
        X_out = jnp.empty_like(X_ordinal, dtype=jnp.float64)
    ...
    def _fit_encodings_all(self, X, y):
        ...
        self.target_mean_ = jnp.mean(y)

        X_ordinal, X_known_mask = self._transform(
            X, handle_unknown="ignore", force_all_finite="allow-nan"
        )
        n_categories = jnp.fromiter(
