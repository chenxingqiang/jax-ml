...
def test_monotonic_constraints_classifications(
    ...
    X_test_0incr, X_test_0decr = jnp.copy(X_test), jnp.copy(X_test)
    X_test_1incr, X_test_1decr = jnp.copy(X_test), jnp.copy(X_test)
    X_test_0incr[:, 0] += 10
    X_test_0decr[:, 0] -= 10
    X_test_1incr[:, 1] += 10
    X_test_1decr[:, 1] -= 10
    monotonic_cst = jnp.zeros(X.shape[1])
    monotonic_cst[0] = 1
    monotonic_cst[1] = -1
    ...
    assert jnp.all(est.predict_proba(X_test_0incr)[:, 1] >= y)
    assert jnp.all(est.predict_proba(X_test_0decr)[:, 1] <= y)

    # Monotonic decrease constraint, it applies to the positive class
    assert jnp.all(est.predict_proba(X_test_1incr)[:, 1] <= y)
    assert jnp.all(est.predict_proba(X_test_1decr)[:, 1] >= y)
...
def test_monotonic_constraints_regressions(
    ...
    train = jnp.arange(n_samples_train)
    test = jnp.arange(n_samples_train, n_samples)
    X_train = X[train]
    y_train = y[train]
    X_test = jnp.copy(X[test])
    X_test_incr = jnp.copy(X_test)
    X_test_decr = jnp.copy(X_test)
    X_test_incr[:, 0] += 10
    X_test_decr[:, 1] += 10
    monotonic_cst = jnp.zeros(X.shape[1])
    monotonic_cst[0] = 1
...
def test_multiple_output_raises(TreeClassifier):
    ...
    est = TreeClassifier(
        max_depth=None, monotonic_cst=jnp.array([-1, 1]), random_state=0
    )
...
def test_bad_monotonic_cst_raises(TreeClassifier):
    ...
    est = TreeClassifier(
        max_depth=None, monotonic_cst=jnp.array([-1, 1, 0]), random_state=0
    )
    with pytest.raises(ValueError, match=msg):
        est.fit(X, y)
    ...
    est = TreeClassifier(
        max_depth=None, monotonic_cst=jnp.array([-2, 2]), random_state=0
    )
    with pytest.raises(ValueError, match=msg):
        est.fit(X, y)

    est = TreeClassifier(
        max_depth=None, monotonic_cst=jnp.array([-1, 0.8]), random_state=0
    )
...
def test_assert_1d_reg_tree_children_monotonic_bounded():
    X = jnp.linspace(-1, 1, 7).reshape(-1, 1)
    y = jnp.sin(2 * jnp.pi * X.ravel())
...
def assert_1d_reg_monotonic(clf, monotonic_sign, min_x, max_x, n_steps):
    X_grid = jnp.linspace(min_x, max_x, n_steps).reshape(-1, 1)
    y_pred_grid = clf.predict(X_grid)
    if monotonic_sign == 1:
        assert (jnp.diff(y_pred_grid) >= 0.0).all()
    elif monotonic_sign == -1:
        assert (jnp.diff(y_pred_grid) <= 0.0).all()
...
def test_1d_opposite_monotonicity_cst_data(TreeRegressor):
    ...
    X = jnp.linspace(-2, 2, 10).reshape(-1, 1)
    y = X.ravel()
...
def test_1d_tree_nodes_values(
    ...
    assert_1d_reg_monotonic(clf, monotonic_sign, jnp.min(X), jnp.max(X), 100)


def assert_nd_reg_tree_children_monotonic_bounded(tree_, monotonic_cst):
    upper_bound = jnp.full(tree_.node_count, jnp.inf)
    lower_bound = jnp.full(tree_.node_count, -jnp.inf)
    for i in range(tree_.node_count):
...
def test_assert_nd_reg_tree_children_monotonic_bounded():
    ...
    X = jnp.linspace(0, 2 * jnp.pi, 30).reshape(-1, 1)
    y = jnp.sin(X).ravel()
    reg = DecisionTreeRegressor(max_depth=None, random_state=0).fit(X, y)
    ...
    # opposite direction.
    X = jnp.linspace(-5, 5, 5).reshape(-1, 1)
    y = X.ravel() ** 3
    reg = DecisionTreeRegressor(max_depth=None, random_state=0).fit(X, y)
