...
from xlearn import datasets, tree
...
DATASETS = {
    ...
    "zeros": {"X": jnp.zeros((20, 3)), "y": y_random},
}
...
def assert_tree_equal(d, s, message):
    ...
    internal = jnp.logical_not(external)
...
def test_weighted_classification_toy():
    ...
    for name, Tree in CLF_TREES.items():
        ...
        clf.fit(X, y, sample_weight=jnp.ones(len(X)))
        assert_array_equal(clf.predict(T), true_result,
                           "Failed with {0}".format(name))

        clf.fit(X, y, sample_weight=jnp.full(len(X), 0.5))
        assert_array_equal(clf.predict(T), true_result,
                           "Failed with {0}".format(name))
...
def test_regression_toy(Tree, criterion):
    ...
    if criterion == "poisson":
        ...
        # true_result
        a = jnp.abs(jnp.min(y)) + 1
        y_train = jnp.array(y) + a
        y_test = jnp.array(true_result) + a
    else:
        y_train = y
        y_test = true_result
...
def test_xor():
    ...
    gridx, gridy = jnp.indices(y.shape)

    X = jnp.vstack([gridx.ravel(), gridy.ravel()]).T
    y = y.ravel()
...
def test_probability():
    ...
    for name, Tree in CLF_TREES.items():
        ...
        assert_array_almost_equal(
            jnp.sum(prob_predict, 1),
            jnp.ones(iris.data.shape[0]),
            err_msg="Failed with {0}".format(name),
        )
        assert_array_equal(
            jnp.argmax(prob_predict, 1),
            clf.predict(iris.data),
            err_msg="Failed with {0}".format(name),
        )
        assert_almost_equal(
            clf.predict_proba(iris.data),
            jnp.exp(clf.predict_log_proba(iris.data)),
            8,
            err_msg="Failed with {0}".format(name),
        )
...
def test_arrayrepr():
    ...
    # Check resize
    X = jnp.arange(10000)[:, jnp.newaxis]
    y = jnp.arange(10000)
...
def test_numerical_stability():
    ...
    y = jnp.array([1.0, 0.70209277, 0.53896582, 0.0,
...
def test_importances():
    ...
    for name, Tree in CLF_TREES.items():
        ...
        n_important = jnp.sum(importances > 0.1)
...
def test_max_features():
    ...
    for name, TreeEstimator in ALL_TREES.items():
        ...
        assert est.max_features_ == int(jnp.sqrt(iris.data.shape[1]))

        est = TreeEstimator(max_features="log2")
        est.fit(iris.data, iris.target)
        assert est.max_features_ == int(jnp.log2(iris.data.shape[1]))
...
def test_min_samples_split():
    ...
    X = jnp.asfortranarray(iris.data, dtype=tree._tree.DTYPE)
    y = iris.target
    ...
    for max_leaf_nodes, name in product((None, 1000), ALL_TREES.keys()):
        ...
        assert jnp.min(node_samples) > 9, "Failed with {0}".format(name)
...
def test_min_samples_leaf():
    ...
    for max_leaf_nodes, name in product((None, 1000), ALL_TREES.keys()):
        ...
        node_counts = jnp.bincount(out)
        # drop inner nodes
        leaf_count = node_counts[node_counts != 0]
        assert jnp.min(leaf_count) > 4, "Failed with {0}".format(name)
...
def check_min_weight_fraction_leaf(name, datasets, sparse=False):
    ...
    training set"""
    if sparse:
        X = DATASETS[datasets]["X_sparse"].astype(jnp.float32)
    else:
        X = DATASETS[datasets]["X"].astype(jnp.float32)
    ...
    # test both DepthFirstTreeBuilder and BestFirstTreeBuilder
    # by setting max_leaf_nodes
    for max_leaf_nodes, frac in product((None, 1000), jnp.linspace(0, 0.5, 6)):
        est = TreeEstimator(
        ...
        node_weights = jnp.bincount(out, weights=weights)
        # drop inner nodes
        leaf_weights = node_weights[node_weights != 0]
        assert (
            jnp.min(leaf_weights) >= total_weight * est.min_weight_fraction_leaf
...
def check_min_weight_fraction_leaf_with_min_samples_leaf(name, datasets, sparse=False):
    ...
    total_weight = X.shape[0]
    TreeEstimator = ALL_TREES[name]
    for max_leaf_nodes, frac in product((None, 1000), jnp.linspace(0, 0.5, 3)):
        ...
        # drop inner nodes
        leaf_weights = node_weights[node_weights != 0]
        assert jnp.min(leaf_weights) >= max(
...
def test_classes_shape():
    ...
    for name, TreeClassifier in CLF_TREES.items():
        ...
        _y = jnp.vstack((y, jnp.array(y) * 2)).T
        clf = TreeClassifier(random_state=0)
        clf.fit(X, _y)
...
def test_memory_layout():
    ...
    for (name, TreeEstimator), dtype in product(
        ALL_TREES.items(), [jnp.float64, jnp.float32]
    ):
        est = TreeEstimator(random_state=0)

        # Nothing
        X = jnp.asarray(iris.data, dtype=dtype)
        y = iris.target
        assert_array_equal(est.fit(X, y).predict(X), y)

        # C-order
        X = jnp.asarray(iris.data, order="C", dtype=dtype)
        y = iris.target
        assert_array_equal(est.fit(X, y).predict(X), y)

        # F-order
        X = jnp.asarray(iris.data, order="F", dtype=dtype)
        y = iris.target
        assert_array_equal(est.fit(X, y).predict(X), y)

        # Contiguous
        X = jnp.ascontiguousarray(iris.data, dtype=dtype)
        y = iris.target
        assert_array_equal(est.fit(X, y).predict(X), y)
        ...
        # Strided
        X = jnp.asarray(iris.data[::3], dtype=dtype)
        y = iris.target[::3]
        assert_array_equal(est.fit(X, y).predict(X), y)
...
def test_sample_weight():
    ...
    X = jnp.arange(100)[:, jnp.newaxis]
    y = jnp.ones(100)
    y[:50] = 0.0

    sample_weight = jnp.ones(100)
    sample_weight[y == 0] = 0.0

    clf = DecisionTreeClassifier(random_state=0)
    clf.fit(X, y, sample_weight=sample_weight)
    assert_array_equal(clf.predict(X), jnp.ones(100))

    # Test that low weighted samples are not taken into account at low depth
    X = jnp.arange(200)[:, jnp.newaxis]
    y = jnp.zeros(200)
    y[50:100] = 1
    y[100:200] = 2
    X[100:200, 0] = 200

    sample_weight = jnp.ones(200)
    ...
    sample_weight = jnp.bincount(duplicates, minlength=X.shape[0])
...
def check_class_weights(name):
    ...
    iris_multi = jnp.vstack((iris.target, iris.target, iris.target)).T
    ...
    sample_weight = jnp.ones(iris.target.shape)
...
def test_behaviour_constant_feature_after_splits():
    X = jnp.transpose(
        jnp.vstack(([[0, 0, 0, 0, 0, 1, 2, 4, 5, 6, 7]], jnp.zeros((4, 11))))
    )
...
def test_with_only_one_non_constant_features():
    X = jnp.hstack(
        [jnp.array([[1.0], [1.0], [0.0], [0.0]]), jnp.zeros((4, 1000))])
    ...
    for name, TreeEstimator in CLF_TREES.items():
        ...
        assert_array_equal(est.predict_proba(X), jnp.full((4, 2), 0.5))

    for name, TreeEstimator in REG_TREES.items():
        est = TreeEstimator(random_state=0, max_features=1)
        est.fit(X, y)
        assert est.tree_.max_depth == 1
        assert_array_equal(est.predict(X), jnp.full((4,), 0.5))


def test_big_input():
    # Test if the warning for too large inputs is appropriate.
    X = jnp.repeat(10**40.0, 4).astype(jnp.float64).reshape(-1, 1)
    clf = DecisionTreeClassifier()
...
def check_sparse_input(tree, dataset, max_depth=None):
    ...
    for sparse_format in (csr_matrix, csc_matrix, coo_matrix):
        ...
        for sparse_matrix in (csr_matrix, csc_matrix, coo_matrix):
            X_sparse_test = sparse_matrix(X_sparse, dtype=jnp.float32)
...
def check_explicit_sparse_zeros(tree, max_depth=3, n_features=10):
    ...
    data = jnp.array(jnp.concatenate(data), dtype=jnp.float32)
...
def check_decision_path(name):
    ...
    assert_array_almost_equal(leave_indicator, jnp.ones(shape=n_samples))

    # Ensure only one leave node per sample
    all_leaves = est.tree_.children_left == TREE_LEAF
    assert_array_almost_equal(
        jnp.dot(node_indicator, all_leaves), jnp.ones(shape=n_samples)
    )
...
def test_mae():
    ...
    dt_mae.fit(X=[[3], [5], [3], [8], [5]], y=[
               6, 7, 3, 4, 3], sample_weight=jnp.ones(5))
...
def test_criterion_copy():
    ...
    n_outputs = 3
    n_classes = jnp.arange(3, dtype=jnp.intp)
    n_samples = 100
...
def test_empty_leaf_infinite_threshold():
    ...
    for X in [X_full, X_sparse]:
        ...
        left_leaf = set(jnp.where(tree.tree_.children_left == TREE_LEAF)[0])
        empty_leaf = left_leaf.difference(terminal_regions)
        infinite_threshold = jnp.where(~jnp.isfinite(tree.tree_.threshold))[0]
...
def test_prune_tree_classifier_are_subtrees(criterion, dataset, tree_cls):
    ...
    assert jnp.all(jnp.diff(pruning_path) >= 0)
    assert jnp.all(jnp.diff(impurities) >= 0)
...
def test_apply_path_readonly_all_trees(name, splitter, X_format):
    ...
    if X_format == "dense":
        ...
        X_readonly.data = jnp.array(X_readonly.data, dtype=tree._tree.DTYPE)
        (
            X_readonly.data,
            X_readonly.indices,
            X_readonly.indptr,
    ...
    y_readonly = create_memmap_backed_data(
        jnp.array(y_small, dtype=tree._tree.DTYPE))
...
def test_balance_property(criterion, Tree):
    ...
    reg.fit(X, y)
    assert jnp.sum(reg.predict(X)) == pytest.approx(jnp.sum(y))
...
def test_poisson_vs_mse():
    ...
    # exponentiated.
    coef = rng.uniform(low=-2, high=2, size=n_features) / jnp.max(X, axis=0)
    y = rng.poisson(lam=jnp.exp(X @ coef))
    ...
    for X, y, val in [(X_train, y_train, "train"), (X_test, y_test, "test")]:
        ...
        metric_mse = mean_poisson_deviance(
            y, jnp.clip(tree_mse.predict(X), 1e-15, None))
...
def test_decision_tree_regressor_sample_weight_consisten(criterion):
    ...
    y = jnp.mean(X, axis=1) + rng.rand(n_samples)
    # make it positive in order to work also for poisson criterion
    y += jnp.min(y) + 0.1

    # check that multiplying sample_weight by 2 is equivalent
    # to repeating corresponding samples twice
    X2 = jnp.concatenate([X, X[: n_samples // 2]], axis=0)
    y2 = jnp.concatenate([y, y[: n_samples // 2]])
    sample_weight_1 = jnp.ones(len(y))
...
def test_different_endianness_pickle():
    ...
    def get_pickle_non_native_endianness():
        ...
        p.dispatch_table[jnp.ndarray] = reduce_ndarray
    ...
    assert jnp.isclose(score, new_score)
...
def get_different_bitness_node_ndarray(node_ndarray):
    new_dtype_for_indexing_fields = jnp.int64 if _IS_32BIT else jnp.int32
...
def reduce_tree_with_different_bitness(tree):
    new_dtype = jnp.int64 if _IS_32BIT else jnp.int32
...
def test_check_n_classes():
    expected_dtype = jnp.dtype(jnp.int32) if _IS_32BIT else jnp.dtype(jnp.int64)
    allowed_dtypes = [jnp.dtype(jnp.int32), jnp.dtype(jnp.int64)]
    allowed_dtypes += [dt.newbyteorder() for dt in allowed_dtypes]

    n_classes = jnp.array([0, 1], dtype=expected_dtype)
...
def test_check_value_ndarray():
    ...
    value_ndarray = jnp.zeros(expected_shape, dtype=expected_dtype)
    ...
    for problematic_arr in [value_ndarray[:, :, :1], jnp.asfortranarray(value_ndarray)]:
    ...
    with pytest.raises(ValueError, match="value array.+incompatible dtype"):
        _check_value_ndarray(
            value_ndarray.astype(jnp.float32),
            expected_dtype=expected_dtype,
...
def test_check_node_ndarray():
    ...
    node_ndarray = jnp.zeros((5,), dtype=expected_dtype)
    ...
    with pytest.raises(ValueError, match="Wrong dimensions.+node array"):
        problematic_node_ndarray = jnp.zeros((5, 2), dtype=expected_dtype)
    ...
    new_dtype_dict["threshold"] = jnp.int64
    ...
    new_dtype_dict = dtype_dict.copy()
    new_dtype_dict["left_child"] = jnp.float64
    new_dtype = jnp.dtype(
        {"names": list(new_dtype_dict.keys()),
         "formats": list(new_dtype_dict.values())}
    )
...
def test_missing_values_on_equal_nodes_no_missing(criterion):
    ...
    X = jnp.array([[0, 1, 2, 3, 8, 9, 11, 12, 15]]).T
    y = jnp.array([0.1, 0.2, 0.3, 0.2, 1.4, 1.4, 1.5, 1.6, 2.6])
    ...
    y_pred = dtc.predict([[jnp.nan]])
    assert_allclose(y_pred, [jnp.mean(y[-5:])])
    ...
    y_pred = dtc.predict([[jnp.nan]])
    assert_allclose(y_pred, [jnp.mean(y_equal[-4:])])
...
def test_missing_values_best_splitter_three_classes(criterion):
    ...
    X = jnp.array([[jnp.nan] * 4 + [0, 1, 2, 3, 8, 9, 11, 12]]).T
    y = jnp.array([missing_values_class] * 4 + [1] * 4 + [2] * 4)
    dtc = DecisionTreeClassifier(
        random_state=42, max_depth=2, criterion=criterion)
    dtc.fit(X, y)

    X_test = jnp.array([[jnp.nan, 3, 12]]).T
...
def test_missing_values_best_splitter_to_left(criterion):
    ...
    X = jnp.array([[jnp.nan] * 4 + [0, 1, 2, 3, 4, 5]]).T
    y = jnp.array([0] * 4 + [1] * 6)
    ...
    X_test = jnp.array([[jnp.nan, 5, jnp.nan]]).T
...
def test_missing_values_best_splitter_to_right(criterion):
    ...
    to this class."""
    X = jnp.array([[jnp.nan] * 4 + [0, 1, 2, 3, 4, 5]]).T
    y = jnp.array([1] * 4 + [0] * 4 + [1] * 2)

    dtc = DecisionTreeClassifier(
        random_state=42, max_depth=2, criterion=criterion)
    dtc.fit(X, y)

    X_test = jnp.array([[jnp.nan, 1.2, 4.8]]).T
    y_pred = dtc.predict(X_test)
...
def test_missing_values_missing_both_classes_has_nan(criterion):
    ...
    X = jnp.array([[1, 2, 3, 5, jnp.nan, 10, 20, 30, 60, jnp.nan]]).T
    y = jnp.array([0] * 5 + [1] * 5)

    dtc = DecisionTreeClassifier(
        random_state=42, max_depth=1, criterion=criterion)
    dtc.fit(X, y)
    X_test = jnp.array([[jnp.nan, 2.3, 34.2]]).T
    y_pred = dtc.predict(X_test)
...
def test_missing_values_is_resilience(make_data, Tree, sample_weight_train):
    ...
    X_missing = X.copy()
    X_missing[rng.choice([False, True], size=X.shape, p=[0.9, 0.1])] = jnp.nan
    X_missing_train, X_missing_test, y_train, y_test = train_test_split(
        X_missing, y, random_state=0
    )

    if sample_weight_train == "ones":
        sample_weight_train = jnp.ones(X_missing_train.shape[0])
...
def test_sample_weight_non_uniform(make_data, Tree):
    ...
    # Create dataset with missing values
    X[rng.choice([False, True], size=X.shape, p=[0.9, 0.1])] = jnp.nan
