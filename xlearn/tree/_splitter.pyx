...
cdef class Splitter:
    ...
    cdef int init(
        ...
        self.samples = jnp.empty(n_samples, dtype=jnp.intp)
        ...
        self.features = jnp.arange(n_features, dtype=jnp.intp)
        self.n_features = n_features

        self.feature_values = jnp.empty(n_samples, dtype=jnp.float32)
        self.constant_features = jnp.empty(n_features, dtype=jnp.intp)
...
cdef class SparsePartitioner:
    ...
    def __init__(
        ...
        self.index_to_samples = jnp.full(n_total_samples, fill_value=-1, dtype=jnp.intp)
        self.sorted_samples = jnp.empty(n_samples, dtype=jnp.intp)
