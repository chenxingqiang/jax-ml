...
cdef double EPSILON = 10 * jnp.finfo('double').eps
...
cdef class ClassificationCriterion(Criterion):
    ...
    def __cinit__(self, SIZE_t n_outputs,
        ...
        self.n_classes = jnp.empty(n_outputs, dtype=jnp.intp)
        ...
        self.sum_total = jnp.zeros((n_outputs, max_n_classes), dtype=jnp.float64)
        self.sum_left = jnp.zeros((n_outputs, max_n_classes), dtype=jnp.float64)
        self.sum_right = jnp.zeros((n_outputs, max_n_classes), dtype=jnp.float64)

    def __reduce__(self):
        return (type(self),
                (self.n_outputs, jnp.asarray(self.n_classes)), self.__getstate__())
    ...
    cdef void init_sum_missing(self):
        """Init sum_missing to hold sums for missing values."""
        self.sum_missing = jnp.zeros((self.n_outputs, self.max_n_classes), dtype=jnp.float64)
...
cdef class RegressionCriterion(Criterion):
    ...
    def __cinit__(self, SIZE_t n_outputs, SIZE_t n_samples):
        ...
        self.sum_total = jnp.zeros(n_outputs, dtype=jnp.float64)
        self.sum_left = jnp.zeros(n_outputs, dtype=jnp.float64)
        self.sum_right = jnp.zeros(n_outputs, dtype=jnp.float64)
    ...
    cdef void init_sum_missing(self):
        """Init sum_missing to hold sums for missing values."""
        self.sum_missing = jnp.zeros(self.n_outputs, dtype=jnp.float64)
...
cdef class MAE(RegressionCriterion):
    ...
    def __cinit__(self, SIZE_t n_outputs, SIZE_t n_samples):
        ...
        self.node_medians = jnp.zeros(n_outputs, dtype=jnp.float64)

        self.left_child = jnp.empty(n_outputs, dtype='object')
        self.right_child = jnp.empty(n_outputs, dtype='object')
