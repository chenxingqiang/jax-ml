...
cdef double INFINITY = jnp.inf
cdef double EPSILON = jnp.finfo('double').eps
...
cdef Node dummy
NODE_DTYPE = jnp.asarray(<Node[:1]>(&dummy)).dtype
    ...
    cdef inline _check_input(
        ...
        if issparse(X):
            ...
            if X.data.dtype != DTYPE:
                X.data = jnp.ascontiguousarray(X.data, dtype=DTYPE)

            if X.indices.dtype != jnp.int32 or X.indptr.dtype != jnp.int32:
                raise ValueError("No support for jnp.int64 index based "
                                 "sparse matrices")
        ...
        if y.base.dtype != DOUBLE or not y.base.flags.contiguous:
            y = jnp.ascontiguousarray(y, dtype=DOUBLE)
        ...
        if (
            ...
            sample_weight = jnp.asarray(sample_weight, dtype=DOUBLE, order="C")
...
cdef class Tree:
    ...
    def __cinit__(self, int n_features, cnp.ndarray n_classes, int n_outputs):
        """Constructor."""
        cdef SIZE_t dummy = 0
        size_t_dtype = jnp.array(dummy).dtype
        ...
        self.max_n_classes = jnp.max(n_classes)
    ...
    cdef inline cnp.ndarray _apply_dense(self, object X):
        ...
        if not isinstance(X, jnp.ndarray):
            raise ValueError("X should be in jnp.ndarray format, got %s"
                             % type(X))

        if X.dtype != DTYPE:
            raise ValueError("X.dtype should be jnp.float32, got %s" % X.dtype)
        ...
        # Initialize output
        cdef SIZE_t[:] out = jnp.zeros(n_samples, dtype=jnp.intp)
    ...
    cdef inline object _decision_path_dense(self, object X):
        ...
        # Initialize output
        cdef SIZE_t[:] indptr = jnp.zeros(n_samples + 1, dtype=jnp.intp)
        cdef SIZE_t[:] indices = jnp.zeros(
            n_samples * (1 + self.max_depth), dtype=jnp.intp
        )
        ...
        cdef SIZE_t[:] data = jnp.ones(shape=len(indices), dtype=jnp.intp)
    ...
    cpdef compute_node_depths(self):
        ...
        Returns
        -------
        depths : ndarray of shape (self.node_count,), dtype=jnp.int64
            The depth of each node in the tree.
        """
        cdef:
            cnp.int64_t[::1] depths = jnp.empty(self.node_count, dtype=jnp.int64)
    ...
    cpdef compute_feature_importances(self, normalize=True):
        ...
        cdef cnp.float64_t[:] importances = jnp.zeros(self.n_features)
        ...
        if normalize:
            normalizer = jnp.sum(importances)
        ...
        return jnp.asarray(importances)
    ...
    def compute_partial_dependence(self, DTYPE_t[:, ::1] X,
        ...
        """
        cdef:
            double[::1] weight_stack = jnp.zeros(self.node_count,
                                                dtype=jnp.float64)
            SIZE_t[::1] node_idx_stack = jnp.zeros(self.node_count,
                                                  dtype=jnp.intp)
            SIZE_t sample_idx
            SIZE_t feature_idx
            int stack_size
...
cdef class _PathFinder(_CCPPruneController):
    ...
    def __cinit__(self,  int node_count):
        self.ccp_alphas = jnp.zeros(shape=(node_count), dtype=jnp.float64)
        self.impurities = jnp.zeros(shape=(node_count), dtype=jnp.float64)
        self.count = 0
...
cdef _cost_complexity_prune(unsigned char[:] leaves_in_subtree,  # OUT
    ...
    cdef:
        ...
        DOUBLE_t[:] r_node = jnp.empty(shape=n_nodes, dtype=jnp.float64)

        SIZE_t[:] child_l = orig_tree.children_left
        SIZE_t[:] child_r = orig_tree.children_right
        SIZE_t[:] parent = jnp.zeros(shape=n_nodes, dtype=jnp.intp)
        ...
        SIZE_t[:] n_leaves = jnp.zeros(shape=n_nodes, dtype=jnp.intp)
        DOUBLE_t[:] r_branch = jnp.zeros(shape=n_nodes, dtype=jnp.float64)
        DOUBLE_t current_r
        SIZE_t leaf_idx
        SIZE_t parent_idx

        # candidate nodes that can be pruned
        unsigned char[:] candidate_nodes = jnp.zeros(shape=n_nodes,
                                                    dtype=jnp.uint8)
        # nodes in subtree
        unsigned char[:] in_subtree = jnp.ones(shape=n_nodes, dtype=jnp.uint8)
        SIZE_t pruned_branch_node_idx
        DOUBLE_t subtree_alpha
        DOUBLE_t effective_alpha
        SIZE_t n_pruned_leaves
        DOUBLE_t r_diff
        DOUBLE_t max_float64 = jnp.finfo(jnp.float64).max
...
def _build_pruned_tree_ccp(
    ...
    cdef:
        SIZE_t n_nodes = orig_tree.node_count
        unsigned char[:] leaves_in_subtree = jnp.zeros(
            shape=n_nodes, dtype=jnp.uint8)
...
def ccp_pruning_path(Tree orig_tree):
    ...
    cdef:
        unsigned char[:] leaves_in_subtree = jnp.zeros(
            shape=orig_tree.node_count, dtype=jnp.uint8)
    ...
    cdef:
        ...
        DOUBLE_t[:] ccp_alphas = jnp.empty(shape=total_items, dtype=jnp.float64)
        DOUBLE_t[:] impurities = jnp.empty(shape=total_items, dtype=jnp.float64)
        UINT32_t count = 0
    ...
    return {
        'ccp_alphas': jnp.asarray(ccp_alphas),
        'impurities': jnp.asarray(impurities),
    }
