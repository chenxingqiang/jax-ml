...
class BaseDecisionTree(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _compute_missing_values_in_feature_mask(self, X):
        ...
        with jnp.errstate(over="ignore"):
            overall_sum = jnp.sum(X)

        if not jnp.isfinite(overall_sum):
    ...
    def _fit(
        ...
        if check_input:
            ...
            if issparse(X):
                ...
                if X.indices.dtype != jnp.intc or X.indptr.dtype != jnp.intc:
                    raise ValueError(
                        "No support for jnp.int64 index based sparse matrices"
                    )
        ...
        if is_classification:
            ...
            y_encoded = jnp.zeros(y.shape, dtype=int)
            for k in range(self.n_outputs_):
                classes_k, y_encoded[:, k] = jnp.unique(
            ...
            self.n_classes_ = jnp.array(self.n_classes_, dtype=jnp.intp)

        if getattr(y, "dtype", None) != DOUBLE or not y.flags.contiguous:
            y = jnp.ascontiguousarray(y, dtype=DOUBLE)

        max_depth = jnp.iinfo(
            jnp.int32).max if self.max_depth is None else self.max_depth
        ...
        if self.monotonic_cst is None:
            ...
            # Note: we do not cast `jnp.asarray(self.monotonic_cst, dtype=jnp.int8)`
            ...
            monotonic_cst = jnp.asarray(self.monotonic_cst)
            ...
            valid_constraints = jnp.isin(monotonic_cst, (-1, 0, 1))
            if not jnp.all(valid_constraints):
                unique_constaints_value = jnp.unique(monotonic_cst)
                raise ValueError(
        ...
        if is_classifier(self):
            ...
            self.tree_ = Tree(
                ...
                jnp.array([1] * self.n_outputs_, dtype=jnp.intp),
                self.n_outputs_,
            )
    ...
    def _validate_X_predict(self, X, check_input):
        ...
        if check_input:
            ...
            if issparse(X) and (
                X.indices.dtype != jnp.intc or X.indptr.dtype != jnp.intc
            ):
                raise ValueError(
                    "No support for jnp.int64 index based sparse matrices")
        else:
    ...
    def _prune_tree(self):
        ...
        # build pruned tree
        if is_classifier(self):
            n_classes = jnp.atleast_1d(self.n_classes_)
...
class DecisionTreeClassifier(ClassifierMixin, BaseDecisionTree):
    ...
    def predict_proba(self, X, check_input=True):
        ...
        if self.n_outputs_ == 1:
            ...
            normalizer = proba.sum(axis=1)[:, jnp.newaxis]
            ...
            for k in range(self.n_outputs_):
                ...
                normalizer = proba_k.sum(axis=1)[:, jnp.newaxis]
                normalizer[normalizer == 0.0] = 1.0
                proba_k /= normalizer
