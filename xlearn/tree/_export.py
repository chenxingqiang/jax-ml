...
def _color_brew(n):
    ...
    for h in jnp.arange(25, 385, 360.0 / n).astype(int):
...
class _BaseTreeExporter:
    ...
    def get_color(self, value):
        ...
        if self.colors["bounds"] is None:
            # Classification tree
            color = list(self.colors["rgb"][jnp.argmax(value)])
    ...
    def get_fill_color(self, tree, node_id):
        ...
        if "rgb" not in self.colors:
            ...
            if tree.n_outputs != 1:
                ...
                self.colors["bounds"] = (
                    jnp.min(-tree.impurity), jnp.max(-tree.impurity))
            elif tree.n_classes[0] == 1 and len(jnp.unique(tree.value)) != 1:
                # Find max and min values in leaf nodes for regression
                self.colors["bounds"] = (
                    jnp.min(tree.value), jnp.max(tree.value))
        if tree.n_outputs == 1:
    ...
    def node_to_str(self, tree, node_id, criterion):
        ...
        if tree.n_classes[0] == 1:
            # Regression
            value_text = jnp.around(value, self.precision)
        elif self.proportion:
            # Classification
            value_text = jnp.around(value, self.precision)
        elif jnp.all(jnp.equal(jnp.mod(value, 1), 0)):
        ...
        if (
            ...
            if self.class_names is not True:
                class_name = self.class_names[jnp.argmax(value)]
            else:
                class_name = "y%s%s%s" % (
                    characters[1],
                    jnp.argmax(value),
        ...
        if self.max_depth is None or depth <= self.max_depth:
            ...
            if parent is not None:
                ...
                if parent == 0:
                    ...
                    angles = jnp.array([45, -45]) * ((self.rotate - 0.5) * -2)
