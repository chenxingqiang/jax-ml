...
def _assess_dimension(spectrum, rank, n_samples):
    ...
    for i in range(1, rank + 1):
        pu += (
            ...
            - log(jnp.pi) * (n_features - i + 1) / 2.0
        )

    pl = jnp.sum(jnp.log(spectrum[:rank]))
    pl = -pl * n_samples / 2.0

    v = max(eps, jnp.sum(spectrum[rank:]) / (n_features - rank))
    pv = -jnp.log(v) * n_samples * (n_features - rank) / 2.0

    m = n_features * rank - rank * (rank + 1.0) / 2.0
    pp = log(2.0 * jnp.pi) * (m + rank) / 2.0
...
def _infer_dimension(spectrum, n_samples):
    ...
    """
    ll = jnp.empty_like(spectrum)
    ll[0] = -jnp.inf  # we don't want to return n_components = 0
...
class PCA(_BasePCA):
    ...
    def fit_transform(self, X, y=None):
        ...
        C-ordered array, use 'jnp.ascontiguousarray'.
        """
        U, S, Vt = self._fit(X)
    ...
    def _fit(self, X):
        ...
        X = self._validate_data(
            X, dtype=[jnp.float64, jnp.float32], ensure_2d=True, copy=self.copy
        )
