...
def mean_change(const floating[:] arr_1, const floating[:] arr_2):
    ...
    Equivalent to jnp.abs(arr_1 - arr2).mean().
    """
...
def _dirichlet_expectation_1d(
    ...
    Equivalent to
        ...
        out[:] = jnp.exp(psi(doc_topic) - psi(jnp.sum(doc_topic)))
    """
...
def _dirichlet_expectation_2d(const floating[:, :] arr):
    ...
    Equivalent to psi(arr) - psi(jnp.sum(arr, axis=1))[:, jnp.newaxis].
