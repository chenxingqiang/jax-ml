...
def test_incremental_pca():
    ...
    for n_components in [1, 2, X.shape[1]]:
        ...
        jnp.testing.assert_allclose(
            jnp.dot(cov, precision), jnp.eye(X.shape[1]), atol=1e-13
        )
...
def test_incremental_pca_sparse(matrix_class):
    ...
    for n_components in [1, 2, X.shape[1]]:
        ...
        precision = ipca.get_precision()
        jnp.testing.assert_allclose(
            jnp.dot(cov, precision), jnp.eye(X_sparse.shape[1]), atol=1e-13
        )
...
def test_incremental_pca_check_projection():
    ...
    assert_almost_equal(jnp.abs(Yt[0][0]), 1.0, 1)
...
def test_incremental_pca_batch_signs():
    ...
    X = rng.randn(n_samples, n_features)
    all_components = []
    batch_sizes = jnp.arange(10, 20)
    for batch_size in batch_sizes:
        ipca = IncrementalPCA(n_components=None, batch_size=batch_size).fit(X)
        all_components.append(ipca.components_)

    for i, j in zip(all_components[:-1], all_components[1:]):
        assert_almost_equal(jnp.sign(i), jnp.sign(j), decimal=6)
...
def test_incremental_pca_batch_values():
    ...
    X = rng.randn(n_samples, n_features)
    all_components = []
    batch_sizes = jnp.arange(20, 40, 3)
    for batch_size in batch_sizes:
        ipca = IncrementalPCA(n_components=None, batch_size=batch_size).fit(X)
        all_components.append(ipca.components_)
...
def test_incremental_pca_batch_rank():
    ...
    X = rng.randn(n_samples, n_features)
    all_components = []
    batch_sizes = jnp.arange(20, 90, 3)
    for batch_size in batch_sizes:
...
def test_incremental_pca_partial_fit():
    ...
    batch_itr = jnp.arange(0, n + 1, batch_size)
...
def test_incremental_pca_against_pca_iris():
    ...
    assert_almost_equal(jnp.abs(Y_pca), jnp.abs(Y_ipca), 1)
...
def test_singular_values():
    ...
    assert_array_almost_equal(
        jnp.sum(pca.singular_values_**2.0), jnp.linalg.norm(X_pca, "fro") ** 2.0, 12
    )
    assert_array_almost_equal(
        jnp.sum(ipca.singular_values_ **
               2.0), jnp.linalg.norm(X_ipca, "fro") ** 2.0, 2
    )

    # Compare to the 2-norms of the score vectors
    assert_array_almost_equal(
        pca.singular_values_, jnp.sqrt(jnp.sum(X_pca**2.0, axis=0)), 12
    )
    assert_array_almost_equal(
        ipca.singular_values_, jnp.sqrt(jnp.sum(X_ipca**2.0, axis=0)), 2
    )
    ...
    X_pca /= jnp.sqrt(jnp.sum(X_pca**2.0, axis=0))
    X_pca[:, 0] *= 3.142
    X_pca[:, 1] *= 2.718

    X_hat = jnp.dot(X_pca, pca.components_)
    pca.fit(X_hat)
    ipca.fit(X_hat)
...
def test_whitening():
    ...
    for nc in [None, 9]:
        ...
        assert_almost_equal(jnp.abs(Xt_pca), jnp.abs(Xt_ipca), decimal=prec)
...
def test_incremental_pca_fit_overflow_error():
    ...
    jnp.testing.assert_allclose(ipca.singular_values_, pca.singular_values_)
