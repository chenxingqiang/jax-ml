...
def test_factor_analysis():
    ...
    for method in ["randomized", "lapack"]:
        ...
        diff = jnp.all(jnp.diff(fa.loglike_))
        assert diff > 0.0, "Log likelihood dif not increase"

        # Sample Covariance
        scov = jnp.cov(X, rowvar=0.0, bias=1.0)

        # Model Covariance
        mcov = fa.get_covariance()
        diff = jnp.sum(jnp.abs(scov - mcov)) / W.size
        assert diff < 0.1, "Mean absolute difference is %f" % diff
        fa = FactorAnalysis(
            n_components=n_components, noise_variance_init=jnp.ones(n_features)
        )
        with pytest.raises(ValueError):
            fa.fit(X[:, :2])

    def f(x, y):
        return jnp.abs(getattr(x, y))  # sign will not be equal
    ...
    for n_components in [0, 2, X.shape[1]]:
        ...
        assert_array_almost_equal(
            jnp.dot(cov, precision), jnp.eye(X.shape[1]), 12)
    ...
    for rot1, rot2 in combinations([None, "varimax", "quartimax"], 2):
        assert not jnp.allclose(results[rot1], results[rot2])
        assert jnp.allclose(projections[rot1], projections[rot2], atol=3)
