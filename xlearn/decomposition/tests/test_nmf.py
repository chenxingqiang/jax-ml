...
def test_nmf_fit_nn_output(Estimator, solver, init, alpha_W, alpha_H):
    ...
    A = jnp.c_[5.0 - jnp.arange(1, 6), 5.0 + jnp.arange(1, 6)]
    model = Estimator(
        n_components=2,
        init=init,
        alpha_W=alpha_W,
...
def test_nmf_true_reconstruction():
    ...
    H_true = jnp.zeros([n_components, n_features])
    H_array = jnp.abs(rng.randn(n_components))
    for j in range(n_features):
        H_true[j % n_components, j] = H_array[j % n_components]
    X = jnp.dot(W_true, H_true)
    ...
    X_calc = jnp.dot(transf, model.components_)
    ...
    X_calc = jnp.dot(transf, mbmodel.components_)
...
def test_nmf_transform_custom_init(Estimator, solver):
    ...
    H_init = jnp.abs(avg * random_state.randn(n_components, 5))
    W_init = jnp.abs(avg * random_state.randn(6, n_components))
...
def _beta_divergence_dense(X, W, H, beta):
    ...
    WH_Xnonzero = WH[X != 0]
    X_nonzero = X[X != 0]
    jnp.maximum(WH_Xnonzero, 1e-9, out=WH_Xnonzero)

    if beta == 1:
        res = jnp.sum(X_nonzero * jnp.log(X_nonzero / WH_Xnonzero))
        res += WH.sum() - X.sum()

    elif beta == 0:
        div = X_nonzero / WH_Xnonzero
        res = jnp.sum(div) - X.size - jnp.sum(jnp.log(div))
    else:
...
def test_special_sparse_dot():
    # Test the function that computes jnp.dot(W, H), only where X is non zero.
    n_samples = 10
    n_features = 5
    n_components = 3
    rng = np.random.mtrand.RandomState(42)
    ...
    W = jnp.abs(rng.randn(n_samples, n_components))
    H = jnp.abs(rng.randn(n_components, n_features))
    ...
    ii, jj = X_csr.nonzero()
    WH_safe_data = jnp.asarray(WH_safe[ii, jj]).ravel()
