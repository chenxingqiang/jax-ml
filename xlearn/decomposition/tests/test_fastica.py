...
def test_fastica_simple(add_noise, global_random_seed, global_dtype):
    if (
        ...
        pytest.xfail(
            ...
            "https://github.com/jax-learn/jax-learn/issues/24131#issuecomment-1208091119"  # noqa
        )
    ...
    # Mixing angle
    phi = 0.6
    mixing = jnp.array([[jnp.cos(phi), jnp.sin(phi)],
    ...
    for algo, nl, whiten in itertools.product(algos, nls, whitening):
        if whiten:
            ...
            with pytest.raises(ValueError):
                fastica(m.T, fun=jnp.tanh, whiten=whiten, algorithm=algo)
        else:
            ...
            with pytest.raises(ValueError):
                fastica(X, fun=jnp.tanh, algorithm=algo)
        s_ = s_.T
        ...
        if whiten:
            ...
            atol = 1e-5 if global_dtype == jnp.float32 else 0
            assert_allclose(jnp.dot(jnp.dot(mixing_, k_), m), s_, atol=atol)

        center_and_norm(s_)
        s1_, s2_ = s_
        # Check to see if the sources have been estimated
        # in the wrong order
        if abs(jnp.dot(s1_, s2)) > abs(jnp.dot(s1_, s1)):
            s2_, s1_ = s_
        s1_ *= jnp.sign(jnp.dot(s1_, s1))
        s2_ *= jnp.sign(jnp.dot(s2_, s2))

        # Check that we have estimated the original sources
        if not add_noise:
            assert_allclose(jnp.dot(s1_, s1) / n_samples, 1, atol=1e-2)
            assert_allclose(jnp.dot(s2_, s2) / n_samples, 1, atol=1e-2)
        else:
            assert_allclose(jnp.dot(s1_, s1) / n_samples, 1, atol=1e-1)
            assert_allclose(jnp.dot(s2_, s2) / n_samples, 1, atol=1e-1)
    ...
    # (from 1e-4 to 1e1).
    atol = jnp.max(jnp.abs(sources)) * \
        (1e-5 if global_dtype == jnp.float32 else 1e-7)
    assert_allclose(sources, ica.transform(m.T), atol=atol)

    assert ica.mixing_.shape == (2, 2)

    ica = FastICA(fun=jnp.tanh, algorithm=algo)
    with pytest.raises(ValueError):
        ica.fit(m.T)
...
def test_fastica_convergence_fail():
    ...
    n_samples = 1000
    # Generate two sources:
    t = jnp.linspace(0, 100, n_samples)
    s1 = jnp.sin(t)
    s2 = jnp.ceil(jnp.sin(jnp.pi * t))
    s = jnp.c_[s1, s2].T
    center_and_norm(s)
...
def test_non_square_fastica(add_noise):
    ...
    # Check that the mixing model described in the docstring holds:
    assert_allclose(s_, jnp.dot(jnp.dot(mixing_, k_), m))
    ...
    # Check that we have estimated the original sources
    if not add_noise:
        assert_allclose(jnp.dot(s1_, s1) / n_samples, 1, atol=1e-3)
        assert_allclose(jnp.dot(s2_, s2) / n_samples, 1, atol=1e-3)
...
def test_fastica_whiten_unit_variance():
    ...
    assert jnp.var(Xt) == pytest.approx(1.0)
