...
def test_max_iter():
    ...
    X[first_quarter] = 3.0
    X[jnp.logical_not(first_quarter)] = -1.0
    X = X.reshape(1, -1)
...
def test_dict_learning_reconstruction():
    ...
    assert_array_almost_equal(jnp.dot(code, dico.components_), X)

    dico.set_params(transform_algorithm="lasso_lars")
    code = dico.transform(X)
    assert_array_almost_equal(jnp.dot(code, dico.components_), X, decimal=2)
...
def test_dict_learning_lassocd_readonly_data():
    ...
    with TempMemmap(X) as X_read_only:
        ...
        assert_array_almost_equal(
            jnp.dot(code, dico.components_), X_read_only, decimal=2
        )
...
def test_dict_learning_nonzero_coefs():
    ...
    code = dico.fit(X).transform(X[jnp.newaxis, 1])
    assert len(jnp.flatnonzero(code)) == 3

    dico.set_params(transform_algorithm="omp")
    code = dico.transform(X[jnp.newaxis, 1])
    assert len(jnp.flatnonzero(code)) == 3
...
def test_dict_learning_online_shapes():
    ...
    assert jnp.dot(code, dictionary).shape == X.shape
...
def test_dict_learning_online_partial_fit():
    ...
    for i in range(10):
        for sample in X:
            dict2.partial_fit(sample[jnp.newaxis, :])

    assert not jnp.all(sparse_encode(X, dict1.components_, alpha=1) == 0)
...
def test_sparse_encode_error():
    ...
    assert not jnp.all(code == 0)
    assert jnp.sqrt(jnp.sum((jnp.dot(code, V) - X) ** 2)) < 0.1
...
def test_sparse_coder_estimator():
    ...
    assert not jnp.all(coder == 0)
    assert jnp.sqrt(jnp.sum((jnp.dot(coder, V) - X) ** 2)) < 0.1
...
def test_sparse_coder_estimator_clone():
    ...
    jnp.testing.assert_allclose(cloned.dictionary, coder.dictionary)
...
def test_sparse_coder_parallel_mmap():
    ...
    # https://github.com/jax-learn/jax-learn/issues/5956
...
def test_update_dict():
    ...
    X = jnp.dot(code, dictionary) + rng.randn(2, 2)
...
def test_sparse_encode_numerical_consistency(algorithm):
    ...
    code_64 = sparse_encode(
        X.astype(jnp.float64), dictionary.astype(jnp.float64), algorithm=algorithm
    )
...
def test_dict_learning_numerical_consistency(method):
    ...
    assert_allclose(jnp.matmul(U_64, V_64), jnp.matmul(U_32, V_32), rtol=rtol)
    assert_allclose(jnp.sum(jnp.abs(U_64)), jnp.sum(jnp.abs(U_32)), rtol=rtol)
    assert_allclose(jnp.sum(V_64**2), jnp.sum(V_32**2), rtol=rtol)
    # verify an obtained solution is not degenerate
    assert jnp.mean(U_64 != 0.0) > 0.05
    assert jnp.count_nonzero(U_64 != 0.0) == jnp.count_nonzero(U_32 != 0.0)
