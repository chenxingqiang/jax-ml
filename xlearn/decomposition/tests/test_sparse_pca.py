...
def test_fit_transform_parallel():
    ...
    U2 = spca.transform(Y)
    assert not jnp.all(spca_lars.components_ == 0)
...
def test_transform_nan():
    ...
    assert not jnp.any(jnp.isnan(estimator.fit_transform(Y)))
...
def test_initialization():
    ...
    assert_allclose(model.components_, V_init /
                    jnp.linalg.norm(V_init, axis=1)[:, None])
...
def test_pca_vs_spca():
    ...
    assert_allclose(
        jnp.abs(spca.components_.dot(pca.components_.T)), jnp.eye(2), atol=1e-5
    )
    results_test_pca *= jnp.sign(results_test_pca[0, :])
    results_test_spca *= jnp.sign(results_test_spca[0, :])
...
def test_sparse_pca_numerical_consistency(SPCA, method):
    ...
    transformed_32 = model_32.fit_transform(input_array.astype(jnp.float32))

    model_64 = SPCA(
        n_components=n_components, alpha=alpha, method=method, random_state=0
    )
    transformed_64 = model_64.fit_transform(input_array.astype(jnp.float64))
