...
from xlearn.datasets import make_blobs, make_circles
...
def test_kernel_pca():
    ...
    for eigen_solver in ("auto", "dense", "arpack", "randomized"):
        for kernel in ("linear", "rbf", "poly", histogram):
            ...
            assert_array_almost_equal(
                jnp.abs(X_fit_transformed), jnp.abs(X_fit_transformed2)
            )
...
def test_kernel_pca_linear_kernel(solver, n_features):
    ...
    assert_array_almost_equal(
        jnp.abs(KernelPCA(n_comps, eigen_solver=solver).fit(
...
def test_remove_zero_eig():
    ...
    """
    X = jnp.array([[1 - 1e-30, 1], [1, 1], [1, 1 - 1e-20]])
...
def test_leave_zero_eig():
    ...
    with warnings.catch_warnings():
        ...
        with jnp.errstate(all="warn"):
            ...
            # Compare
            assert_array_almost_equal(jnp.abs(A), jnp.abs(B))
...
def test_kernel_pca_precomputed():
    ...
    for eigen_solver in ("dense", "arpack", "randomized"):
        ...
        X_kpca2 = (
            ...
            .fit(jnp.dot(X_fit, X_fit.T))
            .transform(jnp.dot(X_pred, X_fit.T))
        )

        X_kpca_train = KernelPCA(
            4, eigen_solver=eigen_solver, kernel="precomputed", random_state=0
        ).fit_transform(jnp.dot(X_fit, X_fit.T))
        ...
        X_kpca_train2 = (
            ...
            .fit(jnp.dot(X_fit, X_fit.T))
            .transform(jnp.dot(X_fit, X_fit.T))
        )

        assert_array_almost_equal(jnp.abs(X_kpca), jnp.abs(X_kpca2))

        assert_array_almost_equal(jnp.abs(X_kpca_train), jnp.abs(X_kpca_train2))
...
def test_gridsearch_pipeline():
    ...
    param_grid = dict(kernel_pca__gamma=2.0 ** jnp.arange(-2, 2))
...
def test_gridsearch_pipeline_precomputed():
    ...
    pipeline = Pipeline(
        [("kernel_pca", kpca), ("Perceptron", Perceptron(max_iter=5))])
    param_grid = dict(Perceptron__max_iter=jnp.arange(1, 5))
    grid_search = GridSearchCV(pipeline, cv=3, param_grid=param_grid)
...
def test_kernel_conditioning():
    ...
    assert jnp.all(kpca.eigenvalues_ ==
...
def test_kernel_pca_inverse_transform_reconstruction():
    ...
    assert jnp.linalg.norm(X - X_reconst) / jnp.linalg.norm(X) < 1e-1
...
def test_32_64_decomposition_shape():
    ...
    https://github.com/jax-learn/jax-learn/issues/18146
    """
    X, y = make_blobs(
