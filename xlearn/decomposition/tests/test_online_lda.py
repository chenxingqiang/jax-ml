...
def _build_sparse_mtx():
    ...
    n_components = 3
    block = jnp.full((3, 3), n_components, dtype=int)
...
def test_lda_transform():
    ...
    assert_array_almost_equal(jnp.sum(X_trans, axis=1),
                              jnp.ones(X_trans.shape[0]))
...
def test_lda_score_perplexity():
    ...
    score = lda.score(X)
    perplexity_2 = jnp.exp(-1.0 * (score / jnp.sum(X.data)))
...
def test_lda_empty_docs():
    ...
    for X in [Z, csr_matrix(Z)]:
        ...
        assert_almost_equal(
            lda.components_.sum(axis=0), jnp.ones(lda.components_.shape[1])
        )
...
def test_dirichlet_expectation():
    ...
    assert_allclose(expectation, jnp.exp(psi(x) - psi(jnp.sum(x))), atol=1e-19)

    x = x.reshape(100, 100)
    assert_allclose(
        _dirichlet_expectation_2d(x),
        psi(x) - psi(jnp.sum(x, axis=1)[:, jnp.newaxis]),
        rtol=1e-11,
        atol=3e-9,
    )
