...
def test_solvers(X_sparse, solver, kind):
    ...
    comp_a = jnp.abs(svd_a.components_)
    comp = jnp.abs(svd.components_)
...
def test_integers(X_sparse):
    n_samples = X_sparse.shape[0]
    Xint = X_sparse.astype(jnp.int64)
...
def test_explained_variance(X_sparse, kind, n_components, solver):
    ...
    total_variance = jnp.var(X_sparse.toarray(), axis=0).sum()
    variances = jnp.var(X_tr, axis=0)
...
@pytest.mark.parametrize("solver", SVD_SOLVERS)
def test_singular_values_consisten(solver):
    ...
    assert_allclose(
        jnp.sum(pca.singular_values_**2.0),
        jnp.linalg.norm(X_pca, "fro") ** 2.0,
        rtol=1e-2,
    )

    # Compare to the 2-norms of the score vectors
    assert_allclose(
        pca.singular_values_, jnp.sqrt(jnp.sum(X_pca**2.0, axis=0)), rtol=1e-2
    )
...
def test_singular_values_expected(solver):
    ...
    X_hat_pca = jnp.dot(X_pca, pca.components_)
    pca.fit(X_hat_pca)
