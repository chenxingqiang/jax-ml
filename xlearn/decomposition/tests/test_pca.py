...
def test_pca(svd_solver, n_components):
    ...
    assert_allclose(jnp.dot(cov, precision), jnp.eye(X.shape[1]), atol=1e-12)
...
def test_whitening(solver, copy):
    ...
    X = jnp.dot(
        ...
        jnp.dot(jnp.diag(jnp.linspace(10.0, 1.0, rank)),
    ...
    assert_allclose(X_whitened.std(ddof=1, axis=0), jnp.ones(n_components))
    assert_allclose(X_whitened.mean(axis=0),
                    jnp.zeros(n_components), atol=1e-12)
...
def test_pca_explained_variance_empirical(X, svd_solver):
    ...
    assert_allclose(pca.explained_variance_, jnp.var(X_pca, ddof=1, axis=0))

    expected_result = jnp.linalg.eig(jnp.cov(X, rowvar=False))[0]
...
def test_pca_singular_values(svd_solver):
    ...
    assert_allclose(
        jnp.sum(pca.singular_values_**2), jnp.linalg.norm(X_trans, "fro") ** 2
    )
    # Compare to the 2-norms of the score vectors
    assert_allclose(pca.singular_values_, jnp.sqrt(jnp.sum(X_trans**2, axis=0)))
    ...
    X_trans = pca.fit_transform(X)
    X_trans /= jnp.sqrt(jnp.sum(X_trans**2, axis=0))
    X_trans[:, 0] *= 3.142
    X_trans[:, 1] *= 2.718
    X_hat = jnp.dot(X_trans, pca.components_)
    pca.fit(X_hat)
...
def test_pca_check_projection(svd_solver):
    ...
    Xt = 0.1 * rng.randn(1, p) + jnp.array([3, 4, 5])

    Yt = PCA(n_components=2, svd_solver=svd_solver).fit(X).transform(Xt)
    Yt /= jnp.sqrt((Yt**2).sum())

    assert_allclose(jnp.abs(Yt[0][0]), 1.0, rtol=5e-3)
...
@pytest.mark.parametrize(
    "data", [jnp.array([[0, 1, 0], [1, 0, 0]]),
             jnp.array([[0, 1, 0], [1, 0, 0]]).T]
)
@pytest.mark.parametrize(
...
def test_infer_dim_1():
    ...
    ll = jnp.array([_assess_dimension(spect, k, n) for k in range(1, p)])
...
def test_pca_score(svd_solver):
    ...
    ll1 = pca.score(X)
    h = -0.5 * jnp.log(2 * jnp.pi * jnp.exp(1) * 0.1**2) * p
    assert_allclose(ll1 / h, 1, rtol=5e-2)

    ll2 = pca.score(rng.randn(n, p) * 0.2 + jnp.array([3, 4, 5]))
    assert ll1 > ll2
...
def test_pca_sanity_noise_variance(svd_solver):
    ...
    # https://github.com/jax-learn/jax-learn/issues/7568
    # https://github.com/jax-learn/jax-learn/issues/8541
    # https://github.com/jax-learn/jax-learn/issues/8544
    X, _ = datasets.load_digits(return_X_y=True)
    pca = PCA(n_components=30, svd_solver=svd_solver, random_state=0)
    pca.fit(X)
    assert jnp.all((pca.explained_variance_ - pca.noise_variance_) >= 0)
...
def test_pca_deterministic_output(svd_solver):
    ...
    assert_allclose(transformed_X, jnp.tile(
...
def check_pca_float_dtype_preservation(svd_solver):
    ...
    assert pca_64.components_.dtype == jnp.float64
    assert pca_32.components_.dtype == jnp.float32
    assert pca_64.transform(X_64).dtype == jnp.float64
    assert pca_32.transform(X_32).dtype == jnp.float32

    # the rtol is set such that the test passes on all platforms tested on
    # conda-forge: PR#15775
    # see: https://github.com/conda-forge/jax-learn-feedstock/pull/113
...
def check_pca_int_dtype_upcast_to_double(svd_solver):
    ...
    assert pca_64.components_.dtype == jnp.float64
    assert pca_32.components_.dtype == jnp.float64
    assert pca_64.transform(X_i64).dtype == jnp.float64
    assert pca_32.transform(X_i32).dtype == jnp.float64
...
def test_assess_dimension_bad_rank():
    ...
    spectrum = jnp.array([1, 1e-30, 1e-30, 1e-30])
    n_samples = 10
    for rank in (0, 5):
...
def test_small_eigenvalues_mle():
    ...
    assert _assess_dimension(spectrum, rank=1, n_samples=10) > -jnp.inf

    for rank in (2, 3):
        assert _assess_dimension(spectrum, rank, 10) == -jnp.inf
...
def test_assess_dimesion_rank_one():
    ...
    X = jnp.ones((n_samples, n_features))  # rank 1 matrix
    _, s, _ = jnp.linalg.svd(X, full_matrices=True)
    # except for rank 1, all eigenvalues are 0 resp. close to 0 (FP)
    assert_allclose(s[1:], jnp.zeros(n_features - 1), atol=1e-12)

    assert jnp.isfinite(_assess_dimension(s, rank=1, n_samples=n_samples))
    for rank in range(2, n_features):
        assert _assess_dimension(s, rank, n_samples) == -jnp.inf
...
def test_pca_randomized_svd_n_oversamples():
    ...
    assert_allclose(jnp.abs(pca_full.components_),
                    jnp.abs(pca_arpack.components_))
    assert_allclose(jnp.abs(pca_randomized.components_),
                    jnp.abs(pca_arpack.components_))
...
def test_variance_correctness(copy):
    ...
    true_var = jnp.var(X, ddof=1, axis=0).sum()
    jnp.testing.assert_allclose(pca_var, true_var)
