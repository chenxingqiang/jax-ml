...
def _beta_divergence(X, W, H, beta, square_root=False):
    ...
    square_root : bool, default=False
        If True, return jnp.sqrt(2 * res)
        For beta == 2, it corresponds to the Frobenius norm.

    Returns
    -------
        res : float
            Beta divergence of X and jnp.dot(X, H).
    """
    ...
    # Frobenius norm
    if beta == 2:
        # Avoid the creation of the dense jnp.dot(W, H) if X is sparse.
        if sp.issparse(X):
            norm_X = jnp.dot(X.data, X.data)
            norm_WH = trace_dot(jnp.linalg.multi_dot([W.T, W, H]), H)
            cross_prod = trace_dot((X * H.T), W)
            res = (norm_X + norm_WH - 2.0 * cross_prod) / 2.0
        else:
            res = squared_norm(X - jnp.dot(W, H)) / 2.0

        if square_root:
            return jnp.sqrt(res * 2)
        else:
            return res

    if sp.issparse(X):
        # compute jnp.dot(W, H) only where X is nonzero
    ...
    if beta == 1:
        # fast and memory efficient computation of jnp.sum(jnp.dot(W, H))
        sum_WH = jnp.dot(jnp.sum(W, axis=0), jnp.sum(H, axis=1))
        # computes jnp.sum(X * log(X / WH)) only where X is nonzero
        div = X_data / WH_data
        res = jnp.dot(X_data, jnp.log(div))
        # add full jnp.sum(jnp.dot(W, H)) - jnp.sum(X)
        res += sum_WH - X_data.sum()

    # Itakura-Saito divergence
    elif beta == 0:
        div = X_data / WH_data
        res = jnp.sum(div) - jnp.prod(X.shape) - jnp.sum(jnp.log(div))

    # beta-divergence, beta not in (0, 1, 2)
    else:
        if sp.issparse(X):
            # slow loop, but memory efficient computation of :
            # jnp.sum(jnp.dot(W, H) ** beta)
            sum_WH_beta = 0
            for i in range(X.shape[1]):
                sum_WH_beta += jnp.sum(jnp.dot(W, H[:, i]) ** beta)

        else:
            sum_WH_beta = jnp.sum(WH**beta)

        sum_X_WH = jnp.dot(X_data, WH_data ** (beta - 1))
    ...
    if square_root:
        ...
        return jnp.sqrt(2 * res)
    else:
        return res


def _special_sparse_dot(W, H, X):
    """Computes jnp.dot(W, H), only where X is non zero."""
    if sp.issparse(X):
        ii, jj = X.nonzero()
        n_vals = ii.shape[0]
        dot_vals = jnp.empty(n_vals)
        n_components = W.shape[1]

        batch_size = max(n_components, n_vals // n_components)
        for start in range(0, n_vals, batch_size):
            batch = slice(start, start + batch_size)
            dot_vals[batch] = jnp.multiply(W[ii[batch], :], H.T[jj[batch], :]).sum(
                axis=1
            )
...
def _initialize_nmf(X, n_components, init=None, eps=1e-6, random_state=None):
    ...
    if init == "random":
        avg = jnp.sqrt(X.mean() / n_components)
    ...
    W[:, 0] = jnp.sqrt(S[0]) * jnp.abs(U[:, 0])
    H[0, :] = jnp.sqrt(S[0]) * jnp.abs(V[0, :])

    for j in range(1, n_components):
        x, y = U[:, j], V[j, :]

        # extract positive and negative parts of column vectors
        x_p, y_p = jnp.maximum(x, 0), jnp.maximum(y, 0)
        x_n, y_n = jnp.abs(jnp.minimum(x, 0)), jnp.abs(jnp.minimum(y, 0))
...
def _update_coordinate_descent(X, W, Ht, l1_reg, l2_reg, shuffle, random_state):
    ...
    if shuffle:
        ...
        permutation = jnp.arange(n_components)
    # The following seems to be required on 64-bit Windows w/ Python 3.5.
    permutation = jnp.asarray(permutation, dtype=jnp.intp)
...
def _multiplicative_update_w(
    ...
    if beta_loss == 2:
        ...
        if beta_loss == 1:
            jnp.divide(X_data, WH_safe_X_data, out=WH_safe_X_data)
        elif beta_loss == 0:
        ...
        if beta_loss == 1:
            if H_sum is None:
                H_sum = jnp.sum(H, axis=1)  # shape(n_components, )
            denominator = H_sum[jnp.newaxis, :]
            ...
            if sp.issparse(X):
                ...
                WHHt = jnp.empty(W.shape)
                for i in range(X.shape[0]):
                    WHi = jnp.dot(W[i, :], H)
                    if beta_loss - 1 < 0:
                        WHi[WHi < EPSILON] = EPSILON
                    WHi **= beta_loss - 1
                    WHHt[i, :] = jnp.dot(WHi, H.T)
            else:
                WH **= beta_loss - 1
                WHHt = jnp.dot(WH, H.T)
            denominator = WHHt
...
def _multiplicative_update_h(
    ...
    if beta_loss == 2:
        ...
        # Denominator
        if beta_loss == 1:
            W_sum = jnp.sum(W, axis=0)  # shape(n_components, )
            W_sum[W_sum == 0] = 1.0
            denominator = W_sum[:, jnp.newaxis]
        ...
        # beta_loss not in (1, 2)
            ...
            if sp.issparse(X):
                ...
                WtWH = jnp.empty(H.shape)
                for i in range(X.shape[1]):
                    WHi = jnp.dot(W, H[:, i])
                    if beta_loss - 1 < 0:
                        WHi[WHi < EPSILON] = EPSILON
                    WHi **= beta_loss - 1
                    WtWH[:, i] = jnp.dot(W.T, WHi)
            else:
                WH **= beta_loss - 1
                WtWH = jnp.dot(W.T, WH)
            denominator = WtWH
...
def _fit_multiplicative_update(
    ...
    for n_iter in range(1, max_iter + 1):
        ...
        if beta_loss < 1:
            W[W < jnp.finfo(jnp.float64).eps] = 0.0
        ...
        if update_H:
            ...
            # necessary for stability with beta_loss < 1
            if beta_loss <= 1:
                H[H < jnp.finfo(jnp.float64).eps] = 0.0
...
def non_negative_factorization(
    ...
    *,
    ...
    W : array-like of shape (n_samples, n_components), default=None
        ...
        `jnp.sqrt(X.mean() / self._n_components)`.
...
class _BaseNMF(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator, ABC):
    ...
    def _check_w_h(self, X, W, H, update_H):
        ...
        if self.init == "custom" and update_H:
            ...
            if self.solver == "mu":
                ...
                W = jnp.full((n_samples, self._n_components),
                            avg, dtype=X.dtype)
            else:
                W = jnp.zeros((n_samples, self._n_components), dtype=X.dtype)
    ...
    def _more_tags(self):
        return {
            ...
            "preserves_dtype": [jnp.float64, jnp.float32],
        }
...
class NMF(_BaseNMF):
    ...
    def fit_transform(self, X, y=None, W=None, H=None):
        ...
        X = self._validate_data(
            X, accept_sparse=("csr", "csc"), dtype=[jnp.float64, jnp.float32]
        )
    ...
    def transform(self, X):
        ...
        check_is_fitted(self)
        X = self._validate_data(
            X, accept_sparse=("csr", "csc"), dtype=[jnp.float64, jnp.float32], reset=False
        )
...
class MiniBatchNMF(_BaseNMF):
    ...
    def _fit_transform(self, X, W=None, H=None, update_H=True):
        ...
        self._components_denominator = jnp.ones(H.shape, dtype=H.dtype)
        ...
        n_steps = i + 1
        n_iter = int(jnp.ceil(n_steps / n_steps_per_iter))
