...
class KernelPCA(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def _fit_transform(self, K):
        ...
        self.eigenvectors_, _ = svd_flip(
            self.eigenvectors_, jnp.zeros_like(self.eigenvectors_).T
        )
        ...
        #       self.eigenvectors_ = self.eigenvectors_ / jnp.sqrt(self.eigenvalues_)
        #
    ...
    def fit(self, X, y=None):
        ...
        if self.fit_inverse_transform:
            ...
            X_transformed = self.eigenvectors_ * jnp.sqrt(self.eigenvalues_)
    ...
    def transform(self, X):
        ...
        non_zeros = jnp.flatnonzero(self.eigenvalues_)
        scaled_alphas = jnp.zeros_like(self.eigenvectors_)
        scaled_alphas[:, non_zeros] = self.eigenvectors_[:, non_zeros] / jnp.sqrt(
            self.eigenvalues_[non_zeros]
        )

        # Project with a scalar product between K and the scaled eigenvectors
        return jnp.dot(K, scaled_alphas)
