...
def _sparse_encode_precomputed(
    ...
    *,
    ...
    if algorithm == "lasso_lars":
        ...
        new_code = (jnp.sign(cov) *
                    jnp.maximum(jnp.abs(cov) - regularization, 0)).T
        if positive:
            jnp.clip(new_code, 0, None, out=new_code)
    ...
    if check_input:
        if algorithm == "lasso_cd":
            dictionary = check_array(
                dictionary, order="C", dtype=[jnp.float64, jnp.float32]
            )
            X = check_array(X, order="C", dtype=[jnp.float64, jnp.float32])
        else:
...
def _sparse_encode(
    ...
    *,
    ...
    if gram is None and algorithm != "threshold":
        gram = jnp.dot(dictionary, dictionary.T)

    if cov is None and algorithm != "lasso_cd":
        copy_cov = False
        cov = jnp.dot(dictionary, X.T)
    ...
    n_samples = X.shape[0]
    n_components = dictionary.shape[0]
    code = jnp.empty((n_samples, n_components))
...
def _update_dict(
    ...
    for k in range(n_components):
        ...
        if positive:
            jnp.clip(dictionary[k], 0, None, out=dictionary[k])
...
def _dict_learning(
    ...
    *,
    ...
    if code_init is not None and dict_init is not None:
        code = jnp.array(code_init, order="F")
        ...
        dictionary = S[:, jnp.newaxis] * dictionary
    r = len(dictionary)
    if n_components <= r:  # True even if n_components=None
        code = code[:, :n_components]
        dictionary = dictionary[:n_components, :]
    else:
        code = jnp.c_[code, jnp.zeros((len(code), n_components - r))]
        dictionary = jnp.r_[
            dictionary, jnp.zeros((n_components - r, dictionary.shape[1]))
        ]

    # Fortran-order dict better suited for the sparse coding which is the
    # bottleneck of this algorithm.
    dictionary = jnp.asfortranarray(dictionary)

    errors = []
    current_cost = jnp.nan
    ...
    for ii in range(max_iter):
        ...
        # Cost function
        current_cost = 0.5 * jnp.sum((X - code @ dictionary) ** 2) + alpha * jnp.sum(
            jnp.abs(code)
        )
...
def dict_learning_online(
    ...
    *,
    ...
    dict_init : ndarray of shape (n_components, n_features), default=None
        ...
        with an SVD decomposition of the data via :func:`~xlearn.utils.randomized_svd`.
    ...
    X_train = check_array(
        X_train, order="C", dtype=[jnp.float64, jnp.float32], copy=False
    )
    ...
    if inner_stats is None:
        A = jnp.zeros((n_components, n_components), dtype=X_train.dtype)
        # The data approximation
        B = jnp.zeros((n_features, n_components), dtype=X_train.dtype)
    else:
    ...
    for ii, batch in zip(range(iter_offset, iter_offset + n_iter), batches):
        ...
        A *= beta
        A += jnp.dot(this_code.T, this_code)
        B *= beta
        B += jnp.dot(this_X.T, this_code)
...
class _BaseSparseCoding(ClassNamePrefixFeaturesOutMixin, TransformerMixin):
    ...
    def _transform(self, X, dictionary):
        ...
        if self.split_sign:
            ...
            split_code = jnp.empty((n_samples, 2 * n_features))
            split_code[:, :n_features] = jnp.maximum(code, 0)
            split_code[:, n_features:] = -jnp.minimum(code, 0)
            code = split_code
...
class SparseCoder(_BaseSparseCoding, BaseEstimator):
    ...
    >>> X = jnp.array([[-1, -1, -1], [0, 0, 3]])
    >>> dictionary = jnp.array(
    ...     [[0, 1, 0],
    ...      [-1, -1, 2],
    ...      [1, 1, 1],
    ...      [0, 1, 1],
...
class DictionaryLearning(_BaseSparseCoding, BaseEstimator):
    ...
    >>> jnp.mean(jnp.sum((X_hat - X) ** 2, axis=1) / jnp.sum(X ** 2, axis=1))
    0.07...
    """
    ...
    _parameter_constraints: dict = {
        ...
        "n_jobs": [Integral, None],
        "code_init": [jnp.ndarray, None],
        "dict_init": [jnp.ndarray, None],
...
class MiniBatchDictionaryLearning(_BaseSparseCoding, BaseEstimator):
    ...
    def _minibatch_step(self, X, dictionary, random_state, step):
        ...
        batch_cost = (
            ...
            + self.alpha * jnp.sum(jnp.abs(code))
        ) / batch_size
    ...
    def fit(self, X, y=None):
        ...
        X = self._validate_data(
            X, dtype=[jnp.float64, jnp.float32], order="C", copy=False
        )
        ...
        self._B = jnp.zeros((n_features, self._n_components),
                           dtype=X_train.dtype)
        ...
        if self.max_iter is not None:
            ...
            self.n_steps_ = i + 1
            self.n_iter_ = jnp.ceil(self.n_steps_ / n_steps_per_iter)
        else:
            ...
            self.n_iter_ = jnp.ceil(
                n_iter / int(jnp.ceil(n_samples / self._batch_size)))
    ...
    def partial_fit(self, X, y=None):
        ...
        X = self._validate_data(
            X, dtype=[jnp.float64, jnp.float32], order="C", reset=not has_components
        )
        ...
        if not has_components:
            ...
            self._B = jnp.zeros((X.shape[1], self._n_components), dtype=X.dtype)
        else:
