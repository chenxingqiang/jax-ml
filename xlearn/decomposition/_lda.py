...
def _update_doc_distribution(
    ...
    if random_state:
        ...
        doc_topic_distr = jnp.ones((n_samples, n_topics), dtype=X.dtype)

    # In the literature, this is `exp(E[log(theta)])`
    exp_doc_topic = jnp.exp(_dirichlet_expectation_2d(doc_topic_distr))

    # diff on `component_` (only calculate it when `cal_diff` is True)
    suff_stats = (
        jnp.zeros(exp_topic_word_distr.shape,
    ...
    for idx_d in range(n_samples):
        ...
        for _ in range(0, max_doc_update_iter):
            ...
            norm_phi = jnp.dot(exp_doc_topic_d, exp_topic_word_d) + eps

            doc_topic_d = exp_doc_topic_d * \
                jnp.dot(cnts / norm_phi, exp_topic_word_d.T)
        ...
        if cal_sstats:
            norm_phi = jnp.dot(exp_doc_topic_d, exp_topic_word_d) + eps
            suff_stats[:, ids] += jnp.outer(exp_doc_topic_d, cnts / norm_phi)
...
class LatentDirichletAllocation(
    ...
    components_ : ndarray of shape (n_components, n_features)
        ...
        after normalization:
        ``model.components_ / model.components_.sum(axis=1)[:, jnp.newaxis]``.
    ...
    def _init_latent_vars(self, n_features, dtype=jnp.float64):
        ...
        # In the literature, this is `exp(E[log(beta)])`
        self.exp_dirichlet_component_ = jnp.exp(
    ...
    def _e_step(self, X, cal_sstats, random_init, parallel=None):
        ...
        doc_topic_distr = jnp.vstack(doc_topics)

        if cal_sstats:
            # This step finishes computing the sufficient statistics for the
            # M-step.
            suff_stats = jnp.zeros(self.components_.shape,
    ...
    def _check_non_neg_array(self, X, reset_n_features, whom):
        ...
        """
        dtype = [jnp.float64,
                 jnp.float32] if reset_n_features else self.components_.dtype
    ...
    def transform(self, X):
        ...
        doc_topic_distr /= doc_topic_distr.sum(axis=1)[:, jnp.newaxis]
        return doc_topic_distr
    ...
    def _approx_bound(self, X, doc_topic_distr, sub_sampling):
        ...
        def _loglikelihood(prior, distr, dirichlet_distr, size):
            ...
            score = jnp.sum((prior - distr) * dirichlet_distr)
            score += jnp.sum(gammaln(distr) - gammaln(prior))
            score += jnp.sum(gammaln(prior * size) - gammaln(jnp.sum(distr, 1)))
            return score
        ...
        for idx_d in range(0, n_samples):
            if is_sparse_x:
                ...
                cnts = X[idx_d, ids]
            temp = (
                dirichlet_doc_topic[idx_d, :, jnp.newaxis] +
                dirichlet_component_[:, ids]
            )
            norm_phi = logsumexp(temp, axis=0)
            score += jnp.dot(cnts, norm_phi)
    ...
    def _perplexity_precomp_distr(self, X, doc_topic_distr=None, sub_sampling=False):
        ...
        return jnp.exp(-1.0 * perword_bound)
