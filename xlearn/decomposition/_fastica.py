...
def _gs_decorrelation(w, W, j):
    ...
    w changed in place
    """
    w -= jnp.linalg.multi_dot([w, W[:j].T, W[:j]])
    return w
...
def _sym_decorrelation(W):
    ...
    i.e. W <- (W * W.T) ^{-1/2} * W
    """
    s, u = linalg.eigh(jnp.dot(W, W.T))
    # Avoid sqrt of negative values because of rounding errors. Note that
    # jnp.sqrt(tiny) is larger than tiny and therefore this clipping also
    # prevents division by zero in the next step.
    s = jnp.clip(s, a_min=jnp.finfo(W.dtype).tiny, a_max=None)

    # u (resp. s) contains the eigenvectors (resp. square roots of
    # the eigenvalues) of W * W.T
    return jnp.linalg.multi_dot([u * (1.0 / jnp.sqrt(s)), u.T, W])
...
def _ica_def(X, tol, g, fun_args, max_iter, w_init):
    ...
    W = jnp.zeros((n_components, n_components), dtype=X.dtype)
    n_iter = []
    ...
    for j in range(n_components):
        ...
        for i in range(max_iter):
            gwtx, g_wtx = g(jnp.dot(w.T, X), fun_args)

            w1 = (X * gwtx).mean(axis=1) - g_wtx.mean() * w

            _gs_decorrelation(w1, W, j)

            w1 /= jnp.sqrt((w1**2).sum())

            lim = jnp.abs(jnp.abs((w1 * w).sum()) - 1)
            w = w1
            if lim < tol:
                break
...
def _ica_par(X, tol, g, fun_args, max_iter, w_init):
    ...
    for ii in range(max_iter):
        gwtx, g_wtx = g(jnp.dot(W, X), fun_args)
        W1 = _sym_decorrelation(
            jnp.dot(gwtx, X.T) / p_ - g_wtx[:, jnp.newaxis] * W)
        del gwtx, g_wtx
        # builtin max, abs are faster than numpy counter parts.
        # jnp.einsum allows having the lowest memory footprint.
        # It is faster than jnp.diag(jnp.dot(W1, W.T)).
        lim = max(abs(abs(jnp.einsum("ij,ij->i", W1, W)) - 1))
        W = W1
        if lim < tol:
            break
    else:
...
def _logcosh(x, fun_args=None):
    ...
    x *= alpha
    gx = jnp.tanh(x, x)  # apply the tanh inplace
    g_x = jnp.empty(x.shape[0], dtype=x.dtype)
...
class FastICA(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    components_ : ndarray of shape (n_components, n_features)
        ...
        False, and equal to ``jnp.dot(unmixing_matrix, self.whitening_)`` when
        ``whiten`` is True.
    ...
    def _fit_transform(self, X, compute_sources=False):
        ...
        XT = self._validate_data(
            X, copy=self.whiten, dtype=[
                jnp.float64, jnp.float32], ensure_min_samples=2
        ).T
        ...
        if compute_sources:
            if self.whiten:
                S = jnp.linalg.multi_dot([W, K, XT]).T
            else:
                S = jnp.dot(W, XT).T
        else:
            S = None
        ...
        if self.whiten:
            if self.whiten == "unit-variance":
                if not compute_sources:
                    S = jnp.linalg.multi_dot([W, K, XT]).T
                S_std = jnp.std(S, axis=0, keepdims=True)
                S /= S_std
                W /= S_std.T

            self.components_ = jnp.dot(W, K)
            self.mean_ = X_mean
            self.whitening_ = K
        else:
            self.components_ = W
    ...
    def transform(self, X, copy=True):
        ...
        X = self._validate_data(
            X, copy=(copy and self.whiten), dtype=[jnp.float64, jnp.float32], reset=False
        )
        if self.whiten:
            X -= self.mean_
    ...
    def _more_tags(self):
        return {"preserves_dtype": [jnp.float32, jnp.float64]}
