...
class FactorAnalysis(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    noise_variance_init : array-like of shape (n_features,), default=None
        ...
        If None, it defaults to jnp.ones(n_features).
    ...
    def fit(self, X, y=None):
        ...
        """
        X = self._validate_data(X, copy=self.copy, dtype=jnp.float64)
        ...
        llconst = n_features * log(2.0 * jnp.pi) + n_components
        var = jnp.var(X, axis=0)

        if self.noise_variance_init is None:
            psi = jnp.ones(n_features, dtype=X.dtype)
        else:
            ...
            psi = jnp.array(self.noise_variance_init)
        ...
        for i in range(self.max_iter):
            ...
            sqrt_psi = jnp.sqrt(psi) + SMALL
            s, Vt, unexp_var = my_svd(X / (sqrt_psi * nsqrt))
            s **= 2
            # Use 'maximum' here to avoid sqrt problems.
            W = jnp.sqrt(jnp.maximum(s - 1.0, 0.0))[:, jnp.newaxis] * Vt
            del Vt
            W *= sqrt_psi

            # loglikelihood
            ll = llconst + jnp.sum(jnp.log(s))
            ll += unexp_var + jnp.sum(jnp.log(psi))
            ll *= -n_samples / 2.0
            loglike.append(ll)
            ...
            psi = jnp.maximum(var - jnp.sum(W**2, axis=0), SMALL)
        else:
            warnings.warn(
    ...
    def transform(self, X):
        ...
        Ih = jnp.eye(len(self.components_))

        X_transformed = X - self.mean_

        Wpsi = self.components_ / self.noise_variance_
        cov_z = linalg.inv(Ih + jnp.dot(Wpsi, self.components_.T))
        tmp = jnp.dot(X_transformed, Wpsi.T)
        X_transformed = jnp.dot(tmp, cov_z)
    ...
    def get_covariance(self):
        ...
        cov = jnp.dot(self.components_.T, self.components_)
        # modify diag inplace
    ...
    def get_precision(self):
        ...
        if self.n_components == 0:
            return jnp.diag(1.0 / self.noise_variance_)
        if self.n_components == n_features:
            return linalg.inv(self.get_covariance())

        # Get precision using matrix inversion lemma
        components_ = self.components_
        precision = jnp.dot(components_ / self.noise_variance_, components_.T)
        precision.flat[:: len(precision) + 1] += 1.0
        precision = jnp.dot(components_.T, jnp.dot(
            linalg.inv(precision), components_))
        precision /= self.noise_variance_[:, jnp.newaxis]
        precision /= -self.noise_variance_[jnp.newaxis, :]
    ...
    def score_samples(self, X):
        ...
        log_like = -0.5 * (Xr * (jnp.dot(Xr, precision))).sum(axis=1)
        log_like -= 0.5 * (n_features * log(2.0 * jnp.pi) -
    ...
    def score(self, X, y=None):
        ...
        """
        return jnp.mean(self.score_samples(X))
...
def _ortho_rotation(components, method="varimax", tol=1e-6, max_iter=100):
    ...
    rotation_matrix = jnp.eye(ncol)
    var = 0

    for _ in range(max_iter):
        comp_rot = jnp.dot(components, rotation_matrix)
        if method == "varimax":
            tmp = comp_rot * jnp.transpose((comp_rot**2).sum(axis=0) / nrow)
    ...
    return jnp.dot(components, rotation_matrix).T
