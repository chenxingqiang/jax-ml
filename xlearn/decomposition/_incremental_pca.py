...
class IncrementalPCA(_BasePCA):
    ...
    of ``batch_size * n_features``, enabling use of jnp.memmap files without
    ...
    *Matrix Computations, Third Edition, G. Holub and C. Van Loan, Chapter 5,
    ...
    def partial_fit(self, X, y=None, check_input=True):
        ...
        if check_input:
            ...
            X = self._validate_data(
                X, copy=self.copy, dtype=[
                    jnp.float64, jnp.float32], reset=first_pass
            )
        n_samples, n_features = X.shape
        if first_pass:
            self.components_ = None
        ...
        col_mean, col_var, n_total_samples = _incremental_mean_and_var(
            ...
            last_sample_count=jnp.repeat(self.n_samples_seen_, X.shape[1]),
        )
        ...
        if self.n_samples_seen_ == 0:
            ...
            col_batch_mean = jnp.mean(X, axis=0)
            X -= col_batch_mean
            # Build matrix of combined previous basis and new data
            mean_correction = jnp.sqrt(
        ...
        explained_variance_ratio = S**2 / jnp.sum(col_var * n_total_samples)
