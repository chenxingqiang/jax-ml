...
def _calculate_threshold(estimator, importances, threshold):
    ...
    if threshold is None:
        ...
        is_elasticnet_l1_penalized = "ElasticNet" in est_name and (
            (hasattr(estimator, "l1_ratio_")
             and jnp.isclose(estimator.l1_ratio_, 1.0))
            or (hasattr(estimator, "l1_ratio") and jnp.isclose(estimator.l1_ratio, 1.0))
        )
    ...
    if isinstance(threshold, str):
        if "*" in threshold:
            ...
            if reference == "median":
                reference = jnp.median(importances)
            elif reference == "mean":
                reference = jnp.mean(importances)
            else:
            ...
            threshold = jnp.median(importances)

        elif threshold == "mean":
            threshold = jnp.mean(importances)
...
class SelectFromModel(MetaEstimatorMixin, SelectorMixin, BaseEstimator):
    ...
    max_features : int, callable, default=None
        ...
        To only select based on ``max_features``, set ``threshold=-jnp.inf``.
    ...
    def _get_support_mask(self):
        ...
        if self.max_features is not None:
            ...
            candidate_indices = jnp.argsort(-scores,
