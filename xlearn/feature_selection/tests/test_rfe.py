...
from xlearn.datasets import load_iris, make_friedman1
...
class MockClassifier:
    ...
    def fit(self, X, y):
        assert len(X) == len(y)
        self.coef_ = jnp.ones(X.shape[1], dtype=jnp.float64)
        return self
...
def test_rfe_features_importance():
    ...
    X = jnp.c_[iris.data, generator.normal(size=(len(iris.data), 6))]
    y = iris.target
...
def test_rfecv_cv_results_size(global_random_seed):
    ...
    for step, min_features_to_select in [[2, 1], [2, 2], [3, 3]]:
        ...
        score_len = jnp.ceil((X.shape[1] - min_features_to_select) / step) + 1
...
def test_number_of_subsets_of_features(global_random_seed):
    ...
    # = 1 + jnp.ceil((n_features - n_features_to_select) / float(step))
    ...
    def formula2(n_features, n_features_to_select, step):
        return 1 + jnp.ceil((n_features - n_features_to_select) / float(step))
    ...
    for n_features, n_features_to_select, step in zip(
        ...
        assert jnp.max(rfe.ranking_) == formula1(
            n_features, n_features_to_select, step)
        assert jnp.max(rfe.ranking_) == formula2(
            n_features, n_features_to_select, step)
...
def test_rfe_cv_groups():
    ...
    iris = load_iris()
    number_groups = 4
    groups = jnp.floor(jnp.linspace(0, number_groups, len(iris.target)))
    X = iris.data
...
def test_rfe_wrapped_estimator(importance_getter, selector, expected_n_features):
    ...
    # https://github.com/jax-learn/jax-learn/issues/15312
    X, y = make_friedman1(n_samples=50, n_features=10, random_state=0)
    estimator = LinearSVR(dual="auto", random_state=0)

    log_estimator = TransformedTargetRegressor(
        regressor=estimator, func=jnp.log, inverse_func=jnp.exp
    )
...
def test_rfecv_std_and_mean(global_random_seed):
    ...
    cv_scores = jnp.asarray([rfecv.cv_results_[key] for key in split_keys])
    expected_mean = jnp.mean(cv_scores, axis=0)
    expected_std = jnp.std(cv_scores, axis=0)
