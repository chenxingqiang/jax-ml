...
def test_n_features_to_select_stopping_criterion(direction):
    ...
    added_X = jnp.hstack(
        [
            selected_X,
            (X[:, rng.choice(added_candidates)])[:, jnp.newaxis],
        ]
    )

    removed_candidate = rng.choice(list(range(sfs.n_features_to_select_)))
    removed_X = jnp.delete(selected_X, removed_candidate, axis=1)
...
@pytest.mark.parametrize("y", ("no_validation", 1j, 99.9, jnp.nan, 3))
...
def test_cv_generator_support():
    ...
    groups = jnp.zeros_like(y, dtype=int)
    groups[y.size // 2:] = 1
