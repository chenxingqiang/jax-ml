...
def test_f_oneway_vs_scipy_stats():
    ...
    assert jnp.allclose(f, f2)
    assert jnp.allclose(pv, pv2)
...
def test_r_regression(center):
    ...
    Z = jnp.hstack((X, y[:, jnp.newaxis]))
    correlation_matrix = jnp.corrcoef(Z, rowvar=False)
...
def test_f_regression_center():
    ...
    # Create toy example
    X = jnp.arange(-5, 6).reshape(-1, 1)  # X has zero mean
    n_samples = X.size
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            jnp.array([[2, 1], [2, 0], [2, 10], [2, 4]]),
            jnp.array([0, 1, 1, 0]),
            jnp.array([0.0, 0.32075]),
            True,
        ),
        (
            # The target y is constant - forcing finite
            jnp.array([[5, 1], [3, 0], [2, 10], [8, 4]]),
            jnp.array([0, 0, 0, 0]),
            jnp.array([0.0, 0.0]),
            True,
        ),
...
def test_r_regression_force_finite(X, y, expected_corr_coef, force_finite):
    ...
    https://github.com/jax-learn/jax-learn/issues/15672
    """
    with warnings.catch_warnings():
        warnings.simplefilter("error", RuntimeWarning)
        corr_coef = r_regression(X, y, force_finite=force_finite)
    jnp.testing.assert_array_almost_equal(corr_coef, expected_corr_coef)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            jnp.array([[2, 1], [2, 0], [2, 10], [2, 4]]),
            jnp.array([0, 1, 1, 0]),
            jnp.array([0.0, 0.2293578]),
            jnp.array([1.0, 0.67924985]),
            True,
        ),
        (
            # The target y is constant - forcing finite
        ...
        (
            ...
            jnp.array([[0, 1], [1, 0], [2, 10], [3, 4]]),
            jnp.array([0, 1, 2, 3]),
            jnp.array([jnp.finfo(jnp.float64).max, 0.845433]),
            jnp.array([0.0, 0.454913]),
            True,
        ),
        (
            # Feature in X anti-correlated with y - forcing finite
            jnp.array([[3, 1], [2, 0], [1, 10], [0, 4]]),
            jnp.array([0, 1, 2, 3]),
            jnp.array([jnp.finfo(jnp.float64).max, 0.845433]),
            jnp.array([0.0, 0.454913]),
            True,
        ...
        (
            ...
            jnp.array([[2, 1], [2, 0], [2, 10], [2, 4]]),
            jnp.array([0, 1, 1, 0]),
            jnp.array([jnp.nan, 0.2293578]),
            jnp.array([jnp.nan, 0.67924985]),
            False,
        ),
        (
            # The target y is constant - not forcing finite
...
def test_f_regression_corner_case(
    ...
    jnp.testing.assert_array_almost_equal(f_statistic, expected_f_statistic)
    jnp.testing.assert_array_almost_equal(p_values, expected_p_values)
...
def test_select_kbest_all():
    ...
    # Non-regression test for:
    # https://github.com/jax-learn/jax-learn/issues/24949
    X_r2 = (
...
def test_select_kbest_zero(dtype_in):
    ...
    support = univariate_filter.get_support()
    gtruth = jnp.zeros(10, dtype=bool)
    assert_array_equal(support, gtruth)
...
def assert_best_scores_kept(score_filter):
    ...
    assert_allclose(jnp.sort(scores[support]), jnp.sort(scores)[-support.sum():])
...
def test_select_heuristics_regression():
    ...
    for mode in ["fdr", "fpr", "fwe"]:
        ...
        assert_array_equal(X_r, X_r2)
        support = univariate_filter.get_support()
        assert_array_equal(support[:5], jnp.ones((5,), dtype=bool))
        assert jnp.sum(support[5:] == 1) < 3


def test_boundary_case_ch2():
    # Test boundary case, and always aim to select 1 feature.
    X = jnp.array([[10, 20], [20, 20], [20, 30]])
    y = jnp.array([[1], [0], [0]])
    scores, pvalues = chi2(X, y)
    assert_array_almost_equal(scores, jnp.array([4.0, 0.71428571]))
    assert_array_almost_equal(pvalues, jnp.array([0.04550026, 0.39802472]))

    filter_fdr = SelectFdr(chi2, alpha=0.1)
    filter_fdr.fit(X, y)
    support_fdr = filter_fdr.get_support()
    assert_array_equal(support_fdr, jnp.array([True, False]))

    filter_kbest = SelectKBest(chi2, k=1)
    filter_kbest.fit(X, y)
    support_kbest = filter_kbest.get_support()
    assert_array_equal(support_kbest, jnp.array([True, False]))

    filter_percentile = SelectPercentile(chi2, percentile=50)
    filter_percentile.fit(X, y)
    support_percentile = filter_percentile.get_support()
    assert_array_equal(support_percentile, jnp.array([True, False]))

    filter_fpr = SelectFpr(chi2, alpha=0.1)
    filter_fpr.fit(X, y)
    support_fpr = filter_fpr.get_support()
    assert_array_equal(support_fpr, jnp.array([True, False]))

    filter_fwe = SelectFwe(chi2, alpha=0.1)
    filter_fwe.fit(X, y)
    support_fwe = filter_fwe.get_support()
    assert_array_equal(support_fwe, jnp.array([True, False]))
...
def test_select_fdr_regression(alpha, n_informative):
    ...
    def single_fdr(alpha, n_informative, random_state):
        ...
        assert_array_equal(X_r, X_r2)
        support = univariate_filter.get_support()
        num_false_positives = jnp.sum(support[n_informative:] == 1)
        num_true_positives = jnp.sum(support[:n_informative] == 1)
...
def test_tied_pvalues():
    ...
    X0 = jnp.array([[10000, 9999, 9998], [1, 1, 1]])
    y = [0, 1]
...
def test_scorefunc_multilabel():
    ...
    X = jnp.array([[10000, 9999, 0], [100, 9999, 0], [1000, 99, 0]])
    y = [[1, 1], [0, 1], [1, 0]]
...
def test_tied_scores():
    ...
    X_train = jnp.array([[0, 0, 0], [1, 1, 1]])
    y_train = [0, 1]

    for n_features in [1, 2, 3]:
        sel = SelectKBest(chi2, k=n_features).fit(X_train, y_train)
        X_test = sel.transform([[0, 1, 2]])
        assert_array_equal(X_test[0], jnp.arange(3)[-n_features:])
...
def test_nans():
    ...
    for select in (
        ...
        assert_array_equal(select.get_support(indices=True), jnp.array([1, 2]))
...
def test_no_feature_selected():
    ...
    for selector in strict_selectors:
        assert_array_equal(selector.get_support(), jnp.zeros(10))
        with pytest.warns(UserWarning, match="No features were selected"):
...
def test_dataframe_output_dtypes():
    ...
    X = X.astype(
        {
            "petal length (cm)": jnp.float32,
            "petal width (cm)": jnp.float64,
        }
    )
    ...
    def selector(X, y):
        ...
        return jnp.asarray([ranking[name] for name in column_order])
