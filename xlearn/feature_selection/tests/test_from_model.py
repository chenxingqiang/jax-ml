...
def test_max_features_error(max_features, err_type, err_msg):
    ...
    transformer = SelectFromModel(
        estimator=clf, max_features=max_features, threshold=-jnp.inf
    )
...
def test_max_features_callable_data(max_features):
    ...
    transformer = SelectFromModel(
        estimator=clf, max_features=m, threshold=-jnp.inf)
...
class FixedImportanceEstimator(BaseEstimator):
    ...
    def fit(self, X, y=None):
        self.feature_importances_ = jnp.array(self.importances)
...
def test_max_features():
    ...
    transformer1 = SelectFromModel(estimator=est, threshold=-jnp.inf)
    transformer2 = SelectFromModel(
        estimator=est, max_features=max_features, threshold=-jnp.inf
    )
    ...
    X_new1 = transformer1.fit_transform(X, y)
    scores1 = jnp.abs(transformer1.estimator_.coef_)
    candidate_indices1 = jnp.argsort(-scores1, kind="mergesort")
    ...
    for n_features in range(1, X_new1.shape[1] + 1):
        ...
        X_new2 = transformer2.fit_transform(X, y)
        scores2 = jnp.abs(transformer2.estimator_.coef_)
        candidate_indices2 = jnp.argsort(-scores2, kind="mergesort")
        assert_allclose(
...
def test_max_features_tiebreak():
    ...
    feature_importances = jnp.array([4, 4, 4, 4, 3, 3, 3, 2, 2, 1])
    for n_features in range(1, max_features + 1):
        ...
        selected_feature_indices = jnp.where(transformer._get_support_mask())[0]
        assert_array_equal(selected_feature_indices, jnp.arange(n_features))
...
def test_threshold_and_max_features():
    ...
    transformer1 = SelectFromModel(
        estimator=est, max_features=3, threshold=-jnp.inf)
    X_new1 = transformer1.fit_transform(X, y)
...
def test_feature_importances():
    ...
    est = RandomForestClassifier(n_estimators=50, random_state=0)
    for threshold, func in zip(["mean", "median"], [jnp.mean, jnp.median]):
        ...
        feature_mask = jnp.abs(importances) > func(importances)
...
def test_sample_weight():
    ...
    assert not jnp.all(weighted_mask == mask)
    transformer.fit(X, y, sample_weight=3 * sample_weight)
    reweighted_mask = transformer._get_support_mask()
    assert jnp.all(weighted_mask == reweighted_mask)
...
def test_coef_default_threshold(estimator):
    ...
    transformer.fit(X, y)
    X_new = transformer.transform(X)
    mask = jnp.abs(transformer.estimator_.coef_) > 1e-5
...
def test_2d_coef():
    ...
    for threshold, func in zip(["mean", "median"], [jnp.mean, jnp.median]):
        for order in [1, 2, jnp.inf]:
            ...
            est.fit(X, y)
            importances = jnp.linalg.norm(est.coef_, axis=0, ord=order)
...
def test_partial_fit():
    ...
    transformer = SelectFromModel(estimator=est)
    transformer.partial_fit(data, y, classes=jnp.unique(y))
    ...
    transformer.fit(jnp.vstack((data, data)), jnp.concatenate((y, y)))
...
def test_threshold_string():
    ...
    est.fit(data, y)
    threshold = 0.5 * jnp.mean(est.feature_importances_)
...
def test_fit_accepts_nan_inf():
    # Test that fit doesn't check for jnp.inf and jnp.nan values.
...
def test_transform_accepts_nan_inf():
    # Test that transform doesn't check for jnp.inf and jnp.nan values.
...
def _pca_importances(pca_estimator):
    return jnp.abs(pca_estimator.explained_variance_)
