...
def _clean_nans(scores):
    ...
    scores[jnp.isnan(scores)] = jnp.finfo(scores.dtype).min
    return scores
...
def f_oneway(*args):
    ...
    *args : {array-like, sparse matrix}
    ...
    n_samples_per_class = jnp.array([a.shape[0] for a in args])
    n_samples = jnp.sum(n_samples_per_class)
    ss_alldata = sum(safe_sqr(a).sum(axis=0) for a in args)
    sums_args = [jnp.asarray(a.sum(axis=0)) for a in args]
    ...
    msb = ssbn / float(dfbn)
    msw = sswn / float(dfwn)
    constant_features_idx = jnp.where(msw == 0.0)[0]
    if jnp.nonzero(msb)[0].size != msb.size and constant_features_idx.size:
        warnings.warn("Features %s are constant." %
                      constant_features_idx, UserWarning)
    f = msb / msw
    # flatten matrix to vector in sparse case
    f = jnp.asarray(f).ravel()
...
def f_classif(X, y):
    ...
    args = [X[safe_mask(X, y == k)] for k in jnp.unique(y)]
...
def _chisquare(f_obs, f_exp):
    ...
    optimizations.
    """
    f_obs = jnp.asarray(f_obs, dtype=jnp.float64)
...
def chi2(X, y):
    ...
    X = check_array(X, accept_sparse="csr", dtype=(jnp.float64, jnp.float32))
    if jnp.any((X.data if issparse(X) else X) < 0):
    ...
    expected = jnp.dot(class_prob.T, feature_count)
...
def r_regression(X, y, *, center=True, force_finite=True):
    ...
    force_finite : bool, default=True
        ...
        `force_finite=False`, a correlation of `jnp.nan` is returned to
    ...
    if center:
        ...
        X_norms = jnp.sqrt(row_norms(X.T, squared=True) -
                          n_samples * X_means**2)
    else:
    ...
    with jnp.errstate(divide="ignore", invalid="ignore"):
        ...
        correlation_coefficient /= jnp.linalg.norm(y)

    if force_finite and not jnp.isfinite(correlation_coefficient).all():
        # case where the target or some features are constant
        # the correlation coefficient(s) is/are set to the minimum (i.e. 0.0)
        nan_mask = jnp.isnan(correlation_coefficient)
...
def f_regression(X, y, *, center=True, force_finite=True):
    ...
    force_finite : bool, default=True
        ...
        - when the target `y` or some features in `X` are constant. In this
          ...
          `jnp.nan` values in the F-statistic and p-value. When
          `force_finite=True`, the F-statistic is set to `0.0` and the
          associated p-value is set to `1.0`.
        - when a feature in `X` is perfectly correlated (or
          anti-correlated) with the target `y`. In this case, the F-statistic
          is expected to be `jnp.inf`. When `force_finite=True`, the F-statistic
          is set to `jnp.finfo(dtype).max` and the associated p-value is set to
          `0.0`.
    ...
    if force_finite and not jnp.isfinite(f_statistic).all():
        # case where there is a perfect (anti-)correlation
        # f-statistics can be set to the maximum and p-values to zero
        mask_inf = jnp.isinf(f_statistic)
        f_statistic[mask_inf] = jnp.finfo(f_statistic.dtype).max
        # case where the target or some features are constant
        # f-statistics would be minimum and thus p-values large
        mask_nan = jnp.isnan(f_statistic)
...
class _BaseFilter(SelectorMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if isinstance(score_func_ret, (list, tuple)):
            ...
            self.pvalues_ = jnp.asarray(self.pvalues_)
        else:
            self.scores_ = score_func_ret
            self.pvalues_ = None

        self.scores_ = jnp.asarray(self.scores_)
...
class SelectPercentile(_BaseFilter):
    ...
    def _get_support_mask(self):
        ...
        if self.percentile == 100:
            return jnp.ones(len(self.scores_), dtype=bool)
        elif self.percentile == 0:
            return jnp.zeros(len(self.scores_), dtype=bool)

        scores = _clean_nans(self.scores_)
        threshold = jnp.percentile(scores, 100 - self.percentile)
        mask = scores > threshold
        ties = jnp.where(scores == threshold)[0]
        if len(ties):
...
class SelectKBest(_BaseFilter):
    ...
    def _get_support_mask(self):
        ...
        if self.k == "all":
            return jnp.ones(self.scores_.shape, dtype=bool)
        elif self.k == 0:
            return jnp.zeros(self.scores_.shape, dtype=bool)
        else:
            scores = _clean_nans(self.scores_)
            ...
            mask[jnp.argsort(scores, kind="mergesort")[-self.k:]] = 1
            return mask
...
class SelectFdr(_BaseFilter):
    ...
    def _get_support_mask(self):
        ...
        sv = jnp.sort(self.pvalues_)
        selected = sv[
            sv <= float(self.alpha) / n_features * jnp.arange(1, n_features + 1)
        ]
        if selected.size == 0:
            return jnp.zeros_like(self.pvalues_, dtype=bool)
...
class GenericUnivariateSelect(_BaseFilter):
    ...
    def _more_tags(self):
        return {"preserves_dtype": [jnp.float64, jnp.float32]}
