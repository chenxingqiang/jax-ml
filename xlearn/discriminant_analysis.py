...
def _class_cov(X, y, priors, shrinkage=None, covariance_estimator=None):
    ...
    cov = jnp.zeros(shape=(X.shape[1], X.shape[1]))
    for idx, group in enumerate(classes):
        Xg = X[y == group, :]
        cov += priors[idx] * \
            jnp.atleast_2d(_cov(Xg, shrinkage, covariance_estimator))
    return cov
...
class LinearDiscriminantAnalysis(
    ...
    def _solve_lstsq(self, X, y, shrinkage, covariance_estimator):
        ...
        self.intercept_ = -0.5 * jnp.diag(jnp.dot(self.means_, self.coef_.T)) + jnp.log(
            self.priors_
        )
    ...
    def _solve_eigen(self, X, y, shrinkage, covariance_estimator):
        ...
        self.explained_variance_ratio_ = jnp.sort(evals / jnp.sum(evals))[::-1][
            : self._max_components
        ]
        evecs = evecs[:, jnp.argsort(evals)[::-1]]  # sort eigenvectors

        self.scalings_ = evecs
        self.coef_ = jnp.dot(self.means_, evecs).dot(evecs.T)
        self.intercept_ = -0.5 * jnp.diag(jnp.dot(self.means_, self.coef_.T)) + jnp.log(
            self.priors_
        )
...
class QuadraticDiscriminantAnalysis(ClassifierMixin, BaseEstimator):
    ...
    reg_param : float, default=0.0
        ...
        ``S2 = (1 - reg_param) * S2 + reg_param * jnp.eye(n_features)``,
    ...
    def fit(self, X, y):
        ...
        if self.priors is None:
            self.priors_ = jnp.bincount(y) / float(n_samples)
        else:
            self.priors_ = jnp.array(self.priors)
        ...
        for ind in range(n_classes):
            ...
            Xgc = Xg - meang
            # Xgc = U * S * V.T
            _, S, Vt = jnp.linalg.svd(Xgc, full_matrices=False)
            rank = jnp.sum(S > self.tol)
            if rank < n_features:
            ...
            if self.store_covariance or store_covariance:
                # cov = V * (S^2 / (n-1)) * V.T
                cov.append(jnp.dot(S2 * Vt.T, Vt))
            scalings.append(S2)
            rotations.append(Vt.T)
        if self.store_covariance or store_covariance:
            self.covariance_ = cov
        self.means_ = jnp.asarray(means)
    ...
    def _decision_function(self, X):
        ...
        for i in range(len(self.classes_)):
            ...
            S = self.scalings_[i]
            Xm = X - self.means_[i]
            X2 = jnp.dot(Xm, R * (S ** (-0.5)))
            norm2.append(jnp.sum(X2**2, axis=1))
        norm2 = jnp.array(norm2).T  # shape = [len(X), n_classes]
        u = jnp.asarray([jnp.sum(jnp.log(s)) for s in self.scalings_])
        return -0.5 * (norm2 + u) + jnp.log(self.priors_)
    ...
    def predict_proba(self, X):
        ...
        likelihood = jnp.exp(values - values.max(axis=1)[:, jnp.newaxis])
        # compute posterior probabilities
        return likelihood / likelihood.sum(axis=1)[:, jnp.newaxis]
