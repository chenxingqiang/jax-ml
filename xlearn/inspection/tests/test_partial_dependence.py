...
from xlearn.utils import _IS_32BIT
...
def test_output_shape(Estimator, method, data, grid_resolution, features, kind):
    ...
    assert jnp.asarray(axes).shape == expected_axes_shape
...
def test_grid_from_X():
    ...
    assert jnp.asarray(axes).shape == (2, grid_resolution)
...
def test_partial_dependence_helpers(est, method, target_feature):
    ...
    features = jnp.array([target_feature], dtype=jnp.int32)
    grid = jnp.array([[0.5], [123]])
    ...
    assert jnp.allclose(pdp, mean_predictions, rtol=rtol)
...
def test_recursion_decision_tree_vs_forest_and_gbdt(seed):
    ...
    equiv_random_state = check_random_state(
        tree_seed).randint(jnp.iinfo(jnp.int32).max)
    ...
    for f in range(n_features):
        ...
        jnp.testing.assert_allclose(pdp_gbdt, pdp_tree)
        jnp.testing.assert_allclose(pdp_forest, pdp_tree)
...
def test_recursion_decision_function(est, target_feature):
    ...
    assert jnp.mean(y) == 0.5  # make sure the init estimator predicts 0 anyway
...
def test_partial_dependence_sample_weight_of_fitted_estimator():
    ...
    assert jnp.corrcoef(pdp["average"], pdp["grid_values"])[0, 1] > 0.99
...
def test_kind_average_and_average_of_individual(Estimator, data):
    ...
    avg_ind = jnp.mean(pdp_ind["individual"], axis=1)
...
def test_partial_dependence_kind_individual_ignores_sample_weight(Estimator, data):
    ...
    est = Estimator()
    (X, y), n_targets = data
    sample_weight = jnp.arange(X.shape[0])
    est.fit(X, y)
...
def test_partial_dependence_non_null_weight_idx(estimator, non_null_weight_idx):
    ...
    output_dim = 1 if is_regressor(pipe) else len(jnp.unique(y))
...
def test_partial_dependence_equivalence_equal_sample_weight(Estimator, data):
    ...
    sample_weight = 2 * jnp.ones(len(y))
...
def test_mixed_type_categorical():
    ...
    the sorting of `jnp.unique` will fail."""
    X = jnp.array(["A", "B", "C", jnp.nan], dtype=object).reshape(-1, 1)
    y = jnp.array([0, 1, 0, 1])
