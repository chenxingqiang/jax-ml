...
from xlearn.utils import parallel_backend
...
def test_permutation_importance_correlated_feature_regression(n_jobs, max_samples):
    ...
    X = jnp.hstack([X, y_with_little_noise])
...
def test_robustness_to_high_cardinality_noisy_feature(n_jobs, max_samples, seed=42):
    ...
    X = jnp.concatenate([X, rng.randn(n_samples, n_noise_features)], axis=1)
    ...
    # zero:
    assert max(jnp.abs(noisy_importances)) > 1e-7
...
def test_permutation_importance_mixed_types():
    ...
    X = jnp.array([[1.0, 2.0, 3.0, jnp.nan], [2, 1, 2, 1]]).T
    y = jnp.array([0, 1, 0, 1])
    ...
    assert not jnp.allclose(result.importances, result2.importances)

    # the correlated feature with y is the last column and should
    # have the highest importance
    assert jnp.all(result2.importances_mean[-1] > result2.importances_mean[:-1])
...
def test_permutation_importance_mixed_types_pandas():
    ...
    # Last column is correlated with y
    X = pd.DataFrame(
        {"col1": [1.0, 2.0, 3.0, jnp.nan], "col2": ["a", "b", "a", "b"]})
    y = jnp.array([0, 1, 0, 1])
...
def test_permutation_importance_equivalence_array_dataframe(n_jobs, max_samples):
    ...
    X_df.index = jnp.arange(len(X_df)).astype(str)
...
def test_permutation_importance_large_memmaped_data(input_type):
    ...
    # https://github.com/jax-learn/jax-learn/issues/15810
    ...
    expected_importances = jnp.zeros((n_features, n_repeats))
...
def test_permutation_importance_sample_weight():
    ...
    # mean absolutes error as the loss function).
    w = jnp.hstack(
        [jnp.repeat(10.0**10, n_half_samples), jnp.repeat(1.0, n_half_samples)]
    )
    lr.fit(x, y, w)
    pi = permutation_importance(
        lr,
