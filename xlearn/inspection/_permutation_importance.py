...
def _calculate_permutation_scores(
    ...
    scores = []
    shuffling_idx = jnp.arange(X_permuted.shape[0])
...
def _create_importances_bunch(baseline_score, permuted_score):
    ...
    return Bunch(
        importances_mean=jnp.mean(importances, axis=1),
        importances_std=jnp.std(importances, axis=1),
...
def permutation_importance(
    ...
    *,
    ...
    random_state = check_random_state(random_state)
    random_seed = random_state.randint(jnp.iinfo(jnp.int32).max + 1)
    ...
    if isinstance(baseline_score, dict):
        return {
            name: _create_importances_bunch(
                ...
                jnp.array([scores[col_idx][name]
                         for col_idx in range(X.shape[1])]),
            )
            for name in baseline_score
        }
    else:
        return _create_importances_bunch(baseline_score, jnp.array(scores))
