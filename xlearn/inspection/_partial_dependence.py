...
def _grid_from_X(X, percentiles, is_categorical, grid_resolution):
    ...
    values = []
    # TODO: we should handle missing values (i.e. `jnp.nan`) specifically and store them
    # in a different Bunch attribute.
    for feature, is_cat in enumerate(is_categorical):
        try:
            uniques = jnp.unique(_safe_indexing(X, feature, axis=1))
        except TypeError as exc:
            # `jnp.unique` will fail in the presence of `jnp.nan` and `str` categories
            ...
            raise ValueError(
                ...
                "contains `jnp.nan` values together with `str` categories. Such use "
                "case is not yet supported in jax-learn."
            ) from exc
        if is_cat or uniques.shape[0] < grid_resolution:
            ...
            if jnp.allclose(emp_percentiles[0], emp_percentiles[1]):
                raise ValueError(
...
def _partial_dependence_brute(
    ...
    for new_values in grid:
        ...
        try:
            ...
            averaged_predictions.append(jnp.average(
    ...
    # - n_tasks for multi-output regression
    # - n_classes for multiclass classification.
    averaged_predictions = jnp.array(averaged_predictions).T
...
def partial_dependence(
    ...
    *,
    ...
    if is_classifier(estimator) and isinstance(estimator.classes_[0], jnp.ndarray):
    ...
    if _determine_key_type(features, accept_slice=False) == "int":
        ...
        if jnp.any(jnp.less(features, 0)):
            raise ValueError(
                "all features must be in [0, {}]".format(X.shape[1] - 1))

    features_indices = jnp.asarray(
        _get_column_indices(X, features), dtype=jnp.int32, order="C"
    ).ravel()
