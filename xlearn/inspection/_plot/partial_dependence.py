...
class PartialdependenceDisplay:
    ...
    def from_estimator(
        ...
        *,
        ...
        if hasattr(estimator, "classes_") and jnp.size(estimator.classes_) > 2:
            if target is None:
                raise ValueError("target must be specified for multi-class")
            target_idx = jnp.searchsorted(estimator.classes_, target)
            if (
        ...
        for kind_plot, fxs in zip(kind_, features):
            ...
            ice_for_two_way_pd.append(
                kind_plot != "average" and jnp.size(fxs) > 1)
        ...
        if categorical_features is None:
            ...
            categorical_features = jnp.array(categorical_features, copy=False)
            ...
            for cats in is_categorical:
                if jnp.size(cats) == 2 and (cats[0] != cats[1]):
                    raise ValueError(
        ...
        if ax is not None and not isinstance(ax, plt.Axes):
            axes = jnp.asarray(ax, dtype=object)
    ...
    def _plot_average_dependence(
        ...
        """
        if categorical:
            bar_idx = jnp.unravel_index(pd_line_idx, self.bars_.shape)
            self.bars_[bar_idx] = ax.bar(
                feature_values, avg_preds, **bar_kw)[0]
            ax.tick_params(axis="x", rotation=90)
        else:
            line_idx = jnp.unravel_index(pd_line_idx, self.lines_.shape)
            self.lines_[line_idx] = ax.plot(
                feature_values,
                avg_preds,
                **line_kw,
    ...
    def _plot_one_way_partial_dependence(
        ...
        vlines_idx = jnp.unravel_index(pd_plot_idx, self.deciles_vlines_.shape)
    ...
    def _plot_two_way_partial_dependence(
        ...
        if categorical:
            ...
            text = jnp.empty_like(data, dtype=object)
            # print text with appropriate color depending on background
            thresh = (data.max() + data.min()) / 2.0

            for flat_index in range(data.size):
                row, col = jnp.unravel_index(flat_index, data.shape)
            ...
            ax.set(
                xticks=jnp.arange(len(feature_values[1])),
                yticks=jnp.arange(len(feature_values[0])),
            ...
            heatmap_idx = jnp.unravel_index(pd_plot_idx, self.heatmaps_.shape)
            self.heatmaps_[heatmap_idx] = im
        else:
            from matplotlib import transforms  # noqa

            XX, YY = jnp.meshgrid(feature_values[0], feature_values[1])
            Z = avg_preds[self.target_idx].T
            CS = ax.contour(XX, YY, Z, levels=Z_level,
                            linewidths=0.5, colors="k")
            contour_idx = jnp.unravel_index(pd_plot_idx, self.contours_.shape)
    ...
    def plot(
        ...
        *,
        ...
        if isinstance(ax, plt.Axes):
            ...
            n_rows = int(jnp.ceil(n_features / float(n_cols)))

            self.axes_ = jnp.empty((n_rows, n_cols), dtype=object)
            if all(is_average_plot):
                self.lines_ = jnp.empty((n_rows, n_cols), dtype=object)
            else:
                self.lines_ = jnp.empty((n_rows, n_cols, n_lines), dtype=object)
            self.contours_ = jnp.empty((n_rows, n_cols), dtype=object)
            self.bars_ = jnp.empty((n_rows, n_cols), dtype=object)
            self.heatmaps_ = jnp.empty((n_rows, n_cols), dtype=object)
            ...
            ax = jnp.asarray(ax, dtype=object)
            ...
            self.axes_ = ax
            if all(is_average_plot):
                self.lines_ = jnp.empty_like(ax, dtype=object)
            else:
                self.lines_ = jnp.empty(ax.shape + (n_lines,), dtype=object)
            self.contours_ = jnp.empty_like(ax, dtype=object)
            self.bars_ = jnp.empty_like(ax, dtype=object)
            self.heatmaps_ = jnp.empty_like(ax, dtype=object)

        # create contour levels for two-way plots
        if 2 in pdp_lim:
            Z_level = jnp.linspace(*pdp_lim[2], num=8)

        self.deciles_vlines_ = jnp.empty_like(self.axes_, dtype=object)
        self.deciles_hlines_ = jnp.empty_like(self.axes_, dtype=object)
