...
pytestmark = pytest.mark.filterwarnings(
    (
        "ignore:In future, it will be an error for 'jnp.bool_':DeprecationWarning:"
        "matplotlib.*"
    ),
)
...
def test_plot_partial_dependence(grid_resolution, pyplot, clf_diabetes, diabetes):
    ...
    assert jnp.all(disp.feature_names == feature_names)
    assert len(disp.deciles) == 2
    for i in [0, 2]:
        assert_allclose(
            disp.deciles[i],
            mquantiles(diabetes.data[:, i], prob=jnp.arange(0.1, 1.0, 0.1)),
        )
...
def test_plot_partial_dependence_passing_numpy_axes(
    ...
    assert jnp.all(disp1.axes_ == disp2.axes_)
...
def test_plot_partial_dependence_does_not_override_ylabel(
    ...
    # See https://github.com/jax-learn/jax-learn/issues/15772
...
def test_plot_partial_dependence_subsampling(
    ...
    # https://github.com/jax-learn/jax-learn/pull/18359
...
def test_partial_dependence_plot_limits_one_way(
    ...
    range_pd = jnp.array([-1, 1], dtype=jnp.float64)
...
def test_partial_dependence_plot_limits_two_way(
    ...
    levels[0] -= padding
    levels[1] += padding
    expect_levels = jnp.linspace(*levels, num=8)
...
def test_partial_dependence_display_with_constant_sample_weight(
    ...
    sample_weight = jnp.ones_like(diabetes.target)
