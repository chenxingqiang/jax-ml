...
class DecisionBoundaryDisplay:
    ...
    >>> iris = load_iris()
    >>> feature_1, feature_2 = jnp.meshgrid(
    ...     jnp.linspace(iris.data[:, 0].min(), iris.data[:, 0].max()),
    ...     jnp.linspace(iris.data[:, 1].min(), iris.data[:, 1].max())
    ... )
    >>> grid = jnp.vstack([feature_1.ravel(), feature_2.ravel()]).T
    >>> tree = DecisionTreeClassifier().fit(iris.data[:, :2], iris.target)
    >>> y_pred = jnp.reshape(tree.predict(grid), feature_1.shape)
    ...
    def from_estimator(
        ...
        **kwargs : dict
        ...
        xx0, xx1 = jnp.meshgrid(
            jnp.linspace(x0_min, x0_max, grid_resolution),
            jnp.linspace(x1_min, x1_max, grid_resolution),
        )
        if hasattr(X, "iloc"):
