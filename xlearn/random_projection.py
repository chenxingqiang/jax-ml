...
def johnson_lindenstrauss_min_dim(n_samples, *, eps=0.1):
    ...
    if jnp.any(eps <= 0.0) or jnp.any(eps >= 1):
        raise ValueError(
            "The JL bound is defined for eps in ]0, 1[, got %r" % eps)

    if jnp.any(n_samples <= 0):
        raise ValueError(
    ...
    return (4 * jnp.log(n_samples) / denominator).astype(jnp.int64)


def _check_density(density, n_features):
    """Factorize density check according to Li et al."""
    if density == "auto":
        density = 1 / jnp.sqrt(n_features)
...
def _gaussian_random_matrix(n_components, n_features, random_state=None):
    ...
    components = rng.normal(
        loc=0.0, scale=1.0 / jnp.sqrt(n_components), size=(n_components, n_features)
    )
    return components
...
def _sparse_random_matrix(n_components, n_features, density="auto", random_state=None):
    ...
    if density == 1:
        ...
        return 1 / jnp.sqrt(n_components) * components
        ...
        data = rng.binomial(1, 0.5, size=jnp.size(indices)) * 2 - 1
        ...
        return jnp.sqrt(1 / density) / jnp.sqrt(n_components) * components
...
class BaseRandomProjection(
    ...
    def inverse_transform(self, X):
        ...
        X = check_array(X, dtype=[jnp.float64, jnp.float32],
...
class GaussianRandomProjection(BaseRandomProjection):
    ...
    def transform(self, X):
        ...
        check_is_fitted(self)
        X = self._validate_data(
            X, accept_sparse=["csr", "csc"], reset=False, dtype=[jnp.float64, jnp.float32]
        )
...
class SparseRandomProjection(BaseRandomProjection):
    ...
    >>> jnp.mean(transformer.components_ != 0)
    0.0182...
    """
