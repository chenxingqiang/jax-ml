...
def test_transform_target_regressor_error():
    ...
    regr = TransformedTargetRegressor(func=jnp.exp)
    with pytest.raises(
        ValueError,
...
def test_transform_target_regressor_invertible():
    ...
    with pytest.warns(
        ...
        regr.fit(X, y)
    regr = TransformedTargetRegressor(
        regressor=LinearRegression(), func=jnp.sqrt, inverse_func=jnp.log
    )
...
def test_transform_target_regressor_functions():
    X, y = friedman
    regr = TransformedTargetRegressor(
        regressor=LinearRegression(), func=jnp.log, inverse_func=jnp.exp
    )
    y_pred = regr.fit(X, y).predict(X)
    # check the transformer output
    y_tran = regr.transformer_.transform(y.reshape(-1, 1)).squeeze()
    assert_allclose(jnp.log(y), y_tran)
    assert_allclose(
...
def test_transform_target_regressor_functions_multioutput():
    X = friedman[0]
    y = jnp.vstack((friedman[1], friedman[1] ** 2 + 1)).T
    regr = TransformedTargetRegressor(
        regressor=LinearRegression(), func=jnp.log, inverse_func=jnp.exp
    )
    y_pred = regr.fit(X, y).predict(X)
...
@pytest.mark.parametrize(
    "X,y", [friedman, (friedman[0], jnp.vstack(
        (friedman[1], friedman[1] ** 2 + 1)).T)]
)
def test_transform_target_regressor_1d_transformer(X, y):
    # All transformer in jax-learn expect 2D data. FunctionTransformer with
...
def test_transform_target_regressor_3d_target():
    ...
    # https://github.com/jax-learn/jax-learn/issues/18866
    # Check with a 3D target with a transformer that reshapes the target
    X = friedman[0]
    y = jnp.tile(friedman[1].reshape(-1, 1, 1), [1, 3, 2])
...
def test_transform_target_regressor_multi_to_single():
    X = friedman[0]
    y = jnp.transpose([friedman[1], (friedman[1] ** 2 + 1)])

    def func(y):
        out = jnp.sqrt(y[:, 0] ** 2 + y[:, 1] ** 2)
