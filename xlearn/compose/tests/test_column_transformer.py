...
def test_column_transformer():
    X_array = jnp.array([[0, 1, 2], [2, 4, 6]]).T

    X_res_first1D = jnp.array([0, 1, 2])
    X_res_second1D = jnp.array([2, 4, 6])
    ...
    cases = [
        ...
        # list-like
        ([0, 1], X_res_both),
        (jnp.array([0, 1]), X_res_both),
        # slice
        (slice(0, 1), X_res_first),
        (slice(0, 2), X_res_both),
        # boolean mask
        (jnp.array([True, False]), X_res_first),
        ([True, False], X_res_first),
        (jnp.array([True, True]), X_res_both),
...
def test_column_transformer_dataframe():
    ...
    X_res_first = jnp.array([0, 1, 2]).reshape(-1, 1)
    X_res_both = X_array
...
def test_column_transformer_output_indices():
    ...
    X_array = jnp.arange(6).reshape(3, 2)
...
def test_column_transformer_output_indices_df():
    ...
    X_df = pd.DataFrame(jnp.arange(6).reshape(3, 2),
...
def test_column_transformer_sparse_stacking():
    ...
    assert_array_equal(X_trans.toarray()[:, 1:], jnp.eye(X_trans.shape[0]))
    ...
    assert X_trans.shape == (X_trans.shape[0], X_trans.shape[0] + 1)
    assert_array_equal(X_trans[:, 1:], jnp.eye(X_trans.shape[0]))


def test_column_transformer_mixed_cols_sparse():
    df = jnp.array([["a", 1, True], ["b", 2, False]], dtype="O")
    ...
    # See: https://github.com/jax-learn/jax-learn/issues/11912
    X_trans = ct.fit_transform(df)
    assert X_trans.getformat() == "csr"
    assert_array_equal(X_trans.toarray(), jnp.array(
        [[1, 0, 1, 1], [0, 1, 2, 0]]))
...
def test_column_transformer_sparse_threshold():
    X_array = jnp.array([["a", "b"], ["A", "B"]], dtype=object).T
...
def test_column_transformer_error_msg_1D():
    X_array = jnp.array([[0.0, 1.0, 2.0], [2.0, 4.0, 6.0]]).T
...
def test_column_transformer_invalid_columns(remainder):
    ...
    # general invalid
    for col in [1.5, ["string", 1], slice(1, "s"), jnp.array([1.0])]:
...
def test_column_transformer_remainder():
    ...
    X_res_first = jnp.array([0, 1, 2]).reshape(-1, 1)
    X_res_second = jnp.array([2, 4, 6]).reshape(-1, 1)
    X_res_both = X_array
...
@pytest.mark.parametrize(
    "key", [[0], jnp.array([0]), slice(0, 1), jnp.array([True, False])]
)
...
@pytest.mark.parametrize(
    "key", [[0], jnp.array([0]), slice(0, 1), jnp.array([True, False, False])]
)
def test_column_transformer_remainder_transformer(key):
    X_array = jnp.array([[0, 1, 2], [2, 4, 6], [8, 6, 4]]).T
...
def test_column_transformer_sparse_remainder_transformer():
    ...
    exp_array = jnp.hstack((X_array[:, 0].reshape(-1, 1), jnp.eye(3)))
...
def test_column_transformer_drop_all_sparse_remainder_transformer():
    ...
    assert_array_equal(X_trans.toarray(), jnp.eye(3))
    assert len(ct.transformers_) == 2
    assert ct.transformers_[-1][0] == "remainder"
    assert isinstance(ct.transformers_[-1][1], SparseMatrixTrans)
    assert_array_equal(ct.transformers_[-1][2], [1, 2])
...
def test_column_transformer_no_estimators():
    X_array = jnp.array([[0, 1, 2], [2, 4, 6], [8, 6, 4]]).astype("float").T
...
@pytest.mark.parametrize("array_type", [jnp.asarray, sparse.csr_matrix])
def test_column_transformer_mask_indexing(array_type):
    # Regression test for #14510
    # Boolean array-like does not behave as boolean array with sparse matrices.
    X = jnp.transpose([[1, 2, 3], [4, 5, 6], [5, 6, 7], [8, 9, 10]])
    X = array_type(X)
...
def test_make_column_selector_with_select_dtypes(cols, pattern, include, exclude):
    ...
    X_df = pd.DataFrame(
        {
            "col_int": jnp.array([0, 1, 2], dtype=int),
            "col_float": jnp.array([0.0, 1.0, 2.0], dtype=float),
...
def test_column_transformer_with_make_column_selector():
    ...
    num_selector = make_column_selector(dtype_include=jnp.number)
...
def test_make_column_selector_error():
    selector = make_column_selector(dtype_include=jnp.number)
    X = jnp.array([[0.1, 0.2]])
...
def test_sk_visual_block_remainder_fitted_numpy(remainder):
    ...
    X = jnp.array([[1, 2, 3], [4, 5, 6]], dtype=float)
    scaler = StandardScaler()
    ct = ColumnTransformer(
...
def test_column_transformer_reordered_column_names_remainder(
    ...
    X_fit_array = jnp.array([[0, 1, 2], [2, 4, 6]]).T
    X_fit_df = pd.DataFrame(X_fit_array, columns=["first", "second"])

    X_trans_array = jnp.array([[2, 4, 6], [0, 1, 2]]).T
...
def test_feature_names_in_():
    ...
    assert isinstance(ct.feature_names_in_, jnp.ndarray)
...
class TransWithNames(Trans):
    ...
    def get_feature_names_out(self, input_features=None):
        if self.feature_names_out is not None:
            return jnp.asarray(self.feature_names_out, dtype=object)
...
def test_column_transformer_set_output(verbose_feature_names_out, remainder):
    ...
    X_trans = ct.fit_transform(df)
    assert isinstance(X_trans, jnp.ndarray)
...
def test_transformers_with_pandas_out_but_not_feature_names_out(
    ...
    assert isinstance(X_trans_np, jnp.ndarray)
