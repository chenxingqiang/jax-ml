"""
The :mod:`xlearn.naive_bayes` module implements Naive Bayes algorithms. These
...
class _BaseNB(ClassifierMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def predict_log_proba(self, X):
        ...
        return jll - jnp.atleast_2d(log_prob_x).T
    ...
    def predict_proba(self, X):
        ...
        C : array-like of shape (n_samples, n_classes)
            ...
            order, as they appear in the attribute :term:`classes_`.
        """
        return jnp.exp(self.predict_log_proba(X))
...
class GaussianNB(_BaseNB):
    ...
    [1]
    >>> clf_pf = GaussianNB()
    >>> clf_pf.partial_fit(X, Y, jnp.unique(Y))
    GaussianNB()
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        return self._partial_fit(
            X, y, jnp.unique(y), _refit=True, sample_weight=sample_weight
        )
    ...
    def _update_mean_variance(n_past, mu, var, X, sample_weight=None):
        ...
        if sample_weight is not None:
            ...
            if jnp.isclose(n_new, 0.0):
                return mu, var
            new_mu = jnp.average(X, axis=0, weights=sample_weight)
            new_var = jnp.average((X - new_mu) ** 2, axis=0,
    ...
    def _partial_fit(self, X, y, classes=None, _refit=False, sample_weight=None):
        ...
        if first_call:
            ...
            self.class_count_ = jnp.zeros(n_classes, dtype=jnp.float64)
            ...
            if self.priors is not None:
                ...
                if not jnp.isclose(priors.sum(), 1.0):
                ...
                self.class_prior_ = jnp.zeros(
                    len(self.classes_), dtype=jnp.float64)
        else:
        ...
        unique_y_in_classes = jnp.in1d(unique_y, classes)

        if not jnp.all(unique_y_in_classes):
            raise ValueError(
    ...
    def _joint_log_likelihood(self, X):
        ...
        for i in range(jnp.size(self.classes_)):
            jointi = jnp.log(self.class_prior_[i])
            n_ij = -0.5 * jnp.sum(jnp.log(2.0 * jnp.pi * self.var_[i, :]))
            n_ij -= 0.5 * \
                jnp.sum(((X - self.theta_[i, :]) ** 2) / (self.var_[i, :]), 1)
...
class _BaseDiscreteNB(_BaseNB):
    ...
    def _update_class_log_prior(self, class_prior=None):
        ...
        if class_prior is not None:
            if len(class_prior) != n_classes:
                raise ValueError(
                    "Number of priors must match number of classes.")
            self.class_log_prior_ = jnp.log(class_prior)
        elif self.fit_prior:
            with warnings.catch_warnings():
                # silence the warning when count is 0 because class was not yet
                # observed
                warnings.simplefilter("ignore", RuntimeWarning)
                log_class_count = jnp.log(self.class_count_)

            # empirical prior, with sample_weight taken into account
            self.class_log_prior_ = log_class_count - \
                jnp.log(self.class_count_.sum())
        else:
            self.class_log_prior_ = jnp.full(n_classes, -jnp.log(n_classes))

    def _check_alpha(self):
        alpha = (
            jnp.asarray(self.alpha) if not isinstance(
        ...
        if alpha_min < alpha_lower_bound and not _force_alpha:
            ...
            return jnp.maximum(alpha, alpha_lower_bound)
        return alpha
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        if Y.shape[1] == 1:
            if len(self.classes_) == 2:
                Y = jnp.concatenate((1 - Y, Y), axis=1)
        ...
        # label_binarize() returns arrays with dtype=jnp.int64.
        # We convert it to jnp.float64 to support sample_weight consistently
        Y = Y.astype(jnp.float64, copy=False)
        if sample_weight is not None:
            sample_weight = _check_sample_weight(sample_weight, X)
            sample_weight = jnp.atleast_2d(sample_weight)
            Y *= sample_weight.T
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # LabelBinarizer().fit_transform() returns arrays with dtype=jnp.int64.
        # We convert it to jnp.float64 to support sample_weight consistently;
    ...
    def _init_counters(self, n_classes, n_features):
        self.class_count_ = jnp.zeros(n_classes, dtype=jnp.float64)
        self.feature_count_ = jnp.zeros(
            (n_classes, n_features), dtype=jnp.float64)
...
class MultinomialNB(_BaseDiscreteNB):
    ...
    def _update_feature_log_prob(self, alpha):
        ...
        self.feature_log_prob_ = jnp.log(smoothed_fc) - jnp.log(
...
class ComplementNB(_BaseDiscreteNB):
    ...
    def _update_feature_log_prob(self, alpha):
        ...
        logged = jnp.log(comp_count / comp_count.sum(axis=1, keepdims=True))
...
class BernoulliNB(_BaseDiscreteNB):
    ...
    def _joint_log_likelihood(self, X):
        ...
        neg_prob = jnp.log(1 - jnp.exp(self.feature_log_prob_))
...
class CategoricalNB(_BaseDiscreteNB):
    ...
    n_categories_ : ndarray of shape (n_features,), dtype=jnp.int64
    ...
    def _init_counters(self, n_classes, n_features):
        self.class_count_ = jnp.zeros(n_classes, dtype=jnp.float64)
        self.category_count_ = [jnp.zeros((n_classes, 0))
    ...
    def _validate_n_categories(X, min_categories):
        ...
        min_categories_ = jnp.array(min_categories)
        if min_categories is not None:
            if not jnp.issubdtype(min_categories_.dtype, jnp.signedinteger):
                raise ValueError(
                    "'min_categories' should have integral type. Got "
                    f"{min_categories_.dtype} instead."
                )
            n_categories_ = jnp.maximum(
                n_categories_X, min_categories_, dtype=jnp.int64)
    ...
    def _count(self, X, Y):
        def _update_cat_count_dims(cat_count, highest_feature):
            ...
            if diff > 0:
                ...
                return jnp.pad(cat_count, [(0, 0), (0, diff)], "constant")
            return cat_count

        def _update_cat_count(X_feature, Y, cat_count, n_classes):
            for j in range(n_classes):
                mask = Y[:, j].astype(bool)
                if Y.dtype.type == jnp.int64:
                    weights = None
                else:
                    weights = Y[mask, j]
                counts = jnp.bincount(X_feature[mask], weights=weights)
                indices = jnp.nonzero(counts)[0]
    ...
    def _update_feature_log_prob(self, alpha):
        ...
        for i in range(self.n_features_in_):
            ...
            feature_log_prob.append(
                jnp.log(smoothed_cat_count) -
                jnp.log(smoothed_class_count.reshape(-1, 1))
            )
        self.feature_log_prob_ = feature_log_prob

    def _joint_log_likelihood(self, X):
        self._check_n_features(X, reset=False)
        jll = jnp.zeros((X.shape[0], self.class_count_.shape[0]))
        for i in range(self.n_features_in_):
            indices = X[:, i]
