...
def sag_solver(
    ...
    if "coef" in warm_start_mem.keys():
        ...
        coef_init = jnp.zeros((n_features, n_classes), dtype=X.dtype, order="C")
    ...
    if fit_intercept:
        ...
        intercept_init = jnp.zeros(n_classes, dtype=X.dtype)

    if "intercept_sum_gradient" in warm_start_mem.keys():
        intercept_sum_gradient = warm_start_mem["intercept_sum_gradient"]
    else:
        intercept_sum_gradient = jnp.zeros(n_classes, dtype=X.dtype)

    if "gradient_memory" in warm_start_mem.keys():
        gradient_memory_init = warm_start_mem["gradient_memory"]
    else:
        gradient_memory_init = jnp.zeros(
            (n_samples, n_classes), dtype=X.dtype, order="C"
        )
    if "sum_gradient" in warm_start_mem.keys():
        sum_gradient_init = warm_start_mem["sum_gradient"]
    else:
        sum_gradient_init = jnp.zeros(
            (n_features, n_classes), dtype=X.dtype, order="C")

    if "seen" in warm_start_mem.keys():
        seen_init = warm_start_mem["seen"]
    else:
        seen_init = jnp.zeros(n_samples, dtype=jnp.int32, order="C")
    ...
    sag = sag64 if X.dtype == jnp.float64 else sag32
    ...
    if fit_intercept:
        coef_init = jnp.vstack((coef_init, intercept_init))
