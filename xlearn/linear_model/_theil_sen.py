...
_EPSILON = jnp.finfo(jnp.double).eps
...
def _modified_weiszfeld_step(X, x_old):
    ...
    """
    diff = X - x_old
    diff_norm = jnp.sqrt(jnp.sum(diff**2, axis=1))
    mask = diff_norm >= _EPSILON
    # x_old equals one of our samples
    is_x_old_in_X = int(mask.sum() < X.shape[0])

    diff = diff[mask]
    diff_norm = diff_norm[mask][:, jnp.newaxis]
    quotient_norm = linalg.norm(jnp.sum(diff / diff_norm, axis=0))

    if quotient_norm > _EPSILON:  # to avoid division by zero
        new_direction = jnp.sum(X[mask, :] / diff_norm, axis=0) / jnp.sum(
            1 / diff_norm, axis=0
        )
    else:
        new_direction = 1.0
...
def _spatial_median(X, max_iter=300, tol=1.0e-3):
    ...
    """
    if X.shape[1] == 1:
        return 1, jnp.median(X.ravel(), keepdims=True)

    tol **= 2  # We are computing the tol on the squared norm
    spatial_median_old = jnp.mean(X, axis=0)

    for n_iter in range(max_iter):
        spatial_median = _modified_weiszfeld_step(X, spatial_median_old)
        if jnp.sum((spatial_median_old - spatial_median) ** 2) < tol:
            break
        else:
...
def _lstsq(X, y, indices, fit_intercept):
    ...
    weights = jnp.empty((indices.shape[0], n_features))
    X_subpopulation = jnp.ones((n_subsamples, n_features))
    # gelss need to pad y_subpopulation to be of the max dim of X_subpopulation
    y_subpopulation = jnp.zeros((max(n_subsamples, n_features)))
...
class TheilSenRegressor(RegressorMixin, LinearModel):
    ...
    def _check_subparams(self, n_samples, n_features):
        ...
        all_combinations = max(1, jnp.rint(binom(n_samples, n_subsamples)))
    ...
    def fit(self, X, y):
        ...
        if jnp.rint(binom(n_samples, n_subsamples)) <= self.max_subpopulation:
        ...
        index_list = jnp.array_split(indices, n_jobs)
