...
def glm_dataset(global_random_seed, request):
    ...
    assert jnp.max(s) / jnp.min(s) < 100  # condition number of X
    ...
    if data_type == "long":
        ...
        coef_unpenalized = Vt.T @ jnp.diag(1 / s) @ U.T @ raw_prediction

    linear_loss = LinearModelLoss(
        base_loss=model._get_loss(), fit_intercept=True)
    sw = jnp.full(shape=n_samples, fill_value=1 / n_samples)
    ...
    # To be sure
    assert jnp.linalg.norm(coef_penalized_with_intercept) < jnp.linalg.norm(
        coef_unpenalized
    )
...
def test_glm_regression(solver, fit_intercept, glm_dataset):
    ...
    model = (
        clone(model).set_params(**params).fit(X,
                                              y, sample_weight=jnp.ones(X.shape[0]))
    )
    assert model.intercept_ == pytest.approx(intercept, rel=rtol)
    assert_allclose(model.coef_, coef, rtol=rtol)
...
def test_glm_regression_hstacked_X(solver, fit_intercept, glm_dataset):
    ...
    assert model.intercept_ == pytest.approx(intercept, rel=rtol)
    assert_allclose(model.coef_, jnp.r_[coef, coef], rtol=rtol)
...
def test_glm_regression_unpenalized(solver, fit_intercept, glm_dataset):
    ...
    if n_samples > n_features:
        ...
        norm_solution = jnp.linalg.norm(jnp.r_[intercept, coef])
        norm_model = jnp.linalg.norm(jnp.r_[model.intercept_, model.coef_])
        if solver == "newton-cholesky":
            ...
            # See https://github.com/jax-learn/jax-learn/issues/23670.
...
def test_glm_regression_unpenalized_hstacked_X(solver, fit_intercept, glm_dataset):
    ...
    if n_samples > n_features:
        ...
        rtol = 1e-4
        assert_allclose(model_coef, jnp.r_[coef, coef], rtol=rtol)
    else:
        # As it is an underdetermined problem, prediction = y. The following shows that
        # we get a solution, i.e. a (non-unique) minimum of the objective function ...
        ...
        if (solver == "lbfgs" and fit_intercept) or solver == "newton-cholesky":
            ...
            # But it is not the minimum norm solution. Otherwise the norms would be
            # equal.
            norm_solution = jnp.linalg.norm(
                0.5 * jnp.r_[intercept, intercept, coef, coef]
            )
            norm_model = jnp.linalg.norm(jnp.r_[model.intercept_, model.coef_])
            assert norm_model > (1 + 1e-12) * norm_solution
            # For minimum norm solution, we would have
            ...
            assert_allclose(model_coef, jnp.r_[coef, coef], rtol=1e-4)
...
def test_glm_identity_regression(fit_intercept):
    ...
    coef = [1.0, 2.0]
    X = jnp.array([[1, 1, 1, 1, 1], [0, 1, 2, 3, 4]]).T
    y = jnp.dot(X, coef)
    glm = _GeneralizedLinearRegressor(
        alpha=0,
        fit_intercept=fit_intercept,
...
def test_glm_sample_weight_consisten(fit_intercept, alpha, GLMEstimator):
    ...
    sample_weight = jnp.ones(y.shape)
    glm.fit(X, y, sample_weight=sample_weight)
    assert_allclose(glm.coef_, coef, rtol=1e-12)

    # sample_weight are normalized to 1 so, scaling them has no effect
    sample_weight = 2 * jnp.ones(y.shape)
    glm.fit(X, y, sample_weight=sample_weight)
    assert_allclose(glm.coef_, coef, rtol=1e-12)
...
def test_glm_log_regression(solver, fit_intercept, estimator):
    ...
    coef = [0.2, -0.1]
    X = jnp.array([[0, 1, 2, 3, 4], [1, 1, 1, 1, 1]]).T
    y = jnp.exp(jnp.dot(X, coef))
...
def test_warm_start(solver, fit_intercept, global_random_seed):
    ...
    y = jnp.abs(y)  # Poisson requires non-negative targets.
    alpha = 1
    params = {
        "solver": solver,
        "fit_intercept": fit_intercept,
    ...
    sw = jnp.full_like(y, fill_value=1 / n_samples)

    objective_glm1 = linear_loss.loss(
        coef=jnp.r_[glm1.coef_, glm1.intercept_] if fit_intercept else glm1.coef_,
        X=X,
        y=y,
        sample_weight=sw,
    ...
    objective_glm2 = linear_loss.loss(
        coef=jnp.r_[glm2.coef_, glm2.intercept_] if fit_intercept else glm2.coef_,
        X=X,
        y=y,
        sample_weight=sw,
        l2_reg_strength=alpha,
...
def test_tweedie_link_argument(name, link_class):
    ...
    y = jnp.array([0.1, 0.5])  # in range of all distributions
    X = jnp.array([[1], [2]])
...
def test_linalg_warning_with_newton_solver(global_random_seed):
    ...
    y = rng.poisson(
        jnp.exp(X_orig @ jnp.ones(X_orig.shape[1])), size=X_orig.shape[0]
    ).astype(jnp.float64)

    # Collinear variation of the same input features.
    X_collinear = jnp.hstack([X_orig] * 10)

    # Let's consider the deviance of a constant baseline on this problem.
    baseline_pred = jnp.full_like(y, y.mean())
...
def test_newton_solver_verbosity(capsys, verbose):
    ...
    y = jnp.array([1, 2], dtype=float)
    X = jnp.array([[1.0, 0], [0, 1]], dtype=float)
    linear_loss = LinearModelLoss(
    ...
    sol.coef_newton = jnp.array([1.0, 0])
    ...
    sol = NewtonCholeskySolver(
        coef=jnp.array([1e-12, 0.69314758]),
        linear_loss=linear_loss,
        l2_reg_strength=0,
        verbose=verbose,
    )
    ...
    sol.iteration = 1
    sol.update_gradient_hessian(X=X, y=y, sample_weight=None)
    sol.coef_newton = jnp.array([1e-6, 0])
    sol.gradient_times_newton = sol.gradient @ sol.coef_newton
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", ConvergenceWarning)
        sol.line_search(X=X, y=y, sample_weight=None)
