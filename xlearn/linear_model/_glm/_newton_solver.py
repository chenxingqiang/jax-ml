...
class NewtonSolver(ABC):
    ...
    def fallback_lbfgs_solve(self, X, y, sample_weight):
        ...
        opt_res = scipy.optimize.minimize(
            ...
            options={
                ...
                "gtol": self.tol,
                "ftol": 64 * jnp.finfo(jnp.float64).eps,
            },
    ...
    def line_search(self, X, y, sample_weight):
        ...
        eps = 16 * jnp.finfo(self.loss_value.dtype).eps
        t = 1  # step size
        ...
        # jnp.sum(jnp.abs(self.gradient_old))
        sum_abs_grad_old = -1
        ...
        for i in range(21):  # until and including t = beta**20 ~ 1e-6
            ...
            tiny_loss = jnp.abs(self.loss_value_old * eps)
            check = jnp.abs(loss_improvement) <= tiny_loss
            if is_verbose:
                print(
                    "      check loss |improvement| <= eps * |loss_old|:"
                    f" {jnp.abs(loss_improvement)} <= {tiny_loss} {check}"
                )
            if check:
    ...
    def check_convergence(self, X, y, sample_weight):
        ...
        # check = jnp.max(jnp.abs(coef_step) / jnp.maximum(1, jnp.abs(self.coef_old)))

        # 1. Criterion: maximum |gradient| <= tol
        #    The gradient was already updated in line_search()
        check = jnp.max(jnp.abs(self.gradient))
        if self.verbose:
...
class NewtonCholeskySolver(NewtonSolver):
    ...
    def setup(self, X, y, sample_weight):
        ...
        if self.linear_loss.fit_intercept:
            n_dof += 1
        self.gradient = jnp.empty_like(self.coef)
        self.hessian = jnp.empty_like(self.coef, shape=(n_dof, n_dof))
        ...
        try:
            with warnings.catch_warnings():
                ...
                if self.gradient_times_newton > 0:
                    ...
                    self.use_fallback_lbfgs_solve = True
                    return
        except (jnp.linalg.LinAlgError, scipy.linalg.LinAlgWarning) as e:
            warnings.warn(
