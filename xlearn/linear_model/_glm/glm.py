...
# some parts and tricks stolen from other xlearn files.
...
class _GeneralizedLinearRegressor(RegressorMixin, BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.solver == "lbfgs":
            ...
            # (and contiguous).
            loss_dtype = jnp.float64
        else:
            loss_dtype = min(max(y.dtype, X.dtype), jnp.float64)
        ...
        if self.warm_start and hasattr(self, "coef_"):
            if self.fit_intercept:
                ...
                coef = jnp.concatenate(
                    (self.coef_, jnp.array([self.intercept_])))
            else:
                coef = self.coef_
            ...
            if self.fit_intercept:
                coef[-1] = linear_loss.base_loss.link.link(
                    jnp.average(y, weights=sample_weight)
                )
    ...
    def score(self, X, y, sample_weight=None):
        ...
        # https://github.com/jax-learn/jax-learn/pull/22118 is merged.
        #
        ...
        constant = jnp.mean(base_loss.constant_to_optimal_zero(y_true=y))
        if sample_weight is not None:
            constant *= sample_weight.shape[0] / jnp.sum(sample_weight)
        ...
        y_mean = base_loss.link.link(jnp.average(y, weights=sample_weight))
        deviance_null = base_loss(
            y_true=y,
            raw_prediction=jnp.tile(y_mean, y.shape[0]),
            sample_weight=sample_weight,
            n_threads=1,
        )
