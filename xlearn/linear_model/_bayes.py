...
class BayesianRidge(RegressorMixin, LinearModel):
    ...
    X_offset_ : ndarray of shape (n_features,)
        ...
        zero mean. Set to jnp.zeros(n_features) otherwise.

    X_scale_ : ndarray of shape (n_features,)
        Set to jnp.ones(n_features).
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        X, y = self._validate_data(
            X, y, dtype=[jnp.float64, jnp.float32], y_numeric=True)
        ...
        # Add `eps` in the denominator to omit division by zero if `jnp.var(y)`
        # is zero
        alpha_ = self.alpha_init
        lambda_ = self.lambda_init
        if alpha_ is None:
            alpha_ = 1.0 / (jnp.var(y) + eps)
        if lambda_ is None:
            lambda_ = 1.0
        ...
        for iter_ in range(max_iter):
            ...
            gamma_ = jnp.sum((alpha_ * eigen_vals_) /
                            (lambda_ + alpha_ * eigen_vals_))
            lambda_ = (gamma_ + 2 * lambda_1) / \
                (jnp.sum(coef_**2) + 2 * lambda_2)
            alpha_ = (n_samples - gamma_ + 2 * alpha_1) / (rmse_ + 2 * alpha_2)

            # Check for convergence
            if iter_ != 0 and jnp.sum(jnp.abs(coef_old_ - coef_)) < self.tol:
                if verbose:
                    print("Convergence after ", str(iter_), " iterations")
                break
            coef_old_ = jnp.copy(coef_)
        ...
        if self.compute_score:
            ...
            self.scores_.append(s)
            self.scores_ = jnp.array(self.scores_)

        # posterior covariance is given by 1/alpha_ * scaled_sigma_
        scaled_sigma_ = jnp.dot(
            Vh.T, Vh / (eigen_vals_ + lambda_ / alpha_)[:, jnp.newaxis]
        )
    ...
    def predict(self, X, return_std=False):
        ...
        if not return_std:
            ...
            sigmas_squared_data = (jnp.dot(X, self.sigma_) * X).sum(axis=1)
            y_std = jnp.sqrt(sigmas_squared_data + (1.0 / self.alpha_))
            return y_mean, y_std
    ...
    def _update_coef_(
        ...
        scaled_sigma_ = (lambda_/alpha_ * jnp.eye(n_features)
                         + jnp.dot(X.T, X))^-1
        """

        if n_samples > n_features:
            coef_ = jnp.linalg.multi_dot(
    ...
    def _log_marginal_likelihood(
        ...
        # sigma = (lambda_ * jnp.eye(n_features) + alpha_ * jnp.dot(X.T, X))^-1
        if n_samples > n_features:
            logdet_sigma = -jnp.sum(jnp.log(lambda_ + alpha_ * eigen_vals))
        else:
            logdet_sigma = jnp.full(n_features, lambda_,
                                   dtype=jnp.array(lambda_).dtype)
            logdet_sigma[:n_samples] += alpha_ * eigen_vals
            logdet_sigma = -jnp.sum(jnp.log(logdet_sigma))
        ...
        score += 0.5 * (
            ...
            - alpha_ * rmse
            - lambda_ * jnp.sum(coef**2)
            + logdet_sigma
            - n_samples * log(2 * jnp.pi)
        )
...
class ARDRegression(RegressorMixin, LinearModel):
    ...
    def fit(self, X, y):
        ...
        X, y = self._validate_data(
            X, y, dtype=[jnp.float64, jnp.float32], y_numeric=True, ensure_min_samples=2
        )

        n_samples, n_features = X.shape
        coef_ = jnp.zeros(n_features, dtype=X.dtype)
        ...
        # is zero
        alpha_ = 1.0 / (jnp.var(y) + eps)
        lambda_ = jnp.ones(n_features, dtype=X.dtype)

        self.scores_ = list()
        coef_old_ = None

        def update_coeff(X, y, coef_, alpha_, keep_lambda, sigma_):
            coef_[keep_lambda] = alpha_ * jnp.linalg.multi_dot(
        ...
        for iter_ in range(max_iter):
            ...
            if self.compute_score:
                s = (lambda_1 * jnp.log(lambda_) - lambda_2 * lambda_).sum()
                s += alpha_1 * log(alpha_) - alpha_2 * alpha_
                s += 0.5 * (
                    fast_logdet(sigma_)
                    + n_samples * log(alpha_)
                    + jnp.sum(jnp.log(lambda_))
                )
    ...
    def _update_sigma_woodbury(self, X, alpha_, lambda_, keep_lambda):
        ...
        sigma_ = pinvh(
            jnp.eye(n_samples, dtype=X.dtype) / alpha_
            + jnp.dot(X_keep * inv_lambda, X_keep.T)
        )
        sigma_ = jnp.dot(sigma_, X_keep * inv_lambda)
        sigma_ = -jnp.dot(inv_lambda.reshape(-1, 1) * X_keep.T, sigma_)
        sigma_[jnp.diag_indices(sigma_.shape[1])] += 1.0 / lambda_[keep_lambda]
        return sigma_
    ...
    def _update_sigma(self, X, alpha_, lambda_, keep_lambda):
        ...
        eye = jnp.eye(gram.shape[0], dtype=X.dtype)
