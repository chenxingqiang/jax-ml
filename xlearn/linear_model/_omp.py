...
def _cholesky_omp(X, y, n_nonzero_coefs, tol=None, copy_X=True, return_path=False):
    ...
    min_float = jnp.finfo(X.dtype).eps
    ...
    gamma = jnp.empty(0)
    n_active = 0
    indices = jnp.arange(X.shape[1])  # keeping track of swapping
    ...
    if return_path:
        coefs = jnp.empty_like(L)

    while True:
        lam = jnp.argmax(jnp.abs(jnp.dot(X.T, residual)))
        ...
        if n_active > 0:
            ...
            L[n_active, :n_active] = jnp.dot(X[:, :n_active].T, X[:, lam])
        ...
        residual = y - jnp.dot(X[:, :n_active], gamma)
...
def _gram_omp(
    ...
    n_active : int
        Number of active features at convergence.
    """
    Gram = Gram.copy("F") if copy_Gram else jnp.asfortranarray(Gram)

    if copy_Xy or not Xy.flags.writeable:
        Xy = Xy.copy()

    min_float = jnp.finfo(Gram.dtype).eps
    nrm2, swap = linalg.get_blas_funcs(("nrm2", "swap"), (Gram,))
    (potrs,) = get_lapack_funcs(("potrs",), (Gram,))

    indices = jnp.arange(len(Gram))  # keeping track of swapping
    alpha = Xy
    tol_curr = tol_0
    delta = 0
    gamma = jnp.empty(0)
    ...
    while True:
        ...
        if return_path:
            coefs[:n_active, n_active - 1] = gamma
        beta = jnp.dot(Gram[:, :n_active], gamma)
        alpha = Xy - beta
        if tol is not None:
            tol_curr += delta
            delta = jnp.inner(gamma, beta[:n_active])
            tol_curr -= delta
            if abs(tol_curr) <= tol:
                break
        elif n_active == max_features:
...
def orthogonal_mp(
    ...
    *,
    ...
    if precompute:
        G = jnp.dot(X.T, X)
        G = jnp.asfortranarray(G)
        Xy = jnp.dot(X.T, y)
        if tol is not None:
            norms_squared = jnp.sum((y**2), axis=0)
        else:
            norms_squared = None
    ...
    if return_path:
        coef = jnp.zeros((X.shape[1], y.shape[1], X.shape[1]))
    else:
        coef = jnp.zeros((X.shape[1], y.shape[1]))
    n_iters = []
    ...
    if return_n_iter:
        return jnp.squeeze(coef), n_iters
    else:
        return jnp.squeeze(coef)
...
def orthogonal_mp_gram(
    ...
    *,
    ...
    if return_path:
        coef = jnp.zeros((len(Gram), Xy.shape[1], len(Gram)), dtype=Gram.dtype)
    else:
        coef = jnp.zeros((len(Gram), Xy.shape[1]), dtype=Gram.dtype)
...
class OrthogonalMatchingPursuit(MultiOutputMixin, RegressorMixin, LinearModel):
    ...
    def fit(self, X, y):
        ...
        if Gram is False:
            ...
            norms_sq = jnp.sum(y**2, axis=0) if self.tol is not None else None
...
def _omp_path_residues(
    ...
    return jnp.dot(coefs.T, X_test.T) - y_test
...
class OrthogonalMatchingPursuitCV(RegressorMixin, LinearModel):
    ...
    def fit(self, X, y):
        ...
        best_n_nonzero_coefs = jnp.argmin(mse_folds.mean(axis=0)) + 1
