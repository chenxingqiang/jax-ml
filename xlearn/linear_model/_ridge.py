...
def _solve_sparse_cg(
    ...
    if sample_weight_sqrt is None:
        sample_weight_sqrt = jnp.ones(X.shape[0], dtype=X.dtype)
    ...
    coefs = jnp.empty((y.shape[1], n_features), dtype=X.dtype)
...
def _solve_lsqr(
    ...
    *,
    ...
    n_samples, n_features = X.shape
    coefs = jnp.empty((y.shape[1], n_features), dtype=X.dtype)
    n_iter = jnp.empty(y.shape[1], dtype=jnp.int32)
...
def _solve_cholesky(X, y, alpha):
    ...
    one_alpha = jnp.array_equal(alpha, len(alpha) * [alpha[0]])

    if one_alpha:
        A.flat[:: n_features + 1] += alpha[0]
        return linalg.solve(A, Xy, assume_a="pos", overwrite_a=True).T
    else:
        coefs = jnp.empty([n_targets, n_features], dtype=X.dtype)
...
def _solve_cholesky_kernel(K, y, alpha, sample_weight=None, copy=False):
    ...
    has_sw = isinstance(sample_weight, jnp.ndarray) or sample_weight not in [
        1.0, None]

    if has_sw:
        # Unlike other solvers, we need to support sample_weight directly
        # because K might be a pre-computed kernel.
        sw = jnp.sqrt(jnp.atleast_1d(sample_weight))
        y = y * sw[:, jnp.newaxis]
        K *= jnp.outer(sw, sw)
    ...
    if one_alpha:
        ...
        if has_sw:
            dual_coef *= sw[:, jnp.newaxis]

        return dual_coef
    else:
        # One penalty per target. We need to solve each target separately.
        dual_coefs = jnp.empty([n_targets, n_samples], K.dtype)
...
def _solve_svd(X, y, alpha):
    ...
    s_nnz = s[idx][:, jnp.newaxis]
    UTy = jnp.dot(U.T, y)
    d = jnp.zeros((s.size, alpha.size), dtype=X.dtype)
...
def _solve_lbfgs(
    ...
    if positive:
        config["bounds"] = [(0, jnp.inf)] * n_features
...
def _ridge_regression(
    ...
    if check_input:
        _dtype = [jnp.float64, jnp.float32]
    ...
    if alpha is not None and not isinstance(alpha, jnp.ndarray):
        alpha = check_scalar(
            alpha,
            "alpha",
    ...
    alpha = jnp.asarray(alpha, dtype=X.dtype).ravel()
    ...
    if alpha.size == 1 and n_targets > 1:
        alpha = jnp.repeat(alpha, n_targets)
    ...
    if solver == "sparse_cg":
        ...
        coef = jnp.empty((y.shape[1], n_features), dtype=X.dtype)
        n_iter = jnp.empty(y.shape[1], dtype=jnp.int32)
        intercept = jnp.zeros((y.shape[1],), dtype=X.dtype)
        for i, (alpha_i, target) in enumerate(zip(alpha, y.T)):
            init = {
                "coef": jnp.zeros((n_features + int(return_intercept), 1), dtype=X.dtype)
            }
...
class _BaseRidge(LinearModel, metaclass=ABCMeta):
    _parameter_constraints: dict = {
        "alpha": [Interval(Real, 0, None, closed="left"), jnp.ndarray],
...
def _find_smallest_angle(query, vectors):
    ...
    """
    abs_cosine = jnp.abs(query.dot(vectors))
    index = jnp.argmax(abs_cosine)
    return index
...
class _X_CenterStackOp(sparse.linalg.LinearOperator):
    ...
    jnp.hstack([X - sqrt_sw[:, None] * X_mean, sqrt_sw[:, None]])
    """
...
class _XT_CenterStackOp(sparse.linalg.LinearOperator):
    ...
    This operator behaves as
    jnp.hstack([X - sqrt_sw[:, None] * X_mean, sqrt_sw[:, None]]).T
    """
    ...
    def _matvec(self, v):
        ...
        res = jnp.empty(n_features, dtype=self.X.dtype)
        res[:-1] = safe_sparse_dot(self.X.T, v, dense_output=True) - (
            self.X_mean * self.sqrt_sw.dot(v)
        )
        res[-1] = jnp.dot(v, self.sqrt_sw)
        return res

    def _matmat(self, v):
        n_features = self.shape[0]
        res = jnp.empty((n_features, v.shape[1]), dtype=self.X.dtype)
        res[:-1] = safe_sparse_dot(self.X.T, v, dense_output=True) - self.X_mean[
            :, None
        ] * self.sqrt_sw.dot(v)
        res[-1] = jnp.dot(self.sqrt_sw, v)
        return res
...
class _RidgeGCV(LinearModel):
    ...
    def _diag_dot(D, B):
        ...
        if len(B.shape) > 1:
            ...
            D = D[(slice(None),) + (jnp.newaxis,) * (len(B.shape) - 1)]
        return D * B
    ...
    def _compute_gram(self, X, sqrt_sw):
        ...
        if not center:
            ...
            X_mean = jnp.zeros(X.shape[1], dtype=X.dtype)
        ...
        X_mX_m = jnp.outer(sqrt_sw, sqrt_sw) * jnp.dot(X_mean, X_mean)
        return (
    ...
    def _compute_covariance(self, X, sqrt_sw):
        ...
        return (
            ...
            - weight_sum * jnp.outer(X_mean, X_mean),
            X_mean,
        )
    ...
    def _sparse_multidot_diag(self, X, A, X_mean, sqrt_sw):
        ...
        Returns
        -------
        diag : jnp.ndarray, shape (n_samples,)
            The computed diagonal.
        """
        intercept_col = scale = sqrt_sw
        batch_size = X.shape[1]
        diag = jnp.empty(X.shape[0], dtype=X.dtype)
    ...
    def _solve_eigen_gram(self, alpha, y, sqrt_sw, X_mean, eigvals, Q, QT_y):
        ...
        if self.fit_intercept:
            ...
            # sum(sample_weight).
            normalized_sw = sqrt_sw / jnp.linalg.norm(sqrt_sw)
            intercept_dim = _find_smallest_angle(normalized_sw, Q)
            w[intercept_dim] = 0  # cancel regularization for the intercept

        c = jnp.dot(Q, self._diag_dot(w, QT_y))
        G_inverse_diag = self._decomp_diag(w, Q)
        # handle case where y is 2-d
        if len(y.shape) != 1:
            G_inverse_diag = G_inverse_diag[:, jnp.newaxis]
    ...
    def _eigen_decompose_covariance(self, X, y, sqrt_sw):
        ...
        """
        n_samples, n_features = X.shape
        cov = jnp.empty((n_features + 1, n_features + 1), dtype=X.dtype)
    ...
    def _solve_eigen_covariance_intercept(
        ...
        intercept_sv = jnp.zeros(V.shape[0])
        intercept_sv[-1] = 1
    ...
    def _svd_decompose_design_matrix(self, X, y, sqrt_sw):
        ...
        if self.fit_intercept:
            ...
            X = jnp.hstack((X, intercept_column))
    ...
    def _solve_svd_design_matrix(self, alpha, y, sqrt_sw, X_mean, singvals_sq, U, UT_y):
        ...
        c = jnp.dot(U, self._diag_dot(w, UT_y)) + (alpha**-1) * y
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.alphas = jnp.asarray(self.alphas)
        ...
        if sample_weight is not None:
            ...
            sqrt_sw = jnp.ones(n_samples, dtype=X.dtype)
        ...
        n_alphas = 1 if jnp.ndim(self.alphas) == 0 else len(self.alphas)

        if self.store_cv_values:
            self.cv_values_ = jnp.empty(
                (n_samples * n_y, n_alphas), dtype=X.dtype)

        best_coef, best_score, best_alpha = None, None, None

        for i, alpha in enumerate(jnp.atleast_1d(self.alphas)):
            ...
            if best_score is None:
                ...
                if self.alpha_per_target and n_y > 1:
                    best_coef = c
                    best_score = jnp.atleast_1d(alpha_score)
                    best_alpha = jnp.full(n_y, alpha)
                else:
                    best_coef = c
...
class _BaseRidgeCV(LinearModel):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if isinstance(self.alphas, (jnp.ndarray, list, tuple)):
            n_alphas = 1 if jnp.ndim(self.alphas) == 0 else len(self.alphas)
            if n_alphas != 1:
        ...
        alphas = jnp.asarray(self.alphas)
