...
def _set_order(X, y, order="C"):
    ...
    if order is not None:
        ...
        if sparse_X:
            ...
            X = jnp.asarray(X, order=order)
        if sparse_y:
            y = y.asformat(sparse_format)
        else:
            y = jnp.asarray(y, order=order)
    return X, y
...
def _alpha_grid(
    ...
    Xy : array-like of shape (n_features,) or (n_features, n_outputs),\
         default=None
        Xy = jnp.dot(X.T, y) that can be precomputed.
    ...
    if sparse_center:
        if fit_intercept:
            Xy -= mean_dot[:, jnp.newaxis]

    alpha_max = jnp.sqrt(jnp.sum(Xy**2, axis=1)).max() / (n_samples * l1_ratio)

    if alpha_max <= jnp.finfo(float).resolution:
        alphas = jnp.empty(n_alphas)
        alphas.fill(jnp.finfo(float).resolution)
        return alphas

    return jnp.geomspace(alpha_max, alpha_max * eps, num=n_alphas)
    ...
    **params : kwargs
    ...
    if not multi_output and sparse.isspmatrix(X):
        if X_offset_param is not None:
            ...
            X_sparse_scaling = jnp.asarray(X_sparse_scaling, dtype=X.dtype)
        else:
            X_sparse_scaling = jnp.zeros(n_features, dtype=X.dtype)
    ...
    if alphas is None:
        ...
        alphas = jnp.sort(alphas)[::-1]  # make sure alphas are properly ordered

    n_alphas = len(alphas)
    dual_gaps = jnp.empty(n_alphas)
    n_iters = []
    ...
    if not multi_output:
        coefs = jnp.empty((n_features, n_alphas), dtype=X.dtype)
    else:
        coefs = jnp.empty((n_targets, n_features, n_alphas), dtype=X.dtype)

    if coef_init is None:
        coef_ = jnp.zeros(coefs.shape[:-1], dtype=X.dtype, order="F")
    else:
        coef_ = jnp.asfortranarray(coef_init, dtype=X.dtype)
    ...
    for i, alpha in enumerate(alphas):
        ...
        elif isinstance(precompute, jnp.ndarray):
...
class ElasticNet(MultiOutputMixin, RegressorMixin, LinearModel):
    ...
    def fit(self, X, y, sample_weight=None, check_input=True):
        ...
        if sample_weight is not None:
            ...
            #
            #    1/2 * jnp.average(squared error, weights=sw)
            #    + alpha * penalty                                             (1)
            #
            ...
            sample_weight = sample_weight * (n_samples / jnp.sum(sample_weight))
            # Note: Alternatively, we could also have rescaled alpha instead
            # of sample_weight:
            #
            #     alpha *= jnp.sum(sample_weight) / n_samples
        ...
        if not self.warm_start or not hasattr(self, "coef_"):
            coef_ = jnp.zeros((n_targets, n_features), dtype=X.dtype, order="F")
        else:
            coef_ = self.coef_
            if coef_.ndim == 1:
                coef_ = coef_[jnp.newaxis, :]

        dual_gaps_ = jnp.zeros(n_targets, dtype=X.dtype)
        self.n_iter_ = []
        ...
        if not all(jnp.isfinite(w).all() for w in [self.coef_, self.intercept_]):
            raise ValueError(
...
def _path_residuals(
    ...
    if sample_weight is None:
        ...
        sw_train *= n_samples / jnp.sum(sw_train)
        # Note: Alternatively, we could also have rescaled alpha instead
        # of sample_weight:
        #
        #     alpha *= jnp.sum(sample_weight) / n_samples
    ...
    if y.ndim == 1:
        ...
        coefs = coefs[jnp.newaxis, :, :]
        y_offset = jnp.atleast_1d(y_offset)
        y_test = y_test[:, jnp.newaxis]

    intercepts = y_offset[:, jnp.newaxis] - jnp.dot(X_offset, coefs)
    X_test_coefs = safe_sparse_dot(X_test, coefs)
    residues = X_test_coefs - y_test[:, :, jnp.newaxis]
    residues += intercepts
    if sample_weight is None:
        this_mse = (residues**2).mean(axis=0)
    else:
        this_mse = jnp.average(residues**2, weights=sw_test, axis=0)
...
class LinearModelCV(MultiOutputMixin, LinearModel, ABC):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        check_y_params = dict(
            copy=False, dtype=[jnp.float64, jnp.float32], ensure_2d=False
        )
        if isinstance(X, jnp.ndarray) or sparse.isspmatrix(X):
            ...
            check_X_params = dict(
                accept_sparse="csc", dtype=[jnp.float64, jnp.float32], copy=False
            )
            X, y = self._validate_data(
                X, y, validate_separately=(check_X_params, check_y_params)
            )
            if sparse.isspmatrix(X):
                if hasattr(reference_to_old_X, "data") and not jnp.may_share_memory(
                    reference_to_old_X.data, X.data
                ):
                    # X is a sparse matrix and has been copied
                    copy_X = False
            elif not jnp.may_share_memory(reference_to_old_X, X):
                # X has been copied
                copy_X = False
        ...
        if "l1_ratio" in path_params:
            l1_ratios = jnp.atleast_1d(path_params["l1_ratio"])
        ...
        if alphas is None:
            ...
            alphas = jnp.tile(jnp.sort(alphas)[::-1], (n_l1_ratio, 1))
        ...
        mse_paths = jnp.reshape(mse_paths, (n_l1_ratio, len(folds), -1))
        # The mean is computed over folds.
        mean_mse = jnp.mean(mse_paths, axis=1)
        self.mse_path_ = jnp.squeeze(jnp.moveaxis(mse_paths, 2, 1))
        for l1_ratio, l1_alphas, mse_alphas in zip(l1_ratios, alphas, mean_mse):
            i_best_alpha = jnp.argmin(mse_alphas)
        ...
        if self.alphas is None:
            self.alphas_ = jnp.asarray(alphas)
            if n_l1_ratio == 1:
                self.alphas_ = self.alphas_[0]
        # Remove duplicate alphas in case alphas is provided.
        else:
            self.alphas_ = jnp.asarray(alphas[0])
...
class MultiTaskElasticNet(Lasso):
    ...
    def fit(self, X, y):
        ...
        self.coef_ = jnp.asfortranarray(self.coef_)  # coef contiguous in memory
