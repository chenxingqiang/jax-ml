        ...
        if solver in ["highs", "highs-ds", "highs-ipm"]:
            ...
            if self.fit_intercept:
                ones = sparse.csc_matrix(
                    jnp.ones(shape=(n_indices, 1), dtype=X.dtype))
                A_eq = sparse.hstack(
            ...
            if self.fit_intercept:
                ...
                A_eq = jnp.concatenate([ones, X, -ones, -X, eye, -eye], axis=1)
            else:
                A_eq = jnp.concatenate([X, -X, eye, -eye], axis=1)
