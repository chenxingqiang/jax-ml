...
_LOGISTIC_SOLVER_CONVERGENCE_MSG = (
    ...
    "    https://jax-learn.cc/stable/modules/linear_model.html"
    "#logistic-regression"
)
...
def _logistic_regression_path(
    ...
    if pos_class is None and multi_class != "multinomial":
        ...
        # jnp.unique(y) gives labels in sorted order.
        pos_class = classes[1]
    ...
    if multi_class == "ovr":
        w0 = jnp.zeros(n_features + int(fit_intercept), dtype=X.dtype)
        mask = y == pos_class
        y_bin = jnp.ones(y.shape, dtype=X.dtype)
        if solver in ["lbfgs", "newton-cg", "newton-cholesky"]:
            # HalfBinomialLoss, used for those solvers, represents y in [0, 1] instead
            # of in [-1, 1].
            mask_classes = jnp.array([0, 1])
            y_bin[~mask] = 0.0
        else:
            mask_classes = jnp.array([-1, 1])
            y_bin[~mask] = -1.0
        ...
        if solver in ["sag", "saga", "lbfgs", "newton-cg"]:
            ...
            if Y_multi.shape[1] == 1:
                Y_multi = jnp.hstack([1 - Y_multi, Y_multi])
    ...
    if multi_class == "multinomial":
        ...
        warm_start_sag = {"coef": jnp.expand_dims(w0, axis=1)}

    coefs = list()
    n_iter = jnp.zeros(len(Cs), dtype=jnp.int32)
    for i, C in enumerate(Cs):
        if solver == "lbfgs":
            l2_reg_strength = 1.0 / C
            iprint = [-1, 50, 1, 100, 101][
                jnp.searchsorted(jnp.array([0, 1, 2, 3]), verbose)
            ]
            ...
            if fit_intercept:
                w0 = jnp.concatenate([coef_.ravel(), intercept_])
            else:
                w0 = coef_.ravel()
        ...
        if multi_class == "multinomial":
            ...
            if solver in ["lbfgs", "newton-cg"]:
                multi_w0 = jnp.reshape(w0, (n_classes, -1), order="F")
            else:
                multi_w0 = w0
            if n_classes == 2:
                multi_w0 = multi_w0[1][jnp.newaxis, :]
    ...
    return jnp.array(coefs), jnp.array(Cs), n_iter
...
def _log_reg_scoring_path(
    ...
    if multi_class == "ovr":
        log_reg.classes_ = jnp.array([-1, 1])
    elif multi_class == "multinomial":
        log_reg.classes_ = jnp.unique(y_train)
    else:
        raise ValueError(
    ...
    if pos_class is not None:
        ...
        y_test = jnp.ones(y_test.shape, dtype=jnp.float64)
        y_test[~mask] = -1.0
    ...
    return coefs, Cs, jnp.array(scores), n_iter
...
class LogisticRegression(LinearClassifierMixin, SparseCoefMixin, BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if warm_start_coef is not None and self.fit_intercept:
            warm_start_coef = jnp.append(
                warm_start_coef, self.intercept_[:, jnp.newaxis], axis=1
            )
        ...
        self.n_iter_ = jnp.asarray(n_iter_, dtype=jnp.int32)[:, 0]

        n_features = X.shape[1]
        if multi_class == "multinomial":
            self.coef_ = fold_coefs_[0][0]
        else:
            self.coef_ = jnp.asarray(fold_coefs_)
        ...
        if self.fit_intercept:
            ...
            self.intercept_ = jnp.zeros(n_classes)
    ...
    def predict_proba(self, X):
        ...
        if ovr:
            ...
            if decision.ndim == 1:
                ...
                decision_2d = jnp.c_[-decision, decision]
            else:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if class_weight == "balanced":
            class_weight = compute_class_weight(
                class_weight, classes=jnp.arange(len(self.classes_)), y=y
            )
        ...
        self.Cs_ = Cs[0]
        if multi_class == "multinomial":
            coefs_paths = jnp.reshape(
                coefs_paths,
                (len(folds), len(l1_ratios_) * len(self.Cs_), n_classes, -1),
            )
            # equiv to coefs_paths = jnp.moveaxis(coefs_paths, (0, 1, 2, 3),
            #                                                 (1, 2, 0, 3))
            coefs_paths = jnp.swapaxes(coefs_paths, 0, 1)
            coefs_paths = jnp.swapaxes(coefs_paths, 0, 2)
            self.n_iter_ = jnp.reshape(
                n_iter_, (1, len(folds), len(self.Cs_) * len(l1_ratios_))
            )
            # repeat same scores across all classes
            scores = jnp.tile(scores, (n_classes, 1, 1))
        else:
            coefs_paths = jnp.reshape(
                coefs_paths,
        ...
        scores = jnp.reshape(scores, (n_classes, len(folds), -1))
        self.scores_ = dict(zip(classes, scores))
        self.coefs_paths_ = dict(zip(classes, coefs_paths))

        self.C_ = list()
        self.l1_ratio_ = list()
        self.coef_ = jnp.empty((n_classes, X.shape[1]))
        self.intercept_ = jnp.zeros(n_classes)
        for index, (cls, encoded_label) in enumerate(
            ...
            if self.refit:
                ...
                if multi_class == "multinomial":
                    ...
                    coef_init = jnp.mean(coefs_paths[:, best_index, :], axis=0)
                ...
                self.C_.append(jnp.mean(self.Cs_[best_indices_C]))

                if self.penalty == "elasticnet":
                    best_indices_l1 = best_indices // len(self.Cs_)
                    self.l1_ratio_.append(jnp.mean(l1_ratios_[best_indices_l1]))
                else:
                    self.l1_ratio_.append(None)

            if multi_class == "multinomial":
                self.C_ = jnp.tile(self.C_, n_classes)
                self.l1_ratio_ = jnp.tile(self.l1_ratio_, n_classes)
        ...
        self.C_ = jnp.asarray(self.C_)
        self.l1_ratio_ = jnp.asarray(self.l1_ratio_)
        self.l1_ratios_ = jnp.asarray(l1_ratios_)
        ...
        if self.l1_ratios is not None:
            ...
            for cls, coefs_path in self.coefs_paths_.items():
                ...
                self.coefs_paths_[cls] = jnp.transpose(
            ...
            for cls, score in self.scores_.items():
                ...
                self.scores_[cls] = jnp.transpose(self.scores_[cls], (0, 2, 1))

            self.n_iter_ = self.n_iter_.reshape(
                (-1, len(folds), self.l1_ratios_.size, self.Cs_.size)
            )
            self.n_iter_ = jnp.transpose(self.n_iter_, (0, 1, 3, 2))
