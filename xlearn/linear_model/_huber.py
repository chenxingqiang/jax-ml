...
def _huber_loss_and_gradient(w, X, y, epsilon, alpha, sample_weight=None):
    ...
    abs_linear_loss = jnp.abs(linear_loss)
    outliers_mask = abs_linear_loss > epsilon * sigma

    # Calculate the linear loss due to the outliers.
    # This is equal to (2 * M * |y - X'w -c / sigma| - M**2) * sigma
    outliers = abs_linear_loss[outliers_mask]
    num_outliers = jnp.count_nonzero(outliers_mask)
    n_non_outliers = X.shape[0] - num_outliers

    # n_sq_outliers includes the weight give to the outliers while
    # num_outliers is just the number of outliers.
    outliers_sw = sample_weight[outliers_mask]
    n_sw_outliers = jnp.sum(outliers_sw)
    outlier_loss = (
        2.0 * epsilon * jnp.sum(outliers_sw * outliers)
    ...
    weighted_loss = jnp.dot(weighted_non_outliers.T, non_outliers)
    squared_loss = weighted_loss / sigma

    if fit_intercept:
        grad = jnp.zeros(n_features + 2)
    else:
        grad = jnp.zeros(n_features + 1)
    ...
    signed_outliers = jnp.ones_like(outliers)
    ...
    if fit_intercept:
        grad[-2] = -2.0 * jnp.sum(weighted_non_outliers) / sigma
        grad[-2] -= 2.0 * epsilon * jnp.sum(sw_outliers)

    loss = n_samples * sigma + squared_loss + outlier_loss
    loss += alpha * jnp.dot(w, w)
    return loss, grad
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.warm_start and hasattr(self, "coef_"):
            ...
            if self.fit_intercept:
                parameters = jnp.zeros(X.shape[1] + 2)
            else:
                parameters = jnp.zeros(X.shape[1] + 1)
        ...
        bounds = jnp.tile([-jnp.inf, jnp.inf], (parameters.shape[0], 1))
        bounds[-1][0] = jnp.finfo(jnp.float64).eps * 10
        ...
        residual = jnp.abs(y - safe_sparse_dot(X, self.coef_) - self.intercept_)
