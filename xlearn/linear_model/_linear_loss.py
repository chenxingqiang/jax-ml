...
class LinearModelLoss:
    ...
    def init_zero_coef(self, X, dtype=None):
        ...
        if self.base_loss.is_multiclass:
            ...
            coef = jnp.zeros_like(X, shape=n_dof, dtype=dtype)
        return coef
    ...
    def loss_gradient(
        ...
        if not self.base_loss.is_multiclass:
            grad = jnp.empty_like(coef, dtype=weights.dtype)
            grad[:n_features] = X.T @ grad_pointwise + \
                l2_reg_strength * weights
            if self.fit_intercept:
                grad[-1] = grad_pointwise.sum()
        else:
            grad = jnp.empty((n_classes, n_dof), dtype=weights.dtype, order="F")
    ...
    def gradient_hessian(
        ...
        # positive.
        hessian_warning = jnp.mean(hess_pointwise <= 0) > 0.25
        hess_pointwise = jnp.abs(hess_pointwise)
        ...
        if not self.base_loss.is_multiclass:
            ...
            if hessian_out is None:
                hess = jnp.empty(shape=(n_dof, n_dof), dtype=weights.dtype)
            else:
                hess = hessian_out
            ...
            if sparse.issparse(X):
                ...
                # jnp.einsum may use less memory but the following, using BLAS matrix
                # multiplication (gemm), is by far faster.
                WX = hess_pointwise[:, None] * X
                hess[:n_features, :n_features] = jnp.dot(X.T, WX)
    ...
    def gradient_hessian_product(
        ...
        if not self.base_loss.is_multiclass:
            ...
            def hessp(s):
                ret = jnp.empty_like(s)
                if sparse.issparse(X):
                    ret[:n_features] = X.T @ (hX @ s[:n_features])
                else:
                    ret[:n_features] = jnp.linalg.multi_dot(
                        [X.T, hX, s[:n_features]])
            ...
            #
            # See also https://github.com/jax-learn/jax-learn/pull/3646#discussion_r17461411  # noqa
            def hessp(s):
                ...
                tmp += (-proba * tmp).sum(axis=1)[:, jnp.newaxis]  # - sum_l ..
                tmp *= proba  # * p_i_k
                if sample_weight is not None:
                    tmp *= sample_weight[:, jnp.newaxis]
                # hess_prod = empty_like(grad), but we ravel grad below and this
                # function is run after that.
                hess_prod = jnp.empty((n_classes, n_dof),
