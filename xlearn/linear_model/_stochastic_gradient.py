...
class _ValidationScoreCallback:
    ...
    def __call__(self, coef, intercept):
        ...
        est.intercept_ = jnp.atleast_1d(intercept)
...
class BaseSGD(SparseCoefMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    _parameter_constraints: dict = {
        ...
        "warm_start": ["boolean"],
        "average": [Interval(Integral, 0, None, closed="left"), bool, jnp.bool_],
    }
    ...
    def _allocate_parameter_mem(
        ...
        if n_classes > 2:
            ...
            if coef_init is not None:
                coef_init = jnp.asarray(coef_init, dtype=input_dtype, order="C")
            ...
            if coef_init is not None:
                ...
                self.coef_ = jnp.zeros(n_features, dtype=input_dtype, order="C")

            # allocate intercept_
            if intercept_init is not None:
                intercept_init = jnp.asarray(intercept_init, dtype=input_dtype)
                ...
                if one_class:
                    self.offset_ = jnp.zeros(1, dtype=input_dtype, order="C")
                else:
                    self.intercept_ = jnp.zeros(1, dtype=input_dtype, order="C")

        # initialize average parameters
        if self.average > 0:
            self._standard_coef = self.coef_
            self._average_coef = jnp.zeros(
            ...
            self._average_intercept = jnp.zeros(
    ...
    def _make_validation_split(self, y, sample_mask):
        ...
        """
        n_samples = y.shape[0]
        validation_mask = jnp.zeros(n_samples, dtype=jnp.bool_)
        ...
        idx_train, idx_val = next(cv.split(jnp.zeros(shape=(y.shape[0], 1)), y))

        if not jnp.any(sample_mask[idx_val]):
            raise ValueError(
...
def _prepare_fit_binary(est, y, i, input_dtye):
    ...
    """
    y_i = jnp.ones(y.shape, dtype=input_dtye, order="C")
...
def fit_binary(
    ...
    classes = jnp.array([-1, 1], dtype=y_i.dtype)
    ...
    tol = est.tol if est.tol is not None else -jnp.inf
...
def _get_plain_sgd_function(input_dtype):
    return _plain_sgd32 if input_dtype == jnp.float32 else _plain_sgd64
...
class BaseSGDClassifier(LinearClassifierMixin, BaseSGD, metaclass=ABCMeta):
    ...
    def _fit(
        ...
        # jnp.unique sorts in asc order; largest class id is positive class
    ...
    def _fit_binary(self, X, y, alpha, C, sample_weight, learning_rate, max_iter):
        ...
        if self.average > 0:
            if self.average <= self.t_ - 1:
                ...
                self._standard_intercept = jnp.atleast_1d(intercept)
                self.intercept_ = self._standard_intercept
        else:
            self.coef_ = coef.reshape(1, -1)
            # intercept is a float, need to convert it to an array of length 1
            self.intercept_ = jnp.atleast_1d(intercept)
    ...
    def _fit_multiclass(self, X, y, alpha, C, learning_rate, sample_weight, max_iter):
        ...
        if self.average > 0:
            if self.average <= self.t_ - 1.0:
                ...
                self._standard_intercept = jnp.atleast_1d(self.intercept_)
                self.intercept_ = self._standard_intercept
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        classes : ndarray of shape (n_classes,), default=None
            ...
            Can be obtained by via `jnp.unique(y_all)`, where y_all is the
...
class SGDClassifier(BaseSGDClassifier):
    ...
    def predict_proba(self, X):
        ...
        if self.loss == "log_loss":
            ...
            if binary:
                prob2 = jnp.ones((scores.shape[0], 2))
                prob = prob2[:, 1]
            else:
                prob = scores

            jnp.clip(scores, -1, 1, prob)
            prob += 1.0
            prob /= 2.0
...
class BaseSGDRegressor(RegressorMixin, BaseSGD):
    ...
    def _partial_fit(
        ...
        if self.average > 0 and getattr(self, "_average_coef", None) is None:
            self._average_coef = jnp.zeros(n_features, dtype=X.dtype, order="C")
            self._average_intercept = jnp.zeros(1, dtype=X.dtype, order="C")
    ...
    def _fit_regressor(
        ...
        tol = self.tol if self.tol is not None else -jnp.inf
        ...
        if self.average > 0:
            self._average_intercept = jnp.atleast_1d(average_intercept)
            self._standard_intercept = jnp.atleast_1d(intercept)

            if self.average <= self.t_ - 1.0:
                # made enough updates for averaging to be taken into account
                self.coef_ = average_coef
                self.intercept_ = jnp.atleast_1d(average_intercept)
            else:
                self.coef_ = coef
                self.intercept_ = jnp.atleast_1d(intercept)
...
class SGDOneClassSVM(BaseSGD, OutlierMixin):
    ...
    def _fit_one_class(self, X, alpha, C, sample_weight, learning_rate, max_iter):
        """Uses SGD implementation with X and y=jnp.ones(n_samples)."""

        # The One-Class SVM uses the SGD implementation with
        # y=jnp.ones(n_samples).
        n_samples = X.shape[0]
        y = jnp.ones(n_samples, dtype=X.dtype, order="C")
        ...
        # numpy mtrand expects a C long which is a signed 32 bit integer under
        # Windows
        seed = random_state.randint(0, jnp.iinfo(jnp.int32).max)
        ...
        if self.average > 0:
            ...
            if self.average <= self.t_ - 1.0:
                # made enough updates for averaging to be taken into account
                self.coef_ = average_coef
                self.offset_ = 1 - jnp.atleast_1d(average_intercept)
            else:
                self.coef_ = coef
                self.offset_ = 1 - jnp.atleast_1d(intercept)
    ...
    def predict(self, X):
        ...
        """
        y = (self.decision_function(X) >= 0).astype(jnp.int32)
