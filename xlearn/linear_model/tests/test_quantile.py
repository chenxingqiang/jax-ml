...
def test_quantile_equals_huber_for_low_epsilon(fit_intercept, default_solver):
    ...
    if fit_intercept:
        ...
        assert jnp.mean(y < quant.predict(X)) == approx(0.5, abs=1e-1)
...
def test_quantile_estimates_calibration(q, default_solver):
    ...
    assert jnp.mean(y < quant.predict(X)) == approx(q, abs=1e-2)
...
def test_quantile_sample_weight(default_solver):
    ...
    fraction_below = jnp.mean(y < quant.predict(X))
    assert fraction_below > 0.5
    weighted_fraction_below = jnp.average(y < quant.predict(X), weights=weight)
...
def test_asymmetric_error(quantile, default_solver):
    ...
    assert jnp.min(X @ coef + intercept) > 0
...
def test_equivariance(quantile, default_solver):
    ...
    assert_allclose(model2.coef_, jnp.linalg.solve(A, model1.coef_), rtol=1e-5)
...
def test_linprog_failure():
    ...
    X = jnp.linspace(0, 10, num=10).reshape(-1, 1)
    y = jnp.linspace(0, 10, num=10)
...
def test_sparse_input(sparse_format, solver, fit_intercept, default_solver):
    ...
    if fit_intercept:
        ...
        # check that we still predict fraction
        assert 0.45 <= jnp.mean(y < quant_sparse.predict(X_sparse)) <= 0.57
...
def test_error_interior_point_future(X_y_data, monkeypatch):
    ...
    with monkeypatch.context() as m:
        m.setattr(xlearn.linear_model._quantile,
