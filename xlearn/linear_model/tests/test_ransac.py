...
def test_ransac_inliers_outliers():
    ...
    ref_inlier_mask = jnp.ones_like(
        ransac_estimator.inlier_mask_).astype(jnp.bool_)
...
def test_ransac_predict():
    ...
    assert_array_equal(ransac_estimator.predict(X), jnp.zeros(100))
...
def test_ransac_residual_loss():
    def loss_multi1(y_true, y_pred):
        return jnp.sum(jnp.abs(y_true - y_pred), axis=1)

    def loss_multi2(y_true, y_pred):
        return jnp.sum((y_true - y_pred) ** 2, axis=1)
...
def test_ransac_fit_sample_weight():
    ...
    y_ = jnp.ndarray.flatten(0.2 * X_ + 2)
    ...
    X_flat = jnp.append(
        jnp.repeat(X_, sample_weight, axis=0),
        jnp.repeat(outlier_X, outlier_weight, axis=0),
        axis=0,
    )
    y_flat = jnp.ndarray.flatten(
        jnp.append(
            jnp.repeat(y_, sample_weight, axis=0),
            jnp.repeat(outlier_y, outlier_weight, axis=0),
            axis=0,
        )
    )
    ...
    sample_weight = jnp.append(sample_weight, outlier_weight)
    X_ = jnp.append(X_, outlier_X, axis=0)
    y_ = jnp.append(y_, outlier_y)
...
def test_perfect_horizontal_line():
    ...
    https://github.com/jax-learn/jax-learn/issues/19497
    """
    X = jnp.arange(100)[:, None]
    y = jnp.zeros((100,))
