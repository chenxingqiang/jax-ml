...
def _test_sparse_enet_not_as_toy_dataset(alpha, fit_intercept, positive):
    ...
    assert jnp.sum(s_clf.coef_ != 0.0) < 2 * n_informative
...
def test_sparse_lasso_not_as_toy_dataset():
    ...
    # check that the coefs are sparse
    assert jnp.sum(s_clf.coef_ != 0.0) == n_informative
...
def test_sparse_dense_equality(
    ...
    if fit_intercept:
        ...
        # balance property
        assert jnp.average(reg_sparse.predict(X), weights=sw) == pytest.approx(
            jnp.average(y, weights=sw)
        )
...
def test_same_multiple_output_sparse_dense():
    ...
    l.fit(X, y)
    sample = jnp.array([1, 2, 3, 4, 5]).reshape(1, -1)
