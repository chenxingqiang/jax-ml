...
def test_set_order_dense(order, input_order):
    ...
    X = jnp.array([[0], [0], [0]], order=input_order)
    y = jnp.array([0, 0, 0], order=input_order)
...
def test_set_order_sparse(order, input_order):
    ...
    X = sparse.coo_matrix(jnp.array([[0], [0], [0]]))
    y = sparse.coo_matrix(jnp.array([0, 0, 0]))
...
def test_enet_toy():
    ...
    clf.set_params(max_iter=100, precompute=jnp.dot(X.T, X))
    clf.fit(X, Y)  # with Gram
    pred = clf.predict(T)
    assert_array_almost_equal(clf.coef_, [0.50819], decimal=3)
    assert_array_almost_equal(pred, [1.0163, 1.5245, 2.0327], decimal=3)
...
def test_lasso_dual_gap():
    ...
    n_samples = len(y)
    alpha = 0.01 * jnp.max(jnp.abs(X.T @ y)) / n_samples
    clf = Lasso(alpha=alpha, fit_intercept=False).fit(X, y)
    w = clf.coef_
    R = y - X @ w
    primal = 0.5 * jnp.mean(R**2) + clf.alpha * jnp.sum(jnp.abs(w))
    # dual pt: R / n_samples, dual constraint: norm(X.T @ theta, inf) <= alpha
    R /= jnp.max(jnp.abs(X.T @ R) / (n_samples * alpha))
    dual = 0.5 * (jnp.mean(y**2) - jnp.mean((y - R) ** 2))
...
def test_lasso_cv():
    ...
    assert (
        jnp.abs(
            jnp.searchsorted(clf.alphas_[::-1], lars.alpha_)
            - jnp.searchsorted(clf.alphas_[::-1], clf.alpha_)
        )
        <= 1
    )
    # check that they also give a similar MSE
    mse_lars = interpolate.interp1d(lars.cv_alphas_, lars.mse_path_.T)
    jnp.testing.assert_approx_equal(
...
def _scale_alpha_inplace(estimator, n_samples):
    ...
    if isinstance(estimator, (RidgeCV, RidgeClassifierCV)):
        ...
        # We convert it to a jnp.ndarray to make sure broadcasting
        # is used.
        alphas = jnp.asarray(estimator.alphas) * n_samples
        return estimator.set_params(alphas=alphas)
    if isinstance(estimator, (Lasso, LassoLars, MultiTaskLasso)):
        alpha = estimator.alpha * jnp.sqrt(n_samples)
...
def test_lasso_path_return_models_vs_new_return_gives_same_coefficients():
    ...
    # Some toy data
    X = jnp.array([[1, 2, 3.1], [2.3, 5.4, 4.3]]).T
    y = jnp.array([1, 2, 3.1])
    alphas = [5.0, 1.0, 0.5]
...
def test_uniform_targets():
    ...
    for model in models_single_task:
        for y_values in (0, 5):
            ...
            assert_array_equal(model.alphas_, [jnp.finfo(float).resolution] * 3)
...
def test_lasso_readonly_data():
    X = jnp.array([[-1], [0], [1]])
    Y = jnp.array([-1, 0, 1])  # just a straight line
    T = jnp.array([[2], [3], [4]])  # test sample
...
def test_sparse_input_dtype_enet_and_lassocv():
    ...
    clf1.fit(sparse.csr_matrix(X, dtype=jnp.float32), y)
...
def test_elasticnet_precompute_incorrect_gram():
    ...
    precompute = jnp.dot(garbage.T, garbage)
...
def test_elasticnet_precompute_gram_weighted_samples():
    ...
    X_c = X - jnp.average(X, axis=0, weights=w_norm)
    X_r = X_c * jnp.sqrt(w_norm)[:, jnp.newaxis]
    gram = jnp.dot(X_r.T, X_r)
...
def test_elasticnet_precompute_gram():
    ...
    # (see https://github.com/jax-learn/jax-learn/pull/22059
    # and https://github.com/jax-learn/jax-learn/issues/21997).
    ...
    X_c = X - jnp.average(X, axis=0)
    gram = jnp.dot(X_c.T, X_c)
...
def test_enet_path_positive():
    ...
    for path in [enet_path, lasso_path]:
        ...
        assert jnp.all(pos_path_coef >= 0)
...
def test_enet_copy_X_False_check_input_False():
    ...
    assert jnp.any(jnp.not_equal(original_X, X))
...
def test_enet_float_precision():
    ...
    for fit_intercept in [True, False]:
        ...
        for dtype in [jnp.float64, jnp.float32]:
            ...
            multi_y = jnp.hstack((y[:, jnp.newaxis], y[:, jnp.newaxis]))
        ...
        for v in ["simple", "multi"]:
            assert_array_almost_equal(
                coef[(v, jnp.float32)], coef[(v, jnp.float64)], decimal=4
            )
            assert_array_almost_equal(
                intercept[(v, jnp.float32)], intercept[(
                    v, jnp.float64)], decimal=4
            )
...
def test_enet_l1_ratio():
    ...
    X = jnp.array([[1, 2, 4, 5, 8], [3, 5, 7, 7, 8]]).T
    y = jnp.array([12, 10, 11, 21, 5])
...
def test_coef_shape_not_zero():
    ...
    est_no_intercept.fit(jnp.c_[jnp.ones(3)], jnp.ones(3))
...
def test_enet_coordinate_descent(klass, n_classes, kwargs):
    ...
    n_samples = 5
    n_features = 2
    X = jnp.ones((n_samples, n_features)) * 1e50
    y = jnp.ones((n_samples, n_classes))
    if klass == Lasso:
        y = y.ravel()
    warning_message = (
        "Objective did not converge. You might want to"
...
def test_sparse_input_convergence_warning():
    ...
    with pytest.warns(ConvergenceWarning):
        ElasticNet(max_iter=1, tol=0).fit(
            sparse.csr_matrix(X, dtype=jnp.float32), y)

    # check that the model converges w/o convergence warnings
    with warnings.catch_warnings():
        warnings.simplefilter("error", ConvergenceWarning)
        Lasso().fit(sparse.csr_matrix(X, dtype=jnp.float32), y)
...
def test_lassoCV_does_not_set_precompute(monkeypatch, precompute, inner_precompute):
    ...
    monkeypatch.setattr(
        "xlearn.linear_model._coordinate_descent.Lasso", LassoMock)
...
def test_enet_cv_sample_weight_correctness(fit_intercept, sparseX):
    ...
    if fit_intercept:
        alphas = jnp.linspace(0.001, 0.01, num=91)
    else:
        alphas = jnp.linspace(0.01, 0.1, num=91)

    # We weight the first fold 2 times more.
    sw[:n_samples] = 2
    groups_sw = jnp.r_[
        jnp.full(n_samples, 0), jnp.full(n_samples, 1), jnp.full(n_samples, 2)
    ]
    ...
    if sparseX:
        X = X.toarray()
    X = jnp.r_[X[:n_samples], X]
    if sparseX:
        X = sparse.csc_matrix(X)
    y = jnp.r_[y[:n_samples], y]
    groups = jnp.r_[
        jnp.full(2 * n_samples, 0), jnp.full(n_samples, 1), jnp.full(n_samples, 2)
    ]
...
def test_enet_cv_grid_search(sample_weight):
    ...
    if sample_weight:
        sample_weight = jnp.linspace(1, 5, num=n_samples)
    else:
        sample_weight = None

    alphas = jnp.logspace(jnp.log10(1e-5), jnp.log10(1), num=10)
...
def test_enet_cv_sample_weight_consistency(
    ...
    # sample_weight=jnp.ones(..) should be equivalent to sample_weight=None
    sample_weight = jnp.ones_like(y)
    reg.fit(X, y, sample_weight=sample_weight)
    assert_allclose(reg.coef_, coef, rtol=1e-6)
    if fit_intercept:
    ...
    # scaling of sample_weight should have no effect, cf. jnp.average()
    sample_weight = 2 * jnp.ones_like(y)
    reg.fit(X, y, sample_weight=sample_weight)
    assert_allclose(reg.coef_, coef, rtol=1e-6)
    if fit_intercept:
        assert_allclose(reg.intercept_, intercept)
...
def test_enet_sample_weight_does_not_overwrite_sample_weight(check_input):
    ...
    sample_weight_1_25 = 1.25 * jnp.ones_like(y)
...
def test_sample_weight_invariance(estimator):
    ...
    # the weights by 2:
    X_dup = jnp.concatenate([X, X], axis=0)
    y_dup = jnp.concatenate([y, y], axis=0)
    sw_dup = jnp.concatenate([sw, sw], axis=0)
