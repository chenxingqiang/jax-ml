...
def test_huber_sample_weights():
    ...
    scale = max(jnp.mean(jnp.abs(huber.coef_)),
                jnp.mean(jnp.abs(huber.intercept_)))

    huber.fit(X, y, sample_weight=jnp.ones(y.shape[0]))
    assert_array_almost_equal(huber.coef_ / scale, huber_coef / scale)
    assert_array_almost_equal(
        huber.intercept_ / scale, huber_intercept / scale)

    X, y = make_regression_with_outliers(n_samples=5, n_features=20)
    X_new = jnp.vstack((X, jnp.vstack((X[1], X[1], X[3]))))
    y_new = jnp.concatenate((y, [y[1]], [y[1]], [y[3]]))
...
def test_huber_scaling_invariant():
    ...
    assert not jnp.all(n_outliers_mask_1)
...
def test_huber_better_r2_score():
    ...
    huber.fit(X, y)
    linear_loss = jnp.dot(X, huber.coef_) + huber.intercept_ - y
    mask = jnp.abs(linear_loss) < huber.epsilon * huber.scale_
