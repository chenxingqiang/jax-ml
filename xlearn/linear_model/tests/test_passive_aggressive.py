...
indices = jnp.arange(iris.data.shape[0])
...
class MyPassiveAggressive(ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        self.w = jnp.zeros(n_features, dtype=jnp.float64)
        self.b = 0.0
        ...
        for t in range(self.n_iter):
            for i in range(n_samples):
                ...
                if self.loss in ("hinge", "squared_hinge"):
                    ...
                    loss = max(jnp.abs(p - y[i]) - self.epsilon, 0)

                sqnorm = jnp.dot(X[i], X[i])
...
def test_partial_fit_weight_class_balanced():
    ...
    with pytest.raises(ValueError):
        clf.partial_fit(X, y, classes=jnp.unique(y))
    ...
    for data in (X, X_csr):
        for fit_intercept in (True, False):
            for average in (False, True):
                ...
                assert jnp.mean((pred - y_bin) ** 2) < 1.7
                if average:
