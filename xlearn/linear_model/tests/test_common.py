...
@pytest.mark.parametrize(
    ...
    [
        ...
        # https://github.com/jax-learn/jax-learn/issues/21305
        pytest.param(
            LogisticRegression(
...
def test_balance_property(model, with_sample_weight, global_random_seed):
    ...
    if is_classifier(model):
        assert jnp.average(model.predict_proba(X)[:, 1], weights=sw) == pytest.approx(
            jnp.average(y, weights=sw), rel=rel
        )
    else:
        assert jnp.average(model.predict(X), weights=sw, axis=0) == pytest.approx(
            jnp.average(y, weights=sw, axis=0), rel=rel
        )
