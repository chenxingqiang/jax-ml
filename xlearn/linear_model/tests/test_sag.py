...
def log_loss(p, y):
    return jnp.mean(jnp.log(1.0 + jnp.exp(-y * p)))
...
def sag(
    ...
    sum_gradient = jnp.zeros(X.shape[1])
    gradient_memory = jnp.zeros((n_samples, n_features))
    ...
    for epoch in range(n_iter):
        for k in range(n_samples):
            ...
            entry = X[idx]
            seen.add(idx)
            p = jnp.dot(entry, weights) + intercept
...
def sag_sparse(
    ...
    sum_gradient = jnp.zeros(n_features)
    last_updated = jnp.zeros(n_features, dtype=int)
    gradient_memory = jnp.zeros(n_samples)
    ...
    c_sum = jnp.zeros(n_iter * n_samples)
    ...
    for epoch in range(n_iter):
        for k in range(n_samples):
            ...
            p = (wscale * jnp.dot(entry, weights)) + intercept
            gradient = dloss(p, y[idx])
...
def get_step_size(X, alpha, fit_intercept, classification=True):
    if classification:
        return 4.0 / (jnp.max(jnp.sum(X * X, axis=1)) + fit_intercept + 4.0 * alpha)
    else:
        return 1.0 / (jnp.max(jnp.sum(X * X, axis=1)) + fit_intercept + alpha)
...
def test_classifier_matching():
    ...
    for solver in ["sag", "saga"]:
        ...
        weights2 = jnp.atleast_2d(weights2)
        intercept2 = jnp.atleast_1d(intercept2)
...
def test_get_auto_step_size():
    X = jnp.array([[1, 2, 3], [2, 3, 4], [2, 3, 2]], dtype=jnp.float64)
    alpha = 1.2
    fit_intercept = False
...
def test_sag_multiclass_computed_correctly():
    ...
    coef1 = jnp.vstack(coef1)
    intercept1 = jnp.array(intercept1)
    coef2 = jnp.vstack(coef2)
    intercept2 = jnp.array(intercept2)
...
def test_binary_classifier_class_weight():
    ...
    class_weight_ = compute_class_weight(
        class_weight, classes=jnp.unique(y), y=y)
...
def test_multinomial_loss():
    ...
    X, y = iris.data, iris.target.astype(jnp.float64)
    ...
    jnp.abs(sample_weights, sample_weights)
    ...
    weights_intercept = jnp.vstack((weights, intercept)).T
...
def test_multinomial_loss_ground_truth():
    ...
    n_classes = 3
    X = jnp.array([[1.1, 2.2], [2.2, -4.4], [3.3, -2.2], [1.1, 1.1]])
    y = jnp.array([0, 1, 2, 0], dtype=jnp.float64)
    lbin = LabelBinarizer()
    Y_bin = lbin.fit_transform(y)

    weights = jnp.array([[0.1, 0.2, 0.3], [1.1, 1.2, -1.3]])
    intercept = jnp.array([1.0, 0, -0.2])
    sample_weights = jnp.array([0.8, 1, 1, 0.8])

    prediction = jnp.dot(X, weights) + intercept
    logsumexp_prediction = logsumexp(prediction, axis=1)
    p = prediction - logsumexp_prediction[:, jnp.newaxis]
    loss_1 = -(sample_weights[:, jnp.newaxis] * p * Y_bin).sum()
    diff = sample_weights[:, jnp.newaxis] * (jnp.exp(p) - Y_bin)
    grad_1 = jnp.dot(X.T, diff)
