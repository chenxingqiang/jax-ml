...
def test_bayesian_ridge_score_values():
    ...
    M = 1.0 / alpha_ * jnp.eye(n_samples) + 1.0 / lambda_ * jnp.dot(X, X.T)
    M_inv_dot_y = jnp.linalg.solve(M, y)
    score += -0.5 * (
        fast_logdet(M) + jnp.dot(y.T, M_inv_dot_y) + n_samples * log(2 * jnp.pi)
    )
...
def test_bayesian_ridge_parameter():
    ...
    X = jnp.array([[1, 1], [3, 4], [5, 7], [4, 1], [2, 6], [3, 10], [3, 2]])
    y = jnp.array([1, 2, 3, 2, 0, 4, 5]).T
...
def test_toy_bayesian_ridge_object():
    ...
    X = jnp.array([[1], [2], [6], [8], [10]])
    Y = jnp.array([1, 2, 6, 8, 10])
    clf = BayesianRidge(compute_score=True)
    clf.fit(X, Y)
...
def test_bayesian_initial_params():
    ...
    X = jnp.vander(jnp.linspace(0, 4, 5), 4)
    y = jnp.array([0.0, 1.0, 0.0, -1.0, 0.0])  # y = (x^3 - 6x^2 + 8x) / 3
...
def test_prediction_bayesian_ridge_ard_with_constant_input():
    ...
    y = jnp.full(n_samples, constant_value,
                dtype=jnp.array(constant_value).dtype)
    expected = jnp.full(n_samples, constant_value,
                       dtype=jnp.array(constant_value).dtype)
...
def test_ard_accuracy_on_easy_problem(global_random_seed, n_samples, n_features):
    ...
    abs_coef_error = jnp.abs(1 - regressor.coef_[1])
...
def test_update_sigma(global_random_seed):
    ...
    alpha = 1
    lmbda = jnp.arange(1, n_features + 1)
    keep_lambda = jnp.array([True] * n_features)
    ...
    jnp.testing.assert_allclose(sigma, sigma_woodbury)
...
def test_dtype_correctness(Estimator):
    ...
    y = jnp.array([1, 2, 3, 2, 0, 4, 5]).T
    model = Estimator()
    coef_32 = model.fit(X.astype(jnp.float32), y).coef_
    coef_64 = model.fit(X.astype(jnp.float64), y).coef_
    jnp.testing.assert_allclose(coef_32, coef_64, rtol=1e-4)
