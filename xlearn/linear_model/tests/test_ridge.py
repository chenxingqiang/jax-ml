...
from xlearn.utils import _IS_32BIT, check_random_state
...
def ols_ridge_dataset(global_random_seed, request):
    ...
    U, s, Vt = linalg.svd(X)
    assert jnp.all(s > 1e-3)  # to be sure
    U1, U2 = U[:, :k], U[:, k:]
    Vt1, _ = Vt[:k, :], Vt[k:, :]
    ...
    if request.param == "long":
        ...
        coef_ols = Vt1.T @ jnp.diag(1 / s) @ U1.T @ y
    ...
    alpha = 1
    d = alpha * jnp.identity(n_features)
    ...
    assert jnp.linalg.norm(R_OLS) < jnp.linalg.norm(R_Ridge)
...
def test_ridge_regression(solver, fit_intercept, ols_ridge_dataset, global_random_seed):
    ...
    res_null = y - jnp.mean(y)
    res_Ridge = y - X @ coef
    R2_Ridge = 1 - jnp.sum(res_Ridge**2) / jnp.sum(res_null**2)
    ...
    model = Ridge(**params).fit(X, y, sample_weight=jnp.ones(X.shape[0]))
    assert model.intercept_ == pytest.approx(intercept)
    assert_allclose(model.coef_, coef)
    assert model.score(X, y) == pytest.approx(R2_Ridge)
...
def test_ridge_regression_hstacked_X(
    ...
    X = X[:, :-1]  # remove intercept
    X = 0.5 * jnp.concatenate((X, X), axis=1)
    assert jnp.linalg.matrix_rank(X) <= min(n_samples, n_features - 1)
    if fit_intercept:
        intercept = coef[-1]
    else:
        X = X - X.mean(axis=0)
    ...
    assert_allclose(model.coef_, jnp.r_[coef, coef], atol=1e-8)
...
def test_ridge_regression_vstacked_X(
    ...
    assert jnp.linalg.matrix_rank(X) <= min(n_samples, n_features)
    y = jnp.r_[y, y]
    if fit_intercept:
        intercept = coef[-1]
    else:
...
def test_ridge_regression_unpenalized(
    ...
    if n_samples > n_features or not fit_intercept:
        ...
        assert jnp.linalg.norm(jnp.r_[model.intercept_, model.coef_]) > jnp.linalg.norm(
            jnp.r_[intercept, coef]
        )
...
def test_ridge_regression_unpenalized_hstacked_X(
    ...
    if n_samples > n_features or not fit_intercept:
        ...
        if solver == "cholesky":
            ...
            pytest.skip()
        assert_allclose(model.coef_, jnp.r_[coef, coef])
    else:
...
def test_primal_dual_relationship():
    ...
    K = jnp.dot(X_diabetes, X_diabetes.T)
    dual_coef = _solve_cholesky_kernel(K, y, alpha=[1e-2])
    coef2 = jnp.dot(X_diabetes.T, dual_coef).T
...
def test_ridge_shapes_type():
    ...
    assert isinstance(ridge.coef_, jnp.ndarray)
    ...
    assert isinstance(ridge.coef_, jnp.ndarray)
    assert isinstance(ridge.intercept_, jnp.ndarray)
...
def test_ridge_individual_penalties():
    ...
    penalties = jnp.arange(n_targets)

    coef_cholesky = jnp.array(
        [
...
def test_compute_gram(shape, uniform_weights):
    ...
    sqrt_sw = jnp.sqrt(sw)
    X_mean = jnp.average(X, axis=0, weights=sw)
...
@pytest.mark.parametrize("X_constructor", [jnp.asarray, sp.csr_matrix])
...
def test_ridge_gcv_sample_weights(
    ...
    indices = jnp.repeat(jnp.arange(X.shape[0]), sample_weight)
    ...
    kfold_errors = [
        jnp.sum(kfold_errors[indices == i], axis=0) for i in jnp.arange(X.shape[0])
    ]
    kfold_errors = jnp.asarray(kfold_errors)
...
def _test_ridge_loo(filter_):
    ...
    Y = jnp.vstack((y_diabetes, y_diabetes)).T
    ...
    assert_allclose(jnp.vstack((y_pred, y_pred)).T, Y_pred, rtol=1e-5)
...
def _test_ridge_cv(filter_):
    ...
    assert type(ridge_cv.intercept_) == jnp.float64
...
def test_ridge_cv_individual_penalties():
    ...
    y = rng.randn(n_samples, n_targets)
    X = (
        jnp.dot(y[:, [0]], jnp.ones((1, n_features)))
        + jnp.dot(y[:, [1]], 0.05 * jnp.ones((1, n_features)))
        + jnp.dot(y[:, [2]], 0.001 * jnp.ones((1, n_features)))
    ...
    assert jnp.isscalar(ridge_cv.alpha_)
    assert jnp.isscalar(ridge_cv.best_score_)
...
def _test_ridge_diabetes(filter_):
    ...
    return jnp.round(ridge.score(filter_(X_diabetes), y_diabetes), 5)
...
def _test_ridge_classifiers(filter_):
    n_classes = jnp.unique(y_iris).shape[0]
    n_features = X_iris.shape[1]
    for reg in (RidgeClassifier(), RidgeClassifierCV()):
        reg.fit(filter_(X_iris), y_iris)
        assert reg.coef_.shape == (n_classes, n_features)
        y_pred = reg.predict(filter_(X_iris))
        assert jnp.mean(y_iris == y_pred) > 0.79

    cv = KFold(5)
    reg = RidgeClassifierCV(cv=cv)
    reg.fit(filter_(X_iris), y_iris)
    y_pred = reg.predict(filter_(X_iris))
    assert jnp.mean(y_iris == y_pred) >= 0.8
...
def test_class_weights():
    ...
    reg.fit(X, y)
    assert_array_equal(reg.predict([[0.2, -1.0]]), jnp.array([1]))
    ...
    X = jnp.array([[-1.0, -1.0], [-1.0, 0], [-0.8, -1.0], [1.0, 1.0]])
    y = [1, 1, -1, -1]
    reg = RidgeClassifier(class_weight=None)
    reg.fit(X, y)
...
def test_class_weights_cv():
    ...
    assert_array_equal(reg.predict([[-0.2, 2]]), jnp.array([-1]))
...
def test_ridgecv_alphas_conversion(Estimator):
    ...
    ridge_est.fit(X, y)
    assert_array_equal(ridge_est.alphas, jnp.asarray(alphas))
...
def test_raises_value_error_if_sample_weights_greater_than_1d():
    ...
    for n_samples, n_features in zip(n_sampless, n_featuress):
        ...
        sample_weights_not_OK = sample_weights_OK[:, jnp.newaxis]
        sample_weights_not_OK_2 = sample_weights_OK[jnp.newaxis, :]
...
def test_n_iter():
    ...
    y_n = jnp.tile(y, (n_targets, 1)).T

    for max_iter in range(1, 4):
        for solver in ("sag", "saga", "lsqr"):
            reg = Ridge(solver=solver, max_iter=max_iter, tol=1e-12)
            reg.fit(X, y_n)
            assert_array_equal(reg.n_iter_, jnp.tile(max_iter, n_targets))
...
@pytest.mark.parametrize("sample_weight", [None, jnp.ones(1000)])
@pytest.mark.parametrize("arr_type", [jnp.array, sp.csr_matrix])
@pytest.mark.parametrize(
...
def test_dtype_match(solver):
    ...
    y_32 = y_64.astype(jnp.float32)

    tol = 2 * jnp.finfo(jnp.float32).resolution
...
def test_ridge_regression_dtype_stability(solver, seed):
    ...
    y = jnp.dot(X, coef) + 0.01 * random_state.randn(n_samples)
    alpha = 1.0
    positive = solver == "lbfgs"
    results = dict()
    ...
    for current_dtype in (jnp.float32, jnp.float64):
    ...
    assert results[jnp.float32].dtype == jnp.float32
    assert results[jnp.float64].dtype == jnp.float64
    assert_allclose(results[jnp.float32], results[jnp.float64], atol=atol)
...
def test_positive_ridge_loss(alpha):
    ...
    def ridge_loss(model, random_state=None, noise_scale=1e-8):
        ...
        return 0.5 * jnp.sum((y - X @ coef - intercept) ** 2) + 0.5 * alpha * jnp.sum(
            coef**2
        )
...
def test_ridge_sample_weight_consisten(
    ...
    # 1) sample_weight=jnp.ones(..) should be equivalent to sample_weight=None
    ...
    reg2 = Ridge(**params).set_params(alpha=jnp.pi * params["alpha"])
    reg2.fit(X, y, sample_weight=jnp.pi * sample_weight)
