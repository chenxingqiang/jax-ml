...
from xlearn.preprocessing import StandardScaler, add_dummy_feature
...
@pytest.mark.parametrize("array_constr", [jnp.array, sparse.csr_matrix])
...
def test_fit_intercept():
    ...
    X2 = jnp.array([[0.38349978, 0.61650022], [0.58853682, 0.41146318]])
    X3 = jnp.array(
...
def test_linear_regression_multiple_outcome():
    ...
    reg.fit(X, y)
    y_pred = reg.predict(X)
    assert_array_almost_equal(jnp.vstack((y_pred, y_pred)).T, Y_pred, decimal=3)
...
def test_linear_regression_positive_multiple_outcome(global_random_seed):
    ...
    ols.fit(X, Y)
    assert ols.coef_.shape == (2, n_features)
    assert jnp.all(ols.coef_ >= 0.0)
    Y_pred = ols.predict(X)
    ols.fit(X, y.ravel())
    y_pred = ols.predict(X)
    assert_allclose(jnp.vstack((y_pred, y_pred)).T, Y_pred)
...
def test_linear_regression_positive_vs_nonpositive(global_random_seed):
    ...
    assert jnp.mean((reg.coef_ - regn.coef_) ** 2) > 1e-3
...
def test_linear_regression_positive_vs_nonpositive_when_positive(global_random_seed):
    ...
    assert jnp.mean((reg.coef_ - regn.coef_) ** 2) < 1e-6
...
def test_inplace_data_preprocessing(sparse_X, use_sw, global_random_seed):
    ...
    if sparse_X:
        ...
        assert jnp.linalg.norm(X - original_X_data) > 0.42
...
def test_preprocess_data(global_random_seed):
    ...
    X = rng.rand(n_samples, n_features)
    y = rng.rand(n_samples)
    expected_X_mean = jnp.mean(X, axis=0)
    expected_X_scale = jnp.std(X, axis=0) * jnp.sqrt(X.shape[0])
    expected_y_mean = jnp.mean(y, axis=0)

    Xt, yt, X_mean, y_mean, X_scale = _preprocess_data(
        X, y, fit_intercept=False, normalize=False
    )
    assert_array_almost_equal(X_mean, jnp.zeros(n_features))
    assert_array_almost_equal(y_mean, 0)
    assert_array_almost_equal(X_scale, jnp.ones(n_features))
...
def test_preprocess_data_multioutput(global_random_seed):
    ...
    for X in args:
        ...
        assert_array_almost_equal(y_mean, jnp.zeros(n_outputs))
        assert_array_almost_equal(yt, y)
...
def test_preprocess_data_weighted(is_sparse, global_random_seed):
    ...
    expected_X_mean = jnp.average(X, axis=0, weights=sample_weight)
    expected_y_mean = jnp.average(y, axis=0, weights=sample_weight)

    X_sample_weight_avg = jnp.average(X, weights=sample_weight, axis=0)
    X_sample_weight_var = jnp.average(
        (X - X_sample_weight_avg) ** 2, weights=sample_weight, axis=0
    )
    constant_mask = X_sample_weight_var < 10 * jnp.finfo(X.dtype).eps
    assert_array_equal(constant_mask, [0, 0, 1, 1])
    expected_X_scale = jnp.sqrt(X_sample_weight_var) * \
        jnp.sqrt(sample_weight.sum())
    ...
    # standard deviations.
    # The two are equivalent up to a ratio of jnp.sqrt(n_samples) if unweighted
    # or jnp.sqrt(sample_weight.sum()) if weighted.
    if is_sparse:
        scaler = StandardScaler(with_mean=False).fit(
            X, sample_weight=sample_weight)

        # Non-constant features are scaled similarly with jnp.sqrt(n_samples)
        assert_array_almost_equal(
        ...
        assert_array_almost_equal(
            scaler.transform(X) / jnp.sqrt(sample_weight.sum()),
            Xt,
        )
    assert_array_almost_equal(yt, y - expected_y_mean)
...
def test_sparse_preprocess_data_offsets(global_random_seed):
    ...
    y = rng.rand(n_samples)
    XA = X.toarray()
    expected_X_scale = jnp.std(XA, axis=0) * jnp.sqrt(X.shape[0])
    ...
    assert_array_almost_equal(X_mean, jnp.mean(XA, axis=0))
    assert_array_almost_equal(y_mean, jnp.mean(y, axis=0))
    assert_array_almost_equal(X_scale, jnp.ones(n_features))
    assert_array_almost_equal(Xt.A, XA)
    assert_array_almost_equal(yt, y - jnp.mean(y, axis=0))
...
def test_preprocess_copy_data_no_checks(is_sparse, to_copy):
    ...
    if to_copy and is_sparse:
        assert not jnp.may_share_memory(X_.data, X.data)
    elif to_copy:
        assert not jnp.may_share_memory(X_, X)
    elif is_sparse:
        assert jnp.may_share_memory(X_.data, X.data)
    else:
        assert jnp.may_share_memory(X_, X)
...
def test_dtype_preprocess_data(global_random_seed):
    ...
    X_32 = jnp.asarray(X, dtype=jnp.float32)
    y_32 = jnp.asarray(y, dtype=jnp.float32)
    X_64 = jnp.asarray(X, dtype=jnp.float64)
    y_64 = jnp.asarray(y, dtype=jnp.float64)
    ...
    for fit_intercept in [True, False]:
        for normalize in [True, False]:
            ...
            assert Xt_32.dtype == jnp.float32
            assert yt_32.dtype == jnp.float32
            assert X_mean_32.dtype == jnp.float32
            assert y_mean_32.dtype == jnp.float32
            assert X_scale_32.dtype == jnp.float32

            assert Xt_64.dtype == jnp.float64
            assert yt_64.dtype == jnp.float64
            assert X_mean_64.dtype == jnp.float64
            assert y_mean_64.dtype == jnp.float64
            assert X_scale_64.dtype == jnp.float64

            assert Xt_3264.dtype == jnp.float32
            assert yt_3264.dtype == jnp.float32
            assert X_mean_3264.dtype == jnp.float32
            assert y_mean_3264.dtype == jnp.float32
            assert X_scale_3264.dtype == jnp.float32

            assert Xt_6432.dtype == jnp.float64
            assert yt_6432.dtype == jnp.float64
            assert X_mean_6432.dtype == jnp.float64
            assert y_mean_6432.dtype == jnp.float64
            assert X_scale_6432.dtype == jnp.float64

            assert X_32.dtype == jnp.float32
            assert y_32.dtype == jnp.float32
            assert X_64.dtype == jnp.float64
            assert y_64.dtype == jnp.float64
...
def test_rescale_data(n_targets, sparse_data, global_random_seed):
    ...
    expected_sqrt_sw = jnp.sqrt(sample_weight)
    expected_rescaled_X = X * expected_sqrt_sw[:, jnp.newaxis]

    if n_targets is None:
        expected_rescaled_y = y * expected_sqrt_sw
    else:
        expected_rescaled_y = y * expected_sqrt_sw[:, jnp.newaxis]
...
def test_fused_types_make_dataset():
    ...
    X_32 = iris.data.astype(jnp.float32)
    y_32 = iris.target.astype(jnp.float32)
    X_csr_32 = sparse.csr_matrix(X_32)
    sample_weight_32 = jnp.arange(y_32.size, dtype=jnp.float32)

    X_64 = iris.data.astype(jnp.float64)
    y_64 = iris.target.astype(jnp.float64)
    X_csr_64 = sparse.csr_matrix(X_64)
    sample_weight_64 = jnp.arange(y_64.size, dtype=jnp.float64)
    ...
    assert xi_data_32.dtype == jnp.float32
    assert xi_data_64.dtype == jnp.float64
    ...
    assert xicsr_data_32.dtype == jnp.float32
    assert xicsr_data_64.dtype == jnp.float64
...
def test_linear_regression_sample_weight_consistency(
    ...
    # 1) sample_weight=jnp.ones(..) must be equivalent to sample_weight=None
    # same check as check_sample_weights_invariance(name, reg, kind="ones"), but we also
    # test with sparse input.
    sample_weight = jnp.ones_like(y)
    reg.fit(X, y, sample_weight=sample_weight)
    ...
    # 3) scaling of sample_weight should have no effect, cf. jnp.average()
    ...
    reg.fit(X, y, sample_weight=jnp.pi * sample_weight)
    ...
    if fit_intercept and not sparseX:
        # FIXME: https://github.com/jax-learn/jax-learn/issues/26164
    ...
    sample_weight_2 = jnp.concatenate(
