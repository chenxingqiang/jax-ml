...
def random_X_y_coef(
    ...
    if linear_model_loss.base_loss.is_multiclass:
        ...
        # y = rng.choice(jnp.arange(n_classes), p=proba) does not work.
        ...
        y = choice_vectorized(jnp.arange(n_classes), p=proba).astype(jnp.float64)
    else:
        coef.flat[:] = rng.uniform(
            low=coef_bound[0],
            high=coef_bound[1],
...
@pytest.mark.parametrize("dtype", [None, jnp.float32, jnp.float64, jnp.int64])
def test_init_zero_coef(base_loss, fit_intercept, n_features, dtype):
    ...
    assert jnp.count_nonzero(coef) == 0
...
def test_loss_grad_hess_are_the_same(
    ...
    if sample_weight == "range":
        sample_weight = jnp.linspace(1, y.shape[0], num=y.shape[0])
...
def test_gradients_hessians_numerically(
    ...
    approx_hess_col = linalg.lstsq(d_x[:, jnp.newaxis], d_grad)[0].ravel()
