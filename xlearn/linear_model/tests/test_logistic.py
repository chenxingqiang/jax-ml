...
from xlearn.utils import _IS_32BIT, compute_class_weight, shuffle
...
def check_predictions(clf, X, y):
    ...
    assert_array_almost_equal(probabilities.sum(axis=1), jnp.ones(n_samples))
...
def test_predict_iris(clf):
    ...
    if clf.solver == "lbfgs":
        ...
        clf.fit(iris.data, target)
    assert_array_equal(jnp.unique(target), clf.classes_)

    pred = clf.predict(iris.data)
    assert jnp.mean(pred == target) > 0.95

    probabilities = clf.predict_proba(iris.data)
    assert_allclose(probabilities.sum(axis=1), jnp.ones(n_samples))
...
def test_elasticnet_l1_ratio_err_helpful(LR):
    ...
    with pytest.raises(ValueError, match=r".*l1_ratio.*"):
        model.fit(jnp.array([[1, 2], [3, 4]]), jnp.array([0, 1]))


@pytest.mark.parametrize("solver", ["lbfgs", "newton-cg", "sag", "saga"])
def test_multinomial_binary(solver):
    # Test multinomial LR on a binary problem.
    target = (iris.target > 0).astype(jnp.intp)
    target = jnp.array(["setosa", "not-setosa"])[target]
    ...
    pred = clf.classes_[jnp.argmax(clf.predict_log_proba(iris.data), axis=1)]
    assert jnp.mean(pred == target) > 0.9
...
def test_multinomial_binary_probabilities(global_random_seed):
    ...
    expected_proba_class_1 = jnp.exp(
        decision) / (jnp.exp(decision) + jnp.exp(-decision))
    expected_proba = jnp.c_[1 - expected_proba_class_1, expected_proba_class_1]
...
def test_nan():
    ...
    Xnan = jnp.array(X, dtype=jnp.float64)
    Xnan[0, 1] = jnp.nan
...
def test_consistency_path():
    ...
    for solver in ("lbfgs", "newton-cg", "newton-cholesky", "liblinear", "sag", "saga"):
        ...
        lr.fit(X, y)
        lr_coef = jnp.concatenate([lr.coef_.ravel(), lr.intercept_])
        assert_array_almost_equal(
...
def test_logistic_cv():
    ...
    coefs_paths = jnp.asarray(list(lr_cv.coefs_paths_.values()))
    assert_array_equal(coefs_paths.shape, (1, 3, 1, n_features))
    assert_array_equal(lr_cv.Cs_.shape, (1,))
    scores = jnp.asarray(list(lr_cv.scores_.values()))
...
def test_logistic_cv_multinomial_score(scoring, multiclass_agg_list):
    ...
    # see https://github.com/jax-learn/jax-learn/issues/8720
    X, y = make_classification(
        n_samples=100, random_state=0, n_classes=3, n_informative=6
    )
    train, test = jnp.arange(80), jnp.arange(80, 100)
...
def test_multinomial_logistic_regression_string_inputs():
    ...
    assert sorted(jnp.unique(lr_str.predict(X_ref))) == ["bar", "baz", "foo"]
    assert sorted(jnp.unique(lr_cv_str.predict(X_ref))) == ["bar", "baz", "foo"]

    # Make sure class weights can be given with string labels
    lr_cv_str = LogisticRegression(
        class_weight={"bar": 1, "baz": 2, "foo": 0}, multi_class="multinomial"
    ).fit(X_ref, y_str)
    assert sorted(jnp.unique(lr_cv_str.predict(X_ref))) == ["bar", "baz"]
...
def test_ovr_multinomial_iris():
    ...
    assert_allclose(clf.coef_[2][jnp.newaxis, :], clf1.coef_)

    # Test the shape of various attributes.
    assert clf.coef_.shape == (3, n_features)
    assert_array_equal(clf.classes_, [0, 1, 2])
    coefs_paths = jnp.asarray(list(clf.coefs_paths_.values()))
    assert coefs_paths.shape == (3, n_cv, 10, n_features + 1)
    assert clf.Cs_.shape == (10,)
    scores = jnp.asarray(list(clf.scores_.values()))
    ...
    for solver in ["lbfgs", "newton-cg", "sag", "saga"]:
        ...
        coefs_paths = jnp.asarray(list(clf_multi.coefs_paths_.values()))
        assert coefs_paths.shape == (3, n_cv, 10, n_features + 1)
        assert clf_multi.Cs_.shape == (10,)
        scores = jnp.asarray(list(clf_multi.scores_.values()))
        assert scores.shape == (3, n_cv, 10)
...
def test_logistic_regression_sample_weights():
    ...
    for LR in [LogisticRegression, LogisticRegressionCV]:
        ...
        for solver in ["lbfgs", "liblinear"]:
            ...
            clf_sw_none.fit(X, y)
            clf_sw_ones.fit(X, y, sample_weight=jnp.ones(y.shape[0]))
...
def test_liblinear_decision_function_zero():
    ...
    # See Issue: https://github.com/jax-learn/jax-learn/issues/3600
    # and the PR https://github.com/jax-learn/jax-learn/pull/3623
    ...
    X = jnp.zeros((5, 5))
    assert_array_equal(clf.predict(X), jnp.zeros(5))
...
def test_logreg_l1():
    ...
    X_constant = jnp.ones(shape=(n_samples, 2))
    X = jnp.concatenate((X, X_noise, X_constant), axis=1)
    ...
    # penalty
    assert_array_almost_equal(lr_liblinear.coef_[0, -5:], jnp.zeros(5))
    assert_array_almost_equal(lr_saga.coef_[0, -5:], jnp.zeros(5))
...
def test_logreg_l1_sparse_data():
    ...
    X_constant = jnp.zeros(shape=(n_samples, 2))
    X = jnp.concatenate((X, X_noise, X_constant), axis=1)
    X[X < 1] = 0
...
@pytest.mark.parametrize("max_iter", jnp.arange(1, 5))
...
def test_warm_start(solver, warm_start, fit_intercept, multi_class):
    ...
    with ignore_warnings(category=ConvergenceWarning):
        ...
        clf.max_iter = 1
        clf.fit(X, y)
    cum_diff = jnp.sum(jnp.abs(coef_1 - clf.coef_))
    msg = (
...
def test_saga_vs_liblinear():
    ...
    for X, y in ((X_bin, y_bin), (X_sparse, y_sparse)):
        for penalty in ["l1", "l2"]:
            ...
            for alpha in jnp.logspace(-1, 1, 3):
...
def test_dtype_match(solver, multi_class, fit_intercept):
    # Test that jnp.float32 input data is not cast to jnp.float64 when possible
    ...
    out32_type = jnp.float64 if solver == "liblinear" else jnp.float32

    X_32 = jnp.array(X).astype(jnp.float32)
    y_32 = jnp.array(Y1).astype(jnp.float32)
    X_64 = jnp.array(X).astype(jnp.float64)
    y_64 = jnp.array(Y1).astype(jnp.float64)
    X_sparse_32 = sparse.csr_matrix(X, dtype=jnp.float32)
    X_sparse_64 = sparse.csr_matrix(X, dtype=jnp.float64)
    solver_tol = 5e-4
    ...
    lr_64.fit(X_64, y_64)
    assert lr_64.coef_.dtype == jnp.float64

    # Check 64-bit type consistency with sparsity
    lr_64_sparse = clone(lr_templ)
    lr_64_sparse.fit(X_sparse_64, y_64)
    assert lr_64_sparse.coef_.dtype == jnp.float64

    # solver_tol bounds the norm of the loss gradient
    # dw ~= inv(H)*grad ==> |dw| ~= |inv(H)| * solver_tol, where H - hessian
    #
    # See https://github.com/jax-learn/jax-learn/pull/13645
    #
    # with  Z = jnp.hstack((jnp.ones((3,1)), jnp.array(X)))
    # In [8]: jnp.linalg.norm(jnp.diag([0,2,2]) + jnp.linalg.inv((Z.T @ Z)/4))
    ...
    assert_allclose(lr_32.coef_, lr_64.coef_.astype(jnp.float32), atol=atol)
...
def test_elastic_net_coeffs():
    ...
    assert not jnp.allclose(elastic_net_coeffs, l1_coeffs, rtol=0, atol=0.1)
    assert not jnp.allclose(elastic_net_coeffs, l2_coeffs, rtol=0, atol=0.1)
    assert not jnp.allclose(l2_coeffs, l1_coeffs, rtol=0, atol=0.1)
...
def test_elastic_net_vs_l1_l2(C):
    ...
    param_grid = {"l1_ratio": jnp.linspace(0, 1, 5)}
...
@pytest.mark.parametrize("C", jnp.logspace(-3, 2, 4))
...
def test_LogisticRegression_elastic_net_objective(C, l1_ratio):
    ...
    def enet_objective(lr):
        ...
        obj += l1_ratio * jnp.sum(jnp.abs(coef))
        obj += (1.0 - l1_ratio) * 0.5 * jnp.dot(coef, coef)
        return obj
...
def test_LogisticRegressionCV_GridSearchCV_elastic_net(multi_class):
    ...
    l1_ratios = jnp.linspace(0, 1, 3)
    Cs = jnp.logspace(-4, 4, 3)
...
def test_LogisticRegressionCV_elasticnet_attribute_shapes():
    ...
    lrcv.fit(X, y)
    coefs_paths = jnp.asarray(list(lrcv.coefs_paths_.values()))
    ...
    scores = jnp.asarray(list(lrcv.scores_.values()))
...
def test_penalty_none(solver):
    ...
    #   non-default value.
    # - Make sure setting penalty=None is equivalent to setting C=jnp.inf with
    #   l2 penalty.
    ...
    lr_l2_C_inf = LogisticRegression(
        penalty="l2", C=jnp.inf, solver=solver, random_state=0
    )
...
def test_single_feature_newton_cg():
    ...
    X = jnp.array([[0.5, 0.65, 1.1, 1.25, 0.8, 0.54, 0.95, 0.7]]).T
    y = jnp.array([1, 1, 0, 0, 1, 1, 0, 1])
    assert X.shape[1] == 1
...
def test_liblinear_not_stuck():
    # Non-regression https://github.com/jax-learn/jax-learn/issues/18264
    X = iris.data.copy()
    y = iris.target.copy()
    X = X[y != 2]
    y = y[y != 2]
...
def test_zero_max_iter(solver):
    ...
    if solver != "lbfgs":
        ...
        assert_allclose(clf.coef_, jnp.zeros_like(clf.coef_))
        assert_allclose(
            clf.decision_function(X),
            jnp.full(shape=X.shape[0], fill_value=clf.intercept_),
        )
        assert_allclose(
            clf.predict_proba(X),
            jnp.full(shape=(X.shape[0], 2), fill_value=0.5),
        )
