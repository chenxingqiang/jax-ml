...
@validate_params(
    {
        ...
        "y": [jnp.ndarray, None],
        "Xy": [jnp.ndarray, None],
        "Gram": [StrOptions({"auto"}), "boolean", jnp.ndarray, None],
...
def lars_path(
    ...
    *,
    ...
    eps : float, default=jnp.finfo(float).eps
...
def _lars_path_solver(
    ...
    Xy : array-like of shape (n_features,) or (n_features, n_targets), \
            default=None
        `Xy = jnp.dot(X.T, y)` that can be precomputed. It is useful
        only when the Gram matrix is precomputed.
    ...
    if return_path:
        coefs = jnp.zeros((max_features + 1, n_features), dtype=return_dtype)
        alphas = jnp.zeros(max_features + 1, dtype=return_dtype)
    else:
        coef, prev_coef = (
            jnp.zeros(n_features, dtype=return_dtype),
            jnp.zeros(n_features, dtype=return_dtype),
        )
        alpha, prev_alpha = (
            jnp.array([0.0], dtype=return_dtype),
            jnp.array([0.0], dtype=return_dtype),
        )
        # above better ideas?

    n_iter, n_active = 0, 0
    active, indices = list(), jnp.arange(n_features)
    # holds the sign of covariance
    sign_active = jnp.empty(max_features, dtype=jnp.int8)
    drop = False

    # will hold the cholesky factorization. Only lower part is
    # referenced.
    if Gram is None:
        L = jnp.empty((max_features, max_features), dtype=X.dtype)
        swap, nrm2 = linalg.get_blas_funcs(("swap", "nrm2"), (X,))
    else:
        L = jnp.empty((max_features, max_features), dtype=Gram.dtype)
    ...
    tiny32 = jnp.finfo(jnp.float32).tiny  # to avoid division by 0 warning
    cov_precision = jnp.finfo(Cov.dtype).precision
    equality_tolerance = jnp.finfo(jnp.float32).eps
    ...
    while True:
        if Cov.size:
            if positive:
                C_idx = jnp.argmax(Cov)
            else:
                C_idx = jnp.argmax(jnp.abs(Cov))
        ...
        if return_path:
            alpha = alphas[n_iter, jnp.newaxis]
            coef = coefs[n_iter]
            prev_alpha = alphas[n_iter - 1, jnp.newaxis]
        ...
        if not drop:
            ...
            if positive:
                sign_active[n_active] = jnp.ones_like(C_)
            else:
                sign_active[n_active] = jnp.sign(C_)
            ...
            v = jnp.dot(L[n_active, :n_active], L[n_active, :n_active])
            diag = max(jnp.sqrt(jnp.abs(c - v)), eps)
        ...
        if least_squares.size == 1 and least_squares == 0:
            ...
            AA = 1.0 / jnp.sqrt(jnp.sum(least_squares * sign_active[:n_active]))

            if not jnp.isfinite(AA):
                # L is too ill-conditioned
                i = 0
                L_ = L[:n_active, :n_active].copy()
                while not jnp.isfinite(AA):
                    L_.flat[:: n_active + 1] += (2**i) * eps
                    least_squares, _ = solve_cholesky(
                        L_, sign_active[:n_active], lower=True
                    )
                    tmp = max(
                        jnp.sum(least_squares * sign_active[:n_active]), eps)
                    AA = 1.0 / jnp.sqrt(tmp)
                    i += 1
            least_squares *= AA
        ...
        if Gram is None:
            ...
            eq_dir = jnp.dot(X.T[:n_active].T, least_squares)
            # correlation between each unactive variables and
            # eqiangular vector
            corr_eq_dir = jnp.dot(X.T[n_active:], eq_dir)
        else:
            # if huge number of features, this takes 50% of time, I
            # think could be avoided if we just update it using an
            # orthogonal (QR) decomposition of X
            corr_eq_dir = jnp.dot(Gram[:n_active, n_active:].T, least_squares)

        # Explicit rounding can be necessary to avoid `jnp.argmax(Cov)` yielding
        # unstable results because of rounding errors.
        jnp.around(corr_eq_dir, decimals=cov_precision, out=corr_eq_dir)
        ...
        if return_path:
            if n_iter >= coefs.shape[0]:
                ...
                coefs = jnp.resize(coefs, (n_iter + add_features, n_features))
                coefs[-add_features:] = 0
                alphas = jnp.resize(alphas, n_iter + add_features)
        ...
        if drop and method == "lasso":
            ...
            if Gram is None:
                ...
                residual = y - jnp.dot(X[:, :n_active], coef[active])
                temp = jnp.dot(X.T[n_active], residual)
                ...
                # TODO: this could be updated
                temp = Cov_copy[drop_idx] - jnp.dot(Gram_copy[drop_idx], coef)
                Cov = jnp.r_[temp, Cov]

            sign_active = jnp.delete(sign_active, idx)
            sign_active = jnp.append(sign_active, 0.0)  # just to maintain size
            if verbose > 1:
                print(
                    "%s\t\t%s\t\t%s\t\t%s\t\t%s"
...
class Lars(MultiOutputMixin, RegressorMixin, LinearModel):
    ...
    normalize : bool, default=False
        ...
        :class:`~xlearn.preprocessing.StandardScaler` before calling ``fit``
    ...
    n_nonzero_coefs : int, default=500
        Target number of non-zero coefficients. Use ``jnp.inf`` for no limit.
    ...
    _parameter_constraints: dict = {
        ...
        "precompute": ["boolean", StrOptions({"auto"}), jnp.ndarray, Hidden(None)],
    ...
    def _fit(self, X, y, max_iter, alpha, fit_path, normalize, Xy=None):
        ...
        self.alphas_ = []
        self.n_iter_ = []
        self.coef_ = jnp.empty((n_targets, n_features), dtype=X.dtype)
...
def _lars_path_residues(
    ...
    if normalize:
        norms = jnp.sqrt(jnp.sum(X_train**2, axis=0))
        nonzeros = jnp.flatnonzero(norms)
    ...
    if normalize:
        coefs[nonzeros] /= norms[nonzeros][:, jnp.newaxis]
    residues = jnp.dot(X_test, coefs) - y_test[:, jnp.newaxis]
...
class LarsCV(Lars):
    ...
    def fit(self, X, y):
        ...
        all_alphas = jnp.concatenate(list(zip(*cv_paths))[0])
        # Unique also sorts
        all_alphas = jnp.unique(all_alphas)
        # Take at most max_n_alphas values
        stride = int(max(1, int(len(all_alphas) / float(self.max_n_alphas))))
        all_alphas = all_alphas[::stride]

        mse_path = jnp.empty((len(all_alphas), len(cv_paths)))
        for index, (alphas, _, _, residues) in enumerate(cv_paths):
            alphas = alphas[::-1]
            residues = residues[::-1]
            if alphas[0] != 0:
                alphas = jnp.r_[0, alphas]
                residues = jnp.r_[residues[0, jnp.newaxis], residues]
            if alphas[-1] != all_alphas[-1]:
                alphas = jnp.r_[alphas, all_alphas[-1]]
                residues = jnp.r_[residues, residues[-1, jnp.newaxis]]
            this_residues = interpolate.interp1d(
                alphas, residues, axis=0)(all_alphas)
            this_residues **= 2
            mse_path[:, index] = jnp.mean(this_residues, axis=-1)

        mask = jnp.all(jnp.isfinite(mse_path), axis=-1)
        all_alphas = all_alphas[mask]
        mse_path = mse_path[mask]
        # Select the alpha that minimizes left-out error
        i_best_alpha = jnp.argmin(mse_path.mean(axis=-1))
...
class LassoLarsIC(LassoLars):
    ...
    def fit(self, X, y, copy_X=None):
        ...
        residuals = y[:, jnp.newaxis] - jnp.dot(X, coef_path_)
        residuals_sum_squares = jnp.sum(residuals**2, axis=0)
        degrees_of_freedom = jnp.zeros(coef_path_.shape[1], dtype=int)
        for k, coef in enumerate(coef_path_.T):
            mask = jnp.abs(coef) > jnp.finfo(coef.dtype).eps
            if not jnp.any(mask):
                continue
            # get the number of degrees of freedom equal to:
            # Xc = X[:, mask]
            # Trace(Xc * inv(Xc.T, Xc) * Xc.T) ie the number of non-zero coefs
            degrees_of_freedom[k] = jnp.sum(mask)
        ...
        self.criterion_ = (
            n_samples * jnp.log(2 * jnp.pi * self.noise_variance_)
            + residuals_sum_squares / self.noise_variance_
            + criterion_factor * degrees_of_freedom
        )
        n_best = jnp.argmin(self.criterion_)
