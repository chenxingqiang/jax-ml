...
def _dynamic_max_trials(n_inliers, n_samples, min_samples, probability):
    ...
    if denom == 1:
        return float("inf")
    return abs(float(jnp.ceil(jnp.log(nom) / jnp.log(denom))))
...
class RANSACRegressor(
    ...
    min_samples : int (>= 1) or float ([0, 1]), default=None
        ...
        ``xlearn.linear_model.LinearRegression()`` estimator is assumed and
    ...
    stop_n_inliers : int, default=jnp.inf
        Stop iteration if at least this number of inliers are found.

    stop_score : float, default=jnp.inf
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.min_samples is None:
            ...
            min_samples = jnp.ceil(self.min_samples * X.shape[0])
        ...
        if self.residual_threshold is None:
            ...
            residual_threshold = jnp.median(jnp.abs(y - jnp.median(y)))
        else:
            residual_threshold = self.residual_threshold

        if self.loss == "absolute_error":
            if y.ndim == 1:
                def loss_function(y_true, y_pred): return jnp.abs(
                    y_true - y_pred)
            else:
                def loss_function(y_true, y_pred): return jnp.sum(
                    jnp.abs(y_true - y_pred), axis=1
                )
        ...
        n_samples = X.shape[0]
        sample_idxs = jnp.arange(n_samples)
        ...
        while self.n_trials_ < max_trials:
            ...
            n_inliers_subset = jnp.sum(inlier_mask_subset)
