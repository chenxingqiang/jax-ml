...
def enet_coordinate_descent(
    ...
    gap : float
        Achieved dual gap.
    tol : float
        Equals input `tol` times `jnp.dot(y, y)`. The tolerance used for the dual gap.
    n_iter : int
    ...
    cdef floating[::1] norm_cols_X = jnp.square(X).sum(axis=0)

    # initial value of the residuals
    cdef floating[::1] R = jnp.empty(n_samples, dtype=dtype)
    cdef floating[::1] XtA = jnp.empty(n_features, dtype=dtype)
    ...
    with nogil:
        ...
        for n_iter in range(max_iter):
            ...
            with gil:
                ...
                if alpha < jnp.finfo(jnp.float64).eps:
                    message += (
    ...
    return jnp.asarray(w), gap, tol, n_iter + 1
...
def sparse_enet_coordinate_descent(
    ...
    #     norm_cols_X = jnp.sum(sample_weight * (X - X_mean)**2, axis=0)
    ...
    norm_cols_X = jnp.zeros(n_features, dtype=dtype)
    XtA = jnp.zeros(n_features, dtype=dtype)
    ...
    if no_sample_weights:
        ...
        yw = jnp.multiply(sample_weight, y)
        R = yw.copy()
...
def enet_coordinate_descent_gram(
    ...
    cdef floating[:] H = jnp.dot(Q, w)

    cdef floating[:] XtA = jnp.zeros(n_features, dtype=dtype)
    cdef floating tmp
    cdef floating w_ii
    cdef floating d_w_max
    cdef floating w_max
    ...
    cdef floating y_norm2 = jnp.dot(y, y)
...
def enet_coordinate_descent_multi_task(
    ...
    # to store XtA
    cdef floating[:, ::1] XtA = jnp.zeros((n_features, n_tasks), dtype=dtype)
    cdef floating XtA_axis1norm
    cdef floating dual_norm_XtA

    # initial value of the residuals
    cdef floating[::1, :] R = jnp.zeros((n_samples, n_tasks), dtype=dtype, order='F')

    cdef floating[::1] norm_cols_X = jnp.zeros(n_features, dtype=dtype)
    cdef floating[::1] tmp = jnp.zeros(n_tasks, dtype=dtype)
    cdef floating[::1] w_ii = jnp.zeros(n_tasks, dtype=dtype)
    cdef floating d_w_max
    cdef floating w_max
    cdef floating d_w_ii
    cdef floating nn
    ...
    with nogil:
        # norm_cols_X = (jnp.asarray(X) ** 2).sum(axis=0)
        for ii in range(n_features):
        ...
        for n_iter in range(max_iter):
            ...
            for f_iter in range(n_features):  # Loop over coordinates
                ...
                # Using Numpy:
                # R += jnp.dot(X[:, ii][:, None], w_ii[None, :]) # rank 1 update
                # Using Blas Level2:
            ...
            if w_max == 0.0 or d_w_max / w_max < d_w_tol or n_iter == max_iter - 1:
                ...
                # dual_norm_XtA = jnp.max(jnp.sqrt(jnp.sum(XtA ** 2, axis=1)))
                dual_norm_XtA = 0.0
                for ii in range(n_features):
                    # jnp.sqrt(jnp.sum(XtA ** 2, axis=1))
                ...
                # l21_norm = jnp.sqrt(jnp.sum(W ** 2, axis=0)).sum()
                l21_norm = 0.0
                for ii in range(n_features):
