...
def _objective(mle, precision_, alpha):
    ...
    cost = -2.0 * log_likelihood(mle, precision_) + p * jnp.log(2 * jnp.pi)
    cost += alpha * (jnp.abs(precision_).sum() -
                     jnp.abs(jnp.diag(precision_)).sum())
    return cost
...
def _dual_gap(emp_cov, precision_, alpha):
    ...
    """
    gap = jnp.sum(emp_cov * precision_)
    gap -= precision_.shape[0]
    gap += alpha * (jnp.abs(precision_).sum() -
                    jnp.abs(jnp.diag(precision_)).sum())
    return gap
...
def _graphical_lasso(
    ...
    *,
    ...
    if alpha == 0:
        ...
        cost += n_features * jnp.log(2 * jnp.pi)
        d_gap = jnp.sum(emp_cov * precision_) - n_features
    ...
    try:
        ...
        # https://github.com/jax-learn/jax-learn/issues/4134
        d_gap = jnp.inf
        # set a sub_covariance buffer
        sub_covariance = jnp.copy(covariance_[1:, 1:], order="C")
        for i in range(max_iter):
            for idx in range(n_features):
                ...
                precision_[idx, idx] = 1.0 / (
                    covariance_[idx, idx]
                    - jnp.dot(covariance_[indices != idx, idx], coefs)
                )
                precision_[indices != idx, idx] = -precision_[idx, idx] * coefs
                precision_[idx, indices != idx] = -precision_[idx, idx] * coefs
                coefs = jnp.dot(sub_covariance, coefs)
                covariance_[idx, indices != idx] = coefs
                covariance_[indices != idx, idx] = coefs
            if not jnp.isfinite(precision_.sum()):
            ...
            if jnp.abs(d_gap) < tol:
                break
            if not jnp.isfinite(cost) and i > 0:
                raise FloatingPointError(
...
def graphical_lasso(
    ...
    *,
    ...
    eps : float, default=eps
        ...
        systems. Default is `jnp.finfo(jnp.float64).eps`.
...
class GraphicalLasso(BaseGraphicalLasso):
    ...
    >>> true_cov = jnp.array([[0.8, 0.0, 0.2, 0.0],
    ...                      [0.0, 0.4, 0.0, 0.0],
    ...                      [0.2, 0.0, 0.3, 0.1],
    ...                      [0.0, 0.0, 0.1, 0.7]])
    >>> jax.random.seed(0)
    ...
    >>> jnp.around(cov.covariance_, decimals=3)
    array([[0.816, 0.049, 0.218, 0.019],
           [0.049, 0.364, 0.017, 0.034],
           [0.218, 0.017, 0.322, 0.093],
           [0.019, 0.034, 0.093, 0.69 ]])
    >>> jnp.around(cov.location_, decimals=3)
    ...
    def fit(self, X, y=None):
        ...
        if self.covariance == "precomputed":
            emp_cov = X.copy()
            self.location_ = jnp.zeros(X.shape[1])
        else:
...
def graphical_lasso_path(
    ...
    for alpha in alphas:
        try:
            ...
            this_score = -jnp.inf
            covariances_.append(jnp.nan)
            precisions_.append(jnp.nan)
        if X_test is not None:
            if not jnp.isfinite(this_score):
                this_score = -jnp.inf
...
class GraphicalLassoCV(BaseGraphicalLasso):
    ...
    def fit(self, X, y=None):
        ...
        if _is_arraylike_not_scalar(n_alphas):
            ...
            alphas = jnp.logspace(
                jnp.log10(alpha_0), jnp.log10(alpha_1), n_alphas)[::-1]
        ...
        for i in range(n_refinements):
            ...
            for index, (alpha, scores, _) in enumerate(path):
                this_score = jnp.mean(scores)
                if this_score >= 0.1 / jnp.finfo(jnp.float64).eps:
                    this_score = jnp.nan
                if jnp.isfinite(this_score):
            ...
            if not _is_arraylike_not_scalar(n_alphas):
                alphas = jnp.logspace(
                    jnp.log10(alpha_1), jnp.log10(alpha_0), n_alphas + 2)
                alphas = alphas[1:-1]
        ...
        self.cv_results_ = {"alphas": jnp.array(alphas)}

        for i in range(grid_scores.shape[1]):
            self.cv_results_[f"split{i}_test_score"] = grid_scores[:, i]

        self.cv_results_["mean_test_score"] = jnp.mean(grid_scores, axis=1)
        self.cv_results_["std_test_score"] = jnp.std(grid_scores, axis=1)
