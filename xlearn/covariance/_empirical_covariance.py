...
def log_likelihood(emp_cov, precision):
    ...
    """
    p = precision.shape[0]
    log_likelihood_ = -jnp.sum(emp_cov * precision) + fast_logdet(precision)
    log_likelihood_ -= p * jnp.log(2 * jnp.pi)
    log_likelihood_ /= 2.0
...
def empirical_covariance(X, *, assume_centered=False):
    ...
    if assume_centered:
        covariance = jnp.dot(X.T, X) / X.shape[0]
    else:
        covariance = jnp.cov(X.T, bias=1)
...
class EmpiricalCovariance(BaseEstimator):
    ...
    def error_norm(self, comp_cov, norm="frobenius", scaling=True, squared=True):
        ...
        if norm == "frobenius":
            squared_norm = jnp.sum(error**2)
        elif norm == "spectral":
            squared_norm = jnp.amax(linalg.svdvals(jnp.dot(error.T, error)))
        else:
    ...
    def mahalanobis(self, X):
        ...
        with config_context(assume_finite=True):
            ...
            dist = pairwise_distances(
                X, self.location_[jnp.newaxis, :], metric="mahalanobis", VI=precision
            )

        return jnp.reshape(dist, (len(X),)) ** 2
