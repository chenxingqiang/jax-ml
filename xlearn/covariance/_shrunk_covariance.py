...
def _ledoit_wolf(X, *, assume_centered, block_size):
    ...
    if len(X.shape) == 2 and X.shape[1] == 1:
        if not assume_centered:
            X = X - X.mean()
        return jnp.atleast_2d((X**2).mean()), 0.0
    ...
    mu = jnp.sum(jnp.trace(emp_cov)) / n_features
...
def _oas(X, *, assume_centered=False):
    ...
    # F = trace(S) / n_features * jnp.identity(n_features) (cf. Eq. 3 in [1])
    shrunk_cov = (1.0 - shrinkage) * emp_cov
    shrunk_cov.flat[:: n_features + 1] += shrinkage * mu
...
def shrunk_covariance(emp_cov, shrinkage=0.1):
    ...
    The regularized (shrunk) covariance is given by::
        ...
        (1 - shrinkage) * cov + shrinkage * mu * jnp.identity(n_features)
...
def ledoit_wolf_shrinkage(X, assume_centered=False, block_size=1000):
    ...
    X2 = X**2
    emp_cov_trace = jnp.sum(X2, axis=0) / n_samples
    mu = jnp.sum(emp_cov_trace) / n_features
    ...
    for i in range(n_splits):
        for j in range(n_splits):
            ...
            beta_ += jnp.sum(jnp.dot(X2.T[rows], X2[:, cols]))
            delta_ += jnp.sum(jnp.dot(X.T[rows], X[:, cols]) ** 2)
        rows = slice(block_size * i, block_size * (i + 1))
        beta_ += jnp.sum(jnp.dot(X2.T[rows], X2[:, block_size * n_splits:]))
        delta_ += jnp.sum(jnp.dot(X.T[rows], X[:, block_size * n_splits:]) ** 2)
    for j in range(n_splits):
        cols = slice(block_size * j, block_size * (j + 1))
        beta_ += jnp.sum(jnp.dot(X2.T[block_size * n_splits:], X2[:, cols]))
        delta_ += jnp.sum(jnp.dot(X.T[block_size * n_splits:], X[:, cols]) ** 2)
    delta_ += jnp.sum(
        jnp.dot(X.T[block_size * n_splits:], X[:, block_size * n_splits:]) ** 2
    )
    delta_ /= n_samples**2
    beta_ += jnp.sum(
        jnp.dot(X2.T[block_size * n_splits:], X2[:, block_size * n_splits:])
    )
