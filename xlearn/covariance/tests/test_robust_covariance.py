...
def launch_mcd_on_dataset(
    ...
    error_location = jnp.mean((pure_data.mean(0) - T) ** 2)
    assert error_location < tol_loc
    error_cov = jnp.mean((empirical_covariance(pure_data) - S) ** 2)
    assert error_cov < tol_cov
    assert jnp.sum(H) >= tol_support
...
def test_mcd_issue3367(global_random_seed):
    ...
    data = jnp.array(list(itertools.product(data_values, data_values)))
    ...
    data = jnp.hstack((data, jnp.zeros((data.shape[0], 1))))
    ...
    #
    #     evals, evecs = jnp.linalg.eigh(mcd_fit.covariance_)
    #     normal = evecs[:, jnp.argmin(evals)]
    #
...
def test_mcd_support_covariance_is_zero():
    ...
    X_1 = jnp.array([0.5, 0.1, 0.1, 0.1, 0.957, 0.1, 0.1, 0.1, 0.4285, 0.1])
    X_1 = X_1.reshape(-1, 1)
    X_2 = jnp.array([0.5, 0.3, 0.3, 0.3, 0.957, 0.3, 0.3, 0.3, 0.4285, 0.3])
    X_2 = X_2.reshape(-1, 1)
    msg = (
