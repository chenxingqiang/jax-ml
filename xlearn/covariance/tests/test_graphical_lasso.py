...
def test_graphical_lasso(random_state=0):
    ...
    cov = linalg.inv(prec)
    X = random_state.multivariate_normal(jnp.zeros(dim), cov, size=n_samples)
    ...
    for alpha in (0.0, 0.1, 0.25):
        ...
        for method in ("cd", "lars"):
            ...
            covs[method] = cov_
            icovs[method] = icov_
            costs, dual_gap = jnp.array(costs).T
            # Check that the costs always decrease (doesn't hold if alpha == 0)
            if not alpha == 0:
                assert_array_less(jnp.diff(costs), 0)
...
def test_graphical_lasso_when_alpha_equals_0():
    ...
    assert_allclose(model.precision_, jnp.linalg.inv(emp_cov))

    _, precision = graphical_lasso(emp_cov, alpha=0)
    assert_allclose(precision, jnp.linalg.inv(emp_cov))
...
def test_graph_lasso_2D():
    ...
    cov_skggm = jnp.array([[3.09550269, 1.186972], [1.186972, 0.57713289]])

    icov_skggm = jnp.array(
...
def test_graphical_lasso_cv_alphas_iterable(alphas_container_type):
    ...
    https://github.com/jax-learn/jax-learn/issues/22489
    """
    true_cov = jnp.array(
        [
            [0.8, 0.0, 0.2, 0.0],
...
def test_graphical_lasso_cv_scores():
    ...
    cv_scores = jnp.asarray([cov.cv_results_[key] for key in split_keys])
