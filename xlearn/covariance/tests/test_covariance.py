...
def test_covariance():
    ...
    X_1sample = jnp.arange(5).reshape(1, 5)
    cov = EmpiricalCovariance()
    ...
    assert_array_almost_equal(
        cov.covariance_, jnp.zeros(shape=(5, 5), dtype=jnp.float64))

    # test integer type
    X_integer = jnp.asarray([[0, 1], [1, 0]])
    result = jnp.asarray([[0.25, -0.25], [-0.25, 0.25]])
    assert_array_almost_equal(empirical_covariance(X_integer), result)

    # test centered case
    cov = EmpiricalCovariance(assume_centered=True)
    cov.fit(X)
    assert_array_equal(cov.location_, jnp.zeros(X.shape[1]))
...
def test_ledoit_wolf():
    ...
    assert_array_almost_equal(
        lw.covariance_, jnp.zeros(shape=(5, 5), dtype=jnp.float64))
...
def _naive_ledoit_wolf_shrinkage(X):
    ...
    beta_ = (
        ...
        * jnp.sum(jnp.dot(X2.T, X2) / n_samples - emp_cov**2)
    )
...
def test_ledoit_wolf_large():
    ...
    assert_almost_equal(lw.covariance_, jnp.eye(20), 0)
    cov = lw.covariance_
...
def test_oas():
    ...
    assert_array_almost_equal(
        oa.covariance_, jnp.zeros(shape=(5, 5), dtype=jnp.float64))
