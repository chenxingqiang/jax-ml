...
def _c_step(
    ...
    # Initialisation
    support = jnp.zeros(n_samples, dtype=bool)
    if initial_estimates is None:
        ...
        dist = (jnp.dot(X_centered, precision) * X_centered).sum(1)
        # compute new estimates
        support[jnp.argsort(dist)[:n_support]] = True
    ...
    previous_det = jnp.inf
    while det < previous_det and remaining_iterations > 0 and not jnp.isinf(det):
        ...
        precision = linalg.pinvh(covariance)
        X_centered = X - location
        dist = (jnp.dot(X_centered, precision) * X_centered).sum(axis=1)
        # compute new estimates
        support = jnp.zeros(n_samples, dtype=bool)
        support[jnp.argsort(dist)[:n_support]] = True
        X_support = X[support]
    ...
    previous_dist = dist
    dist = (jnp.dot(X - location, precision) * (X - location)).sum(axis=1)
    # Check if best fit already found (det => 0, logdet => -inf)
    if jnp.isinf(det):
        results = location, covariance, det, support, dist
    # Check convergence
    if jnp.allclose(det, previous_det):
...
def select_candidates(
    ...
    index_best = jnp.argsort(all_dets_sub)[:select]
    best_locations = jnp.asarray(all_locs_sub)[index_best]
    best_covariances = jnp.asarray(all_covs_sub)[index_best]
    best_supports = jnp.asarray(all_supports_sub)[index_best]
    best_ds = jnp.asarray(all_ds_sub)[index_best]
...
def fast_mcd(
    ...
    if support_fraction is None:
        n_support = int(jnp.ceil(0.5 * (n_samples + n_features + 1)))
    else:
    ...
    if n_features == 1:
        if n_support < n_samples:
            ...
            X_sorted = jnp.sort(jnp.ravel(X))
            diff = X_sorted[n_support:] - X_sorted[: (n_samples - n_support)]
            halves_start = jnp.where(diff == jnp.min(diff))[0]
            ...
            support = jnp.zeros(n_samples, dtype=bool)
            X_centered = X - location
            support[jnp.argsort(jnp.abs(X_centered), 0)[:n_support]] = True
            covariance = jnp.asarray([[jnp.var(X[support])]])
            location = jnp.array([location])
            # get precision matrix in an optimized way
            precision = linalg.pinvh(covariance)
            dist = (jnp.dot(X_centered, precision) * (X_centered)).sum(axis=1)
        else:
            support = jnp.ones(n_samples, dtype=bool)
            covariance = jnp.asarray([[jnp.var(X)]])
            location = jnp.asarray([jnp.mean(X)])
            X_centered = X - location
            # get precision matrix in an optimized way
            precision = linalg.pinvh(covariance)
            dist = (jnp.dot(X_centered, precision) * (X_centered)).sum(axis=1)
    ...
    if (n_samples > 500) and (n_features > 1):
        ...
        h_subset = int(jnp.ceil(n_samples_subsets *
        ...
        all_best_locations = jnp.zeros((n_best_tot, n_features))
        try:
            all_best_covariances = jnp.zeros(
        ...
        for i in range(n_subsets):
            ...
            subset_slice = jnp.arange(i * n_best_sub, (i + 1) * n_best_sub)
            all_best_locations[subset_slice] = best_locations_sub
            all_best_covariances[subset_slice] = best_covariances_sub
        # 2. Pool the candidate supports into a merged set
        # (possibly the full dataset)
        n_samples_merged = min(1500, n_samples)
        h_merged = int(jnp.ceil(n_samples_merged *
                       (n_support / float(n_samples))))
        if n_samples > 1500:
            n_best_merged = 10
        else:
...
class MinCovDet(EmpiricalCovariance):
    ...
    def fit(self, X, y=None):
        ...
        if (linalg.svdvals(jnp.dot(X.T, X)) > 1e-8).sum() != n_features:
            warnings.warn(
        ...
        if self.assume_centered:
            raw_location = jnp.zeros(n_features)
            raw_covariance = self._nonrobust_covariance(
                X[raw_support], assume_centered=True
            )
            # get precision matrix in an optimized way
            precision = linalg.pinvh(raw_covariance)
            raw_dist = jnp.sum(jnp.dot(X, precision) * X, 1)
    ...
    def correct_covariance(self, data):
        ...
        n_support = jnp.sum(self.support_)
        if n_support < n_samples and jnp.allclose(self.raw_covariance_, 0):
            raise ValueError(
                "The covariance matrix of the support data "
                "is equal to 0, try to increase support_fraction"
            )
        correction = jnp.median(self.dist_) / chi2(data.shape[1]).isf(0.5)
    ...
    def reweight_covariance(self, data):
        ...
        if self.assume_centered:
            location_reweighted = jnp.zeros(n_features)
        else:
            location_reweighted = data[mask].mean(0)
        covariance_reweighted = self._nonrobust_covariance(
            data[mask], assume_centered=self.assume_centered
        )
        support_reweighted = jnp.zeros(n_samples, dtype=bool)
        ...
        self.dist_ = jnp.sum(
            jnp.dot(X_centered, self.get_precision()) * X_centered, 1)
