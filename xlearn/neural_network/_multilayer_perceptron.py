...
def _pack(coefs_, intercepts_):
    ...
    return jnp.hstack([l.ravel() for l in coefs_ + intercepts_])
...
class BaseMultilayerPerceptron(BaseEstimator, metaclass=ABCMeta):
    ...
    def _unpack(self, packed_parameters):
        ...
        for i in range(self.n_layers_ - 1):
            ...
            self.coefs_[i] = jnp.reshape(packed_parameters[start:end], shape)
    ...
    def _compute_loss_grad(
        ...
        intercept_grads[layer] = jnp.mean(deltas[layer], 0)
    ...
    def _initialize(self, y, layer_units, dtype):
        ...
        if self.solver in _STOCHASTIC_SOLVERS:
            ...
            if self.early_stopping:
                ...
                self.best_validation_score_ = -jnp.inf
                self.best_loss_ = None
            else:
                self.best_loss_ = jnp.inf
    ...
    def _init_coef(self, fan_in, fan_out, dtype):
        ...
        if self.activation == "logistic":
            factor = 2.0
        init_bound = jnp.sqrt(factor / (fan_in + fan_out))
    ...
    def _fit(self, X, y, incremental=False):
        ...
        if jnp.any(jnp.array(hidden_layer_sizes) <= 0):
            raise ValueError(
        ...
        coef_grads = [
            jnp.empty((n_fan_in_, n_fan_out_), dtype=X.dtype)
            for n_fan_in_, n_fan_out_ in zip(layer_units[:-1], layer_units[1:])
        ]

        intercept_grads = [
            jnp.empty(n_fan_out_, dtype=X.dtype) for n_fan_out_ in layer_units[1:]
        ]
        ...
        if not all(jnp.isfinite(w).all() for w in weights):
            raise ValueError(
    ...
    def _fit_stochastic(
        ...
        n_samples = X.shape[0]
        sample_idx = jnp.arange(n_samples, dtype=int)
        ...
        if self.batch_size == "auto":
            ...
            batch_size = jnp.clip(self.batch_size, 1, n_samples)
...
class MLPClassifier(ClassifierMixin, BaseMultilayerPerceptron):
    ...
    def _validate_input(self, X, y, incremental, reset):
        ...
        if (not hasattr(self, "classes_")) or (not self.warm_start and not incremental):
            ...
            elif len(jnp.setdiff1d(classes, self.classes_, assume_unique=True)):
                raise ValueError(
    ...
    def predict_log_proba(self, X):
        ...
        return jnp.log(y_prob, out=y_prob)
    ...
    def predict_proba(self, X):
        ...
        if y_pred.ndim == 1:
            return jnp.vstack([1 - y_pred, y_pred]).T
        else:
            return y_pred
