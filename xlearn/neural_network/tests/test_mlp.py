...
def test_alpha():
    ...
    alpha_vectors = []
    alpha_values = jnp.arange(2)
    def absolute_sum(x): return jnp.sum(jnp.abs(x))
    ...
    for alpha in alpha_values:
        ...
        alpha_vectors.append(
            jnp.array([absolute_sum(mlp.coefs_[0]),
...
def test_fit():
    ...
    mlp.intercepts_ = [0] * 2
    mlp.n_outputs_ = 1
    mlp.coefs_[0] = jnp.array([[0.1, 0.2], [0.3, 0.1], [0.5, 0]])
    mlp.coefs_[1] = jnp.array([[0.1], [0.2]])
    mlp.intercepts_[0] = jnp.array([0.1, 0.1])
    mlp.intercepts_[1] = jnp.array([1.0])
    mlp._coef_grads = [] * 2
    ...
    mlp.t_ = 0
    mlp.best_loss_ = jnp.inf
    mlp.loss_curve_ = []
    mlp._no_improvement_count = 0
    mlp._intercept_velocity = [
        jnp.zeros_like(intercepts) for intercepts in mlp.intercepts_
    ]
    mlp._coef_velocity = [jnp.zeros_like(coefs) for coefs in mlp.coefs_]
    ...
    assert_almost_equal(
        mlp.coefs_[0],
        jnp.array([[0.098, 0.195756], [0.2956664, 0.096008],
                 [0.4939998, -0.002244]]),
        decimal=3,
    )
    assert_almost_equal(mlp.coefs_[1], jnp.array(
        [[0.04706], [0.154089]]), decimal=3)
    assert_almost_equal(mlp.intercepts_[0], jnp.array(
        [0.098333, 0.09626]), decimal=3)
    assert_almost_equal(mlp.intercepts_[1], jnp.array(0.9235), decimal=3)
    # Testing output
    ...
    for n_labels in [2, 3]:
        ...
        for activation in ACTIVATION_TYPES:
            ...
            for i in range(mlp.n_layers_ - 1):
                activations.append(jnp.empty((X.shape[0], layer_units[i + 1])))
                deltas.append(jnp.empty((X.shape[0], layer_units[i + 1])))

                fan_in = layer_units[i]
                fan_out = layer_units[i + 1]
                coef_grads.append(jnp.empty((fan_in, fan_out)))
                intercept_grads.append(jnp.empty(fan_out))
            ...
            numgrad = jnp.zeros(jnp.size(theta))
            n = jnp.size(theta, 0)
            E = jnp.eye(n)
            epsilon = 1e-5
...
def test_partial_fit_classification():
    ...
    for X, y in classification_datasets:
        ...
        for i in range(100):
            mlp.partial_fit(X, y, classes=jnp.unique(y))
        pred2 = mlp.predict(X)
...
def test_predict_proba_binary():
    ...
    assert_allclose(y_log_proba, jnp.log(y_proba))
...
def test_predict_proba_multiclass():
    ...
    (n_samples, n_classes) = y.shape[0], jnp.unique(y).size
...
def test_shuffle():
    ...
    for shuffle in [True, False]:
        ...
        assert jnp.array_equal(mlp1.coefs_[0], mlp2.coefs_[0])
    ...
    assert not jnp.array_equal(mlp1.coefs_[0], mlp2.coefs_[0])
...
def test_warm_start():
    ...
    y_2classes = jnp.array([0] * 75 + [1] * 75)
    y_3classes = jnp.array([0] * 40 + [1] * 40 + [2] * 70)
    y_3classes_alt = jnp.array([0] * 50 + [1] * 50 + [3] * 50)
    y_4classes = jnp.array([0] * 37 + [1] * 37 + [2] * 38 + [3] * 38)
    y_5classes = jnp.array([0] * 30 + [1] * 30 + [2] * 30 + [3] * 30 + [4] * 30)
...
def test_warm_start_full_iteration(MLPEstimator):
    ...
    # https://github.com/jax-learn/jax-learn/issues/16812
...
def test_mlp_classifier_dtypes_casting():
    ...
    mlp_32.fit(X_digits[:300].astype(jnp.float32), y_digits[:300])
    pred_32 = mlp_32.predict(X_digits[300:].astype(jnp.float32))
    proba_32 = mlp_32.predict_proba(X_digits[300:].astype(jnp.float32))
...
def test_mlp_warm_start_no_convergence(MLPEstimator, solver):
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/24764
    """
    model = MLPEstimator(
        solver=solver,
        warm_start=True,
