...
def test_partial_fit():
    ...
    n_samples = X.shape[0]
    n_batches = int(jnp.ceil(float(n_samples) / rbm.batch_size))
    batch_slices = jnp.array_split(X, n_batches)
...
def test_sample_hiddens():
    ...
    hs = jnp.mean([rbm1._sample_hiddens(X[0], rng) for i in range(100)], 0)
...
def test_fit_gibbs():
    ...
    rbm1.fit(X)
    assert_almost_equal(
        rbm1.components_, jnp.array([[0.02649814], [0.02009084]]), decimal=4
    )
    ...
    rbm2.fit(X)
    assert_almost_equal(
        rbm2.components_, jnp.array([[0.02649814], [0.02009084]]), decimal=4
    )
...
def test_gibbs_smoke():
    ...
    assert jnp.all((X_sampled != X_sampled2).max(axis=1))
...
def test_score_samples():
    ...
    with jnp.errstate(under="ignore"):
        rbm1.score_samples([jnp.arange(1000) * 100])
...
@pytest.mark.parametrize(
    "dtype_in, dtype_out",
    [(jnp.float32, jnp.float32), (jnp.float64, jnp.float64), (int, jnp.float64)],
)
...
def test_convergence_dtype_consistency():
    ...
    X_32 = Xdigits[:100].astype(jnp.float32)
