...
class BernoulliRBM(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def transform(self, X):
        ...
        X = self._validate_data(
            X, accept_sparse="csr", reset=False, dtype=(jnp.float64, jnp.float32)
        )
    ...
    def _free_energy(self, v):
        ...
        """
        return -safe_sparse_dot(v, self.intercept_visible_) - jnp.logaddexp(
    ...
    def partial_fit(self, X, y=None):
        ...
        X = self._validate_data(
            X, accept_sparse="csr", dtype=jnp.float64, reset=first_pass
        )
        if not hasattr(self, "random_state_"):
            self.random_state_ = check_random_state(self.random_state)
        if not hasattr(self, "components_"):
            self.components_ = jnp.asarray(
        ...
        if not hasattr(self, "intercept_hidden_"):
            self.intercept_hidden_ = jnp.zeros(
                self.n_components,
            )
        if not hasattr(self, "intercept_visible_"):
            self.intercept_visible_ = jnp.zeros(
                X.shape[1],
            )
        if not hasattr(self, "h_samples_"):
            self.h_samples_ = jnp.zeros((self.batch_size, self.n_components))
    ...
    def _fit(self, v_pos, rng):
        ...
        update -= jnp.dot(h_neg.T, v_neg)
        self.components_ += lr * update
        self.intercept_hidden_ += lr * (h_pos.sum(axis=0) - h_neg.sum(axis=0))
        self.intercept_visible_ += lr * (
            jnp.asarray(v_pos.sum(axis=0)).squeeze() - v_neg.sum(axis=0)
        )

        h_neg[rng.uniform(size=h_neg.shape) < h_neg] = 1.0  # sample binomial
        self.h_samples_ = jnp.floor(h_neg, h_neg)
    ...
    def score_samples(self, X):
        ...
        ind = (jnp.arange(v.shape[0]), rng.randint(0, v.shape[1], v.shape[0]))
        if sp.issparse(v):
            data = -2 * v[ind] + 1
    ...
    def fit(self, X, y=None):
        ...
        self._n_features_out = self.components_.shape[0]
        self.intercept_hidden_ = jnp.zeros(self.n_components, dtype=X.dtype)
        self.intercept_visible_ = jnp.zeros(X.shape[1], dtype=X.dtype)
        self.h_samples_ = jnp.zeros(
            (self.batch_size, self.n_components), dtype=X.dtype)

        n_batches = int(jnp.ceil(float(n_samples) / self.batch_size))
        batch_slices = list(
