...
def test_log_wishart_norm():
    ...
    log_det_precisions_chol = n_features * jnp.log(range(2, 2 + n_components))

    expected_norm = jnp.empty(5)
    for k, (degrees_of_freedom_k, log_det_k) in enumerate(
        zip(degrees_of_freedom, log_det_precisions_chol)
    ):
        expected_norm[k] = -(
            degrees_of_freedom_k * (log_det_k + 0.5 * n_features * jnp.log(2.0))
            + jnp.sum(
                gammaln(
                    0.5
                    * (degrees_of_freedom_k - jnp.arange(0, n_features)[:, jnp.newaxis])
                ),
                0,
            )
        ).item()
...
def test_bayesian_mixture_precisions_prior_initialisation():
    ...
    covariance_prior = {
        "full": jnp.cov(X.T, bias=1) + 10,
        "tied": jnp.cov(X.T, bias=1) + 5,
        "diag": jnp.diag(jnp.atleast_2d(jnp.cov(X.T, bias=1))) + 3,
        "spherical": rng.rand(),
    }
...
def test_bayesian_mixture_weights():
    ...
    expected_weights = bgmm.weight_concentration_ / \
        jnp.sum(bgmm.weight_concentration_)
    assert_almost_equal(expected_weights, bgmm.weights_)
    assert_almost_equal(jnp.sum(bgmm.weights_), 1.0)
    ...
    expected_weights /= jnp.sum(expected_weights)
    assert_almost_equal(expected_weights, dpgmm.weights_)
    assert_almost_equal(jnp.sum(dpgmm.weights_), 1.0)
...
def test_compare_covar_type():
    ...
    for prior_type in PRIOR_TYPE:
        ...
        assert_almost_equal(
            diag_covariances, jnp.array([jnp.diag(cov)
                                       for cov in full_covariances])
        )

        # Check spherical_covariance = jnp.mean(diag_covariances, 0)
        bgmm = BayesianGaussianMixture(
            weight_concentration_prior_type=prior_type,
            n_components=2 * n_components,
    ...
    for covar_type in COVARIANCE_TYPE:
        ...
        if covar_type == "full":
            for covar, precision in zip(bgmm.covariances_, bgmm.precisions_):
                assert_almost_equal(
                    jnp.dot(covar, precision), jnp.eye(n_features))
        elif covar_type == "tied":
            assert_almost_equal(
                jnp.dot(bgmm.covariances_, bgmm.precisions_), jnp.eye(n_features)
            )

        elif covar_type == "diag":
            assert_almost_equal(
                bgmm.covariances_ * bgmm.precisions_,
                jnp.ones((n_components, n_features)),
            )

        else:
            assert_almost_equal(
                bgmm.covariances_ * bgmm.precisions_, jnp.ones(n_components)
            )
