...
class RandomData:
    def __init__(self, rng, n_samples=200, n_components=2, n_features=2, scale=50):
        ...
        self.Y = jnp.hstack(
            [
                jnp.full(int(jnp.round(w * n_samples)), k, dtype=int)
...
def test_check_weights():
    ...
    msg = re.escape(
        ...
        f" max value {jnp.min(weights_bad_range):.5f}, "
        f"min value {jnp.max(weights_bad_range):.5f}"
    )
    with pytest.raises(ValueError, match=msg):
        g.fit(X)
    ...
    msg = re.escape(
        ...
        f"but got sum(weights) = {jnp.sum(weights_bad_norm):.5f}"
    )
    with pytest.raises(ValueError, match=msg):
        g.fit(X)
...
def test_check_precisions():
    ...
    precisions_bad_shape = {
        "full": jnp.ones((n_components + 1, n_features, n_features)),
        "tied": jnp.ones((n_features + 1, n_features + 1)),
        "diag": jnp.ones((n_components + 1, n_features)),
        "spherical": jnp.ones((n_components + 1)),
    }

    # Define not positive-definite precisions
    precisions_not_pos = jnp.ones((n_components, n_features, n_features))
    precisions_not_pos[0] = jnp.eye(n_features)
    precisions_not_pos[0, 0, 0] = -1.0

    precisions_not_positive = {
        "full": precisions_not_pos,
        "tied": precisions_not_pos[0],
        "diag": jnp.full((n_components, n_features), -1.0),
        "spherical": jnp.full(n_components, -1.0),
    }
...
def test_suffstat_sk_full():
    ...
    X_resp = jnp.sqrt(resp) * X
    nk = jnp.array([n_samples])
    xk = jnp.zeros((1, n_features))
    ...
    precs_pred = jnp.array([jnp.dot(prec, prec.T) for prec in precs_chol_pred])
    precs_est = jnp.array([linalg.inv(cov) for cov in covars_pred])
    assert_array_almost_equal(precs_est, precs_pred)

    # special case 2, assuming resp are all ones
    resp = jnp.ones((n_samples, 1))
    nk = jnp.array([n_samples])
...
def test_suffstat_sk_tied():
    ...
    resp = resp / resp.sum(axis=1)[:, jnp.newaxis]
    X = rng.rand(n_samples, n_features)
    nk = resp.sum(axis=0)
    xk = jnp.dot(resp.T, X) / nk[:, jnp.newaxis]

    covars_pred_full = _estimate_gaussian_covariances_full(resp, X, nk, xk, 0)
    covars_pred_full = (
        jnp.sum(nk[:, jnp.newaxis, jnp.newaxis] * covars_pred_full, 0) / n_samples
    )
    ...
    precs_pred = jnp.dot(precs_chol_pred, precs_chol_pred.T)
...
def test_suffstat_sk_diag():
    ...
    for cov_full, cov_diag in zip(covars_pred_full, covars_pred_diag):
        ecov.covariance_ = jnp.diag(jnp.diag(cov_full))
        cov_diag = jnp.diag(cov_diag)
...
def test_gaussian_suffstat_sk_spherical():
    ...
    covars_pred_spherical = _estimate_gaussian_covariances_spherical(
        resp, X, nk, xk, 0)
    covars_pred_spherical2 = jnp.dot(X.flatten().T, X.flatten()) / (
...
def test_compute_log_det_cholesky():
    ...
    for covar_type in COVARIANCE_TYPE:
        ...
        if covar_type == "full":
            predected_det = jnp.array([linalg.det(cov) for cov in covariance])
        elif covar_type == "tied":
            predected_det = linalg.det(covariance)
        elif covar_type == "diag":
            predected_det = jnp.array([jnp.prod(cov) for cov in covariance])
        ...
        assert_array_almost_equal(expected_det, -0.5 * jnp.log(predected_det))


def _naive_lmvnpdf_diag(X, means, covars):
    resp = jnp.empty((len(X), len(means)))
    stds = jnp.sqrt(covars)
...
def test_gaussian_mixture_log_probabilities():
    ...
    # full covariances
    precs_full = jnp.array([jnp.diag(1.0 / jnp.sqrt(x)) for x in covars_diag])

    log_prob = _estimate_log_gaussian_prob(X, means, precs_full, "full")
    assert_array_almost_equal(log_prob, log_prob_naive)

    # diag covariances
    precs_chol_diag = 1.0 / jnp.sqrt(covars_diag)
    log_prob = _estimate_log_gaussian_prob(X, means, precs_chol_diag, "diag")
    assert_array_almost_equal(log_prob, log_prob_naive)

    # tied
    covars_tied = jnp.array([x for x in covars_diag]).mean(axis=0)
    precs_tied = jnp.diag(jnp.sqrt(1.0 / covars_tied))
    ...
    precs_spherical = 1.0 / jnp.sqrt(covars_diag.mean(axis=1))
    log_prob_naive = _naive_lmvnpdf_diag(
...
def test_gaussian_mixture_estimate_log_prob_resp():
    ...
    for covar_type in COVARIANCE_TYPE:
        ...
        assert_array_almost_equal(resp.sum(axis=1), jnp.ones(n_samples))
...
def test_gaussian_mixture_fit():
    ...
    for covar_type in COVARIANCE_TYPE:
        ...
        assert_allclose(
            jnp.sort(g.weights_), jnp.sort(rand_data.weights), rtol=0.1, atol=1e-2
        )
        ...
        if covar_type == "full":
            ...
            prec_pred = jnp.array([g.precisions_] * n_components)
            prec_test = jnp.array([rand_data.precisions["tied"]] * n_components)
        elif covar_type == "spherical":
            prec_pred = jnp.array(
                [jnp.eye(n_features) * c for c in g.precisions_])
            prec_test = jnp.array(
                [jnp.eye(n_features) * c for c in rand_data.precisions["spherical"]]
            )
        elif covar_type == "diag":
            prec_pred = jnp.array([jnp.diag(d) for d in g.precisions_])
            prec_test = jnp.array([jnp.diag(d)
                                 for d in rand_data.precisions["diag"]])

        arg_idx1 = jnp.trace(prec_pred, axis1=1, axis2=2).argsort()
        arg_idx2 = jnp.trace(prec_test, axis1=1, axis2=2).argsort()
...
def test_gaussian_mixture_aic_bic():
    ...
    sgh = 0.5 * (
        fast_logdet(jnp.cov(X.T, bias=1)) + n_features * (1 + jnp.log(2 * jnp.pi))
    )
    for cv_type in COVARIANCE_TYPE:
        g = GaussianMixture(
            n_components=n_components,
        ...
        bic = 2 * n_samples * sgh + jnp.log(n_samples) * g._n_parameters()
        bound = n_features / jnp.sqrt(n_samples)
...
def test_monotonic_likelihood():
    ...
    for covar_type in COVARIANCE_TYPE:
        ...
        current_log_likelihood = -jnp.inf
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", ConvergenceWarning)
...
def test_regularisation():
    ...
    X = jnp.vstack(
        (jnp.ones((n_samples // 2, n_features)),
         jnp.zeros((n_samples // 2, n_features)))
    )
...
def test_sample():
    ...
    for covar_type in COVARIANCE_TYPE:
        ...
        for k in range(n_components):
            if covar_type == "full":
                assert_array_almost_equal(
                    gmm.covariances_[k], jnp.cov(X_s[y_s == k].T), decimal=1
                )
            elif covar_type == "tied":
                assert_array_almost_equal(
                    gmm.covariances_, jnp.cov(X_s[y_s == k].T), decimal=1
                )
            elif covar_type == "diag":
                assert_array_almost_equal(
                    gmm.covariances_[k], jnp.diag(jnp.cov(X_s[y_s == k].T)), decimal=1
                )
            else:
                assert_array_almost_equal(
                    gmm.covariances_[k],
                    jnp.var(X_s[y_s == k] - gmm.means_[k]),
                    decimal=1,
                )

        means_s = jnp.array([jnp.mean(X_s[y_s == k], 0)
                           for k in range(n_components)])
        assert_array_almost_equal(gmm.means_, means_s, decimal=1)

        # Check shapes of sampled data, see
        # https://github.com/jax-learn/jax-learn/issues/7701
...
def test_init_means_not_duplicated(init_params, global_random_seed):
    ...
    for i_mean, j_mean in itertools.combinations(means, r=2):
        assert not jnp.allclose(i_mean, j_mean)
...
def test_means_for_all_inits(init_params, global_random_seed):
    ...
    assert jnp.all(X.min(axis=0) <= gmm.means_)
    assert jnp.all(gmm.means_ <= X.max(axis=0))
    assert gmm.converged_
...
def test_gaussian_mixture_precisions_init_diag():
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/16944
    """
    ...
    X = jnp.vstack([shifted_gaussian, stretched_gaussian])
    ...
    resp = jnp.zeros((X.shape[0], n_components))
    label = (
        KMeans(n_clusters=n_components, n_init=1, random_state=random_state)
        .fit(X)
        .labels_
    )
    resp[jnp.arange(X.shape[0]), label] = 1
