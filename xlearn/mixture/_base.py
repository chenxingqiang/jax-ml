...
class BaseMixture(DensityMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _initialize_parameters(self, X, random_state):
        ...
        if self.init_params == "kmeans":
            resp = jnp.zeros((n_samples, self.n_components))
            label = (
                cluster.KMeans(
            ...
            resp[jnp.arange(n_samples), label] = 1
        elif self.init_params == "random":
            resp = random_state.uniform(size=(n_samples, self.n_components))
            resp /= resp.sum(axis=1)[:, jnp.newaxis]
        elif self.init_params == "random_from_data":
            resp = jnp.zeros((n_samples, self.n_components))
            indices = random_state.choice(
                n_samples, size=self.n_components, replace=False
            )
            resp[indices, jnp.arange(self.n_components)] = 1
    ...
    def fit_predict(self, X, y=None):
        ...
        X = self._validate_data(
            X, dtype=[jnp.float64, jnp.float32], ensure_min_samples=2)
        ...
        for init in range(n_init):
            ...
            lower_bound = -jnp.inf if do_init else self.lower_bound_
            ...
            if self.max_iter == 0:
                ...
                if lower_bound > max_lower_bound or max_lower_bound == -jnp.inf:
