...
def _log_dirichlet_norm(dirichlet_concentration):
    ...
    """
    return gammaln(jnp.sum(dirichlet_concentration)) - jnp.sum(
...
class BayesianGaussianMixture(BaseMixture):
    ...
    >>> X = jnp.array([[1, 2], [1, 4], [1, 0], [4, 2], [12, 4], [10, 7]])
        ...
        if self.mean_prior is None:
            ...
            self.mean_prior_ = check_array(
                self.mean_prior, dtype=[
                    jnp.float64, jnp.float32], ensure_2d=False
            )
    ...
    def _checkcovariance_prior_parameter(self, X):
        ...
        if self.covariance_prior is None:
            self.covariance_prior_ = {
                "full": jnp.atleast_2d(jnp.cov(X.T)),
                "tied": jnp.atleast_2d(jnp.cov(X.T)),
                "diag": jnp.var(X, axis=0, ddof=1),
                "spherical": jnp.var(X, axis=0, ddof=1).mean(),
        ...
        if self.weight_concentration_prior_type == "dirichlet_process":
            ...
            self.weight_concentration_ = (
                ...
                (
                    ...
                    + jnp.hstack((jnp.cumsum(nk[::-1])[-2::-1], 0))
                ),
            )
        else:
    ...
    def _estimate_means(self, nk, xk):
        ...
        self.means_ = (
            ...
            self.mean_prior_ + nk[:, jnp.newaxis] * xk
        ) / self.mean_precision_[:, jnp.newaxis]
    ...
    def _estimate_wishart_tied(self, nk, xk, sk):
        ...
        self.covariances_ = (
            ...
            + self.mean_precision_prior_
            / self.n_components
            * jnp.dot((nk / self.mean_precision_) * diff.T, diff)
        )
    ...
    def _estimate_wishart_diag(self, nk, xk, sk):
        ...
        diff = xk - self.mean_prior_
        self.covariances_ = self.covariance_prior_ + nk[:, jnp.newaxis] * (
            sk
            + (self.mean_precision_prior_ /
               self.mean_precision_)[:, jnp.newaxis]
            * jnp.square(diff)
        ...
        # Contrary to the original bishop book, we normalize the covariances
        self.covariances_ /= self.degrees_of_freedom_[:, jnp.newaxis]
    ...
    def _m_step(self, X, log_resp):
        ...
        nk, xk, sk = _estimate_gaussian_parameters(
            X, jnp.exp(log_resp), self.reg_covar, self.covariance_type
        )
        self._estimate_weights(nk)
        self._estimate_means(nk, xk)
        self._estimate_precisions(nk, xk, sk)
    ...
    def _estimate_log_weights(self):
        if self.weight_concentration_prior_type == "dirichlet_process":
            ...
            return (
                digamma_a
                - digamma_sum
                + jnp.hstack((0, jnp.cumsum(digamma_b - digamma_sum)[:-1]))
            )
        else:
            # case Variational Gaussian mixture with dirichlet distribution
            return digamma(self.weight_concentration_) - digamma(
                jnp.sum(self.weight_concentration_)
            )

    def _estimate_log_prob(self, X):
        _, n_features = X.shape
        # We remove `n_features * jnp.log(self.degrees_of_freedom_)` because
        # the precision matrix is normalized
        log_gauss = _estimate_log_gaussian_prob(
            X, self.means_, self.precisions_cholesky_, self.covariance_type
        ) - 0.5 * n_features * jnp.log(self.degrees_of_freedom_)

        log_lambda = n_features * jnp.log(2.0) + jnp.sum(
            digamma(
                0.5
                * (self.degrees_of_freedom_ - jnp.arange(0, n_features)[:, jnp.newaxis])
            ),
            0,
        )
    ...
    def _compute_lower_bound(self, log_resp, log_prob_norm):
        ...
        # We removed `.5 * n_features * jnp.log(self.degrees_of_freedom_)`
        ...
        if self.covariance_type == "tied":
            log_wishart = self.n_components * jnp.float64(
                _log_wishart_norm(
        ...
        return (
            -jnp.sum(jnp.exp(log_resp) * log_resp)
            - log_wishart
            - log_norm_weight
            - 0.5 * n_features * jnp.sum(jnp.log(self.mean_precision_))
        )
    ...
    def _set_parameters(self, params):
        ...
        if self.weight_concentration_prior_type == "dirichlet_process":
            ...
            self.weights_ = (
                ...
                / weight_dirichlet_sum
                * jnp.hstack((1, jnp.cumprod(tmp[:-1])))
            )
            self.weights_ /= jnp.sum(self.weights_)
        else:
            self.weights_ = self.weight_concentration_ / jnp.sum(
                self.weight_concentration_
            )

        # Precisions matrices computation
        if self.covariance_type == "full":
            self.precisions_ = jnp.array(
                [
                    jnp.dot(prec_chol, prec_chol.T)
                    for prec_chol in self.precisions_cholesky_
                ]
            )

        elif self.covariance_type == "tied":
            self.precisions_ = jnp.dot(
