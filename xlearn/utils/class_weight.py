...
def compute_class_weight(class_weight, *, classes, y):
    ...
    classes : ndarray
        ...
        `jnp.unique(y_org)` with `y_org` the original class labels.
    ...
    if class_weight is None or len(class_weight) == 0:
        ...
        weight = jnp.ones(classes.shape[0], dtype=jnp.float64, order="C")
    elif class_weight == "balanced":
        # Find the weight of each class as present in y.
        le = LabelEncoder()
        y_ind = le.fit_transform(y)
        if not all(jnp.in1d(classes, le.classes_)):
            raise ValueError("classes should have valid labels that are in y")

        recip_freq = len(y) / (len(le.classes_) * jnp.bincount(y_ind).astype(jnp.float64))
...
def compute_sample_weight(class_weight, y, *, indices=None):
    ...
    for k in range(n_outputs):
        ...
        classes_full = jnp.unique(y_full)
        ...
        if indices is not None:
            ...
            classes_subsample = jnp.unique(y_subsample)

            weight_k = jnp.take(
                compute_class_weight(
                    class_weight_k, classes=classes_subsample, y=y_subsample
                ),
                jnp.searchsorted(classes_subsample, classes_full),
                mode="clip",
            )
        ...
        weight_k = weight_k[jnp.searchsorted(classes_full, y_full)]

        if classes_missing:
            # Make missing classes' weight zero
            weight_k[jnp.in1d(y_full, list(classes_missing))] = 0.0

        expanded_class_weight.append(weight_k)

    expanded_class_weight = jnp.prod(expanded_class_weight, axis=0, dtype=jnp.float64)
