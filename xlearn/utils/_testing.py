...
from xlearn.utils.fixes import threadpool_info
...
# assert_raises_regex but lets keep the backward compat in jax-learn with
    ...
    **kw
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        if hasattr(np, "FutureWarning"):
            ...
            w = [e for e in w if e.category is not jnp.VisibleDeprecationWarning]
...
def assert_allclose(
    ...
    >>> a = jnp.full(shape=10, fill_value=1e-5, dtype=jnp.float32)
    >>> assert_allclose(a, 1e-5)
    """
    dtypes = []

    actual, desired = jnp.asanyarray(actual), jnp.asanyarray(desired)
    dtypes = [actual.dtype, desired.dtype]

    if rtol is None:
        rtols = [1e-4 if dtype == jnp.float32 else 1e-7 for dtype in dtypes]
        rtol = max(rtols)
...
def _create_memmap_backed_array(array, filename, mmap_mode):
    ...
    fp = jnp.memmap(filename, dtype=array.dtype, mode="w+", shape=array.shape)
    fp[:] = array[:]  # write array to memmap array
    fp.flush()
    memmap_backed_array = jnp.memmap(
...
def _create_aligned_memmap_backed_arrays(data, mmap_mode, folder):
    ...
    if isinstance(data, Sequence) and all(
        isinstance(each, jnp.ndarray) for each in data
    ):
        return [
...
def _convert_container(
    ...
    if constructor_name == "list":
        if dtype is None:
            ...
            return jnp.asarray(container, dtype=dtype).tolist()
    elif constructor_name == "tuple":
        if dtype is None:
            return tuple(container)
        else:
            return tuple(jnp.asarray(container, dtype=dtype).tolist())
    elif constructor_name == "array":
        return jnp.asarray(container, dtype=dtype)
...
class MinimalClassifier:
    ...
    * within a `SearchCV` in `test_search.py`.
    ...
    def fit(self, X, y):
        ...
        self.classes_, counts = jnp.unique(y, return_counts=True)
    ...
    def predict_proba(self, X):
        ...
        y_proba = jnp.zeros(shape=proba_shape, dtype=jnp.float64)
...
class MinimalRegressor:
    ...
    * within a `SearchCV` in `test_search.py`.
    ...
    def fit(self, X, y):
        ...
        self._mean = jnp.mean(y)
        return self

    def predict(self, X):
        check_is_fitted(self)
        X = check_array(X)
        return jnp.ones(shape=(X.shape[0],)) * self._mean
