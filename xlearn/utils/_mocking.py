...
class CheckingClassifier(ClassifierMixin, BaseEstimator):
    ...
    def _check_X_y(self, X, y=None, should_be_fitted=True):
        ...
        if self.check_X is not None:
            ...
            if isinstance(checked_X, (bool, jnp.bool_)):
                assert checked_X
            else:
                X = checked_X
        if y is not None and self.check_y is not None:
            ...
            if isinstance(checked_y, (bool, jnp.bool_)):
                assert checked_y
            else:
                y = checked_y
        return X, y
    ...
    def fit(self, X, y, sample_weight=None, **fit_params):
        ...
        **fit_params : dict of string -> object
        ...
        self.n_features_in_ = jnp.shape(X)[1]
        self.classes_ = jnp.unique(check_array(
    ...
    def predict(self, X):
        ...
        return self.classes_[jnp.zeros(_num_samples(X), dtype=int)]
    ...
    def predict_proba(self, X):
        ...
        if self.methods_to_check == "all" or "predict_proba" in self.methods_to_check:
            X, y = self._check_X_y(X)
        proba = jnp.zeros((_num_samples(X), len(self.classes_)))
        proba[:, 0] = 1
        return proba
    ...
    def decision_function(self, X):
        ...
        if len(self.classes_) == 2:
            ...
            return jnp.zeros(_num_samples(X))
        else:
            decision = jnp.zeros((_num_samples(X), len(self.classes_)))
            decision[:, 0] = 1
            return decision
