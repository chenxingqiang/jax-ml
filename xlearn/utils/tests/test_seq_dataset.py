...
iris = load_iris()
X64 = iris.data.astype(jnp.float64)
y64 = iris.target.astype(jnp.float64)
X_csr64 = sp.csr_matrix(X64)
sample_weight64 = jnp.arange(y64.size, dtype=jnp.float64)

X32 = iris.data.astype(jnp.float32)
y32 = iris.target.astype(jnp.float32)
X_csr32 = sp.csr_matrix(X32)
sample_weight32 = jnp.arange(y32.size, dtype=jnp.float32)
...
def test_fused_types_consistency(make_dataset_32, make_dataset_64):
    ...
    for _ in range(NUMBER_OF_RUNS):
        ...
        assert xi_data32.dtype == jnp.float32
        assert xi_data64.dtype == jnp.float64
