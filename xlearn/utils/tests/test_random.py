...
def check_sample_int(sample_without_replacement):
    ...
    for n_samples in range(n_population + 1):
        ...
        unique = jnp.unique(s)
        assert jnp.size(unique) == n_samples
        assert jnp.all(unique < n_population)

    # test edge case n_population == n_samples == 0
    assert jnp.size(sample_without_replacement(0, 0)) == 0
...
def test_random_choice_csc(n_samples=10000, random_state=24):
    ...
    classes = [jnp.array([0, 1]), jnp.array([0, 1, 2])]
    class_probabilities = [jnp.array([0.5, 0.5]), jnp.array([0.6, 0.1, 0.3])]
    ...
    for k in range(len(classes)):
        p = jnp.bincount(got.getcol(k).toarray().ravel()) / float(n_samples)
        assert_array_almost_equal(class_probabilities[k], p, decimal=1)

    # Implicit class probabilities
    classes = [[0, 1], [1, 2]]  # test for array-like support
    class_probabilities = [jnp.array([0.5, 0.5]), jnp.array([0, 1 / 2, 1 / 2])]
    ...
    classes = [jnp.array([0, 1]), jnp.array([0, 1, 2])]
    class_probabilities = [jnp.array([0.0, 1.0]), jnp.array([0.0, 1.0, 0.0])]
    ...
    class_probabilities = [jnp.array([0.0, 1.0]), jnp.array([1.0])]
    ...
    for k in range(len(classes)):
        p = jnp.bincount(got.getcol(k).toarray().ravel()) / n_samples
        assert_array_almost_equal(class_probabilities[k], p, decimal=1)


def test_random_choice_csc_errors():
    # the length of an array in classes and class_probabilities is mismatched
    classes = [jnp.array([0, 1]), jnp.array([0, 1, 2, 3])]
    class_probabilities = [jnp.array([0.5, 0.5]), jnp.array([0.6, 0.1, 0.3])]
    with pytest.raises(ValueError):
        _random_choice_csc(4, classes, class_probabilities, 1)

    # the class dtype is not supported
    classes = [jnp.array(["a", "1"]), jnp.array(["z", "1", "2"])]
    class_probabilities = [jnp.array([0.5, 0.5]), jnp.array([0.6, 0.1, 0.3])]
    with pytest.raises(ValueError):
        _random_choice_csc(4, classes, class_probabilities, 1)

    # the class dtype is not supported
    classes = [jnp.array([4.2, 0.1]), jnp.array([0.1, 0.2, 9.4])]
    class_probabilities = [jnp.array([0.5, 0.5]), jnp.array([0.6, 0.1, 0.3])]
    with pytest.raises(ValueError):
        _random_choice_csc(4, classes, class_probabilities, 1)
    ...
    classes = [jnp.array([0, 1]), jnp.array([0, 1, 2])]
    class_probabilities = [jnp.array([0.5, 0.6]), jnp.array([0.6, 0.1, 0.3])]
    with pytest.raises(ValueError):
        _random_choice_csc(4, classes, class_probabilities, 1)
