...
# toy array
X_toy = jnp.arange(9).reshape((3, 3))
...
def test_resample_stratified():
    ...
    assert jnp.all(y_not_stratified == 1)

    _, y_stratified = resample(X, y, n_samples=10, random_state=0, stratify=y)
    assert not jnp.all(y_stratified == 1)
    assert jnp.sum(y_stratified) == 9  # all 1s, one 0
...
def test_resample_stratified_replace():
    ...
    assert jnp.unique(X_replace).shape[0] < 50
    assert jnp.unique(X_no_replace).shape[0] == 50
    ...
    assert jnp.unique(X_replace).shape[0] == 100
...
def test_column_or_1d():
    EXAMPLES = [
        ...
        ("multiclass-multioutput", [[1, 2, 3]]),
        ("continuous-multioutput", jnp.arange(30).reshape((-1, 3))),
    ]

    for y_type, y in EXAMPLES:
        if y_type in ["binary", "multiclass", "continuous"]:
            assert_array_equal(column_or_1d(y), jnp.ravel(y))
        else:
            with pytest.raises(ValueError):
                column_or_1d(y)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (slice(0, 2), "int"),
        (jnp.array([0, 1, 2], dtype=jnp.int32), "int"),
        (jnp.array([0, 1, 2], dtype=jnp.int64), "int"),
        (jnp.array([0, 1, 2], dtype=jnp.uint8), "int"),
        ([True, False], "bool"),
        ((True, False), "bool"),
        ...
        (("col_0", "col_1", "col_2"), "str"),
        (slice("begin", "end"), "str"),
        (jnp.array(["col_0", "col_1", "col_2"]), "str"),
        (jnp.array(["col_0", "col_1", "col_2"], dtype=object), "str"),
    ],
)
...
def test_shuffle_on_ndim_equals_three():
    ...
    A = jnp.array([[[1, 2], [3, 4]], [[5, 6], [7, 8]]])  # A.shape = (2,2,2)
    S = set(to_tuple(A))
...
def test_shuffle_dont_convert_to_array():
    ...
    a = ["a", "b", "c"]
    b = jnp.array(["a", "b", "c"], dtype=object)
    c = [1, 2, 3]
    d = MockDataFrame(jnp.array([["a", 0], ["b", 1], ["c", 2]], dtype=object))
    e = sp.csc_matrix(jnp.arange(6).reshape(3, 2))
    ...
    assert_array_equal(d_s, jnp.array(
        [["c", 2], ["b", 1], ["a", 0]], dtype=object))
    assert type(d_s) == MockDataFrame

    assert_array_equal(e_s.toarray(), jnp.array([[4, 5], [2, 3], [0, 1]]))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (jnp.nan, True),
        (float(jnp.nan), True),
        (jnp.float32(jnp.nan), True),
        (jnp.float64(jnp.nan), True),
        (0, False),
        (0.0, False),
        (None, False),
        ("", False),
...
def test_approximate_mode():
    """Make sure xlearn.utils._approximate_mode returns valid
    ...
    """
    X = jnp.array([99000, 1000], dtype=jnp.int32)
...
def test_deprecation_joblib_api(tmpdir):
    ...
    # xlearn.utils
    from xlearn.utils import parallel_backend, register_parallel_backend

    assert_no_warnings(parallel_backend, "loky", None)
    assert_no_warnings(register_parallel_backend, "failing", None)

    from xlearn.utils._joblib import joblib

    del joblib.parallel.BACKENDS["failing"]


@pytest.mark.parametrize("sequence", [[jnp.array(1), jnp.array(2)], [[1, 2], [3, 4]]])
