...
def test__wrap_in_pandas_container_dense():
    ...
    X = jnp.asarray([[1, 0, 3], [0, 0, 1]])
    columns = jnp.asarray(["f0", "f1", "f2"], dtype=object)
...
def test__wrap_in_pandas_container_dense_update_columns_and_index():
    ...
    new_columns = jnp.asarray(["f0", "f1", "f2"], dtype=object)
    new_index = [10, 12]
...
class EstimatorWithSetOutput(_SetOutputMixin):
    ...
    def get_feature_names_out(self, input_features=None):
        return jnp.asarray([f"X{i}" for i in range(self.n_features_in_)], dtype=object)
...
def test__safe_set_output():
    ...
    est = EstimatorWithSetOutput().fit(jnp.asarray([[1, 2, 3]]))
    _safe_set_output(est, transform="pandas")
