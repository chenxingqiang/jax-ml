...
RTOL = {jnp.float32: 1e-6, jnp.float64: 1e-12}
...
def test_rotg(dtype):
    ...
    def expected_rotg(a, b):
        ...
        if a == 0 and b == 0:
            ...
            r = jnp.sqrt(a**2 + b**2) * (1 if roe >= 0 else -1)
            c, s = a / r, b / r
...
@pytest.mark.parametrize(
    "opA, transA", [(_no_op, NoTrans), (jnp.transpose, Trans)], ids=["NoTrans", "Trans"]
)
...
def test_ger(dtype, order):
    ...
    expected = alpha * jnp.outer(x, y) + A
    ger(alpha, x, y, A)
...
@pytest.mark.parametrize("dtype", [jnp.float32, jnp.float64])
@pytest.mark.parametrize(
    "opB, transB", [(_no_op, NoTrans), (jnp.transpose, Trans)], ids=["NoTrans", "Trans"]
)
@pytest.mark.parametrize(
    "opA, transA", [(_no_op, NoTrans), (jnp.transpose, Trans)], ids=["NoTrans", "Trans"]
)
