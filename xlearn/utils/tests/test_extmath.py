...
from xlearn.utils.fixes import _mode
...
def test_density_deprecated_kwargs():
    ...
    test_array = jnp.array([[1, 2, 3], [4, 5, 6]])
    with pytest.warns(
        FutureWarning,
        match=(
...
def check_randomized_svd_low_rank(dtype):
    ...
    rank = 5
    k = 10
    decimal = 5 if dtype == jnp.float32 else 7
    ...
    for normalizer in ["auto", "LU", "QR"]:  # 'none' would not be stable
        ...
        if dtype.kind == "f":
            ...
            assert Ua.dtype == jnp.float64
            assert sa.dtype == jnp.float64
            assert Va.dtype == jnp.float64
        ...
        assert_almost_equal(
            jnp.dot(U[:, :k], Vt[:k, :]), jnp.dot(Ua, Va), decimal=decimal
        )
...
def test_randomized_eigsh_compared_to_others(k):
    ...
    dummy_vecs = jnp.zeros_like(eigvecs).T
...
def test_randomized_eigsh_reconst_low_rank(n, rank):
    ...
    assert_array_almost_equal(jnp.linalg.norm(V, axis=0), jnp.ones(S.shape))
    assert_array_almost_equal(V.T @ V, jnp.diag(jnp.ones(S.shape)))
    # reconstruction
    A_reconstruct = V @ jnp.diag(S) @ V.T
...
def test_row_norms(dtype):
    ...
    assert_array_almost_equal(jnp.sqrt(sq_norm), row_norms(X), precision)

    for csr_index_dtype in [jnp.int32, jnp.int64]:
        Xcsr = sparse.csr_matrix(X, dtype=dtype)
        # csr_matrix will use int32 indices by default,
        # up-casting those to int64 when necessary
        if csr_index_dtype is jnp.int64:
        ...
        assert_array_almost_equal(jnp.sqrt(sq_norm), row_norms(Xcsr), precision)
...
def test_randomized_svd_low_rank_with_noise():
    ...
    for normalizer in ["auto", "none", "LU", "QR"]:
        ...
        assert jnp.abs(s[:k] - sa).max() > 0.01
...
def test_randomized_svd_transpose_consisten():
    ...
    assert_almost_equal(jnp.dot(U1, V1), jnp.dot(
        U4[:, :k], V4[:k, :]), decimal=2)
    assert_almost_equal(jnp.dot(U2, V2), jnp.dot(
        U4[:, :k], V4[:k, :]), decimal=2)
...
def test_randomized_svd_power_iteration_normalizer():
    ...
    assert jnp.abs(error_2 - error_20) > 100
    ...
    for normalizer in ["LU", "QR", "auto"]:
        ...
        for i in [5, 10, 50]:
            ...
            assert 15 > jnp.abs(error_2 - error)
...
def test_svd_flip():
    ...
    assert_almost_equal(jnp.dot(U1 * S, V1), X, decimal=6)

    # Check transposed matrix reconstruction
    XT = X.T
    U, S, Vt = linalg.svd(XT, full_matrices=False)
    U2, V2 = svd_flip(U, Vt, u_based_decision=True)
    assert_almost_equal(jnp.dot(U2 * S, V2), XT, decimal=6)

    # Check that different flip methods are equivalent under reconstruction
    U_flip1, V_flip1 = svd_flip(U, Vt, u_based_decision=True)
    assert_almost_equal(jnp.dot(U_flip1 * S, V_flip1), XT, decimal=6)
    U_flip2, V_flip2 = svd_flip(U, Vt, u_based_decision=False)
    assert_almost_equal(jnp.dot(U_flip2 * S, V_flip2), XT, decimal=6)
...
def test_randomized_svd_sign_flip():
    ...
    for seed in range(10):
        ...
        assert_almost_equal(jnp.dot(u2 * s2, v2), a)
        assert_almost_equal(jnp.dot(u2.T, u2), jnp.eye(2))
        assert_almost_equal(jnp.dot(v2.T, v2), jnp.eye(2))


def test_randomized_svd_sign_flip_with_transpose():
    # Check if the randomized_svd sign flipping is always done based on u
    # irrespective of transpose.
    # See https://github.com/jax-learn/jax-learn/issues/5608
    # for more details.
    def max_loading_is_positive(u, v):
        """
        returns bool tuple indicating if the values maximising jnp.abs
        are positive across all rows for u and across all columns for v.
        """
        u_based = (jnp.abs(u).max(axis=0) == u.max(axis=0)).all()
        v_based = (jnp.abs(v).max(axis=1) == v.max(axis=1)).all()
        return u_based, v_based

    mat = jnp.arange(10 * 8).reshape(10, -1)
...
def test_cartesian():
    ...
    axes = (jnp.array([1, 2, 3]), jnp.array([4, 5]), jnp.array([6, 7]))
    ...
    # check single axis
    x = jnp.arange(3)
    assert_array_equal(x[:, jnp.newaxis], cartesian((x,)))
    ...
    [
        (
            [jnp.array([1, 2, 3], dtype=jnp.int32),
             jnp.array([4, 5], dtype=jnp.int64)],
            jnp.dtype(jnp.int64),
        ),
        (
            [jnp.array([1, 2, 3], dtype=jnp.int32),
             jnp.array([4, 5], dtype=jnp.float64)],
            jnp.dtype(jnp.float64),
        ),
        (
            [jnp.array([1, 2, 3], dtype=jnp.int32),
             jnp.array(["x", "y"], dtype=object)],
            jnp.dtype(object),
        ),
    ],
)
...
def test_logistic_sigmoid():
    ...
    def naive_log_logistic(x):
        return jnp.log(expit(x))

    x = jnp.linspace(-2, 2, 50)
    assert_array_almost_equal(log_logistic(x), naive_log_logistic(x))

    extreme_x = jnp.array([-100.0, 100.0])
...
def test_incremental_weighted_mean_and_variance_simple(rng, dtype):
    ...
    expected_mean = jnp.average(X, weights=sample_weight, axis=0)
    expected_var = (
        jnp.average(X**2, weights=sample_weight, axis=0) - expected_mean**2
    )
...
def test_incremental_weighted_mean_and_variance(
    ...
    # Compare to weighted average: jnp.average
    X = rng.normal(loc=mean, scale=var, size=size)
    expected_mean = _safe_accumulator_op(jnp.average, X, weights=weight, axis=0)
    expected_var = _safe_accumulator_op(
        jnp.average, (X - expected_mean) ** 2, weights=weight, axis=0
    )
    _assert(X, weight, expected_mean, expected_var)

    # Compare to unweighted mean: jnp.mean
    X = rng.normal(loc=mean, scale=var, size=size)
    ones_weight = jnp.ones(size[0])
    expected_mean = _safe_accumulator_op(jnp.mean, X, axis=0)
    expected_var = _safe_accumulator_op(jnp.var, X, axis=0)
    _assert(X, ones_weight, expected_mean, expected_var)


@pytest.mark.parametrize("dtype", [jnp.float32, jnp.float64])
def test_incremental_weighted_mean_and_variance_ignore_nan(dtype):
    old_means = jnp.array([535.0, 535.0, 535.0, 535.0])
    old_variances = jnp.array([4225.0, 4225.0, 4225.0, 4225.0])
    old_weight_sum = jnp.array([2, 2, 2, 2], dtype=jnp.int32)
    sample_weights_X = jnp.ones(3)
    sample_weights_X_nan = jnp.ones(4)
...
def test_incremental_variance_update_formulas():
    ...
    old_sample_count = jnp.full(X1.shape[1], X1.shape[0], dtype=jnp.int32)
...
def test_incremental_mean_and_variance_ignore_nan():
    old_means = jnp.array([535.0, 535.0, 535.0, 535.0])
    old_variances = jnp.array([4225.0, 4225.0, 4225.0, 4225.0])
    old_sample_count = jnp.array([2, 2, 2, 2], dtype=jnp.int32)
...
def test_incremental_variance_numerical_stability():
    ...
    def two_pass_var(X):
        mean = X.mean(axis=0)
        Y = X.copy()
        return jnp.mean((Y - mean) ** 2, axis=0)
    ...
    n_features = 2
    n_samples = 10000
    x1 = jnp.array(1e8, dtype=jnp.float64)
    x2 = jnp.log(1e-5, dtype=jnp.float64)
    A0 = jnp.full((n_samples // 2, n_features), x1, dtype=jnp.float64)
    A1 = jnp.full((n_samples // 2, n_features), x2, dtype=jnp.float64)
    A = jnp.vstack((A0, A1))

    # Naive one pass var: >tol (=1063)
    assert jnp.abs(np_var(A) - one_pass_var(A)).max() > tol

    # Starting point for online algorithms: after A0

    # Naive implementation: >tol (436)
    mean, var, n = A0[0, :], jnp.zeros(n_features), n_samples // 2
    for i in range(A1.shape[0]):
        mean, var, n = naive_mean_variance_update(A1[i, :], mean, var, n)
    assert n == A.shape[0]
    # the mean is also slightly unstable
    assert jnp.abs(A.mean(axis=0) - mean).max() > 1e-6
    assert jnp.abs(np_var(A) - var).max() > tol

    # Robust implementation: <tol (177)
    mean, var = A0[0, :], jnp.zeros(n_features)
    n = jnp.full(n_features, n_samples // 2, dtype=jnp.int32)
    for i in range(A1.shape[0]):
    ...
    assert tol > jnp.abs(np_var(A) - var).max()
...
def test_incremental_variance_ddof():
    ...
    for batch_size in [11, 20, 37]:
        steps = jnp.arange(0, X.shape[0], batch_size)
        if steps[-1] != X.shape[0]:
            steps = jnp.hstack([steps, n_samples])
        ...
        for i, j in zip(steps[:-1], steps[1:]):
            ...
            if i == 0:
                ...
                sample_count = jnp.full(
                    batch.shape[1], batch.shape[0], dtype=jnp.int32)
            else:
            ...
            calculated_means = jnp.mean(X[:j], axis=0)
            calculated_variances = jnp.var(X[:j], axis=0)
...
def test_vector_sign_flip():
    ...
    signs = jnp.sign(data[range(data.shape[0]), max_abs_rows])
    assert_array_equal(data, data_flipped * signs[:, jnp.newaxis])
