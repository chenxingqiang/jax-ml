...
def test_checking_classifier(iris, input_type):
    ...
    assert_array_equal(y_pred, jnp.zeros(y_pred.size, dtype=int))
    ...
    first_2_classes = jnp.logical_or(y == 0, y == 1)
...
def test_checking_classifier_fit_params(iris):
    ...
    sample_weight = jnp.ones(len(X) // 2)
...
def test_mock_estimator_on_off_prediction(iris, response_methods):
    ...
    assert_array_equal(estimator.classes_, jnp.unique(y))
