...
def test_nan_not_in_interval(interval):
    """Check that jnp.nan is not in any interval."""
    assert jnp.nan not in interval
...
def test_options():
    ...
    options = Options(Real, {-0.5, 0.5, jnp.inf}, deprecated={-0.5})
    assert options.is_satisfied_by(-0.5)
    assert options.is_satisfied_by(jnp.inf)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (StrOptions({"a", "b", "c"}), "b"),
        (Options(type, {jnp.float32, jnp.float64}), jnp.float64),
...
def test_boolean_constraint_deprecated_int():
    ...
    # True/False and jnp.bool_(True/False) are valid params
    f(True)
    f(jnp.bool_(False))
...
def test_pandas_na_constraint_with_pd_na():
    ...
    assert not na_constraint.is_satisfied_by(jnp.array([1, 2, 3]))
...
def test_third_party_estimator():
    """Check that the validation from a jax-learn estimator inherited by a third
...
def test_real_not_int():
    ...
    assert isinstance(jnp.float64(1), RealNotInt)
    assert not isinstance(jnp.int64(1), RealNotInt)
