...
def test_mmhash3_int_array():
    ...
    for seed in [0, 42]:
        expected = jnp.array([murmurhash3_32(int(k), seed) for k in keys.flat])
...
def test_uniform_distribution():
    ...
    bins = jnp.zeros(n_bins, dtype=jnp.float64)

    for i in range(n_samples):
        bins[murmurhash3_32(i, positive=True) % n_bins] += 1

    means = bins / n_samples
    expected = jnp.full(n_bins, 1.0 / n_bins)

    assert_array_almost_equal(means / expected, jnp.ones(n_bins), 2)
