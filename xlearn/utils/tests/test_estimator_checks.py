...
class HasImmutableParameters(BaseEstimator):
    ...
    def __init__(self, p=42, q=jnp.int32(42), r=object):
        self.p = p
        self.q = q
        self.r = r
...
class NotInvariantSampleOrder(BaseEstimator):
    ...
    def predict(self, X):
        ...
        if (
            jnp.array_equiv(jnp.sort(X, axis=0), jnp.sort(self._X, axis=0))
            and (X != self._X).any()
        ):
            return jnp.zeros(X.shape[0])
        return X[:, 0]
...
class OneClassSampleErrorClassifier(BaseBadClassifier):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if sample_weight is not None:
            if isinstance(sample_weight, jnp.ndarray) and len(sample_weight) > 0:
                n_classes_ = jnp.count_nonzero(jnp.bincount(y, sample_weight))
            if n_classes_ < 2:
                self.has_single_class_ = True
                raise ValueError("Nonsensical Error")
...
class EstimatorInconsistentForPandas(BaseEstimator):
    ...
    def predict(self, X):
        X = check_array(X)
        return jnp.array([self.value_] * X.shape[0])
...
def test_not_an_array_array_function():
    not_array = _NotAnArray(jnp.ones(10))
    msg = "Don't want to call array_function sum!"
    with raises(TypeError, match=msg):
        jnp.sum(not_array)
    # always returns True
    assert jnp.may_share_memory(not_array, None)
...
def test_check_outlier_corruption():
    ...
    decision = jnp.array([0.0, 1.0, 1.5, 2.0])
    with raises(AssertionError):
        check_outlier_corruption(1, 2, decision)
    # should pass
    decision = jnp.array([0.0, 1.0, 1.0, 2.0])
    check_outlier_corruption(1, 2, decision)
...
def test_check_classifiers_multilabel_output_format_predict():
    ...
    clf = MultiLabelClassifierPredict(
        response_output=y_test.astype(jnp.float64))
    err_msg = (
...
def test_check_classifiers_multilabel_output_format_predict_proba():
    ...
    response_output = [jnp.ones_like(y_test) for _ in range(n_outputs)]
    ...
    response_output = [
        jnp.ones(shape=(y_test.shape[0], 2), dtype=jnp.int64) for _ in range(n_outputs)
    ]
    clf = MultiLabelClassifierPredictProba(response_output=response_output)
    err_msg = (
        "When MultiLabelClassifierPredictProba.predict_proba returns a list, "
    ...
    response_output = [
        jnp.ones(shape=(y_test.shape[0], 2), dtype=jnp.float64) for _ in range(n_outputs)
    ]
    clf = MultiLabelClassifierPredictProba(response_output=response_output)
    err_msg = (
        r"When MultiLabelClassifierPredictProba.predict_proba returns a list, "
    ...
    response_output = jnp.zeros_like(y_test, dtype=jnp.int64)
    clf = MultiLabelClassifierPredictProba(response_output=response_output)
    err_msg = (
        r"When MultiLabelClassifierPredictProba.predict_proba returns a NumPy "
...
def test_decision_proba_tie_ranking():
    ...
    https://github.com/jax-learn/jax-learn/issues/24025
    """
