...
def test_get_response_values_classifier_inconsistent_y_pred_for_binary_proba():
    ...
    y_single_class = jnp.zeros_like(y_two_class)
...
def test_get_response_decision_function():
    ...
    jnp.testing.assert_allclose(y_score, classifier.decision_function(X_binary))
    assert pos_label == 1
...
def test_get_response_values_multiclass(estimator, response_method):
    ...
    if response_method == "predict_proba":
        assert jnp.logical_and(predictions >= 0, predictions <= 1).all()
