...
def test_mean_variance_axis0():
    ...
    for input_dtype, output_dtype in expected_dtypes:
        ...
        for X_sparse in (X_csr, X_csc):
            ...
            assert_array_almost_equal(X_means, jnp.mean(X_test, axis=0))
            assert_array_almost_equal(X_vars, jnp.var(X_test, axis=0))
...
def test_mean_variance_axis0_precision(dtype, sparse_constructor):
    ...
    assert var < jnp.finfo(dtype).eps
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [[0, jnp.nan, 2], [0, jnp.nan, jnp.nan]],
            [[0, jnp.nan, 2], [0, jnp.nan, jnp.nan]],
            [1.0, 1.0, 1.0],
        ),
        (
            [[0, 0], [1, jnp.nan], [2, 0], [0, 3], [jnp.nan, jnp.nan], [jnp.nan, 2]],
            [
                [0, 0, 0],
                [1, 1, jnp.nan],
                [2, 2, 0],
...
def test_incr_mean_variance_axis_weighted_axis1(
    ...
    last_mean = jnp.zeros(jnp.shape(Xw)[0], dtype=dtype)
    last_var = jnp.zeros_like(last_mean, dtype=dtype)
    last_n = jnp.zeros_like(last_mean, dtype=jnp.int64)
...
def test_incr_mean_variance_axis_weighted_axis0(
    ...
    last_mean = jnp.zeros(jnp.size(Xw, 1), dtype=dtype)
    last_var = jnp.zeros_like(last_mean)
    last_n = jnp.zeros_like(last_mean, dtype=jnp.int64)
    means0, vars0, n_incr0 = incr_mean_variance_axis(
        X=X_sparse,
        axis=axis,
...
def test_incr_mean_variance_axis():
    for axis in [0, 1]:
        ...
        last_mean = jnp.zeros(n_features) if axis == 0 else jnp.zeros(n_samples)
        last_var = jnp.zeros_like(last_mean)
        last_n = jnp.zeros_like(last_mean, dtype=jnp.int64)

        # Test errors
        X = jnp.array(data_chunks[0])
        X = jnp.atleast_2d(X)
...
def test_incr_mean_variance_axis_dim_mismatch(sparse_constructor):
    ...
    last_var = jnp.zeros_like(last_mean)
    last_n = jnp.zeros(last_mean.shape, dtype=jnp.int64)

    kwargs = dict(last_mean=last_mean, last_var=last_var, last_n=last_n)
    mean0, var0, _ = incr_mean_variance_axis(X, axis=0, **kwargs)
    assert_allclose(jnp.mean(X.toarray(), axis=0), mean0)
    assert_allclose(jnp.var(X.toarray(), axis=0), var0)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            sp.hstack(
                [
                    sp.csr_matrix(jnp.full((13, 1), fill_value=jnp.nan)),
...
def test_incr_mean_variance_axis_equivalence_mean_variance(X1, X2):
    ...
    axis = 0
    last_mean, last_var = jnp.zeros(X1.shape[1]), jnp.zeros(X1.shape[1])
    last_n = jnp.zeros(X1.shape[1], dtype=jnp.int64)
    ...
    X = sp.vstack([X1, X2])
    assert_allclose(updated_mean, jnp.nanmean(X.A, axis=axis))
    assert_allclose(updated_var, jnp.nanvar(X.A, axis=axis))
    assert_allclose(updated_n, jnp.count_nonzero(~jnp.isnan(X.A), axis=0))
...
def test_incr_mean_variance_n_float():
    ...
    last_mean, last_var = jnp.zeros(X.shape[1]), jnp.zeros(X.shape[1])
    last_n = 0
    _, _, new_n = incr_mean_variance_axis(
        X, axis=axis, last_mean=last_mean, last_var=last_var, last_n=last_n
    )
    assert_allclose(new_n, jnp.full(X.shape[1], X.shape[0]))
...
def test_incr_mean_variance_axis_ignore_nan(axis, sparse_constructor):
    ...
    old_sample_count = jnp.array([2, 2, 2, 2], dtype=jnp.int64)

    X = sparse_constructor(
        jnp.array([[170, 170, 170, 170], [
...
def test_densify_rows():
    for dtype in (jnp.float32, jnp.float64):
        ...
        X_rows = jnp.array([0, 2, 3], dtype=jnp.intp)
        out = jnp.ones((6, X.shape[1]), dtype=dtype)
        out_rows = jnp.array([1, 3, 4], dtype=jnp.intp)

        expect = jnp.ones_like(out)
...
def test_inplace_swap_row():
    X = jnp.array(
        [[0, 3, 0], [2, 4, 0], [0, 0, 0], [9, 8, 7], [4, 0, 5]], dtype=jnp.float64
    )
    X_csr = sp.csr_matrix(X)
    ...
    X = jnp.array(
        [[0, 3, 0], [2, 4, 0], [0, 0, 0], [9, 8, 7], [4, 0, 5]], dtype=jnp.float32
    )
    X_csr = sp.csr_matrix(X)
    X_csc = sp.csc_matrix(X)
    swap = linalg.get_blas_funcs(("swap",), (X,))
...
@pytest.mark.parametrize(
    ...
    [(0, jnp.min, jnp.max, False), (jnp.nan, jnp.nanmin, jnp.nanmax, True)],
)
...
def test_min_max_axis_errors():
    X = jnp.array(
        [[0, 3, 0], [2, -1, 0], [0, 0, 0], [9, 8, 7], [4, 0, 5]], dtype=jnp.float64
    )
    X_csr = sp.csr_matrix(X)
    X_csc = sp.csc_matrix(X)
    with pytest.raises(TypeError):
...
def test_count_nonzero():
    ...
    X_nonzero_weighted = X_nonzero * jnp.array(sample_weight)[:, None]
    ...
    try:
        X_csr.indices = X_csr.indices.astype(jnp.int64)
        X_csr.indptr = X_csr.indptr.astype(jnp.int64)
        assert count_nonzero(X_csr, axis=0).dtype == count_nonzero(
            X_csr, axis=1).dtype
        assert (
            count_nonzero(X_csr, axis=0, sample_weight=sample_weight).dtype
        ...
        assert "according to the rule 'safe'" in e.args[0] and jnp.intp(
        ).nbytes < 8, e
...
def test_csc_row_median():
    ...
    X = [[0, -2], [-1, -1], [1, 0], [2, 1]]
    csc = sp.csc_matrix(X)
    assert_array_equal(csc_median_axis_0(csc), jnp.array([0.5, -0.5]))
    X = [[0, -2], [-1, -5], [1, -3]]
    csc = sp.csc_matrix(X)
    assert_array_equal(csc_median_axis_0(csc), jnp.array([0.0, -3]))
        ...
        for dtype in (jnp.float64, jnp.float32):
            ...
            for index_dtype in [jnp.int32, jnp.int64]:
                ...
                if inplace_csr_row_normalize is inplace_csr_row_normalize_l2:
                    X_csr.data **= 2
                assert_array_almost_equal(jnp.abs(X_csr).sum(axis=1), ones)
...
def test_csr_row_norms(dtype):
    ...
    rtol = 1e-6 if dtype == jnp.float32 else 1e-7
