...
def _random_choice_csc(n_samples, classes, class_probability=None, random_state=None):
    ...
    for j in range(len(classes)):
        classes[j] = jnp.asarray(classes[j])
        if classes[j].dtype.kind != "i":
            raise ValueError("class dtype %s is not supported" % classes[j].dtype)
        classes[j] = classes[j].astype(jnp.int64, copy=False)

        # use uniform distribution if no class_probability is given
        if class_probability is None:
            class_prob_j = jnp.empty(shape=classes[j].shape[0])
            class_prob_j.fill(1 / classes[j].shape[0])
        else:
            class_prob_j = jnp.asarray(class_probability[j])

        if not jnp.isclose(jnp.sum(class_prob_j), 1.0):
            raise ValueError(
        ...
        if 0 not in classes[j]:
            classes[j] = jnp.insert(classes[j], 0, 0)
            class_prob_j = jnp.insert(class_prob_j, 0, 0.0)

        # If there are nonzero classes choose randomly using class_probability
        rng = check_random_state(random_state)
        if classes[j].shape[0] > 1:
            index_class_0 = jnp.flatnonzero(classes[j] == 0).item()
            ...
            class_probability_nz_norm = class_probability_nz / jnp.sum(
                class_probability_nz
            )
            classes_ind = jnp.searchsorted(
