...
def inplace_csr_column_scale(X, scale):
    ...
    scale : ndarray of shape (n_features,), dtype={jnp.float32, jnp.float64}
...
def inplace_csr_row_scale(X, scale):
    ...
    X.data *= jnp.repeat(scale, jnp.diff(X.indptr))
...
def incr_mean_variance_axis(X, *, axis, last_mean, last_var, last_n, weights=None):
    ...
    if jnp.size(last_n) == 1:
        last_n = jnp.full(last_mean.shape, last_n, dtype=last_mean.dtype)

    if not (jnp.size(last_mean) == jnp.size(last_var) == jnp.size(last_n)):
        raise ValueError("last_mean, last_var, last_n do not have the same shapes.")

    if axis == 1:
        if jnp.size(last_mean) != X.shape[0]:
            raise ValueError(
                "If axis=1, then last_mean, last_n, last_var should be of "
                f"size n_samples {X.shape[0]} (Got {jnp.size(last_mean)})."
            )
    else:  # axis == 0
        if jnp.size(last_mean) != X.shape[1]:
            raise ValueError(
                "If axis=0, then last_mean, last_n, last_var should be of "
                f"size n_features {X.shape[1]} (Got {jnp.size(last_mean)})."
            )
...
def _minor_reduce(X, ufunc):
    major_index = jnp.flatnonzero(jnp.diff(X.indptr))
...
def _min_or_max_axis(X, axis, min_or_max):
    ...
    not_full = jnp.diff(mat.indptr)[major_index] < N
    value[not_full] = min_or_max(value[not_full], 0)
    mask = value != 0
    major_index = jnp.compress(mask, major_index)
    value = jnp.compress(mask, value)

    if axis == 0:
        res = sp.coo_matrix(
            (value, (jnp.zeros(len(value)), major_index)), dtype=X.dtype, shape=(1, M)
        )
    else:
        res = sp.coo_matrix(
            (value, (major_index, jnp.zeros(len(value)))), dtype=X.dtype, shape=(M, 1)
        )
    return res.A.ravel()
...
def _sparse_min_max(X, axis):
    return (
        _sparse_min_or_max(X, axis, jnp.minimum),
        _sparse_min_or_max(X, axis, jnp.maximum),
    )


def _sparse_nan_min_max(X, axis):
    return (_sparse_min_or_max(X, axis, jnp.fmin), _sparse_min_or_max(X, axis, jnp.fmax))
...
def min_max_axis(X, axis, ignore_nan=False):
    ...
    mins : ndarray of shape (n_features,), dtype={jnp.float32, jnp.float64}
        Feature-wise minima.

    maxs : ndarray of shape (n_features,), dtype={jnp.float32, jnp.float64}
        Feature-wise maxima.
    """
...
def count_nonzero(X, axis=None, sample_weight=None):
    ...
    # We rely here on the fact that jnp.diff(Y.indptr) for a CSR
    ...
    if axis is None:
        if sample_weight is None:
            ...
            return jnp.dot(jnp.diff(X.indptr), sample_weight)
    elif axis == 1:
        out = jnp.diff(X.indptr)
        if sample_weight is None:
        ...
        if sample_weight is None:
            ...
            weights = jnp.repeat(sample_weight, jnp.diff(X.indptr))
            return jnp.bincount(X.indices, minlength=X.shape[1], weights=weights)
    else:
...
def _get_median(data, n_zeros):
    ...
    if not n_elems:
        return jnp.nan
    n_negative = jnp.count_nonzero(data < 0)
...
def csc_median_axis_0(X):
    ...
    It is equivalent to doing jnp.median(X, axis=0).
    ...
    median = jnp.zeros(n_features)

    for f_ind, (start, end) in enumerate(zip(indptr[:-1], indptr[1:])):
        # Prevent modifying X in place
        data = jnp.copy(X.data[start:end])
