...
def squared_norm(x):
    ...
    """
    x = jnp.ravel(x, order="K")
    if jnp.issubdtype(x.dtype, jnp.integer):
        warnings.warn(
            (
                "Array type is integer, jnp.dot may overflow. "
...
def row_norms(X, squared=False):
    ...
    Equivalent to jnp.sqrt((X * X).sum(axis=1)), but also supports sparse
...
def fast_logdet(A):
    ...
    Equivalent to : jnp.log(jnp.det(A)) but more robust.
...
def weighted_mode(a, w, *, axis=0):
    ...
    if a.shape != w.shape:
        w = jnp.full(a.shape, w, dtype=w.dtype)

    scores = jnp.unique(jnp.ravel(a))  # get ALL unique values
    testshape = list(a.shape)
    testshape[axis] = 1
    oldmostfreq = jnp.zeros(testshape)
    oldcounts = jnp.zeros(testshape)
    for score in scores:
        template = jnp.zeros(a.shape)
        ind = a == score
        template[ind] = w[ind]
        counts = jnp.expand_dims(jnp.sum(template, axis), axis)
...
def cartesian(arrays, out=None):
    ...
    array([[1, 4, 6],
           ...
           [3, 5, 7]])
    """
    arrays = [jnp.asarray(x) for x in arrays]
    shape = (len(x) for x in arrays)

    ix = jnp.indices(shape)
    ix = ix.reshape(len(arrays), -1).T

    if out is None:
        dtype = jnp.result_type(*arrays)  # find the most permissive dtype
        out = jnp.empty_like(ix, dtype=dtype)
...
def svd_flip(u, v, u_based_decision=True):
    ...
    u : ndarray
        ...
        dimensions so one can compute `jnp.dot(u * s, v)`.
    ...
    if u_based_decision:
        ...
        signs = jnp.sign(u[max_abs_cols, range(u.shape[1])])
        u *= signs
        v *= signs[:, jnp.newaxis]
    else:
        # rows of v, columns of u
        max_abs_rows = jnp.argmax(jnp.abs(v), axis=1)
        signs = jnp.sign(v[range(v.shape[0]), max_abs_rows])
        u *= signs
        v *= signs[:, jnp.newaxis]
    return u, v
...
def log_logistic(X, out=None):
    ...
    is_1d = X.ndim == 1
    X = jnp.atleast_2d(X)
    X = check_array(X, dtype=jnp.float64)
...
def softmax(X, copy=True):
    ...
    jnp.exp(X) / jnp.sum(jnp.exp(X), axis=1)
...
def _safe_accumulator_op(op, x, *args, **kwargs):
    ...
    ----------
    op : function
        A numpy accumulator function such as jnp.mean or jnp.sum.
    x : ndarray
    ...
    **kwargs : keyword arguments
    ...
    """
    if jnp.issubdtype(x.dtype, jnp.floating) and x.dtype.itemsize < 8:
        result = op(x, *args, **kwargs, dtype=jnp.float64)
    else:
...
def _incremental_mean_and_var(
    ...
    X_nan_mask = jnp.isnan(X)
    if jnp.any(X_nan_mask):
        sum_op = jnp.nansum
    else:
        sum_op = jnp.sum
    if sample_weight is not None:
        # equivalent to jnp.nansum(X * sample_weight, axis=0)
        # safer because jnp.float64(X*W) != jnp.float64(X)*jnp.float64(W)
        new_sum = _safe_accumulator_op(
            jnp.matmul, sample_weight, jnp.where(X_nan_mask, 0, X)
        )
        new_sample_count = _safe_accumulator_op(
            jnp.sum, sample_weight[:, None] * (~X_nan_mask), axis=0
        )
    else:
        new_sum = _safe_accumulator_op(sum_op, X, axis=0)
        n_samples = X.shape[0]
        new_sample_count = n_samples - jnp.sum(X_nan_mask, axis=0)
    ...
    if last_variance is None:
        ...
        temp = X - T
        if sample_weight is not None:
            # equivalent to jnp.nansum((X-T)**2 * sample_weight, axis=0)
            # safer because jnp.float64(X*W) != jnp.float64(X)*jnp.float64(W)
            correction = _safe_accumulator_op(
                jnp.matmul, sample_weight, jnp.where(X_nan_mask, 0, temp)
            )
            temp **= 2
...
def _deterministic_vector_sign_flip(u):
    ...
    """
    max_abs_rows = jnp.argmax(jnp.abs(u), axis=1)
    signs = jnp.sign(u[range(u.shape[0]), max_abs_rows])
    u *= signs[:, jnp.newaxis]
    return u
...
def stable_cumsum(arr, axis=None, rtol=1e-05, atol=1e-08):
    ...
    rtol : float, default=1e-05
        Relative tolerance, see ``jnp.allclose``.
    atol : float, default=1e-08
        Absolute tolerance, see ``jnp.allclose``.
    ...
    """
    out = jnp.cumsum(arr, axis=axis, dtype=jnp.float64)
    expected = jnp.sum(arr, axis=axis, dtype=jnp.float64)
    if not jnp.all(
        jnp.isclose(
...
def _nanaverage(a, weights=None):
    ...
    that :func:`jnp.nan` values are ignored from the average and weights can
    ...
    if weights is None:
        return jnp.nanmean(a)

    weights = jnp.array(weights, copy=False)
    a, weights = a[~mask], weights[~mask]
    try:
        return jnp.average(a, weights=weights)
    except ZeroDivisionError:
        # this is when all weights are zero, then ignore them
        return jnp.average(a)
