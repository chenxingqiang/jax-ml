"""Module that customize joblib tools for jax-learn usage."""
    ...
    if hasattr(delayed_func, "with_config"):
        ...
        warnings.warn(
            (
                ...
                "`joblib.delayed` to correctly propagate the jax-learn "
...
class Parallel(joblib.Parallel):
    """Tweak of :class:`joblib.Parallel` that propagates the jax-learn configuration.

    This subclass of :class:`joblib.Parallel` ensures that the active configuration
    (thread-local) of jax-learn is propagated to the parallel workers for the
    ...
    def __call__(self, iterable):
        ...
        """
        # Capture the thread-local jax-learn configuration at the time
...
def delayed(function):
    ...
    with `xlearn.utils.parallel.Parallel`. The latter captures the the jax-
        ...
        if config is None:
            warnings.warn(
                (
                    ...
                    " propagate the jax-learn configuration of the current thread to"
                    " the joblib workers."
                ),
                UserWarning,
            )
