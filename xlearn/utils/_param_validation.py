...
class _NanConstraint(_Constraint):
    """Constraint representing the indicator `jnp.nan`."""
...
class Interval(_Constraint):
    ...
    def __contains__(self, val):
        ...
        left = -jnp.inf if self.left is None else self.left
        right = jnp.inf if self.right is None else self.right
...
class _Booleans(_Constraint):
    ...
    Convenience class for
    [bool, jnp.bool_, Integral (deprecated)]
    """
...
class MissingValues(_Constraint):
    ...
    def __init__(self, numeric_only=False):
        ...
        self._constraints = [
            _InstancesOf(Integral),
            # we use an interval of Real to ignore jnp.nan that has its own constraint
            Interval(Real, None, None, closed="both"),
            _NanConstraint(),
            _PandasNAConstraint(),
        ]
...
def generate_valid_param(constraint):
    ...
    if isinstance(constraint, _InstancesOf):
        if constraint.type is jnp.ndarray:
