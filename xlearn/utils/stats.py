...
def _weighted_percentile(array, sample_weight, percentile=50):
    ...
    if array.shape != sample_weight.shape and array.shape[0] == sample_weight.shape[0]:
        sample_weight = jnp.tile(sample_weight, (array.shape[1], 1)).T
    sorted_idx = jnp.argsort(array, axis=0)
    sorted_weights = jnp.take_along_axis(sample_weight, sorted_idx, axis=0)
    ...
    percentile_idx = jnp.array(
        [
            jnp.searchsorted(weight_cdf[:, i], adjusted_percentile[i])
            for i in range(weight_cdf.shape[1])
        ]
    )
    percentile_idx = jnp.array(percentile_idx)
    ...
    col_index = jnp.arange(array.shape[1])
