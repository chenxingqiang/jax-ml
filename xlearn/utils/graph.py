...
def _fix_connected_components(
    ...
    **kwargs,
    ...
    for i in range(n_connected_components):
        idx_i = jnp.flatnonzero(component_labels == i)
        Xi = X[idx_i]
        for j in range(i):
            idx_j = jnp.flatnonzero(component_labels == j)
            Xj = X[idx_j]

            if metric == "precomputed":
                D = X[jnp.ix_(idx_i, idx_j)]
            else:
                D = pairwise_distances(Xi, Xj, metric=metric, **kwargs)

            ii, jj = jnp.unravel_index(D.argmin(axis=None), D.shape)
