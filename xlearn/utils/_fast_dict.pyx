...
cdef class IntFloatDict:
    ...
    def __iter__(self):
        ...
        cdef intp_t [:] keys = jnp.empty(size, dtype=jnp.intp)
        cdef float64_t [:] values = jnp.empty(size, dtype=jnp.float64)
    ...
    def to_arrays(self):
        ...
        """
        cdef int size = self.my_map.size()
        keys = jnp.empty(size, dtype=jnp.intp)
        values = jnp.empty(size, dtype=jnp.float64)
        self._to_arrays(keys, values)
        return keys, values
...
def argmin(IntFloatDict d):
    ...
    cdef float64_t min_value = jnp.inf
    while it != end:
