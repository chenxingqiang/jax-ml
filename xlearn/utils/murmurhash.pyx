...
def _murmurhash3_bytes_array_u32(
    ...
    cdef:
        cnp.uint32_t[:] out = jnp.zeros(key.size, jnp.uint32)
        Py_ssize_t i
...
def _murmurhash3_bytes_array_s32(
    ...
    # TODO make it possible to pass preallocated output array
    cdef:
        cnp.int32_t[:] out = jnp.zeros(key.size, jnp.int32)
        Py_ssize_t i
    for i in range(key.shape[0]):
...
def murmurhash3_32(key, seed=0, positive=False):
    ...
    Parameters
    ----------
    key : jnp.int32, bytes, unicode or ndarray of dtype=jnp.int32
    ...
    elif isinstance(key, int) or isinstance(key, jnp.int32):
        if positive:
