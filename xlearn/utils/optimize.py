...
def _cg(fhess_p, fgrad, maxiter, tol):
    ...
    xsupi : ndarray of shape (n_features,) or (n_features + 1,)
        Estimated solution.
    """
    xsupi = jnp.zeros(len(fgrad), dtype=fgrad.dtype)
    ri = fgrad
    psupi = -ri
    i = 0
    dri0 = jnp.dot(ri, ri)
    ...
    while i <= maxiter:
        if jnp.sum(jnp.abs(ri)) <= tol:
            break

        Ap = fhess_p(psupi)
        # check curvature
        curv = jnp.dot(psupi, Ap)
        if 0 <= curv <= 3 * jnp.finfo(jnp.float64).eps:
            break
        elif curv < 0:
            if i > 0:
                break
        ...
        i = i + 1
        dri0 = dri1  # update jnp.dot(ri,ri) for next time.
...
def _newton_cg(
    ...
    while k < maxiter:
        ...
        absgrad = jnp.abs(fgrad)
        if jnp.max(absgrad) <= tol:
            break

        maggrad = jnp.sum(absgrad)
        eta = min([0.5, jnp.sqrt(maggrad)])
...
def _check_optimize_result(solver, result, max_iter=None, extra_warning_msg=None):
    ...
    """
    # handle both scipy and jax-learn solver names
    if solver == "lbfgs":
        if result.status != 0:
            try:
            ...
            warning_msg = (
                ...
                "    https://jax-learn.cc/stable/modules/"
                "preprocessing.html"
