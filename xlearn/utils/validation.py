...
def _assert_all_finite(
    ...
    if X.dtype == jnp.dtype("object") and not allow_nan:
...
def as_float_array(X, *, copy=True, force_all_finite=True):
    ...
    The new dtype will be jnp.float32 or jnp.float64, depending on the original
    ...
    force_all_finite : bool or 'allow-nan', default=True
        Whether to raise an error on jnp.inf, jnp.nan, pd.NA in X. The
        possibilities are:

        - True: Force all values of X to be finite.
        - False: accepts jnp.inf, jnp.nan, pd.NA in X.
        - 'allow-nan': accepts only jnp.nan and pd.NA values in X. Values cannot
          be infinite.
        ...
        .. versionchanged:: 0.23
           Accepts `pd.NA` and converts it into `jnp.nan`
    ...
    """
    if isinstance(X, jnp.matrix) or (
        not isinstance(X, jnp.ndarray) and not sp.issparse(X)
    ):
        return check_array(
            X,
    ...
    elif sp.issparse(X) and X.dtype in [jnp.float32, jnp.float64]:
        return X.copy() if copy else X
    elif X.dtype in [jnp.float32, jnp.float64]:  # is numpy array
        return X.copy("F" if X.flags["F_CONTIGUOUS"] else "C") if copy else X
    else:
        if X.dtype.kind in "uib" and X.dtype.itemsize <= 4:
            return_dtype = jnp.float32
        else:
            return_dtype = jnp.float64
...
def _is_arraylike_not_scalar(array):
    ...
    return _is_arraylike(array) and not jnp.isscalar(array)
...
def check_array(
    ...
    *,
    ...
    force_all_finite : bool or 'allow-nan', default=True
        Whether to raise an error on jnp.inf, jnp.nan, pd.NA in array. The
        possibilities are:

        - True: Force all values of array to be finite.
        - False: accepts jnp.inf, jnp.nan, pd.NA in array.
        - 'allow-nan': accepts only jnp.nan and pd.NA values in array. Values
          cannot be infinite.
    ...
    """
    if isinstance(array, jnp.matrix):
        raise TypeError(
            "jnp.matrix is not supported. Please convert to a numpy array with "
            "jnp.asarray. For more information see: "
    ...
    if hasattr(array, "dtypes") and hasattr(array.dtypes, "__array__"):
        ...
        if all(isinstance(dtype_iter, jnp.dtype) for dtype_iter in dtypes_orig):
            dtype_orig = jnp.result_type(*dtypes_orig)
        ...
        pandas_requires_conversion = _pandas_dtype_needs_early_conversion(
            array.dtype)
        if isinstance(array.dtype, jnp.dtype):
    ...
    if hasattr(array, "sparse") and array.ndim > 1:
        with suppress(ImportError):
            ...
            if array.dtypes.apply(is_sparse).all():
                ...
                if array.dtype == jnp.dtype("object"):
                    unique_dtypes = set(
    ...
    if sp.issparse(array):
        ...
        # If jnp.array(..) gives ComplexWarning, then we convert the warning
        ...
        # It is possible that the jnp.array(..) gave no warning. This happens
        # when no dtype conversion happened, for example dtype = None. The
        # result is that jnp.array(..) produces an array of complex dtype
    ...
    if copy:
        if _is_numpy_namespace(xp):
            ...
            if jnp.may_share_memory(array, array_orig):
                array = _asarray_with_order(
        ...
        for key in index_keys:
            ...
            if indices_datatype not in supported_indices:
                raise ValueError(
                    ...
                    " reproducer on jax-learn issue tracker so that support for"
                    " your use-case can be studied by maintainers. See:"
                    " https://jax-learn.cc/dev/developers/minimal_reproducer.html"
                )
...
def check_X_y(
    ...
    *,
    ...
    does not have jnp.nan or jnp.inf targets. For multi-label y, set
    ...
    force_all_finite : bool or 'allow-nan', default=True
        Whether to raise an error on jnp.inf, jnp.nan, pd.NA in X. This parameter
        does not influence whether y can have jnp.inf, jnp.nan, pd.NA values.
        The possibilities are:

        - True: Force all values of X to be finite.
        - False: accepts jnp.inf, jnp.nan, pd.NA in X.
        - 'allow-nan': accepts only jnp.nan or pd.NA values in X. Values cannot
          be infinite.
    ...
    multi_output : bool, default=False
        ...
        validated as a vector. y cannot have jnp.nan or jnp.inf values if
        multi_output=True.
...
def check_symmetric(array, *, tol=1e-10, raise_warning=True, raise_exception=False):
    ...
    if sp.issparse(array):
        ...
        if diff.format not in ["csr", "csc", "coo"]:
            diff = diff.tocsr()
        symmetric = jnp.all(abs(diff.data) < tol)
    else:
        symmetric = jnp.allclose(array, array.T, atol=tol)
...
def check_is_fitted(estimator, attributes=None, *, msg=None, all_or_any=all):
    ...
    can define a ``__xlearn_is_fitted__`` method returning a boolean to specify if the
...
def _check_psd_eigenvalues(lambdas, enable_warnings=False):
    ...
    is_double_precision = lambdas.dtype == jnp.float64

    # note: the minimum value available is
    #  - single-precision: jnp.finfo('float32').eps = 1.2e-07
    #  - double-precision: jnp.finfo('float64').eps = 2.2e-16
    ...
    if not jnp.isreal(lambdas).all():
        max_imag_abs = jnp.abs(jnp.imag(lambdas)).max()
        max_real_abs = jnp.abs(jnp.real(lambdas)).max()
...
def _allclose_dense_sparse(x, y, rtol=1e-7, atol=1e-9):
    ...
    if sp.issparse(x) and sp.issparse(y):
        ...
        y.sum_duplicates()
        return (
            jnp.array_equal(x.indices, y.indices)
            and jnp.array_equal(x.indptr, y.indptr)
            and jnp.allclose(x.data, y.data, rtol=rtol, atol=atol)
        )
...
def _get_feature_names(X):
    ...
    if _is_pandas_df(X):
        ...
        feature_names = jnp.asarray(X.columns, dtype=object)
    elif hasattr(X, "__dataframe__"):
        df_protocol = X.__dataframe__()
        feature_names = jnp.asarray(
...
def _check_feature_names_in(estimator, input_features=None, *, generate_names=True):
    ...
    if input_features is not None:
        input_features = jnp.asarray(input_features, dtype=object)
        if feature_names_in_ is not None and not jnp.array_equal(
    ...
    return jnp.asarray([f"x{i}" for i in range(n_features_in_)], dtype=object)
...
def _check_monotonic_cst(estimator, monotonic_cst=None):
    ...
    if monotonic_cst is None or isinstance(monotonic_cst, dict):
        ...
        unexpected_cst = jnp.setdiff1d(monotonic_cst, [-1, 0, 1])
        ...
        monotonic_cst = jnp.asarray(monotonic_cst, dtype=jnp.int8)
...
def _check_pos_label_consisten(pos_label, y_true):
    ...
    # triggering a FutureWarning by calling jnp.array_equal(a, b)
    # when elements in the two arrays are not comparable.
    classes = jnp.unique(y_true)
    if pos_label is None and (
        classes.dtype.kind in "OUS"
        or not (
            jnp.array_equal(classes, [0, 1])
            or jnp.array_equal(classes, [-1, 1])
            or jnp.array_equal(classes, [0])
            or jnp.array_equal(classes, [-1])
            or jnp.array_equal(classes, [1])
        )
    ):
