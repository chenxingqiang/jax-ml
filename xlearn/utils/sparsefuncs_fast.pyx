...
def _sqeuclidean_row_norms_sparse(
    ...
    dtype = jnp.float32 if floating is float else jnp.float64

    cdef floating[::1] squared_row_norms = jnp.zeros(n_samples, dtype=dtype)
    ...
    return jnp.asarray(squared_row_norms)
...
def _csr_mean_variance_axis0(
    ...
    cdef:
        ...
        cnp.float64_t[::1] means = jnp.zeros(n_features)
        # variances[j] contains the variance of feature j
        cnp.float64_t[::1] variances = jnp.zeros(n_features)

        cnp.float64_t[::1] sum_weights = jnp.full(
            fill_value=jnp.sum(weights, dtype=jnp.float64), shape=n_features
        )
        cnp.float64_t[::1] sum_weights_nz = jnp.zeros(shape=n_features)
        cnp.float64_t[::1] correction = jnp.zeros(shape=n_features)

        cnp.uint64_t[::1] counts = jnp.full(
            fill_value=weights.shape[0], shape=n_features, dtype=jnp.uint64
        )
        cnp.uint64_t[::1] counts_nz = jnp.zeros(shape=n_features, dtype=jnp.uint64)
    ...
    if floating is float:
        return (
            jnp.array(means, dtype=jnp.float32),
            jnp.array(variances, dtype=jnp.float32),
            jnp.array(sum_weights, dtype=jnp.float32),
        )
    else:
        return (
            jnp.asarray(means), jnp.asarray(variances), jnp.asarray(sum_weights)
        )
...
def incr_mean_variance_axis0(X, last_mean, last_var, last_n, weights=None):
    ...
    X_dtype = X.dtype
    if weights is None:
        weights = jnp.ones(X.shape[0], dtype=X_dtype)
    elif weights.dtype not in [jnp.float32, jnp.float64]:
        weights = weights.astype(jnp.float64, copy=False)
    if last_n.dtype not in [jnp.float32, jnp.float64]:
        last_n = last_n.astype(jnp.float64, copy=False)
...
def _incr_mean_variance_axis0(
    ...
    new_mean = jnp.zeros(n_features, dtype=dtype)
    new_var = jnp.zeros_like(new_mean, dtype=dtype)
    updated_mean = jnp.zeros_like(new_mean, dtype=dtype)
    updated_var = jnp.zeros_like(new_mean, dtype=dtype)
    ...
    updated_n = jnp.zeros(shape=n_features, dtype=dtype)
    last_over_new_n = jnp.zeros_like(updated_n, dtype=dtype)
    ...
    if is_first_pass:
        return jnp.asarray(new_mean), jnp.asarray(new_var), jnp.asarray(new_n)
    ...
    return (
        jnp.asarray(updated_mean),
        jnp.asarray(updated_var),
        jnp.asarray(updated_n),
    )
...
def assign_rows_csr(
    ...
    No-copy supported for both dtype=jnp.float32 and dtype=jnp.float64.

    Parameters
    ----------
    X : scipy.sparse.csr_matrix, shape=(n_samples, n_features)
    X_rows : array, dtype=jnp.intp, shape=n_rows
    out_rows : array, dtype=jnp.intp, shape=n_rows
    out : array, shape=(arbitrary, n_features)
    """
    cdef:
        # npy_intp (jnp.intp in Python) is what jnp.where returns,
