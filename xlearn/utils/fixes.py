...
#     return jnp.percentile(a, q, interpolation=method, **kwargs)
...
def _get_threadpool_controller():
    ...
    if not hasattr(xlearn, "_xlearn_threadpool_controller"):
        xlearn._xlearn_threadpool_controller = threadpoolctl.ThreadpoolController()

    return xlearn._xlearn_threadpool_controller
...
@deprecated(
    "The function `delayed` has been moved from `xlearn.utils.fixes` to "
    "`xlearn.utils.parallel`. This import path will be removed in 1.5."
)
def delayed(function):
    from xlearn.utils.parallel import delayed
