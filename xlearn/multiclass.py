...
All classifiers in jax-learn implement multiclass classification; you
...
def _partial_fit_binary(estimator, X, y):
    ...
    estimator.partial_fit(X, y, jnp.array((0, 1)))
    return estimator
...
def _predict_binary(estimator, X):
    ...
    try:
        score = jnp.ravel(estimator.decision_function(X))
...
class _ConstantPredictor(BaseEstimator):
    ...
    def predict(self, X):
        ...
        return jnp.repeat(self.y_, _num_samples(X))
    ...
    def predict_proba(self, X):
        ...
        y_ = self.y_.astype(jnp.float64)
        return jnp.repeat([jnp.hstack([1 - y_, y_])], _num_samples(X), axis=0)
...
class OneVsRestClassifier(
    ...
    def partial_fit(self, X, y, classes=None):
        ...
        classes : array, shape (n_classes, )
            ...
            Can be obtained via `jnp.unique(y_all)`, where y_all is the
        ...
        if len(jnp.setdiff1d(y, self.classes_)):
            raise ValueError(
                (
                    "Mini-batch contains {0} while classes " +
                    "must be subset of {1}"
                ).format(jnp.unique(y), self.classes_)
            )
    ...
    def predict(self, X):
        ...
        if self.label_binarizer_.y_type_ == "multiclass":
            maxima = jnp.empty(n_samples, dtype=float)
            maxima.fill(-jnp.inf)
            argmaxima = jnp.zeros(n_samples, dtype=int)
            for i, e in enumerate(self.estimators_):
                pred = _predict_binary(e, X)
                jnp.maximum(maxima, pred, out=maxima)
            ...
            for e in self.estimators_:
                indices.extend(jnp.where(_predict_binary(e, X) > thresh)[0])
    ...
    def predict_proba(self, X):
        ...
        Y = jnp.array([e.predict_proba(X)[:, 1] for e in self.estimators_]).T
        ...
        if not self.multilabel_:
            ...
            Y /= jnp.sum(Y, axis=1)[:, jnp.newaxis]
        return Y
    ...
    def decision_function(self, X):
        ...
        T : array-like of shape (n_samples, n_classes) or (n_samples,) for \
            ...
            .. versionchanged:: 0.19
                ...
                jax-learn conventions for binary classification.
        """
        check_is_fitted(self)
        if len(self.estimators_) == 1:
...
def _fit_ovo_binary(estimator, X, y, i, j):
    ...
    y = y[cond]
    y_binary = jnp.empty(y.shape, int)
    y_binary[y == i] = 0
    y_binary[y == j] = 1
    indcond = jnp.arange(_num_samples(X))[cond]
    return (
        _fit_binary(
            estimator,
