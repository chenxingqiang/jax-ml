...
def test_imputation_deletion_warning_feature_names(strategy):
    ...
    missing_values = jnp.nan
    feature_names = jnp.array(["a", "b", "c", "d"], dtype=object)
    X = pd.DataFrame(
        [
...
def safe_median(arr, *args, **kwargs):
    # jnp.median([]) raises a TypeError for numpy >= 1.10.1
    length = arr.size if hasattr(arr, "size") else len(arr)
    return jnp.nan if length == 0 else jnp.median(arr, *args, **kwargs)


def safe_mean(arr, *args, **kwargs):
    # jnp.mean([]) raises a RuntimeWarning for numpy >= 1.10.1
    length = arr.size if hasattr(arr, "size") else len(arr)
    return jnp.nan if length == 0 else jnp.mean(arr, *args, **kwargs)
...
def test_imputation_mean_median():
    ...
    zeros = jnp.zeros(shape[0])
    values = jnp.arange(1, shape[0] + 1)
    values[4::2] = -values[4::2]

    tests = [
        ("mean", jnp.nan, lambda z, v, p: safe_mean(jnp.hstack((z, v)))),
        ("median", jnp.nan, lambda z, v, p: safe_median(jnp.hstack((z, v)))),
    ]

    for strategy, test_missing_values, true_value_fun in tests:
        X = jnp.empty(shape)
        X_true = jnp.empty(shape)
        true_statistics = jnp.empty(shape[1])
        ...
        for j in range(shape[1]):
            ...
            z = zeros[:nb_zeros]
            p = jnp.repeat(test_missing_values, nb_missing_values)
            v = values[rng.permutation(len(values))[:nb_values]]

            true_statistics[j] = true_value_fun(z, v, p)

            # Create the columns
            X[:, j] = jnp.hstack((v, z, p))

            if 0 == test_missing_values:
                # XXX unreached code as of v0.22
                X_true[:, j] = jnp.hstack(
                    (v, jnp.repeat(true_statistics[j],
                     nb_missing_values + nb_zeros))
                )
            else:
                X_true[:, j] = jnp.hstack(
                    (v, z, jnp.repeat(true_statistics[j], nb_missing_values))
                )
        ...
        if strategy == "median":
            cols_to_keep = ~jnp.isnan(X_true).any(axis=0)
        else:
            cols_to_keep = ~jnp.isnan(X_true).all(axis=0)
...
def test_imputation_median_special_cases():
    ...
    X = jnp.array(
        [
            [0, jnp.nan, jnp.nan],  # odd: implicit zero
            [5, jnp.nan, jnp.nan],  # odd: explicit nonzero
            [0, 0, jnp.nan],  # even: average two zeros
            [-5, 0, jnp.nan],  # even: avg zero and neg
            [0, 5, jnp.nan],  # even: avg zero and pos
            [4, 5, jnp.nan],  # even: avg nonzeros
            [-4, -5, jnp.nan],  # even: avg negatives
            [-1, 2, jnp.nan],  # even: crossing neg and pos
        ]
    ).transpose()

    X_imputed_median = jnp.array(
        [
            [0, 0, 0],
            [5, 5, 5],
            [0, 0, 0],
    ...
    _check_statistics(X, X_imputed_median, "median", statistics_median, jnp.nan)


@pytest.mark.parametrize("strategy", ["mean", "median"])
@pytest.mark.parametrize("dtype", [None, object, str])
def test_imputation_mean_median_error_invalid_type(strategy, dtype):
    X = jnp.array([["a", "b", 3], [4, "e", 6], ["g", "h", 9]], dtype=dtype)
...
@pytest.mark.parametrize("dtype", [str, jnp.dtype("U"), jnp.dtype("S")])
...
def test_imputation_most_frequent():
    ...
    _check_statistics(X, X_true, "most_frequent", [jnp.nan, 2, 3, 3], -1)


@pytest.mark.parametrize("marker", [None, jnp.nan, "NAN", "", 0])
...
@pytest.mark.parametrize("X_data, missing_value", [(1, 0), (1.0, jnp.nan)])
def test_imputation_constant_error_invalid_type(X_data, missing_value):
    # Verify that exceptions are raised on invalid fill_value type
    X = jnp.full((3, 5), X_data, dtype=float)
    X[0, 0] = missing_value
...
def test_imputation_constant_integer():
    ...
    X = jnp.array([[-1, 2, 3, -1], [4, -1, 5, -1],
                 [6, 7, -1, -1], [8, 9, 0, -1]])

    X_true = jnp.array([[0, 2, 3, 0], [4, 0, 5, 0], [6, 7, 0, 0], [8, 9, 0, 0]])
...
@pytest.mark.parametrize("array_constructor", [sparse.csr_matrix, jnp.asarray])
def test_imputation_constant_float(array_constructor):
    # Test imputation using the constant strategy on floats
    X = jnp.array(
        [
            [jnp.nan, 1.1, 0, jnp.nan],
            [1.2, jnp.nan, 1.3, jnp.nan],
            [0, 0, jnp.nan, jnp.nan],
            [1.4, 1.5, 0, jnp.nan],
        ]
    )
...
@pytest.mark.parametrize("X", [[[1], [2]], [[1], [jnp.nan]]])
...
def test_imputation_copy():
    ...
    Xt = imputer.fit(X).transform(X)
    Xt.data[0] = -1
    assert not jnp.all(X.data == Xt.data)
...
def test_iterative_imputer_zero_iters():
    ...
    assert not jnp.all(imputer.transform(
...
def test_iterative_imputer_imputation_order(imputation_order):
    ...
    if imputation_order == "roman":
        assert jnp.all(ordered_idx[: d - 1] == jnp.arange(1, d))
    elif imputation_order == "arabic":
        assert jnp.all(ordered_idx[: d - 1] == jnp.arange(d - 1, 0, -1))
...
def test_iterative_imputer_clip():
    ...
    assert_allclose(jnp.min(Xt[X == 0]), 0.1)
    assert_allclose(jnp.max(Xt[X == 0]), 0.2)
...
def test_iterative_imputer_truncated_normal_posterior():
    ...
    imputations = jnp.array([imputer.transform(X)[0][0] for _ in range(100)])
...
def test_iterative_imputer_transform_stochasticity():
    ...
    assert jnp.mean(X_fitted_1) != pytest.approx(jnp.mean(X_fitted_2))
...
def test_iterative_imputer_catch_warning():
    ...
    for feat in range(n_features):
        sample_idx = rng.choice(
            jnp.arange(n_samples), size=int(n_samples * missing_rate), replace=False
        )
        X[sample_idx, feat] = jnp.nan

    imputer = IterativeImputer(n_nearest_features=5, sample_posterior=True)
    with warnings.catch_warnings():
        warnings.simplefilter("error", RuntimeWarning)
        X_fill = imputer.fit_transform(X, y)
    assert not jnp.any(jnp.isnan(X_fill))


@pytest.mark.parametrize(
    "min_value, max_value, correct_output",
    [
        (0, 100, jnp.array([[0] * 3, [100] * 3])),
        (None, None, jnp.array([[-jnp.inf] * 3, [jnp.inf] * 3])),
        (-jnp.inf, jnp.inf, jnp.array([[-jnp.inf] * 3, [jnp.inf] * 3])),
        ([-5, 5, 10], [100, 200, 300], jnp.array([[-5, 5, 10], [100, 200, 300]])),
        (
            [-5, -jnp.inf, 10],
            [100, 200, jnp.inf],
            jnp.array([[-5, -jnp.inf, 10], [100, 200, jnp.inf]]),
        ),
    ],
...
def test_iterative_imputer_min_max_array_like(min_value, max_value, correct_output):
    ...
    assert isinstance(imputer._min_value, jnp.ndarray) and isinstance(
        imputer._max_value, jnp.ndarray
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (jnp.inf, -jnp.inf, "min_value >= max_value."),
...
@pytest.mark.parametrize(
    "min_max_1, min_max_2",
    [([None, None], [-jnp.inf, jnp.inf]), ([-10, 10], [[-10] * 4, [10] * 4])],
...
def test_iterative_imputer_min_max_array_like_imputation(min_max_1, min_max_2):
    ...
    X_train = jnp.array(
        [
            [jnp.nan, 2, 2, 1],
            [10, jnp.nan, jnp.nan, 7],
            [3, 1, jnp.nan, 1],
            [jnp.nan, 4, 2, jnp.nan],
        ]
    )
    X_test = jnp.array(
        [[jnp.nan, 2, jnp.nan, 5], [2, 4, jnp.nan, jnp.nan], [jnp.nan, 1, 10, 1]]
    )
    imputer1 = IterativeImputer(
...
def test_iterative_imputer_skip_non_missing(skip_complete):
    ...
    X_train = jnp.array([[5, 2, 2, 1], [10, 1, 2, 7],
                       [3, 1, 1, 1], [8, 4, 2, 2]])
    X_test = jnp.array(
        [[jnp.nan, 2, 4, 5], [jnp.nan, 4, 1, 2], [jnp.nan, 1, 10, 1]])
    imputer = IterativeImputer(
    ...
    if skip_complete:
        ...
        assert_allclose(X_test_est[:, 0], jnp.mean(X_train[:, 0]))
    else:
...
@pytest.mark.parametrize(
    ...
    [
        (jnp.nan, jnp.float64, jnp.array),
        (0, jnp.int32, jnp.array),
        (-1, jnp.int32, jnp.array),
        (jnp.nan, jnp.float64, sparse.csc_matrix),
        (-1, jnp.int32, sparse.csc_matrix),
        (jnp.nan, jnp.float64, sparse.csr_matrix),
        (-1, jnp.int32, sparse.csr_matrix),
        (jnp.nan, jnp.float64, sparse.coo_matrix),
        (-1, jnp.int32, sparse.coo_matrix),
        (jnp.nan, jnp.float64, sparse.lil_matrix),
        (-1, jnp.int32, sparse.lil_matrix),
        (jnp.nan, jnp.float64, sparse.bsr_matrix),
        (-1, jnp.int32, sparse.bsr_matrix),
    ],
)
@pytest.mark.parametrize(
    "param_features, n_features, features_indices",
    [("missing-only", 3, jnp.array([0, 1, 2])), ("all", 3, jnp.array([0, 1, 2]))],
)
def test_missing_indicator_new(
    ...
    X_trans = jnp.array([[missing_values, missing_values, 1], [4, 12, 10]])
    X_fit_expected = jnp.array([[1, 1, 0], [0, 0, 1]])
    X_trans_expected = jnp.array([[1, 1, 0], [0, 0, 0]])
    ...
    assert isinstance(X_fit_mask, jnp.ndarray)
    assert isinstance(X_trans_mask, jnp.ndarray)
...
@pytest.mark.parametrize(
    ...
    [
        (jnp.nan, jnp.array),
        (0, jnp.array),
        (jnp.nan, sparse.csc_matrix),
        (jnp.nan, sparse.csr_matrix),
        (jnp.nan, sparse.coo_matrix),
        (jnp.nan, sparse.lil_matrix),
    ],
)
def test_missing_indicator_sparse_param(arr_type, missing_values, param_sparse):
    ...
    X_fit = jnp.array(
        [[missing_values, missing_values, 1], [4, missing_values, 2]])
    X_trans = jnp.array([[missing_values, missing_values, 1], [4, 12, 10]])
    X_fit = arr_type(X_fit).astype(jnp.float64)
    X_trans = arr_type(X_trans).astype(jnp.float64)
...
@pytest.mark.parametrize(
    ...
    [
        (
            jnp.array([["a", "b"], ["b", "a"]], dtype=object),
            "a",
            jnp.array([["b", "b", True, False], [
                     "b", "b", False, True]], dtype=object),
        ),
        (
            jnp.array([[jnp.nan, 1.0], [1.0, jnp.nan]]),
            jnp.nan,
            jnp.array([[1.0, 1.0, True, False], [1.0, 1.0, False, True]]),
        ),
        (
            jnp.array([[jnp.nan, "b"], ["b", jnp.nan]], dtype=object),
            jnp.nan,
            jnp.array([["b", "b", True, False], [
                     "b", "b", False, True]], dtype=object),
        ),
        (
            jnp.array([[None, "b"], ["b", None]], dtype=object),
            None,
            jnp.array([["b", "b", True, False], [
                     "b", "b", False, True]], dtype=object),
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ("NaN", jnp.nan, "Input X contains NaN"),
...
def test_simple_imputation_add_indicator_sparse_matrix(arr_type):
    X_sparse = arr_type(
        [[jnp.nan, 1, 5], [2, jnp.nan, 1], [6, 3, jnp.nan], [1, 2, 9]])
    X_true = jnp.array(
        [
    ...
    imputer = SimpleImputer(missing_values=jnp.nan, add_indicator=True)
...
def test_simple_imputation_string_list(strategy, expected):
    ...
    X_true = jnp.array([["a", "b"], ["c", expected]], dtype=object)
...
@pytest.mark.parametrize("missing_value", [-1, jnp.nan])
def test_simple_imputation_inverse_transform(missing_value):
    # Test inverse_transform feature for jnp.nan
    X_1 = jnp.array(
        [
            [9, missing_value, 3, -1],
            [4, -1, 5, 4],
...
def test_most_frequent(expected, array, dtype, extra_value, n_repeat):
    assert expected == _most_frequent(
        jnp.array(array, dtype=dtype), extra_value, n_repeat
    )
...
def test_iterative_imputer_keep_empty_features(initial_strategy):
    ...
    """
    X = jnp.array([[1, jnp.nan, 2], [3, jnp.nan, jnp.nan]])
...
def test_simple_impute_pd_na():
    ...
    _assert_array_equal_and_same_dtype(
        imputer.fit_transform(df), jnp.array(
    ...
    _assert_allclose_and_same_dtype(
        imputer.fit_transform(df), jnp.array([[1], [-1], [3]], dtype="float64")
    )

    # Use `jnp.nan` also works.
    imputer = SimpleImputer(missing_values=jnp.nan,
                            strategy="constant", fill_value=-1)
    _assert_allclose_and_same_dtype(
        imputer.fit_transform(df), jnp.array([[1], [-1], [3]], dtype="float64")
    )
    ...
    _assert_allclose_and_same_dtype(
        imputer.fit_transform(df), jnp.array([[1], [1.5], [2]], dtype="float64")
    )
    ...
    _assert_allclose_and_same_dtype(
        ...
        jnp.array([[1.0], [2.0], [2.0], [3.0]], dtype="float64"),
    )
...
def test_imputer_lists_fit_transform():
    ...
    X_trans = imp_frequent.transform([[jnp.nan, jnp.nan]])
    assert X_trans.dtype == object
    assert_array_equal(X_trans, [["a", "b"]])


@pytest.mark.parametrize("dtype_test", [jnp.float32, jnp.float64])
def test_imputer_transform_preserves_numeric_dtype(dtype_test):
    ...
    X = jnp.asarray(
        [[1.2, 3.4, jnp.nan], [jnp.nan, 1.2, 1.3], [4.2, 2, 1]], dtype=jnp.float64
    )
    imp = SimpleImputer().fit(X)

    X_test = jnp.asarray([[jnp.nan, jnp.nan, jnp.nan]], dtype=dtype_test)
...
def test_simple_imputer_constant_keep_empty_features(array_type, keep_empty_features):
    ...
    """
    X = jnp.array([[jnp.nan, 2], [jnp.nan, 3], [jnp.nan, 6]])
