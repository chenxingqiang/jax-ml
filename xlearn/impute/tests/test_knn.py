...
@pytest.mark.parametrize("na", [jnp.nan, -1])
...
def test_knn_imputer_removes_all_na_features(na):
    ...
    assert not jnp.isnan(X_transform).any()
    assert X_transform.shape == (4, 5)

    X_test = jnp.arange(0, 12).reshape(2, 6)
...
def test_knn_imputer_verify(na):
    ...
    X = jnp.array([[0, 0], [na, 2], [4, 3], [5, 6], [7, 7], [9, 8], [11, 16]])

    X1 = jnp.array([[1, 0], [3, 2], [4, na]])

    X_2_1 = (0 + 3 + 6 + 7 + 8) / 5
    X1_imputed = jnp.array([[1, 0], [3, 2], [4, X_2_1]])
...
def test_knn_imputer_one_n_neighbors(na):
    X = jnp.array([[0, 0], [na, 2], [4, 3], [5, na], [7, 7], [na, 8], [14, 13]])
...
def test_knn_imputer_weight_uniform(na):
    X = jnp.array([[0, 0], [na, 2], [4, 3], [5, 6], [7, 7], [9, 8], [11, 10]])

    # Test with "uniform" weight (or unweighted)
    X_imputed_uniform = jnp.array(
...
def test_knn_imputer_weight_distance(na):
    ...
    manual_imputed_value = jnp.average(X[X_neighbors_idx, 0], weights=weights)

    X_imputed_distance1 = jnp.array(
        [[0, 0], [manual_imputed_value, 2], [4, 3],
            [5, 6], [7, 7], [9, 8], [11, 10]]
    )

    # NearestNeighbor calculation
    X_imputed_distance2 = jnp.array(
    ...
    dist_0_1 = jnp.sqrt((3 / 2) * ((1 - 0) ** 2 + (2 - 0) ** 2))
    dist_0_2 = jnp.sqrt((3 / 2) * ((2 - 0) ** 2 + (3 - 0) ** 2))
    imputed_value = jnp.average([2, 3], weights=[1 / dist_0_1, 1 / dist_0_2])
    ...
    col1_donor_values = jnp.ma.masked_invalid(X[[0, 2, 3, 4, 5], 1]).copy()
    col3_donor_values = jnp.ma.masked_invalid(X[[0, 3, 4, 5, 6], 3]).copy()

    # Final imputed values
    r1c1_imp = jnp.ma.average(col1_donor_values, weights=r1c1_nbor_wt)
    r1c3_imp = jnp.ma.average(col3_donor_values, weights=r1c3_nbor_wt)
    r2c3_imp = jnp.ma.average(col3_donor_values, weights=r2c3_nbor_wt)
    ...
    r0c3 = jnp.average(X[2:-1, -1], weights=r0c3_w)
    r1c3 = jnp.average(X[2:-1, -1], weights=r1c3_w)
    r2c2 = jnp.average(X[(0, 1, 3, 4, 5), 2], weights=r2c2_w)
    r7c0 = jnp.average(X[2:7, 0], weights=r7c0_w)
...
def test_knn_imputer_callable_metric():
    ...
    def custom_callable(x, y, missing_values=jnp.nan, squared=False):
        x = jnp.ma.array(x, mask=jnp.isnan(x))
        y = jnp.ma.array(y, mask=jnp.isnan(y))
        dist = jnp.nansum(jnp.abs(x - y))
        return dist

    X = jnp.array([[4, 3, 3, jnp.nan], [6, 9, 6, 9], [
                 4, 8, 6, 9], [jnp.nan, 9, 11, 10.0]])
...
@pytest.mark.parametrize("na", [-1, jnp.nan])
...
def test_knn_imputer_with_simple_example(na, working_memory):
    ...
    r0c1 = jnp.mean(X[1:6, 1])
    r0c3 = jnp.mean(X[2:-1, -1])
    r1c3 = jnp.mean(X[2:-1, -1])
    r2c2 = jnp.mean(X[[0, 1, 3, 4, 5], 2])
    r7c0 = jnp.mean(X[2:-1, 0])
...
def test_knn_imputer_distance_weighted_not_enough_neighbors(na, working_memory):
    X = jnp.array([[3, na], [2, na], [na, 4], [5, 6], [6, 8], [na, 5]])

    dist = pairwise_distances(
        X, metric="nan_euclidean", squared=False, missing_values=na
    )

    X_01 = jnp.average(X[3:5, 1], weights=1 / dist[0, 3:5])
    X_11 = jnp.average(X[3:5, 1], weights=1 / dist[1, 3:5])
    X_20 = jnp.average(X[3:5, 0], weights=1 / dist[2, 3:5])
    X_50 = jnp.average(X[3:5, 0], weights=1 / dist[5, 3:5])
...
@pytest.mark.parametrize("na, allow_nan", [(-1, False), (jnp.nan, True)])
