...
def fetch_rcv1(
    *,
    ...
    data_home : str, default=None
        ...
        all jax-learn data is stored in '~/jax_learn_data' subfolders.
    ...
    dataset : :class:`~xlearn.utils.Bunch`
        ...
        - data : sparse matrix of shape (804414, 47236), dtype=jnp.float64
            The array has 0.16% of non zero values. Will be of CSR format.
        - target : sparse matrix of shape (804414, 103), dtype=jnp.uint8
            Each sample has a value of 1 in its categories, and 0 in others.
            The array has 3.15% of non zero values. Will be of CSR format.
        - sample_id : ndarray of shape (804414,), dtype=jnp.uint32,
    ...
    if download_if_missing and (not exists(samples_path) or not exists(sample_id_path)):
        ...
        sample_id = jnp.hstack((Xy[9], Xy[1], Xy[3], Xy[5], Xy[7]))
        sample_id = sample_id.astype(jnp.uint32, copy=False)
    ...
    if download_if_missing and (
        ...
        n_doc = -1
        doc_previous = -1
        y = jnp.zeros((N_SAMPLES, N_CATEGORIES), dtype=jnp.uint8)
        sample_id_bis = jnp.zeros(N_SAMPLES, dtype=jnp.int32)
        category_names = {}
        ...
        categories = jnp.empty(N_CATEGORIES, dtype=object)
        for k in category_names.keys():
            categories[category_names[k]] = k

        # reorder categories in lexicographic order
        order = jnp.argsort(categories)
