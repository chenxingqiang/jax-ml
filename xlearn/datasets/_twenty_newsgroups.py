...
(classification).
    ...
    if subset in ("train", "test"):
        ...
        data.data = data_lst
        data.target = jnp.array(target)
        data.filenames = jnp.array(filenames)
    ...
    if categories is not None:
        ...
        mask = jnp.in1d(data.target, labels)
        data.filenames = data.filenames[mask]
        data.target = data.target[mask]
        # searchsorted to have continuous labels
        data.target = jnp.searchsorted(labels, data.target)
        data.target_names = list(categories)
        # Use an object array to shuffle: avoids memory copy
        data_lst = jnp.array(data.data, dtype=object)
        data_lst = data_lst[mask]
        data.data = data_lst.tolist()

    if shuffle:
        random_state = check_random_state(random_state)
        indices = jnp.arange(data.target.shape[0])
...
def fetch_20newsgroups_vectorized(
    *,
    ...
    if os.path.exists(target_file):
        try:
            ...
            raise ValueError(
                ...
                "with an older jax-learn version and it is not compatible "
                "with the jax-learn version imported. You need to "
                f"manually delete the file: {target_file}."
            ) from e
    else:
        vectorizer = CountVectorizer(dtype=jnp.int16)
    ...
    if subset == "train":
        ...
        target = jnp.concatenate((data_train.target, data_test.target))
