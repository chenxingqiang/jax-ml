...
def _load_coverage(F, header_length=6, dtype=jnp.int16):
...
def _load_csv(F):
    ...
    rec = jnp.loadtxt(F, skiprows=0, delimiter=",", dtype="a22,f4,f4")
...
def fetch_species_distributions(*, data_home=None, download_if_missing=True):
    ...
    - For an example of using this dataset with jax-learn, see
      :ref:`examples/applications/plot_species_distribution_modeling.py
      <sphx_glr_auto_examples_applications_plot_species_distribution_modeling.py>`.
    ...
    * `"Maximum entropy modeling of species geographic distributions"
    ...
    if not exists(archive_path):
        ...
        with jnp.load(samples_path) as X:  # samples.zip is a valid npz
            for f in X.files:
                fhandle = BytesIO(X[f])
                if "train" in f:
        ...
        with jnp.load(coverages_path) as X:  # coverages.zip is a valid npz
            coverages = []
            for f in X.files:
                fhandle = BytesIO(X[f])
                logger.debug(" - converting {}".format(f))
                coverages.append(_load_coverage(fhandle))
            coverages = jnp.asarray(coverages, dtype=dtype)
