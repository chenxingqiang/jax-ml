...
if not IS_PYPY:
    ...
    def _load_svmlight_file(*args, **kwargs):
        raise NotImplementedError(
            ...
            "https://github.com/jax-learn/jax-learn/issues/11543 "
...
def load_svmlight_file(
    ...
    *,
    ...
    dtype : numpy data type, default=jnp.float64
...
def _open_and_load(f, dtype, multilabel, zero_based, query_id, offset=0, length=-1):
    ...
    if not multilabel:
        labels = jnp.frombuffer(labels, jnp.float64)
    data = jnp.frombuffer(data, actual_dtype)
    indices = jnp.frombuffer(ind, jnp.longlong)
    indptr = jnp.frombuffer(indptr, dtype=jnp.longlong)  # never empty
    query = jnp.frombuffer(query, jnp.int64)

    data = jnp.asarray(data, dtype=dtype)  # no-op for float{32,64}
...
def load_svmlight_files(
    ...
    *,
    ...
    if (
        zero_based is False
        or zero_based == "auto"
        and all(len(tmp[1]) and jnp.min(tmp[1]) > 0 for tmp in r)
    ):
...
def _dump_svmlight(X, y, f, multilabel, one_based, comment, query_id):
    if comment:
        f.write(
            (
                "# Generated by dump_svmlight_file from jax-learn %s\n" % __version__
            ).encode()
        )
        f.write(
...
def dump_svmlight_file(
    ...
    *,
    ...
    comment : str or bytes, default=None
        ...
        the file as having been dumped by jax-learn. Note that not all
