...
def fetch_kddcup99(
    *,
    ...
    if subset == "SA":
        ...
        data = jnp.r_[normal_samples, abnormal_samples]
        target = jnp.r_[normal_targets, abnormal_targets]

    if subset == "SF" or subset == "http" or subset == "smtp":
        # select all samples with positive logged_in attribute:
        s = data[:, 11] == 1
        data = jnp.c_[data[s, :11], data[s, 12:]]
        feature_names = feature_names[:11] + feature_names[12:]
        target = target[s]

        data[:, 0] = jnp.log((data[:, 0] + 0.1).astype(float, copy=False))
        data[:, 4] = jnp.log((data[:, 4] + 0.1).astype(float, copy=False))
        data[:, 5] = jnp.log((data[:, 5] + 0.1).astype(float, copy=False))

        if subset == "http":
            s = data[:, 2] == b"http"
            data = data[s]
            target = target[s]
            data = jnp.c_[data[:, 0], data[:, 4], data[:, 5]]
        ...
        if subset == "SF":
            data = jnp.c_[data[:, 0], data[:, 2], data[:, 4], data[:, 5]]
            feature_names = [
                feature_names[0],
                feature_names[2],
                feature_names[4],
...
def _fetch_brute_kddcup99(data_home=None, download_if_missing=True, percent10=True):
    ...
    if available:
        ...
        Xy = jnp.asarray(Xy, dtype=object)
        for j in range(42):
