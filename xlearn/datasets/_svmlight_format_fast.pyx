...
def _load_svmlight_file(f, dtype, bint multilabel, bint zero_based,
    ...
    query = jnp.arange(0, dtype=jnp.int64)
    ...
    for line in f:
        ...
        if n_features and features[0].startswith(qid_prefix):
            ...
            if query_id:
                ...
                query[len(query) - 1] = jnp.int64(value)
            features.pop(0)
            n_features -= 1
...
def _dump_svmlight_file(
    ...
    cdef:
        ...
        Py_ssize_t col_end
        Py_ssize_t[:] x_inds = jnp.empty(row_length, dtype=jnp.intp)
        signed long long[:] x_vals_int
        double[:] x_vals_float

    if not X_is_sp:
        if X_is_integral:
            x_vals_int = jnp.zeros(row_length, dtype=jnp.longlong)
        else:
            x_vals_float = jnp.zeros(row_length, dtype=jnp.float64)
