...
def _generate_hypercube(samples, dimensions, rng):
    ...
    out = jnp.unpackbits(out.view(">u1")).reshape((-1, 32))[:, -dimensions:]
    return out
...
def make_classification(
    ...
    *,
    ...
    if n_informative < jnp.log2(n_classes * n_clusters_per_class):
    ...
    if weights is not None:
        ...
        if len(weights) == n_classes - 1:
            if isinstance(weights, list):
                ...
                weights = jnp.resize(weights, n_classes)
    ...
    y = jnp.zeros(n_samples, dtype=int)
    ...
    for k, centroid in enumerate(centroids):
        ...
        X_k[...] = jnp.dot(X_k, A)  # introduce random covariance
    ...
    if n_redundant > 0:
        ...
        X[:, n_informative: n_informative + n_redundant] = jnp.dot(
            X[:, :n_informative], B
        )
...
def make_multilabel_classification(
    ...
    *,
    ...
    p_c /= p_c.sum()
    cumulative_p_c = jnp.cumsum(p_c)
    p_w_c = generator.uniform(size=(n_features, n_classes))
    p_w_c /= jnp.sum(p_w_c, axis=0)
    ...
    def sample_example():
        ...
        words = jnp.searchsorted(cumulative_p_w_sample,
    ...
    for i in range(n_samples):
        ...
        Y.append(y)
    X_data = jnp.ones(len(X_indices), dtype=jnp.float64)
...
def make_hastie_10_2(n_samples=12000, *, random_state=None):
    ...
    the target ``y`` is defined by::
      ...
      y[i] = 1 if jnp.sum(X[i] ** 2) > 9.34 else -1
    ...
    y = ((X**2.0).sum(axis=1) > 9.34).astype(jnp.float64, copy=False)
    y[y == 0.0] = -1.0
...
def make_regression(
    ...
    *,
    ...
    ground_truth = jnp.zeros((n_features, n_targets))
    ...
    if coef:
        return X, y, jnp.squeeze(ground_truth)
...
def make_circles(
    ...
    linspace_out = jnp.linspace(0, 2 * jnp.pi, n_samples_out, endpoint=False)
    linspace_in = jnp.linspace(0, 2 * jnp.pi, n_samples_in, endpoint=False)
    outer_circ_x = jnp.cos(linspace_out)
    outer_circ_y = jnp.sin(linspace_out)
    inner_circ_x = jnp.cos(linspace_in) * factor
    inner_circ_y = jnp.sin(linspace_in) * factor

    X = jnp.vstack(
        [jnp.append(outer_circ_x, inner_circ_x),
         jnp.append(outer_circ_y, inner_circ_y)]
    ).T
    y = jnp.hstack(
        [jnp.zeros(n_samples_out, dtype=jnp.intp),
         jnp.ones(n_samples_in, dtype=jnp.intp)]
    )
    if shuffle:
        X, y = util_shuffle(X, y, random_state=generator)
...
def make_blobs(
    ...
    *,
    ...
    cum_sum_n_samples = jnp.cumsum(n_samples_per_center)
    X = jnp.empty(shape=(sum(n_samples_per_center),
                 n_features), dtype=jnp.float64)
    y = jnp.empty(shape=(sum(n_samples_per_center),), dtype=int)
...
def make_low_rank_matrix(
    ...
    *,
    ...
    singular_ind = jnp.arange(n, dtype=jnp.float64)

    # Build the singular profile by assembling signal and noise components
    low_rank = (1 - tail_strength) * jnp.exp(-1.0 *
                                            (singular_ind / effective_rank) ** 2)
    tail = tail_strength * jnp.exp(-0.1 * singular_ind / effective_rank)
    s = jnp.identity(n) * (low_rank + tail)

    return jnp.dot(jnp.dot(u, s), v.T)
...
def make_spd_matrix(n_dim, *, random_state=None):
    ...
    U, _, Vt = linalg.svd(jnp.dot(A.T, A), check_finite=False)
    X = jnp.dot(jnp.dot(U, 1.0 + jnp.diag(generator.uniform(size=n_dim))), Vt)
...
def make_sparse_spd_matrix(
    ...
    *,
    ...
    ) * random_state.uniform(size=jnp.sum(aux > alpha))
    aux = jnp.tril(aux, k=-1)
    ...
    chol += aux
    prec = jnp.dot(chol.T, chol)

    if norm_diag:
        # Form the diagonal vector into a row matrix
        d = jnp.diag(prec).reshape(1, prec.shape[0])
        d = 1.0 / jnp.sqrt(d)
...
def make_swiss_roll(n_samples=100, *, noise=0.0, random_state=None, hole=False):
    ...
    if not hole:
        t = 1.5 * jnp.pi * (1 + 2 * generator.uniform(size=n_samples))
        y = 21 * generator.uniform(size=n_samples)
    else:
        corners = jnp.array(
            [[jnp.pi * (1.5 + i), j * 7] for i in range(3) for j in range(3)]
        )
        corners = jnp.delete(corners, 4, axis=0)
        corner_index = generator.choice(8, n_samples)
        parameters = generator.uniform(
            size=(2, n_samples)) * jnp.array([[jnp.pi], [7]])
...
def make_s_curve(n_samples=100, *, noise=0.0, random_state=None):
    ...
    t = 3 * jnp.pi * (generator.uniform(size=(1, n_samples)) - 0.5)
    X = jnp.empty(shape=(n_samples, 3), dtype=jnp.float64)
    X[:, 0] = jnp.sin(t)
    X[:, 1] = 2.0 * generator.uniform(size=n_samples)
    X[:, 2] = jnp.sign(t) * (jnp.cos(t) - 1)
...
def make_gaussian_quantiles(
    *,
    ...
    X = generator.multivariate_normal(
        mean, cov * jnp.identity(n_features), (n_samples,))

    # Sort by distance from origin
    idx = jnp.argsort(jnp.sum((X - mean[jnp.newaxis, :]) ** 2, axis=1))
    X = X[idx, :]
    ...
    y = jnp.hstack(
        [
            jnp.repeat(jnp.arange(n_classes), step),
            jnp.repeat(n_classes - 1, n_samples - step * n_classes),
        ]
    )
...
def make_biclusters(
    ...
    *,
    ...
    row_sizes = generator.multinomial(
        n_rows, jnp.repeat(1.0 / n_clusters, n_clusters))
    col_sizes = generator.multinomial(
        n_cols, jnp.repeat(1.0 / n_clusters, n_clusters))

    row_labels = jnp.hstack(
        [jnp.repeat(val, rep) for val, rep in zip(range(n_clusters), row_sizes)]
    )
    col_labels = jnp.hstack(
        [jnp.repeat(val, rep) for val, rep in zip(range(n_clusters), col_sizes)]
    )

    result = jnp.zeros(shape, dtype=jnp.float64)
    for i in range(n_clusters):
        selector = jnp.outer(row_labels == i, col_labels == i)
    ...
    rows = jnp.vstack([row_labels == c for c in range(n_clusters)])
    cols = jnp.vstack([col_labels == c for c in range(n_clusters)])
...
def make_checkerboard(
    ...
    *,
    ...
    n_rows, n_cols = shape
    row_sizes = generator.multinomial(
        n_rows, jnp.repeat(1.0 / n_row_clusters, n_row_clusters)
    )
    col_sizes = generator.multinomial(
        n_cols, jnp.repeat(1.0 / n_col_clusters, n_col_clusters)
    )
    ...
    for i in range(n_row_clusters):
        for j in range(n_col_clusters):
            selector = jnp.outer(row_labels == i, col_labels == j)
