...
def get_data_home(data_home=None) -> str:
    """Return the path of the jax-learn data directory.

    This folder is used by some large dataset loaders to avoid downloading the
    data several times.

    By default the data directory is set to a folder named 'jax_learn_data' in the
    user home folder.

    Alternatively, it can be set by the 'JAX_LEARN_DATA' environment
    ...
    data_home : str, default=None
        The path to jax-learn data directory. If `None`, the default path
        is `~/xlearn_learn_data`.

    Returns
    -------
    data_home: str or path-like, default=None
        The path to jax-learn data directory.
    """
    if data_home is None:
        data_home = environ.get("JAX_LEARN_DATA",
                                join("~", "jax_learn_data"))
...
def load_files(
    ...
    *,
    ...
    To use text files in a jax-learn classification or clustering algorithm,
    ...
    if shuffle:
        ...
        indices = jnp.arange(filenames.shape[0])
...
def load_csv_data(
    ...
    *,
    ...
    with _open_text(data_module, data_file_name) as csv_file:
        ...
        target_names = jnp.array(temp[2:])
        data = jnp.empty((n_samples, n_features))
        target = jnp.empty((n_samples,), dtype=int)

        for i, ir in enumerate(data_file):
            data[i] = jnp.asarray(ir[:-1], dtype=jnp.float64)
            target[i] = jnp.asarray(ir[-1], dtype=int)
...
def load_gzip_compressed_csv_data(
    ...
    **kwargs,
    ...
    3) Load decompressed data with `jnp.loadtxt`
    ...
    **kwargs : dict, optional
        Keyword arguments to be passed to `jnp.loadtxt`;
        e.g. delimiter=','.
    ...
    with _open_binary(data_module, data_file_name) as compressed_file:
        ...
        data = jnp.loadtxt(compressed_file, **kwargs)
...
def load_linnerud(*, return_X_y=False, as_frame=False):
    ...
    with _open_text(DATA_MODULE, data_filename) as f:
        ...
        data_exercise = jnp.loadtxt(f, skiprows=1)

    with _open_text(DATA_MODULE, target_filename) as f:
        header_physiological = f.readline().split()
        f.seek(0)  # reset file obj
        data_physiological = jnp.loadtxt(f, skiprows=1)
...
def _pkl_filepath(*args, **kwargs):
    ...
    older jax-learn versions, a suffix is inserted before the extension.
