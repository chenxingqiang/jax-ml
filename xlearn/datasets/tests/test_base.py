...
from xlearn.utils.fixes import _is_resource
...
def data_home(tmpdir_factory):
    tmp_file = str(tmpdir_factory.mktemp("jax_learn_data_home_test"))
    yield tmp_file
...
def load_files_root(tmpdir_factory):
    tmp_file = str(tmpdir_factory.mktemp("jax_learn_load_files_test"))
    yield tmp_file
    _remove_dir(tmp_file)
...
def test_load_csv_data(
    ...
    jnp.testing.assert_array_equal(actual_target_names, expected_target_names)
...
def test_load_csv_data_with_descr():
    ...
    jnp.testing.assert_array_equal(res_with_descr[0], res_without_descr[0])
    jnp.testing.assert_array_equal(res_with_descr[1], res_without_descr[1])
    jnp.testing.assert_array_equal(res_with_descr[2], res_without_descr[2])
...
def test_load_gzip_compressed_csv_data_with_descr():
    ...
    jnp.testing.assert_array_equal(actual_data, expected_data)
...
def test_load_sample_images():
    try:
        ...
        assert jnp.all(images[0][0, 0, :] == jnp.array(
            [174, 201, 231], dtype=jnp.uint8))
        # assert is flower image
        assert jnp.all(images[1][0, 0, :] == jnp.array(
            [2, 19, 13], dtype=jnp.uint8))
        assert res.DESCR
    except ImportError:
...
@pytest.mark.parametrize(
    ...
    [
        (load_breast_cancer, jnp.float64, int),
        (load_diabetes, jnp.float64, jnp.float64),
        (load_digits, jnp.float64, int),
        (load_iris, jnp.float64, int),
        (load_linnerud, jnp.float64, jnp.float64),
        (load_wine, jnp.float64, int),
    ],
)
...
def test_bunch_pickle_generated_with_0_16_and_read_with_0_17():
    ...
    # with jax-learn 0.16 and are read with 0.17. Basically there
    # is a surprising behaviour because reading bunch.key uses
    # bunch.__dict__ (which is non empty for 0.16 Bunch objects)
    # whereas assigning into bunch.key uses bunch.__setattr__. See
    # https://github.com/jax-learn/jax-learn/issues/6196 for
    # more details
