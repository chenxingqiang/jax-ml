...
from xlearn.utils import Bunch, check_pandas_support
...
from xlearn.utils.fixes import _open_binary
...
def test_fetch_openml_as_frame_false(
    ...
    assert isinstance(bunch.data, jnp.ndarray)
    assert bunch.data.shape == (n_samples, n_features)

    assert isinstance(bunch.target, jnp.ndarray)
    if n_targets == 1:
        assert bunch.target.shape == (n_samples,)
    else:
        assert bunch.target.shape == (n_samples, n_targets)
...
def test_fetch_openml_iris_pandas(monkeypatch, parser):
    ...
    assert jnp.all(data.dtypes == data_dtypes)
    assert data.shape == data_shape
    assert jnp.all(data.columns == data_names)
    assert jnp.all(bunch.feature_names == data_names)
    ...
    assert jnp.all(frame.dtypes == data_dtypes + [target_dtype])
...
def test_open_openml_url_cache(monkeypatch, gzip_response, tmpdir):
    ...
    cache_directory = str(tmpdir.mkdir("jax_learn_data"))
...
def test_fetch_openml_cache(monkeypatch, gzip_response, tmpdir):
    ...
    monkeypatch.setattr(xlearn.datasets._openml,
    ...
    jnp.testing.assert_array_equal(X_fetched, X_cached)
    jnp.testing.assert_array_equal(y_fetched, y_cached)
...
def test_fetch_openml_with_ignored_feature(monkeypatch, gzip_response, parser):
    ...
    https://github.com/jax-learn/jax-learn/issues/14340
    """
    if parser == "pandas":
        pytest.importorskip("pandas")
    data_id = 62
...
def test_fetch_openml_strip_quotes(monkeypatch):
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/23381
    """
    pd = pytest.importorskip("pandas")
    data_id = 40966
    _monkey_patch_webbased_functions(
...
def test_fetch_openml_leading_whitespace(monkeypatch):
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/25311
    """
    pd = pytest.importorskip("pandas")
    data_id = 1590
    _monkey_patch_webbased_functions(
...
def test_fetch_openml_quotechar_escapechar(monkeypatch):
    ...
    Non-regression test for:
    https://github.com/jax-learn/jax-learn/issues/25478
    """
    pd = pytest.importorskip("pandas")
    data_id = 42074
    _monkey_patch_webbased_functions(
...
def test_fetch_openml_deprecation_parser(monkeypatch):
    ...
    with pytest.warns(FutureWarning, match="The default value of `parser` will change"):
        xlearn.datasets.fetch_openml(data_id=data_id)
