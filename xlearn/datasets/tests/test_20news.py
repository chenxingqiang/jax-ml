...
def test_20news(fetch_20newsgroups_fxt):
    ...
    assert jnp.unique(data2cats.target).tolist() == [0, 1]
    ...
    entry2 = data.data[jnp.where(data.target == label)[0][0]]
...
def test_20news_vectorized(fetch_20newsgroups_vectorized_fxt):
    ...
    assert bunch.data.dtype == jnp.float64
...
def test_20news_normalization(fetch_20newsgroups_vectorized_fxt):
    ...
    assert jnp.allclose(jnp.linalg.norm(X_norm.todense(), axis=1), 1)
