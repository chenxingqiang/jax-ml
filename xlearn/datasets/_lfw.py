...
def _load_imgs(file_paths, slice_, color, resize):
    ...
    if not color:
        faces = jnp.zeros((n_faces, h, w), dtype=jnp.float32)
    else:
        faces = jnp.zeros((n_faces, h, w, 3), dtype=jnp.float32)
    ...
    for i, file_path in enumerate(file_paths):
        ...
        face = jnp.asarray(pil_img, dtype=jnp.float32)
...
def _fetch_lfw_people(
    ...
    target_names = jnp.unique(person_names)
    target = jnp.searchsorted(target_names, person_names)
...
def _fetch_lfw_pairs(
    ...
    target = jnp.zeros(n_pairs, dtype=int)
    file_paths = list()
    ...
    return pairs, target, jnp.array(["Different persons", "Same person"])
...
def fetch_lfw_pairs(
    *,
    ...
    data_home : str, default=None
        ...
        default all jax-learn data is stored in '~/jax_learn_data'
        subfolders.
