...
def _smacof_single(
    ...
    sim_flat = ((1 - jnp.tri(n_samples)) * dissimilarities).ravel()
    ...
    for it in range(max_iter):
        ...
        if normalized_stress:
            stress = jnp.sqrt(stress / ((disparities.ravel() ** 2).sum() / 2))
        # Update X using the Guttman transform
        dis[dis == 0] = 1e-5
        ratio = disparities / dis
        B = -ratio
        B[jnp.arange(len(B)), jnp.arange(len(B))] += ratio.sum(axis=1)
        X = 1.0 / n_samples * jnp.dot(B, X)

        dis = jnp.sqrt((X**2).sum(axis=1)).sum()
        if verbose >= 2:
...
def smacof(
    ...
    *,
    ...
    if effective_n_jobs(n_jobs) == 1:
        ...
        seeds = random_state.randint(jnp.iinfo(jnp.int32).max, size=n_init)
