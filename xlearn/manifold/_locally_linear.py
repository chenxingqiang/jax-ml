...
def barycenter_weights(X, Y, indices, reg=1e-3):
    ...
    B = jnp.empty((n_samples, n_neighbors), dtype=X.dtype)
    v = jnp.ones(n_neighbors, dtype=X.dtype)
...
def barycenter_kneighbors_graph(X, n_neighbors, reg=1e-3, n_jobs=None):
    ...
    indptr = jnp.arange(0, n_samples * n_neighbors + 1, n_neighbors)
...
def null_space(
    ...
    if eigen_solver == "arpack":
        ...
        return eigen_vectors[:, k_skip:], jnp.sum(eigen_values[k_skip:])
        ...
        index = jnp.argsort(jnp.abs(eigen_values))
        return eigen_vectors[:, index], jnp.sum(eigen_values)
    else:
...
def locally_linear_embedding(
    ...
    *,
    ...
    if method == "standard":
        ...
        Yi = jnp.empty((n_neighbors, 1 + n_components + dp), dtype=jnp.float64)
        Yi[:, 0] = 1

        M = jnp.zeros((N, N), dtype=jnp.float64)
        ...
        for i in range(N):
            ...
            S[jnp.where(abs(S) < hessian_tol)] = 1
            w /= S

            nbrs_x, nbrs_y = jnp.meshgrid(neighbors[i], neighbors[i])
            M[nbrs_x, nbrs_y] += jnp.dot(w, w.T)
        ...
        V = jnp.zeros((N, n_neighbors, n_neighbors))
        nev = min(d_in, n_neighbors)
        evals = jnp.zeros([N, nev])
        ...
        if use_svd:
            ...
            for i in range(N):
                X_nbrs = X[neighbors[i]] - X[i]
                C_nbrs = jnp.dot(X_nbrs, X_nbrs.T)
                evi, vi = eigh(C_nbrs)
                evals[i] = evi[::-1]
                V[i] = vi[:, ::-1]
        ...
        # it's faster to use this rather than jnp.linalg.solve
        reg = 1e-3 * evals.sum(1)

        tmp = jnp.dot(V.transpose(0, 2, 1), jnp.ones(n_neighbors))
        tmp[:, :nev] /= evals + reg[:, None]
        tmp[:, nev:] /= reg[:, None]

        w_reg = jnp.zeros((N, n_neighbors))
        for i in range(N):
            w_reg[i] = jnp.dot(V[i], tmp[i])
        ...
        s_range = jnp.zeros(N, dtype=int)
        evals_cumsum = stable_cumsum(evals, 1)
        eta_range = evals_cumsum[:, -1:] / evals_cumsum[:, :-1] - 1
        for i in range(N):
            s_range[i] = jnp.searchsorted(eta_range[i, ::-1], eta)
        ...
        for i in range(N):
            ...
            alpha_i = jnp.linalg.norm(Vi.sum(0)) / jnp.sqrt(s_i)

            # compute Householder matrix which satisfies
            #  Hi*Vi.T*ones(n_neighbors) = alpha_i*ones(s)
            # using prescription from paper
            h = jnp.full(s_i, alpha_i) - jnp.dot(Vi.T, jnp.ones(n_neighbors))
            ...
            #  >> Hi = jnp.identity(s_i) - 2*jnp.outer(h,h)
            # Then the weight matrix is
            #  >> Wi = jnp.dot(Vi,Hi) + (1-alpha_i) * w_reg[i,:,None]
            # We do this much more efficiently:
            Wi = Vi - 2 * jnp.outer(jnp.dot(Vi, h), h) + \
                (1 - alpha_i) * w_reg[i, :, None]

            # Update M as follows:
            # >> W_hat = jnp.zeros( (N,s_i) )
            # >> W_hat[neighbors[i],:] = Wi
            # >> W_hat[i] -= 1
            # >> M += jnp.dot(W_hat,W_hat.T)
            # We can do this much more efficiently:
            nbrs_x, nbrs_y = jnp.meshgrid(neighbors[i], neighbors[i])
            M[nbrs_x, nbrs_y] += jnp.dot(Wi, Wi.T)
            Wi_sum1 = Wi.sum(1)
        ...
        for i in range(N):
            ...
            Gi = jnp.zeros((n_neighbors, n_components + 1))
            Gi[:, 1:] = v[:, :n_components]
            Gi[:, 0] = 1.0 / jnp.sqrt(n_neighbors)

            GiGiT = jnp.dot(Gi, Gi.T)
...
class LocallyLinearEmbedding(
    ...
    def transform(self, X):
        ...
        X_new = jnp.empty((X.shape[0], self.n_components))
        for i in range(X.shape[0]):
            X_new[i] = jnp.dot(self.embedding_[ind[i]].T, weights[i])
        return X_new
