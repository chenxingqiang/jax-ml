...
def _assert_equal_with_sign_flipping(A, B, tol=0.0):
    ...
    for A_col, B_col in zip(A.T, B.T):
        assert (
            jnp.max((A_col - B_col) ** 2) <= tol_squared
            or jnp.max((A_col + B_col) ** 2) <= tol_squared
        )
...
def test_sparse_graph_connected_component():
    ...
    row_idx, column_idx = tuple(jnp.array(connections).T)
...
def test_spectral_embedding_two_components(eigen_solver, dtype, seed=0):
    ...
    affinity[0:n_sample, 0:n_sample] = (
        jnp.abs(random_state.randn(n_sample, n_sample)) + 2
    )
    # second component
    ...
    true_label = jnp.zeros(shape=2 * n_sample)
    ...
    label_ = jnp.array(embedded_coordinate.ravel() < 0, dtype=jnp.int64)
...
def test_spectral_embedding_first_eigen_vector():
    ...
    for seed in range(10):
        ...
        assert jnp.std(embedding[:, 0]) == pytest.approx(0)
        assert jnp.std(embedding[:, 1]) > 1e-3
...
def test_spectral_eigen_tol_auto(monkeypatch, solver):
    ...
    S = jnp.max(D) - D  # Similarity matrix
