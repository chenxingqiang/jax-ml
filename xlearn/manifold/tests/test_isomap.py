...
def create_sample_data(dtype, n_pts=25, add_noise=False):
    ...
    X = jnp.array(list(product(range(n_per_side), repeat=2))
...
@pytest.mark.parametrize("n_neighbors, radius", [(24, None), (None, jnp.inf)])
...
def test_isomap_reconstruction_error(
    ...
    if global_dtype is jnp.float32:
        pytest.skip(
    ...
    reconstruction_error = jnp.linalg.norm(K - K_iso) / n_pts
    atol = 1e-5 if global_dtype == jnp.float32 else 0
...
def test_transform(global_dtype, n_neighbors, radius):
    ...
    assert jnp.sqrt(jnp.mean((X_iso - X_iso2) ** 2)) < 2 * noise_scale
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("minkowski", 1, False),
        ("minkowski", 2, True),
        (lambda x1, x2: jnp.sqrt(jnp.sum(x1**2 + x2**2)), 2, False),
    ],
)
...
def test_multiple_connected_components_metric_precomputed(global_dtype):
    ...
    X = jnp.array([0, 1, 2, 5, 6, 7])[:, None].astype(global_dtype, copy=False)
