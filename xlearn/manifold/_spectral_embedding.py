...
def _graph_connected_component(graph, node_id):
    ...
    if sparse.issparse(graph):
        ...
        graph = graph.tocsr()
    connected_nodes = jnp.zeros(n_node, dtype=bool)
    nodes_to_explore = jnp.zeros(n_node, dtype=bool)
    nodes_to_explore[node_id] = True
    for _ in range(n_node):
        last_num_component = connected_nodes.sum()
        jnp.logical_or(connected_nodes, nodes_to_explore, out=connected_nodes)
        if last_num_component >= connected_nodes.sum():
            break
        indices = jnp.where(nodes_to_explore)[0]
        ...
        for i in indices:
            if sparse.issparse(graph):
                ...
                neighbors = graph[i]
            jnp.logical_or(nodes_to_explore, neighbors, out=nodes_to_explore)
...
def _set_diag(laplacian, value, norm_laplacian):
    ...
    if not sparse.isspmatrix(laplacian):
        ...
        n_diags = jnp.unique(laplacian.row - laplacian.col).size
        if n_diags <= 7:
    ...
    * :doi:`"Toward the Optimal Preconditioned Eigensolver: Locally Optimal
    ...
    if (
        ...
        laplacian = check_array(
            laplacian, dtype=[jnp.float64, jnp.float32], accept_sparse=True
        )
        laplacian = _set_diag(laplacian, 1, norm_laplacian)
