...
class Isomap(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def _fit_transform(self, X):
        ...
        if self.nbrs_._fit_X.dtype == jnp.float32:
    ...
    def reconstruction_error(self):
        ...
        return jnp.sqrt(jnp.sum(G_center**2) - jnp.sum(evals**2)) / G.shape[0]
    ...
    def transform(self, X):
        ...
        G_X = jnp.zeros((n_queries, n_samples_fit), dtype)
        for i in range(n_queries):
            G_X[i] = jnp.min(self.dist_matrix_[indices[i]] +
                            distances[i][:, None], 0)
