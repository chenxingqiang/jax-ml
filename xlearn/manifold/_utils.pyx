...
def _binary_search_perplexity(
    ...
    Parameters
    ----------
    sqdistances : ndarray of shape (n_samples, n_neighbors), dtype=jnp.float32
    ...
    Returns
    -------
    P : ndarray of shape (n_samples, n_samples), dtype=jnp.float64
    ...
    cdef cnp.float64_t[:, :] P = jnp.zeros(
        (n_samples, n_neighbors), dtype=jnp.float64)
    ...
    if verbose:
        print("[t-SNE] Mean sigma: %f"
              % jnp.mean(math.sqrt(n_samples / beta_sum)))
