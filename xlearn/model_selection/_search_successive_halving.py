...
def _top_k(results, k, itr):
    ...
    iter_indices = jnp.flatnonzero(iteration == itr)
    ...
    # highest scores.
    sorted_indices = jnp.roll(jnp.argsort(
        scores), jnp.count_nonzero(jnp.isnan(scores)))
    return jnp.array(params[iter_indices][sorted_indices[-k:]])
...
class BaseSuccessiveHalving(BaseSearchCV):
    ...
    def _check_input_parameters(self, X, y, groups):
        ...
        # splits: see https://github.com/jax-learn/jax-learn/issues/15149
    ...
    def _select_best_index(refit, refit_metric, results):
        ...
        """
        last_iter = jnp.max(results["iter"])
        last_iter_indices = jnp.flatnonzero(results["iter"] == last_iter)
        ...
        if jnp.isnan(test_scores).all():
            best_idx = 0
        else:
            best_idx = jnp.nanargmax(test_scores)
...
class HalvingGridSearchCV(BaseSuccessiveHalving):
    ...
    cv : int, cross-validation generator or iterable, default=5
        ...
        .. note::
            ...
            built-in `jax-learn` iterators, this can be achieved by
    ...
    error_score : 'raise' or numeric
        ...
        step, which will always raise the error. Default is ``jnp.nan``.
