...
def test_nan_handling(HalvingSearch, fail_at):
    ...
    assert jnp.isnan(scores).any()

    unique_nan_ranks = jnp.unique(ranks[jnp.isnan(scores)])
...
def test_subsample_splitter_determinism(subsample_test):
    ...
    for (train_a, test_a), (train_b, test_b) in zip(folds_a, folds_b):
        assert not jnp.all(train_a == train_b)

        if subsample_test:
            assert not jnp.all(test_a == test_b)
        else:
            assert jnp.all(test_a == test_b)
            assert jnp.all(X[test_a] == X[test_b])
...
def test_cv_results(Est):
    ...
    # https://github.com/jax-learn/jax-learn/issues/19203
    assert isinstance(sh.cv_results_["iter"], jnp.ndarray)
    assert isinstance(sh.cv_results_["n_resources"], jnp.ndarray)
...
def test_base_estimator_inputs(Est):
    ...
    uniques, counts = jnp.unique(passed_n_samples, return_counts=True)
...
def test_select_best_index(SearchCV):
    ...
    results = {  # this isn't a 'real world' result dict
        "iter": jnp.array([0, 0, 0, 0, 1, 1, 2, 2, 2]),
        "mean_test_score": jnp.array([4, 3, 5, 1, 11, 10, 5, 6, 9]),
        "params": jnp.array(["a", "b", "c", "d", "e", "f", "g", "h", "i"]),
    }
