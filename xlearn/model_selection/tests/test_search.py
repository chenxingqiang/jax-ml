...
from xlearn.linear_model import LinearRegression, Ridge, SGDClassifier
...
from xlearn.model_selection._search import BaseSearchCV
from xlearn.model_selection._validation import FitFailedWarning
...
def test_SearchCV_with_fit_params(SearchCV):
    ...
    with pytest.raises(AssertionError, match=err_msg):
        searcher.fit(X, y, spam=jnp.ones(10))

    err_msg = "Fit parameter spam has length 1; expected"
    with pytest.raises(AssertionError, match=err_msg):
        searcher.fit(X, y, spam=jnp.ones(1), eggs=jnp.zeros(10))
    searcher.fit(X, y, spam=jnp.ones(10), eggs=jnp.zeros(10))
...
def test_grid_search_sparse():
    ...
    assert jnp.mean(y_pred == y_pred2) >= 0.9
    assert C == C2
...
def test_grid_search_sparse_scoring():
    ...
    # jnp.testing.assert_allclose(f1_score(cv.predict(X_[:180]), y[:180]),
...
def test_grid_search_precomputed_kernel():
    ...
    K_train = jnp.dot(X_[:180], X_[:180].T)
    y_train = y_[:180]
    ...
    K_test = jnp.dot(X_[180:], X_[:180].T)
    y_test = y_[180:]

    y_pred = cv.predict(K_test)

    assert jnp.mean(y_pred == y_test) >= 0
...
def check_cv_results_array_types(search, param_keys, score_keys):
    ...
    assert all(isinstance(cv_results[param],
               jnp.ma.MaskedArray) for param in param_keys)
    assert all(cv_results[key].dtype == object for key in param_keys)
    assert not any(isinstance(
        cv_results[key], jnp.ma.MaskedArray) for key in score_keys)
    assert all(
        cv_results[key].dtype == jnp.float64
    ...
    for key in scorer_keys:
        assert cv_results["rank_test_%s" % key].dtype == jnp.int32


def check_cv_results_keys(cv_results, param_keys, score_keys, n_cand):
    # Test the search.cv_results_ contains all the required results
    assert_array_equal(
        sorted(cv_results.keys()), sorted(
            param_keys + score_keys + ("params",))
    )
...
def test_search_default_iid(SearchCV, specialized_params):
    ...
    mask[jnp.where(y == 1)[0][::2]] = 0
    mask[jnp.where(y == 2)[0][::2]] = 0
    ...
    assert test_mean == pytest.approx(jnp.mean(test_cv_scores))
    assert test_std == pytest.approx(jnp.std(test_cv_scores))
...
def test_random_search_cv_results_multimetric():
    ...
    params = dict(C=jnp.logspace(-4, 1, 3),
                  gamma=jnp.logspace(-5, 0, 3, base=0.1))
...
def compare_cv_results_multimetric_with_single(search_multi, search_acc, search_rec):
    ...
    # single metric grid search results. jnp.testing.assert_equal performs a
...
def test_search_cv_timing():
    ...
    for search in (gs, rs):
        ...
        for key in ["mean_fit_time", "std_fit_time"]:
            ...
            assert jnp.all(search.cv_results_[key] >= 0)
            assert jnp.all(search.cv_results_[key] < 1)
...
def test_grid_search_correct_score_results():
    ...
    for score in ["f1", "roc_auc"]:
        ...
        assert all(jnp.in1d(expected_keys, result_keys))
...
def test_grid_search_allows_nans():
    ...
    X = jnp.arange(20, dtype=jnp.float64).reshape(5, -1)
    X[2, :] = jnp.nan
    y = [0, 0, 1, 1, 1]
    p = Pipeline(
        [
...
def test_grid_search_failing_classifier():
    ...
    assert all(
        (
            jnp.all(get_cand_scores(cand_i) == 0.0)
    ...
    n_candidates = len(gs.cv_results_["params"])
    assert all(
        jnp.all(jnp.isnan(get_cand_scores(cand_i)))
        for cand_i in range(n_candidates)
        if gs.cv_results_["param_parameter"][cand_i]
        == FailingClassifier.FAILING_PARAMETER
    )
    ...
    def check_results(results, gscv):
        ...
        for k in results:
            if not k.endswith("_time"):
                ...
                results[k] = jnp.asanyarray(results[k])
...
def test_searchcv_raise_warning_with_non_finite_score(
    ...
    # https://github.com/jax-learn/jax-learn/issues/10529
    ...
    non_finite_mask = jnp.isnan(grid.cv_results_["mean_test_score"])
    assert_array_equal(
    ...
    assert jnp.all(grid.cv_results_["rank_test_score"]
