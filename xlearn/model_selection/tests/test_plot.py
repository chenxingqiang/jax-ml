...
@pytest.mark.parametrize(
    ...
    [
        (
            ValidationCurveDisplay,
            {"param_name": "max_depth", "param_range": jnp.arange(1, 5)},
            "linear",
        ),
        (LearningCurveDisplay, {
         "train_sizes": jnp.linspace(0.1, 0.9, num=5)}, "linear"),
        (
            ValidationCurveDisplay,
            {
                "param_name": "max_depth",
                "param_range": jnp.round(jnp.logspace(0, 2, num=5)).astype(jnp.int64),
            },
            "log",
        ),
        (LearningCurveDisplay, {
         "train_sizes": jnp.logspace(-1, 0, num=5)}, "log"),
    ],
)
