...
class ParameterGrid:
    ...
    def __len__(self):
        ...
        # Product function that can handle iterables (jnp.prod can't).
...
class BaseSearchCV(MetaEstimatorMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _format_results(self, candidate_params, n_splits, out, more_results=None):
        ...
        for key, val in results.items():
            ...
            results[key] = jnp.asarray(val)

        def _store(key_name, array, weights=None, splits=False, rank=False):
            """A small helper to store the scores/times to the cv_results_"""
            # When iterated first by splits, then by parameters
            # We want `array` to have `n_candidates` rows and `n_splits` cols.
            array = jnp.array(array, dtype=jnp.float64).reshape(
            ...
            array_means = jnp.average(array, axis=1, weights=weights)
            results["mean_%s" % key_name] = array_means

            if key_name.startswith(("train_", "test_")) and jnp.any(
                ~jnp.isfinite(array_means)
            ):
                warnings.warn(
                    (
            ...
            array_stds = jnp.sqrt(
                jnp.average(
                    (array - array_means[:, jnp.newaxis]) ** 2, axis=1, weights=weights
                )
            )
            ...
            if rank:
                ...
                if jnp.isnan(array_means).all():
                    # All fit/scoring routines failed.
                    rank_result = jnp.ones_like(array_means, dtype=jnp.int32)
                else:
                    min_array_means = jnp.nanmin(array_means) - 1
                    array_means = jnp.nan_to_num(
                        array_means, nan=min_array_means)
                    rank_result = rankdata(-array_means, method="min").astype(
                        jnp.int32, copy=False
                    )
...
class GridSearchCV(BaseSearchCV):
    ...
    estimator : estimator object
        This is assumed to implement the jax-learn estimator interface.
