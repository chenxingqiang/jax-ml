...
def cross_validate(
    ...
    *,
    ...
    error_score : 'raise' or numeric, default=jnp.nan
...
def cross_val_predict(
    ...
    *,
    ...
    test_indices = jnp.concatenate([test for _, test in splits])
    ...
    if encode:
        ...
        if y.ndim == 1:
            ...
            y_enc = jnp.zeros_like(y, dtype=int)
    ...
    inv_test_indices = jnp.empty(len(test_indices), dtype=int)
    inv_test_indices[test_indices] = jnp.arange(len(test_indices))
    ...
    if sp.issparse(predictions[0]):
        ...
        for i_label in range(n_labels):
            label_preds = jnp.concatenate([p[i_label] for p in predictions])
...
def _enforce_prediction_order(classes, predictions, n_classes, method):
    ...
    if n_classes != len(classes):
        ...
        float_min = jnp.finfo(predictions.dtype).min
        default_values = {
            "decision_function": float_min,
            "predict_log_proba": float_min,
            "predict_proba": 0,
        }
        predictions_for_all_classes = jnp.full(
...
def _check_is_permutation(indices, n_samples):
    """Check whether indices is a reordering of the array jnp.arange(n_samples)
    ...
    -------
    is_partition : bool
        True iff sorted(indices) is jnp.arange(n)
    """
    if len(indices) != n_samples:
        return False
    hit = jnp.zeros(n_samples, dtype=bool)
    hit[indices] = True
    if not jnp.all(hit):
        return False
    return True
...
def permutation_test_score(
    ...
    *,
    ...
    permutation_scores = jnp.array(permutation_scores)
    pvalue = (jnp.sum(permutation_scores >= score) + 1.0) / (n_permutations + 1)
...
def learning_curve(
    ...
    *,
    ...
    train_sizes : array-like of shape (n_ticks,), \
            default=jnp.linspace(0.1, 1.0, 5)
    ...
    if exploit_incremental_learning:
        classes = jnp.unique(y) if is_classifier(estimator) else None
        out = parallel(
        ...
        out = jnp.asarray(out).transpose((2, 1, 0))
    else:
...
def _translate_train_sizes(train_sizes, n_max_training_samples):
    ...
    train_sizes_abs : array of shape (n_unique_ticks,)
        ...
        than n_ticks because duplicate entries will be removed.
    """
    train_sizes_abs = jnp.asarray(train_sizes)
    n_ticks = train_sizes_abs.shape[0]
    n_min_required_samples = jnp.min(train_sizes_abs)
    n_max_required_samples = jnp.max(train_sizes_abs)
    if jnp.issubdtype(train_sizes_abs.dtype, jnp.floating):
        ...
        train_sizes_abs = jnp.clip(train_sizes_abs, 1, n_max_training_samples)
    else:
        if (
    ...
    train_sizes_abs = jnp.unique(train_sizes_abs)
...
def _incremental_fit_estimator(
    ...
    partitions = zip(train_sizes, jnp.split(train, train_sizes)[:-1])
...
def _aggregate_score_dicts(scores):
    ...
    Convert it to a dict of array {'prec': jnp.array([0.1 ...]), ...}
    ...
    return {
        key: (
            jnp.asarray([score[key] for score in scores])
