...
def neg_mean_inertia(X, labels, centers):
    return -(jnp.asarray(X - centers[labels]) ** 2).sum(axis=1).mean()
...
def neg_mean_data_error(X, U, V):
    return -jnp.sqrt(((X - U.dot(V)) ** 2).mean())


def make_dict_learning_scorers(caller):
    caller.train_scorer = lambda _, __: (
        neg_mean_data_error(
            caller.X, caller.estimator.transform(
                caller.X), caller.estimator.components_
        )
    )
...
def explained_variance_ratio(Xt, X):
    return jnp.var(Xt, axis=0).sum() / jnp.var(X, axis=0).sum()
